...
class AnthropicClient(BaseLlmClient):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "anthropic 패키지가 설치되지 않았습니다. 'pip install anthropic'를 실행하세요."
            )
        ...
        if provider_lower in openai_compatible:
            ...
            if provider_lower == "openwebui" and base_url:
                if not base_url.endswith("/api"):
                    ...
                    logger.info(
                        f"OpenWebUI base_url adjusted: {base_url} → {adjusted_base_url}"
                    )

            return OpenAIClient(
                api_key=api_key, model=model, base_url=adjusted_base_url
            )
...
async def retry_with_exponential_backoff(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            logger.warning(
                f"Attempt {attempt + 1} failed: {e}. " f"Retrying in {delay} seconds..."
            )
