...
class CostEstimateRequest(BaseModel):
    ...
    llm_provider: str = Field(
        ..., max_length=50, description="LLM provider (openai, anthropic, ollama)"
    )
    llm_model: str = Field(
        ..., max_length=100, description="LLM model name (gpt-4, claude-3-sonnet)"
    )
    prompt_template: str = Field(
        ..., description="Prompt template with {chunk_text} placeholder"
    )
    max_tokens: int = Field(
        500, gt=0, le=4000, description="Maximum tokens per response"
    )
...
class ModelPricing(BaseModel):
    ...
    input_price_per_1k: float = Field(
        ..., description="Input price per 1K tokens (USD)"
    )
    output_price_per_1k: float = Field(
        ..., description="Output price per 1K tokens (USD)"
    )
...
class LlmAnalysisRequest(BaseModel):
    ...
    llm_config_id: Optional[str] = Field(
        None, max_length=36, description="Backend LLM Config ID (optional)"
    )
    llm_api_key: Optional[str] = Field(None, description="LLM API key (optional)")
    llm_base_url: Optional[str] = Field(
        None, description="Custom LLM endpoint URL (optional)"
    )
...
class LlmAnalysisStatusResponse(BaseModel):
    ...
    job_id: UUID = Field(..., description="Job ID")
    llm_config_id: Optional[str] = Field(
        None, description="Backend LLM Config ID (for resume)"
    )
    status: str = Field(..., description="Analysis status")
    progress: ProgressInfo = Field(..., description="Progress information")
    actual_cost_so_far: Optional[CostInfo] = Field(
        None, description="Actual cost so far"
    )
...
class LlmAnalysisResultsResponse(BaseModel):
    ...
    document_id: UUID = Field(..., description="Document ID")
    job_id: UUID = Field(..., description="Job ID")
    results: List[LlmAnalysisResultItem] = Field(
...
class LlmAnalysisJobListResponse(BaseModel):
    ...
    jobs: List[LlmAnalysisJobSummary] = Field(
        default_factory=list, description="Job list"
    )
