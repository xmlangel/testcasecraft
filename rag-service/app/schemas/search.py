...
class AdvancedSearchRequest(BaseModel):
    ...
    search_method: SearchMethod = Field(
        SearchMethod.HYBRID_RERANK, description="Search method to use\n사용할 검색 방법"
    )
    similarity_threshold: float = Field(
        0.6,
        ge=0.0,
    ...
    max_results: int = Field(
        10, ge=1, le=100, description="Maximum results\n최대 결과 수"
    )
...
class SearchResult(BaseModel):
    ...
    source_type: str = Field(
        ...,
        description="Source type: 'document' or 'testcase'\n출처 타입: 'document' 또는 'testcase'",
    )
    ...
    bm25_score: Optional[float] = Field(
        None, description="BM25 keyword score\nBM25 키워드 점수"
    )
    reranker_score: Optional[float] = Field(
        None, description="Reranker score\nReranker 점수"
    )
    rrf_score: Optional[float] = Field(
        None, description="RRF fusion score\nRRF 융합 점수"
    )
    vector_rank: Optional[int] = Field(
        None, description="Rank in vector search\n벡터 검색에서의 순위"
    )
    bm25_rank: Optional[int] = Field(
        None, description="Rank in BM25 search\nBM25 검색에서의 순위"
    )
