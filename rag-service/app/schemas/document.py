...
class DocumentBase(BaseModel):
    ...
    file_type: str = Field(
        ..., max_length=50, description="File type (extension)\n파일 타입 (확장자)"
    )
    file_size: int = Field(
        ..., gt=0, description="File size in bytes\n파일 크기 (바이트)"
    )
    uploaded_by: Optional[str] = Field(
        None, max_length=255, description="Uploader username\n업로더 사용자명"
    )
    minio_bucket: str = Field(
        ..., max_length=255, description="MinIO bucket name\nMinIO 버킷 이름"
    )
...
class DocumentUpdate(BaseModel):
    ...
    analysis_status: Optional[str] = Field(
        None, max_length=50, description="Analysis status\n분석 상태"
    )
    total_chunks: Optional[int] = Field(
        None, ge=0, description="Total number of chunks\n전체 청크 수"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        None, description="Additional metadata\n추가 메타데이터"
    )
...
class DocumentResponse(BaseModel):
    ...
    uploaded_by: Optional[str] = Field(
        None, description="Uploader username\n업로더 사용자명"
    )
    ...
    analysis_date: Optional[datetime] = Field(
        None, description="Analysis completion date\n분석 완료 날짜"
    )
...
class DocumentUploadResponse(BaseModel):
    ...
    analysis_status: str = Field(..., description="Analysis status\n분석 상태")
    message: str = Field(
        "Document uploaded successfully", description="Status message\n상태 메시지"
    )
...
class DocumentAnalysisResponse(BaseModel):
    ...
    message: str = Field(
        "Document analysis completed", description="Status message\n상태 메시지"
    )
