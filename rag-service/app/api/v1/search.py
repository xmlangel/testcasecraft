...
async def search_similar_chunks(request: SearchRequest, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        query = (
            db.query(RAGEmbedding, RAGDocument)
            .join(RAGDocument, RAGEmbedding.document_id == RAGDocument.id)
            .filter(RAGEmbedding.embedding.isnot(None))
        )
        ...
        for embedding, document in embeddings_with_docs:
            ...
            if similarity_score >= request.similarity_threshold:
                ...
                source_type = (
                    "testcase" if document.file_type == "testcase" else "document"
                )
        ...
        for conversation in conversation_messages:
            ...
            if similarity_score >= request.similarity_threshold:
                metadata = (
                    conversation.metadata_json.copy()
        ...
        logger.info(
            f"Found {len(results)} similar chunks above threshold {request.similarity_threshold}"
        )
...
async def advanced_search(
    ...
    try:
        ...
        logger.info(
            f"Advanced search - Method: {request.search_method}, Query: {request.query_text[:100]}..."
        )
        embedding_service = get_embedding_service()
        query_embedding = embedding_service.generate_embedding(request.query_text)
        ...
        for conversation in conversation_messages:
            ...
            metadata = (
                conversation.metadata_json.copy() if conversation.metadata_json else {}
            )
            metadata.update(
                {
                    "threadId": str(conversation.thread_id),
        ...
        if request.search_method == SearchMethod.VECTOR:
            ...
            results = [
                doc for doc in results if doc["score"] >= request.similarity_threshold
            ]
            results = results[: request.max_results]
            ...
            hybrid_service.build_bm25_index(
                documents, text_field="text", use_korean_tokenizer=True
            )
            results = hybrid_service.search_bm25(
                request.query_text, top_k=request.max_results, use_korean_tokenizer=True
            )
