...
def init_project_db(project_name: str) -> str:
    ...
    if os.path.exists(db_path):
        msg = (
            f"Err001: Database for project '{project_name}' already exists at {db_path}"
        )
        logging.error(msg)
        return msg
...
def search_tickets(
    project_name: str, prompt: str, conditions: str = "", top_n: int = 5
) -> str:
    """
    ...
    """
    try:
        if (not prompt or prompt.strip() == "") and (
            not conditions or conditions.strip() == ""
        ):
...
def add_ticket(project_name: str, ticket: any) -> str:
    ...
    try:
        ...
        ticket_id = (
            ...
            else ticket_data.get("ticket_id")
        )
        ...
        try:
            ...
            """,
                (
                    ...
                    or "",  # Convert ticket_data to a JSON string
                    vector_array,
                ),
            )
            conn.commit()
        ...
        msg = (
            f"Err007: Error adding ticket to database for project '{project_name}': {e}"
        )
        logging.error(msg)
        return msg
...
def connect_db(project_name: str):
    ...
    if not os.path.exists(db_path):
        msg = (
            f"Err010: Database for project '{project_name}' does not exist at {db_path}"
        )
        logging.error(msg)
        return None
...
def del_project_db(project_name: str):
    ...
    if os.path.exists(db_path):
        ...
        msg = (
            f"Err012: Database for project '{project_name}' does not exist at {db_path}"
        )
        logging.error(msg)
        return msg
...
def create_test_db():
    ...
    if not test_prj_name:
        logging.info(
            "TEST_PROJECT_NAME environment variable is not set. Skip test database creation."
        )
        return
    ...
    if os.path.exists(db_path):
        logging.info(
            f"Test database '{test_db_name}' already exists at {db_path}. Skip creation."
        )
        return
