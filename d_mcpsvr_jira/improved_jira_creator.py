...
def create_board_compatible_issue(
    project_key, summary, description, issue_type="10003", priority="3"
):
    """
...
def verify_board_display():
    ...
    try:
        ...
        board_issues = jira.search_issues(
            "project = ICT ORDER BY Rank ASC", maxResults=30
        )
        ...
        for issue in all_issues:
            ...
            rank = getattr(issue.fields, "customfield_10019", "없음")
            print(
                f"  {status} {issue.key}: {issue.fields.status.name} - Rank: {rank[:20]}..."
            )
