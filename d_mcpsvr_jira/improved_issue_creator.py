...
def get_valid_issue_types():
    ...
    try:
        ...
        create_meta = jira.createmeta(
            projectKeys="ICT", expand="projects.issuetypes.fields"
        )
...
def create_issue_safe(issue_data, work_start_data=None):
    ...
    try:
        ...
        if "issuetype" not in issue_data:
            ...
            print(
                f"⚠️ 이슈 타입이 지정되지 않아 기본값 사용: {valid_types[0][1]} (ID: {valid_types[0][0]})"
            )
        ...
        print(
            f'  - 타입: {[name for type_id, name in valid_types if type_id == issue_data["issuetype"]["id"]][0]}'
        )
...
def create_feature_issue(title, description, estimated_hours=None, related_files=None):
    ...
    if estimated_hours or related_files:
        work_start_data = {
            ...
            "risks": ["호환성 문제 가능성", "성능 저하 위험", "사용자 혼란 가능성"],
        }
...
def create_bug_issue(title, description, reproduction_steps=None):
    ...
    work_start_data = {
        ...
        "risks": ["다른 기능에 영향 가능성", "데이터 손실 위험", "사용자 경험 저하"],
    }
