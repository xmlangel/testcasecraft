...
def add_work_start_comment(issue_key, work_description, estimated_hours=None, files=None,
                          technical_context=None, dependencies=None, risks=None):
    ...
    try:
        ...
        in_progress_transitions = [t for t in transitions if '진행' in t['name'] or 'progress' in t['name'].lower()]
...
def add_progress_comment(issue_key, completed_tasks, current_tasks, findings=None, next_steps=None,
                        code_changes=None, test_results=None, performance_metrics=None, blockers=None):
    """작업 진행 상황 코멘트 추가 (상세 정보 포함)"""
    try:
        jira = get_jira_client()
        issue = jira.issue(issue_key)

        comment_text = f"h2. 진행 상황 업데이트 ({datetime.now().strftime('%Y-%m-%d %H:%M')})\n\n"
...
def add_completion_comment(issue_key, completed_work, modified_files, test_results, validation_items=None,
                          performance_impact=None, security_considerations=None, documentation_updates=None,
                          lessons_learned=None, future_improvements=None):
    """작업 완료 코멘트 추가 및 상태 변경 (상세 정보 포함)"""
    try:
        jira = get_jira_client()
        issue = jira.issue(issue_key)

        comment_text = f"h2. ✅ 작업 완료 ({datetime.now().strftime('%Y-%m-%d %H:%M')})\n\n"
        ...
        # 상태를 '완료'로 변경
        transitions = jira.transitions(issue)
        done_transitions = [t for t in transitions if 'done' in t['name'].lower() or '완료' in t['name']]
        if done_transitions:
...
def create_dashboard_compatible_issue(project_key='ICT', summary='', description='', issue_type='10003'):
    """
    ...
    print("대신 board_compatible_creator.py의 create_board_visible_issue()를 사용하세요.")
...
def create_board_visible_issue_simple(project_key='ICT', summary='', description='', issue_type='10003'):
    """
    ...
    try:
        ...
        try:
            ...
            # 보드의 첫 번째 이슈 조회
            board_response = session.get(f'{server_url}/rest/agile/1.0/board/1/issue', 
                                       params={'maxResults': 1})
...
def add_detailed_summary_comment(issue_key, summary_data):
    ...
    try:
        ...
        comment_text = f"h2. 📋 상세 작업 요약 ({datetime.now().strftime('%Y-%m-%d %H:%M')})\n\n"
        ...
        comment_text += f"• *실제 소요 시간*: {summary_data.get('actual_hours', 'N/A')}시간\n"
        comment_text += f"• *예상 vs 실제*: {summary_data.get('time_comparison', 'N/A')}\n"
        comment_text += "{panel}\n"

        # 기술적 상세사항
        tech_details = ""
        if 'tech_stack' in summary_data:
            tech_details += "• *기술 스택*: " + ", ".join(summary_data['tech_stack']) + "\n"
        if 'architecture_patterns' in summary_data:
            tech_details += "• *아키텍처 패턴*: " + ", ".join(summary_data['architecture_patterns']) + "\n"
        if 'frameworks' in summary_data:
            tech_details += "• *프레임워크/라이브러리*: " + ", ".join(summary_data['frameworks']) + "\n"
        if tech_details:
        ...
        if 'code_stats' in summary_data:
            ...
            comment_text += f"• *복잡도 변화*: {stats.get('complexity_change', 'N/A')}\n"
            comment_text += "{panel}\n"
        ...
        if 'detailed_tests' in summary_data:
            ...
            comment_text += f"• *통합 테스트*: {tests.get('integration_tests', 'N/A')}\n"
            comment_text += f"• *E2E 테스트*: {tests.get('e2e_tests', 'N/A')}\n"
            comment_text += f"• *성능 테스트*: {tests.get('performance_tests', 'N/A')}\n"
        ...
        if 'security_checks' in summary_data:
            ...
            comment_text += f"• *취약점 스캔*: {security.get('vulnerability_scan', 'N/A')}\n"
            comment_text += f"• *권한 검증*: {security.get('authorization_check', 'N/A')}\n"
            comment_text += f"• *데이터 검증*: {security.get('data_validation', 'N/A')}\n"
        ...
        if 'database_impact' in summary_data:
            ...
            comment_text += f"• *데이터 마이그레이션*: {db.get('data_migration', 'None')}\n"
        ...
        comment_text += "||*항목*||*점수*||\n"
        comment_text += f"|코드 품질|{summary_data.get('code_quality_score', 'N/A')}/100|\n"
        comment_text += f"|테스트 커버리지|{summary_data.get('test_coverage', 'N/A')}%|\n"
        comment_text += f"|성능 점수|{summary_data.get('performance_score', 'N/A')}/100|\n"
...
def create_comprehensive_work_summary(issue_key, work_type="development"):
    ...
    sample_summary = {
        ...
        'architecture_patterns': ['MVC', 'Repository Pattern', 'Security Service Layer'],
...
def start_work_with_context(issue_key, work_description, files=None, estimated_hours=None):
    """
    컨텍스트가 있는 작업 시작 함수
...
def check_and_start_work(issue_key):
    ...
    try:
        ...
        if ('할일' in current_status or 'To Do' in current_status or 'Open' in current_status or 
            '해야 할 일' in current_status or 'Backlog' in current_status):
            ...
            return f"⚠️ 현재 상태({current_status})에서는 작업 시작을 처리할 수 없습니다."
...
def force_transition_to_progress(issue_key):
    ...
    try:
        ...
        progress_transitions = [
            ...
            if '진행' in t['name'] or 'progress' in t['name'].lower() or 'start' in t['name'].lower()
        ]
