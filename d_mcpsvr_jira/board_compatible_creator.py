...
def create_board_visible_issue(project_key='ICT', summary='', description='', issue_type='10003'):
    """
...
def verify_board_visibility():
    ...
    try:
        ...
        if board_response.status_code == 200:
            ...
            print(f"보드 표시율: {board_count}/{total_count} = {board_count/total_count*100:.1f}%")
...
def main():
    ...
    if test_issue:
        ...
        if new_board_count and board_count:
            print(f"\\n📊 변화량:")
            print(f"  • 보드 이슈: {board_count} → {new_board_count} (+{new_board_count - board_count})")
            if new_total_count and total_count:
                print(f"  • 전체 이슈: {total_count} → {new_total_count} (+{new_total_count - total_count})")
    else:
        print("❌ 테스트 이슈 생성 실패")
