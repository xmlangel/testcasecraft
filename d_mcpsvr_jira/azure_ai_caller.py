...
async def generate_response(messages: list, tools: Any = None) -> str:
    ...
    messages_for_processing = (
        messages.copy()
    )  # Operate on a copy to avoid modifying the caller's list
    ...
    if tools:
        ...
        available_tools = [
            {"type": "function", "function": tool} for tool in available_tools
        ]
    else:
        available_tools = []
    ...
    logging.debug(
        "Return the AI response: ".format(response.choices[0].message.content)
    )
