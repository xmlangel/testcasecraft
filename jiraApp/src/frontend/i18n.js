...
const translations = {
  ko: {
    ...
    "settings.help.policy":
      "아틀라시안 정책때문에 https:// 만 입력가능합니다. 발급받은 API 키는 암호화되어 안전하게 저장됩니다.",
    "settings.help.howToGet":
      "API 키는 사용자 프로필 > API 토큰에서 생성할 수 있습니다.",

    "index.title": "테스트 관리",
    "index.description":
      "현재 이슈({issueKey})와 연결된 테스트 케이스를 확인하거나 결과를 입력할 수 있습니다.",
    "index.loading": "이슈 정보 및 설정을 불러오는 중입니다...",
    "index.error.notConfigured":
      "앱 설정이 완료되지 않았습니다. 관리자에게 문의하여 Testcasecraft 앱 설정을 진행해주세요.",
    "index.error.redirect":
      "테스트 결과 페이지를 열 수 없습니다. 내부망 접근 문제일 수 있습니다.",
  ...
  en: {
    ...
    "settings.help.policy":
      "Due to Atlassian policy, only https:// is allowed. Your API key will be encrypted and stored safely.",
    "settings.help.howToGet":
      "You can generate an API key in User Profile > API Token.",

    "index.title": "Test Management",
    "index.description":
      "You can check test cases connected to the current issue({issueKey}) or enter results.",
    "index.loading": "Loading issue information and settings...",
    "index.error.notConfigured":
      "App configuration is not complete. Please contact your administrator to configure the Testcasecraft app.",
    "index.error.redirect":
      "Could not open the test result page. This might be an internal network access issue.",
    "index.button.open": "Check and Enter Test Results (Shortcut)",
    "index.button.connecting": "Connecting...",
    "index.footer.hint":
      "You will be automatically authenticated via a temporary security token.",
  },
};
