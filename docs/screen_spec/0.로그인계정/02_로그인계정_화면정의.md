...
> S0은 세 개의 독립 라우트로 이루어진다. 라우트별로 영역을 따로 정의한다.
...
| 영역 | 이름 | 역할 | 근거 |
|---|---|---|---|
| **A** | 언어 토글 | 화면 우상단 고정. 로그인 전에 언어를 바꾼다 | `Login.jsx:127-129` |
| **B** | 카드 제목 | 모드에 따라 `로그인` / `회원가입` | `Login.jsx:132-142` |
| **C** | 입력 폼 | 로그인 2필드 / 회원가입 5필드 | `Login.jsx:143-206` |
| **D** | 알림 영역 | 오류(빨강) · 안내(초록) 각 1개 | `Login.jsx:207-216` |
| **E** | 액션 영역 | 주 버튼 + 모드 전환 버튼 + 매뉴얼 링크 | `Login.jsx:217-285` |
...
| 요소 | 표시 | 동작 | 근거 |
|---|---|---|---|
| 인라인 언어 토글 | 한국어 / English | 즉시 화면 문구 교체. 로그인 전이므로 서버에 저장하지 않는다 | `Login.jsx:128`(`InlineLanguageToggle`) |
...
`data-testid="login-title"`이 두 모드에서 같다. 테스트는 이 요소의 **텍스트**로 모드를 판별한다.

### 2.3 C. 입력 폼

| # | 필드 | 라벨 키 | 타입 | 모드 | testid | 필수 |
|---|---|---|---|---|---|---|
| 1 | 아이디 | `login.username` | text | 공통 | `login-username-input` | ○ |
| 2 | 비밀번호 | `login.password` | password | 공통 | `login-password-input` | ○ |
| 3 | 비밀번호 확인 | `register.confirm_password` | password | 가입 | `register-confirm-password-input` | ○ |
| 4 | 이름 | `register.name` | text | 가입 | `register-name-input` | — |
| 5 | 이메일 | `register.email` | text | 가입 | `register-email-input` | — |
...
- 이메일은 `type="email"`이 아니라 `text`다. 형식 검사를 화면에서 하지 않는다 (`Login.jsx:194-204`)

**화면이 직접 거르는 오류 2개** (`Login.jsx:71-83`)
...
두 알림은 동시에 하나만 보인다. 모드 전환·재제출 시 둘 다 비운다(`Login.jsx:33-37`).
...
**로딩 중에는 버튼 두 개가 사라지고 원형 진행 표시로 바뀐다** (`Login.jsx:218-221`). 버튼을 비활성만 하지 않고 교체하는 구조라, 테스트에서 로딩 상태를 버튼 존재 여부로 판별한다.

| 요소 | 표시 | 동작 | testid |
|---|---|---|---|
| 매뉴얼 링크 | `사용자 매뉴얼 · User Manual` — 작은 크기, 흐린 색 | `/manual`을 **새 탭**으로 | `login-manual-link` |
...
근거: `EmailVerification.jsx:80-121`. **`USED`를 오류로 다루지 않는다** — 이미 인증을 마친 사용자가 링크를 다시 눌렀을 때 실패 화면을 보이면 무엇이 잘못된 것처럼 읽힌다.

`token` 파라미터가 없으면 검증을 시작하지 않는다(`EmailVerification.jsx:28`).

### 2.7 G. 매뉴얼 뷰어

`/manual` — 인증 없이 열리는 문서 뷰어다. 한국어판과 영문판을 같은 화면에서 전환한다.

| 요소 | 역할 | testid |
|---|---|---|
| 언어 전환 | 한국어 ↔ English | `manual-lang-ko` · `manual-lang-en` |
| 섹션 목록 | 매뉴얼 18개 절 목록 | `manual-section-list` |
| 절 이동 | 이전 절 · 다음 절 | `manual-prev-section` · `manual-next-section` |
| 목록 복귀 | 섹션 목록으로 | `manual-back-to-sections` |

본문은 `GET /api/manual`이 돌려준 마크다운을 렌더한다. 소스는 `components/Settings/ManualViewer.jsx` · `controller/ManualController.java`.
...
| 항목 | 값 | 출처 |
|---|---|---|
| 데모 관리자 | `admin` / `admin123` | `config/DataInitializer.java` — 배포 직후 변경 대상 |
| 캡처용 계정 | 매뉴얼 §1-2 회원가입 폼 캡처 | `images/03_signup_filled.png` |
...
**인증 실패 사유를 나누지 않는다.** 아이디가 없는 것과 비밀번호가 틀린 것을 같은 문구로 답한다(`Login.jsx:57-58`). 계정 존재 여부를 알려주지 않기 위한 처리다.
