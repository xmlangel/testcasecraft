...
## 12. Phase 0.6 — 플랜·실행을 목록 + 상세 2단으로, 팝업·전체화면 제거 (구현 완료)

구현: 2026-07-31 01:45 KST

신규 레이아웃에서 플랜과 실행을 다루는 방식이 다른 화면과 어긋나 있었다.

- 플랜 목록에서 플랜을 고르면 **팝업(Dialog)** 이 화면을 덮었다
- 실행을 고르면 상단 바·좌측 메뉴가 없는 **전체 화면**으로 빠졌다 (`/projects/:id/executions/:execId` 가 별도 라우트)
- 케이스 결과 화면(`.../testcases/:tcId/result`)도 같은 이유로 껍데기 밖이었다

그래서 플랜을 오가며 실행을 만들거나 결과를 보려면 매번 닫고 다시 열어야 했다.

### 12-1. 바뀐 구조

`PlanExecutionWorkspace` 를 신설해 **왼쪽 목록 + 오른쪽 상세 2단**으로 통일했다. 처음에는 3단(목록·실행목록·상세)으로 만들었는데 복잡해서 2단으로 줄였고, 플랜의 실행 목록은 상세 안의 접히는 섹션으로 넣었다.

| 영역 | 왼쪽 | 오른쪽 |
|---|---|---|
| 테스트 플랜 | 플랜 목록(이름 검색·건수) | 플랜 내용(인라인) + `이 플랜의 실행` 섹션 |
| 테스트 실행 | 실행 목록(상태 칩) | 실행 상세 |

- **각 단은 접힌다** — 목록 단은 폭 260 → 44 로, 실행 섹션은 머리만 남기고 접힌다
- 실행을 고르면 오른쪽이 실행 상세로 바뀌고 `플랜으로 돌아가기` 로 되돌아온다
- 실행 만들기는 고른 플랜을 물려받는다
- URL 로 실행·케이스 결과를 열고 들어와도 껍데기 안에서 열린다(`ExecutionDetailRoute` · `CaseResultRoute` 가 신규 레이아웃이면 `AppContent` 로 넘긴다)

기존 레이아웃(가로 탭)을 고른 사용자는 지금까지의 팝업·전체 화면 동작을 그대로 쓴다. `TestPlanForm` 에 `inline` 프롭을 더해 같은 컴포넌트가 두 방식 모두를 처리한다.

### 12-2. 검증

전체 단위 테스트 **441건 통과**(신규 13건 — `PlanExecutionWorkspace.test.jsx`). 목록·상세 전환, 실행 목록 조회, 실행 만들기의 플랜 상속, 두 단의 접기·펴기, URL 초기 선택 이어받기, 조회 실패 알림을 고정했다. 프런트 빌드·`compileJava` 통과.

구현 중 걸러낸 것 하나 — 실행 목록을 불러오는 콜백이 `t`(번역 함수)를 의존성에 두고 있었다. `t` 가 렌더마다 새 함수인 구현에서는 콜백이 매번 새로 만들어져 effect 가 무한히 재실행된다(테스트가 3분 넘게 멈춰서 발견). 메시지 조립을 렌더 시점으로 옮겨 끊었다.

### 12-3. 후속 보강 (2026-07-31 01:50 KST)

- **결과 입력 화면을 레이아웃 안으로** — `TestCaseResultPage` 와 `TestResultForm` 의 `fullPage` 컨테이너가 `100vh` 를 강제해, 라우트를 껍데기 안으로 넘겨도 화면이 상단 바·좌측 메뉴 아래로 한 화면만큼 밀려났다. `embedded` 프롭을 더해 뷰포트 높이를 쓰지 않도록 했다. 기존 레이아웃(전체 화면)은 그대로.
- **실행 목록에 플랜 이름** — 실행 이름 아래에 상태 칩과 함께 그 실행이 속한 플랜 이름을 링크로 보여준다. 눌러 바로 그 플랜으로 넘어가고(`onOpenPlan`), 실행 선택과는 분리돼 있다(`stopPropagation`).

단위 테스트 443건 통과(플랜 링크 2건 추가).

### 12-4. 왼쪽을 트리로 (2026-07-31 01:55 KST)

목록을 평면 리스트에서 **트리**로 바꿨다. 플랜이 부모, 그 플랜의 실행이 자식이다.
...
- 가지를 펼칠 때 그 플랜의 실행을 그때 불러온다(플랜별 캐시). 플랜을 고르면 자동으로 펼쳐진다
- 플랜 행을 누르면 오른쪽이 플랜 내용, 실행 행을 누르면 실행 상세
- URL 로 실행을 열고 들어오면 그 실행이 속한 가지도 함께 펼쳐진다
- 이름 검색은 플랜 이름과 실행 이름을 함께 본다
- 테스트 플랜·테스트 실행 두 영역이 같은 트리를 공유한다

트리가 실행 목록을 보여주므로 상세 안의 실행 목록 섹션은 없애고 `실행 만들기` 입구만 남겼다. 직전에 넣었던 "실행 행에 플랜 이름 링크"도 부모 행이 곧 플랜이라 함께 정리했다.

단위 테스트 441건 통과(트리 전환·가지 접기·URL 초기 선택 반영).

### 12-5. 두 레이아웃의 결 맞추기 (2026-07-31 02:00 KST)

결과 입력 화면과 플랜·실행 화면이 서로 다른 화면처럼 보였다. 원인은 셋이었다.

1. **카드 안에 또 페이지** — 실행 상세(`TestExecutionForm`)가 `PAGE_CONTAINER_SX.main`(페이지 배경 + 최대폭 + `100vh - 64px` 최소높이)을 그대로 써서, 상세 카드 안에 페이지가 한 겹 더 들어간 모양이었다. `embedded` 면 투명 배경 + `p: 2` 만 쓴다.
2. **떠 있는 헤더 바** — 결과 입력 화면의 `TestResultHeader` 가 불투명 배경·블러·그림자에 `position: sticky` 였다. 카드 안에서는 또 하나의 떠 있는 바로 보인다. `embedded` 면 배경·그림자를 걷고 아래 구분선만 남긴다.
3. **글자 규격이 컴포넌트마다 달랐다** — `0.875rem`·`0.65rem`·`subtitle2`·`caption` 이 손으로 흩어져 있어 레이아웃을 바꾸면 크기가 튀었다.

글자는 `styles/layoutConstants.js` 의 **`CHROME_TYPOGRAPHY`** 한 곳에 모았다.

| 토큰 | 쓰임 | 값 |
|---|---|---|
| `paneTitle` | 트리 머리·상세 제목 | 0.8125rem / 700 |
| `navItem` · `navItemSelected` | 탭 라벨 · 사이드바 항목 · 트리 행 | 0.875rem / 400·700 |
| `countBadge` | 개수 배지 | 0.72rem / 600 |
| `statusChip` | 상태 칩 | height 20 / 0.72rem |
| `hint` | 보조 안내 문구 | 0.78rem |

가로 탭과 좌측 메뉴가 같은 토큰을 쓰므로 레이아웃을 바꿔도 글자 크기·굵기가 그대로다. 가로 탭 라벨의 `textTransform` 도 함께 없애 대소문자 처리가 사이드바와 같아졌다.

### 12-6. 플랜 영역과 실행 영역을 갈라놓기 (2026-07-31 22:00 KST)

12-4 에서 두 영역이 같은 트리를 쓰게 했더니 사이드바에서 `테스트플랜` 을 눌러도 `테스트실행` 을 눌러도 왼쪽이 똑같았다. 목록 제목이 `테스트플랜 / 테스트실행` 으로 하나였고, 실행 영역에서도 플랜 행부터 보였다. 차이는 오른쪽 안내 문구 한 줄뿐이라 어디에 들어왔는지 알 수 없었다.

플랜을 짜는 자리와 실행을 굴리는 자리는 찾는 대상이 다르다. 그래서 왼쪽 목록을 영역별로 나눴다.

| | 플랜 영역 | 실행 영역 |
|---|---|---|
| 목록 제목 | `테스트플랜` + 플랜 수 | `테스트실행` + 불러온 실행 수 |
| 목록 내용 | 플랜 트리 (플랜 아래 그 플랜의 실행) | 프로젝트의 실행을 최신순 평면 목록 |
| 행에 붙는 것 | 실행 개수 배지 | 상태 칩 + 소속 플랜 이름 |
| 이름 검색 | 플랜·실행 이름 (불러온 범위에서) | 실행 이름 — 서버 검색 |
| 오른쪽 | 플랜 내용 / 실행 상세 | 실행 상세 |

- 실행 목록은 `GET /api/test-executions/by-project/{projectId}` 를 50건씩 받는다. 정렬(`createdAt DESC`)·이름 검색·페이지를 서버가 처리하고, 더 있으면 `더 보기` 로 이어 붙인다
- 실행 행에는 소속 플랜 이름을 글자로만 붙인다. 누를 데는 실행 행 하나라 헷갈릴 여지가 없다
- 실행을 저장하면 실행 영역에서는 목록 첫 페이지를, 플랜 영역에서는 그 플랜의 가지를 다시 불러온다

i18n 키 3건(한/영): `filterExecution` · `emptyExecutions` · `loadMore`.
검증: 프런트 단위 테스트 443건 통과(신규 2건 — 플랜 이름 표시, 서버 이름 검색), 빌드·`compileJava` 통과.
