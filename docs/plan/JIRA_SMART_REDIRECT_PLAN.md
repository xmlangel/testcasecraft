...
Jira 이슈 페이지에서 TestcaseCraft로 연결할 때, 내부 ID 대신 지라 이슈 키(예: ON-426)를 사용하는 **스마트 리다이렉트(Smart Redirect)** 방식을 도입합니다. 이 방식은 실행 ID가 변경되더라도 지라 측의 링크를 수정할 필요 없이 항상 최신 작업 화면으로 연결해줍니다.
