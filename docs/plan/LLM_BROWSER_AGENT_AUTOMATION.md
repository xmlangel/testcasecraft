...
| 4 | 버튼 문구가 `로컬 QA 에이전트으로 실행` 로 나왔다. `에이전트` 는 받침이 없어 `로` 가 맞다 | 문구를 `{name} 실행` 로 바꿨다. 이름이 사용자 입력이라 라틴 문자·숫자로도 끝날 수 있어 조사 규칙으로는 늘 맞출 수 없다. 부록 D5-1 의 `{이름}으로 실행` 은 이 정정으로 대체된다 |
| 5 | 프런트가 런타임 설정을 `REACT_APP_API_BASE_URL` 에서 찾는데 앱이 다른 포트에 뜨면 연동 조회가 실패한다 | 실패의 기본값이 숨김이라 탭과 버튼이 조용히 사라진다. 설계대로 동작한 것이며 결함은 아니다. 검증할 때는 포트를 맞춘다 |
| 6 | **주소 한 칸으로는 두 시점을 만족할 수 없다.** 서버는 연결 테스트로, 브라우저는 딥링크로 같은 주소를 쓴다 | 도커 안 제품은 `host.docker.internal` 로만 닿고 브라우저는 그 이름을 풀지 못한다(`ERR_NAME_NOT_RESOLVED`). 반대로 `localhost` 를 넣으면 서버가 자기 자신을 가리켜 연결 확인이 깨지고 버튼이 비활성된다. 안내 문구로 덮으려 했으나 **어떤 값을 넣어도 둘 다 만족하지 못하는 것이 실측으로 드러나** `browser_url` 칸을 신설했다. 비면 서버 주소를 쓰므로 운영에서는 비워 둔다 |
| 7 | 딥링크로 열린 화면이 `토큰이 필요하다` 로 거부됐다 | 토큰을 URL 에 실으면 브라우저 이력·리퍼러·접근 로그에 남는다(A4-1 과 같은 이유). 에이전트 앱에 로그인 화면을 만들어 사람이 한 번 입력하면 httpOnly 쿠키로 기억하게 했다. 화면 경로는 토큰이 없으면 307 로 로그인으로 보내고 원래 주소를 `next` 로 보존한다. 되돌아갈 주소는 이 앱 안으로 묶어 열린 리다이렉트를 막는다 |
| 8 | **시험은 통과하는데 실제로는 전 화면이 500 이었다** | `TemplateResponse` 를 옛 시그니처로 불렀고, Starlette 0.47 은 받지만 1.6 은 제거했다. 시험은 시스템 파이썬(0.47.2)으로, 실행은 venv(1.6.0)로 돌아 판본이 갈려 있었다. 새 시그니처로 통일하고 `scripts/test_all.sh` 가 venv 를 쓰도록 고쳤다. 시험 환경과 실행 환경이 갈리면 시험이 통과해도 아무것도 보장하지 못한다 |
| 9 | 봇 계정이 프로젝트 멤버가 아니면 케이스 목록이 0 건이었다 | 결함이 아니다. 오류 문구가 `봇 계정이 이 프로젝트의 CONTRIBUTOR 이상인지 확인한다` 로 원인을 정확히 짚었다. 프로젝트마다 봇을 멤버로 넣어야 한다 |
...
## E6. 자동화 대시보드 경로 신설 (2026-09-03 22:40 KST)

계획서는 결과 적재를 테스트실행 한 경로로만 두고, 자동화 대시보드로 가는 길은
「첨부 불가」라며 보조로 밀어 뒀다(부록 C3). 실제로 써 보니 **두 화면이 서로 다른 표를
읽어서, 테스트실행에만 올리면 자동화 테스트 화면에는 회차가 아예 보이지 않는다.** 자동화 테스트 화면은
JUnit 3계층(결과 → 스위트 → 케이스)만 읽고 테스트실행 표를 보지 않는다.

### 제품에 케이스 첨부 층을 더했다

자동화 결과 케이스에는 파일을 붙일 자리가 없었다. `JunitTestCase.attachments` 열은
있었지만 아무도 채우지 않고 화면도 그리지 않았다. 테스트 결과 첨부 표를 재사용할 수
없었던 이유는 그쪽이 테스트실행 결과를 필수로 요구하는데, 자동화 결과는 테스트실행을
거치지 않고 올라올 수 있어 그 열을 채울 수 없기 때문이다.

| 무엇 | 어디 |
|---|---|
| 첨부 표 | 자동화 결과 케이스에 매인 별도 표. 지운 것은 상태로 남기고 목록에서 뺀다 |
| 올리기·목록·내려받기·지우기 | 결과를 올릴 수 있는 사람이 붙이고 지운다. 내려받기는 조회 권한 |
| 화면 | 케이스 상세에 「첨부」 탭. 이미지는 격자 미리보기, 누르면 확대 |

이미지는 화면에 그대로 그려야 하므로 `inline` 으로, 그 밖은 내려받기로 넘긴다.
같은 이름을 다시 올리면 쌓지 않고 이미 있는 첨부를 되돌려 준다. 실행을 다시 돌려도
같은 파일이 여러 벌 남지 않는다.

### 에이전트가 두 경로에 함께 올린다

실행이 끝나면 판정을 JUnit XML 로 옮겨 발행하고, 올린 케이스를 되찾아 스크린샷을
붙인다. 판정 대응은 다음과 같다.

| 에이전트 판정 | JUnit | 이유 |
|---|---|---|
| PASS | 자식 없음 | 통과는 표시할 것이 없다 |
| FAIL | `failure` | 제품이 실패로 센다 |
| BLOCKED · NOT_RUN | `skipped` | `error` 로 옮기면 제품 결함으로 세는데, 막힌 것은 아직 판정하지 못한 상태다 |

**발행 실패는 실행 판정을 바꾸지 않는다.** 테스트실행 경로의 기록은 이미 끝났고, 부가
경로 실패로 `완료` 를 `오류와 함께 완료` 로 떨어뜨리면 결과가 실제보다 나쁘게 보인다.
발행 관련 문제는 판정과 분리된 알림 칸에 남는다.

### 실측으로 잡은 것 3건

**케이스를 되찾는 열쇠가 조용히 깨졌다.** 서버가 업로드 응답에 케이스 ID 를 담아 주지
않으므로 이름 앞머리에 케이스 표시 ID 를 박아 두고 나중에 되찾는다. 그 자리에 표시
ID 대신 이름까지 들어간 값을 넘겨 앞머리가 `[SHOP-124 틀린 비밀번호]` 가 됐고, 이름은
그럴듯한데 되찾기는 한 건도 하지 못해 첨부가 통째로 사라졌다. 열쇠로 쓸 수 없는 문자가
섞이면 앞머리를 아예 붙이지 않도록 고쳤다. 붙여 놓고 못 읽으면 조용히 없어진다.

**케이스 목록 응답이 페이지 형태다.** 스위트별 케이스 조회가 `content` 에 담아 주는데
`testCases` 만 보고 있어 실제 제품에서 빈손이 나왔다. 목 서버에서는 통했는데, 응답
형태를 내가 정한 대로 흉내낸 탓이다. 스위트 응답이 이미 케이스를 품고 있어 그 목록을 먼저 쓰고,
없을 때만 별도 조회로 내려가며 페이지 형태까지 받는다.

**질의가 없는 속성을 가리켰다.** 첨부가 속한 프로젝트를 찾는 질의에서 자동화 결과의
프로젝트를 연관 객체로 적었는데 실제로는 문자열 열이다. 컴파일은 통과하고 기동 시점에
처음 죽는다. 기존 리포지토리와 같은 형태로 고쳤다.

### 함께 고친 표시 4건

| 무엇이 문제였나 | 어떻게 고쳤나 |
|---|---|
| 상태 칸이 `TMS 접속` 이라 무엇인지 알 수 없었다 | `테스트 관리 제품 접속` 으로 바꿨다. 약어를 화면에 두지 않는다 |
| codex 로그인이 어느 계정인지 밝히지 않았다 | 자격 파일의 신원 토큰에서 계정·이름·요금제를 읽어 함께 보여 준다. 회사 계정과 개인 계정을 헷갈리면 엉뚱한 계정 사용량으로 모델을 쓴다 |
| 시작 시각이 `1788439582` 로 나왔다 | 보는 사람의 시간대로 옮겨 표시한다. 서버와 시간대가 달라도 어긋나지 않는다 |
| 비용이 늘 `$0.000` 이었다 | codex 는 요금제로 청구되어 토큰당 단가가 없다. 금액 자리를 비우고 토큰 수만 기록한다. 0 을 금액으로 보여 주면 공짜로 읽히고, 비용 상한이 이 제공자에서 아무 일도 하지 않는다는 사실까지 감춰진다 |

마지막 항목에서 하나를 더 알아냈다. **codex 는 토큰을 알려 준다.** `--json` 을 붙이면
턴이 끝날 때 입력·출력·캐시 토큰이 이벤트로 나온다. 그전에는 사용량을 아예 받지 않아
스텝 로그에 토큰이 0 으로 남았다. 금액은 여전히 셀 수 없지만 사용량은 실측할 수 있다.

### 스텝 타임라인도 함께 올린다

판정과 화면만 올려서는 사후 분석이 안 된다. 단계마다 수행한 동작과 관찰 결과가
에이전트 쪽에만 남아 있었다. 제품에는 이미 스텝을 그리는 자리가 있었는데(케이스마다
`testSteps` 열과 상세 화면의 스텝 블록) 그 자리로 가는 길이 없었다.

| 무엇이 어디로 | 화면에서 어떻게 보이나 |
|---|---|
| 스텝 순번 | `STEP 1` · `STEP 2` 머리 |
| 에이전트가 생각한 것 | 스텝 본문 |
| 수행한 동작 | 코드 블록, `수행 동작` 이름표 |
| 관찰한 것 | 실제 결과 칸 |
| 주소·판정·소요·화면 유무 | 이름표와 값으로 나란히 |
| 케이스가 적어 둔 기대 결과 | 상세 위쪽 `EXPECTED` 칸 |
| 에이전트가 낸 요약 | 상세 위쪽 `ACTUAL` 칸 |

제품 쪽에서 세 곳을 손봤다.

**파서가 JSON 배열을 그대로 받게 넓혔다.** 기존 경로는 `Step` 속성을 파이프로 나눠
`{순번, 본문}` 두 칸만 만든다. 그 형태로는 관찰·주소·판정을 가를 수 없다. `TestSteps`
속성에 JSON 배열이 오면 그대로 쓰고, 배열이 아니거나 파싱에 실패하면 아무것도 넣지
않는다. 깨진 값을 넣으면 상세 조회가 매번 파싱에 실패해 스텝이 통째로 사라진다.

**상세 화면에 동작 칸을 더했다.** 기존 코드 블록은 이름표가 `SQL / QUERY:` 여서 브라우저
동작에 맞지 않는다. `수행 동작` 이름표를 붙인 칸을 따로 뒀고, 스텝 본문은 서술 문단으로
그린다.

**스위트 출력이 남의 글을 가져오던 것을 고쳤다.** 자식 요소를 찾는 함수가 모든 하위를
훑어서, 스위트에 자기 `<system-out>` 이 없으면 **첫 케이스의 출력**을 스위트 출력으로
가져왔다. 상세 화면은 스위트 출력을 함께 보여 주므로, 케이스마다 「Suite-level System
Out」 자리에 남의 글이 붙었다. 바로 아래 자식만 보게 고쳤다.

### 스텝 자리에 그 순간의 화면을 함께 그린다

글과 그림이 다른 탭에 떨어져 있으면 어느 단계의 화면인지 사람이 맞춰 보아야 한다.
스텝에 첨부 파일명을 실어 짝지을 열쇠를 만들고, 상세 화면이 그 이름으로 첨부를 찾아
스텝 블록 안에 미리보기를 그린다. 눌러 확대하는 경로는 첨부 탭과 같다.

열쇠는 올릴 때와 같은 규칙으로 만든다(경로의 슬래시를 밑줄로 바꾼다). 이름이 어긋나면
스텝에 그림이 붙지 않는데, 그때도 글은 그대로 남으므로 조용히 망가지지는 않는다.

### 화면 폭이 절반만 차지한 원인

`Execution Properties` 의 JSON 이 왼쪽 절반만 쓰고 오른쪽이 비었다. 이 프로젝트는 MUI 7
이고, 새 `Grid` 는 `item` · `xs` 속성을 더는 쓰지 않는다. 그 속성들이 조용히 무시되어
폭이 내용 기준으로 잡혔다. 상세 패널 세 곳이 모두 옛 API 를 쓰고 있었고(속성 두 곳과
첨부 격자), `size` 로 바꿨다. 첨부 격자가 화면 폭에 따라 1~3열로 놓이는 것도 이 수정을
하고 나서야 동작한다.

### 읽기가 어려웠던 것 2건

**근거가 한 줄에 이어 붙었다.** 슬래시로 이어서 화면에 문단 하나로 흘렀다. 줄마다
나누고 `[근거]` · `[오류]` 머리를 붙였다.

**「제품에서 보기」가 테스트실행으로 갔다.** 스텝 타임라인과 화면이 자동화 결과 상세에
함께 있으므로 그곳으로 보낸다. 사후 분석을 한 화면에서 할 수 있다.

### 화면을 눌러 보고서야 드러난 것 2건

앞의 확인은 API 로만 했다. 목록·내려받기·이미지 바이트까지 맞았는데 **브라우저에서는
이미지가 깨졌다.** 내려받기 주소가 인증 헤더를 요구하므로 `<img src>` 를 그대로 걸면
요청이 거부된다. curl 로는 헤더를 붙였으니 통과한 것이고, 브라우저 경로를 확인하지
않은 탓이다. 제품의 다른 첨부 화면은 인증된 요청으로 본문을 받아 blob 주소로 만들어
쓰고 있었다. 그 방식으로 맞췄고 내려받기 단추도 같은 경로로 바꿨다. `<a download>`
역시 헤더를 싣지 못한다. 토큰을 주소에 실어 푸는 길은 쓰지 않았다. 브라우저 이력·
리퍼러·접근 로그에 남는다.

**첨부를 판정으로 가리지 않는다.** 처음에는 테스트실행 경로와 같은 기준을 써서 실패
건만 3장, 통과 건은 1장을 붙였다. 그런데 자동화 화면은 판정을 모으는 자리가 아니라
무엇을 수행했나를 되짚는 자리다. 마지막 한 장만 있으면 어느 단계에서 무엇이 달라졌는지
알 수 없다. 스텝마다 남긴 화면을 전부 붙이고, 제품 상한(한 케이스 30장)을 넘으면 뒤쪽을
남긴다. 실패는 대개 뒤에서 난다. 설명에 `SHOP-123 스텝 2/4` 처럼 순서를 적어 넘긴다.
