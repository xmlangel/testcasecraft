...
> **버전:** v1.0.102 기준
> **작성일:** 2026-05-27 (최종 갱신 2026-07-31)
...
| **테스트를 실행·보고하는 분** | §7 플랜 → §8 실행 → §9 결과 → §10 자동화 |
| **프로젝트를 관리하는 분** (PM) | §6 대시보드 → §17-9 프로젝트 설정 |
| **시스템을 운영하는 분** (관리자) | §17 시스템 관리자 설정 → §16-3 운영 문서 |
| **용어가 낯선 분** | §18 용어 정리 (언제든 펼쳐 두고 참고) |
...
헤더 **바로 아래**에는 현재 위치를 보여주는 **브레드크럼**(`프로젝트 / 샘플 프로젝트 / 대시보드`)과, 프로젝트 내 영역을 전환하는 **탭바**(대시보드 / 테스트케이스 / 테스트 플랜 / 테스트실행 / 테스트결과 / 자동화 테스트 / RAG 문서 / 탐색 세션)가 표시됩니다. 테스트케이스·테스트 플랜·테스트실행 탭 라벨 옆에는 각각의 개수 배지가 함께 표시됩니다.

> **2026-06-06 변경**: **테스트케이스 / 테스트 플랜 / 테스트실행** 탭 라벨 옆에 각각의 개수를 보여주는 배지가 표시됩니다.

> **2026-07-31 변경**: 프로젝트 안의 영역 이동을 **가로 탭**과 **좌측 메뉴** 중에서 고를 수 있습니다. 고르는 곳은 프로필 → 테마 설정 → `메뉴 구조` 이고(§13-7), 기본값은 지금까지의 가로 탭입니다. 상단 바의 전환 아이콘으로도 바로 바꿉니다.

좌측 메뉴를 고르면 영역이 화면 왼쪽에 세로로 놓입니다. 항목과 개수 배지는 가로 탭과 같습니다.

![좌측 메뉴 레이아웃](images/95_sidebar_layout.png)

메뉴 아래쪽 화살표로 접으면 아이콘만 남아 본문이 넓어집니다.

![좌측 메뉴 접힘](images/96_sidebar_collapsed.png)
...
![브레드크럼의 프로젝트 선택기](images/97_breadcrumb_project_switcher.png)
...
![트리 이름·ID·태그 검색](images/100_tree_filter_search.png)
...
![자동화 테스트 연결](images/112_plan_automated_link.png)
...
두 경우 모두 관리자가 [시스템 설정](#17-6-2-시스템-설정--ai-기능색인-켜고-끄기)에서 다시 켜면 그대로 돌아옵니다.
...
화면 언어는 **프로필 → 언어 설정** 탭에서 변경합니다 ([§13-3](#13-3-언어--시간대-설정) 참고). 변경 즉시 화면 전체에 반영되며 별도 저장 버튼 없이 자동 저장됩니다.
...
| 결과 입력 화면을 열어 봤는데 결과가 바뀌어 있음 | v1.0.80 이상에서는 발생하지 않습니다 — 보기만 하면 저장되지 않도록 개선됨 ([§8-2](#8-2-자동-저장--보기만-하면-절대-저장되지-않습니다) 참고). 과거 버전에서 생긴 빈 미실행 기록은 실제 결과를 입력하면 정리됩니다 |
...
| `/projects/{projectId}/settings` | 프로젝트 설정 (일반·멤버·에이전트 연동) | S1 |
...
![성능 메트릭](images/114_performance_metrics.png)
...
![서버 시간·버전 패널](images/115_server_time_panel.png)

- 시간은 **내 프로필의 시간대로 변환해** 보여 주고, 옆의 배지가 어느 시간대인지 알려 줍니다(`UTC`·`KST` 등). 시간대를 설정하지 않았으면 UTC 로 표시됩니다. 결과 기록 시각이 예상과 다를 때 이 배지를 먼저 확인합니다. 시간대 설정은 [§13-3](#13-3-언어--시간대-설정) 에서 바꿉니다.
...
- **AI 기능을 끄면 관련 예약 작업도 함께 중지됩니다.** 다시 켠 뒤에는 [스케줄러 관리](#17-7-스케줄러-관리) 화면에서 직접 켜야 합니다.
...
| 에이전트 주소 | 에이전트 앱 주소. `http` 또는 `https` 로 시작해야 합니다. |
...
켜고 연결이 확인되면 **자동화 테스트** 화면 헤더에 `{이름}으로 실행` 버튼이 생깁니다. 이 버튼은 에이전트 앱을 새 창으로 열 뿐이고, 결과가 제품으로 돌아오는 경로와는 무관합니다. 그래서 버튼이 동작하지 않아도 에이전트 앱에서 직접 실행할 수 있습니다.
...
| **LEAD_DEVELOPER** (리드 개발자) | 위 모두 + 멤버 추가·역할 부여·제거 |
| **PROJECT_MANAGER (PM)** | 위 모두 + 프로젝트 설정 변경 (이름·설명·정렬 순서) |
| **ADMIN** | 시스템 전역 — 조직/사용자/메일/LLM/스케줄러 설정 (§17 참고). 모든 프로젝트의 멤버 역할과 설정도 바꿀 수 있습니다 |

**역할을 지정하는 곳**

프로젝트 화면 상단 우측의 톱니 아이콘을 누르면 **프로젝트 설정**(`/projects/{projectId}/settings`)이 열립니다. 일반·멤버·에이전트 연동 세 탭이 있습니다.
...
| 에이전트 연동 | 외부 QA 에이전트 연결 ([§17-10](#17-10-에이전트-연동-pm)). 서버 스위치가 꺼져 있으면 숨습니다 | PROJECT_MANAGER, ADMIN |
...
| 2026-08-22 | §17-6 LLM 설정을 네 탭 구조로 확장하고 §17-6-1 RAG 공통 문서 · §17-6-2 시스템 설정(AI 기능·색인 켜고 끄기)을 신설. §11 에 기능이 멈춘 상태에서 무엇이 되고 안 되는지 안내 추가. 연결 테스트의 암호화 키 안내 반영. v1.0.117 기준. |
...
| 2026-07-31 | v1.0.102 기준 2026-07-02 이후 UI 변경 반영. A(레이아웃 선택): §3-1 헤더 브레드크럼 변경 + §13-7 테마 설정에 메뉴 구조 선택 추가. B(플랜·실행 2단): §7·§8에 신규 레이아웃 설명 추가 (신규 레이아웃에서만). C(트리 검색): §4-4 트리 필터 갱신 — 이름·ID·태그 검색, 콤마로 복합 검색, 전체 선택 시 보이는 결과만, 신규 이미지 100_tree_filter_search. D(결과 태그): §8 필터 패널에 태그 항목 추가 — 다중 선택·직접 입력·태그 상속. E(첨부파일): §8-1 결과 입력 화면에 테스트케이스 첨부파일 영역 추가. F(연결 항목): 기존 §4에서 처리(2026-07-02 이후 이미지 갱신만). 영문판 동일 섹션·이미지(images_en) 동시 갱신. |
| 2026-08-19 | 프로젝트 설정 화면 신설 반영: §16-4 주소 표에 `/projects/{projectId}/settings` 추가, §18-4 역할표에 「역할을 지정하는 곳」 신설 — 톱니 아이콘 진입, 일반 탭(이름·설명·정렬 순서, PROJECT_MANAGER·ADMIN)과 멤버 탭(멤버 추가·역할 변경·제거, PROJECT_MANAGER·LEAD_DEVELOPER·ADMIN). 백엔드도 함께 조여 프로젝트 설정 변경을 PM·ADMIN 으로 한정했습니다(이전에는 LEAD_DEVELOPER 와 조직 관리자도 통과). 화면 캡처는 아직 없습니다. 영문판 동일 섹션 동시 갱신. |
| 2026-08-23 | 기능 단위 감사에서 검출된 미문서 화면 5건 추가: §7 「자동화 테스트 연결」(플랜 행의 🔗 아이콘·연결 개수 칩, 관리 역할 전용) · §9-1 「미실행·실패 케이스 목록 바로 열기」(통계 카드의 `▼` 항목 클릭 → 폴더 경로·플랜·이동 4열, 기존 QA 총평은 §9-2 로 이동) · §17-2 전체 대시보드 탭 셋과 「성능 메트릭」(CPU·메모리·디스크, 프로젝트·테스트케이스 캐시 적중률, 수동 새로고침) · §17-2-1 「서버 시간·버전 확인」(ADMIN 전용, 왼쪽 아래 시계 아이콘, 프로필 시간대로 변환·배지 표기, 30초 갱신) · §16-2 요청 제한 초과 안내(같은 IP 1초 60회 초과 시 카운트다운 창·[지금 재시도]). 신규 캡처 4장(112~115)과 캡처 STEPS·prepare 콜백 4종 추가. 한국어 이미지 82장을 **ShopFlow(SHOP)** 로 다시 찍었습니다 — 직전까지 ShopFlow EN 데이터로 찍혀 있어 §3-1 브레드크럼 표기도 `ShopFlow / 대시보드` 로 정정했습니다. 배포 문서 절 번호가 바뀌어 `DOCKER_SETUP.md §9` 참조 2곳을 10-2절로 고쳤습니다. 영문판 동일 섹션·이미지(images_en) 동시 갱신. |
| 2026-09-03 | §17-10 「에이전트 연동 (PM)」 신설. 프로젝트 설정에 세 번째 탭이 붙어 외부 QA 에이전트의 이름·주소·토큰·기본 프로필을 등록하고 연결을 확인합니다. 서버 환경변수 `AGENT_INTEGRATION_ENABLED` 가 꺼져 있으면 탭 자체가 보이지 않습니다. §16-4 주소 표와 §18-4 탭 표를 세 탭으로 갱신. 에이전트는 제품 밖의 별도 앱이며 판정과 증거만 테스트실행으로 들어온다는 점, 실행명에 `[AI]` 접두와 `ai-agent` 태그가 붙는다는 점, 소요·비용·비결정성·미지원 시나리오를 함께 고지했습니다. 화면 캡처는 아직 없습니다. 영문판 동일 섹션 동시 갱신. |
