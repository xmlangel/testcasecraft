...
| 2026-07-31 | v1.0.102-dev 기준 2026-07-02 이후 UI 변경 반영. A(레이아웃 선택): §3-1 헤더 브레드크럼 변경 + §13-7 테마 설정에 메뉴 구조 선택 추가. B(플랜·실행 2단): §7·§8에 신규 레이아웃 설명 추가 (신규 레이아웃에서만). C(트리 검색): §4-4 트리 필터 갱신 — 이름·ID·태그 검색, 콤마로 복합 검색, 전체 선택 시 보이는 결과만, 신규 이미지 100_tree_filter_search. D(결과 태그): §8 필터 패널에 태그 항목 추가 — 다중 선택·직접 입력·태그 상속. E(첨부파일): §8-1 결과 입력 화면에 테스트케이스 첨부파일 영역 추가. F(연결 항목): 기존 §4에서 처리(2026-07-02 이후 이미지 갱신만). 영문판 동일 섹션·이미지(images_en) 동시 갱신. |
