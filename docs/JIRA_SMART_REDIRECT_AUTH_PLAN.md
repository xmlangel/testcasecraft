...
> [!IMPORTANT] > **권장 방안: C. 초기 설치 시 API 키 등록 방식**
>
...
> [!NOTE] > `latest-execution-context`는 `JwtAuthenticationFilter`가 `?apiKey=` 파라미터도 읽도록 확장하거나,
...
const App = () => {
  ...
  return (
    <Stack space="space.100">
      ...
      <Text>
        현재 이슈({issueKey})와 연결된 테스트 케이스를 확인하거나 결과를 입력할
        수 있습니다.
      </Text>
