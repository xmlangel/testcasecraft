...
> v1.0.93부터 앱 DB와 RAG DB가 단일 PostgreSQL(pgvector) 인스턴스로 통합되었습니다 (외부 포트 `5434` 하나).

| 서비스                        | 내부 포트 | 외부 포트     | 설명                                                              |
| :---------------------------- | :-------- | :------------ | :---------------------------------------------------------------- |
| **PostgreSQL + pgvector**     | 5432      | **5434**      | 통합 단일 인스턴스 — 앱 DB(`testcase_management`) + RAG DB(`rag_db`) |
...
# 모든 서비스 백그라운드 실행
docker-compose -f docker-compose.yml up -d postgres minio rag-service
...
`React Frontend` → `Spring Boot (8080)` → `FastAPI RAG Service (8001)` → `PostgreSQL+pgvector (5434, rag_db)` + `MinIO (9000)`
