...
- **드라이버/ORM:** `org.postgresql:postgresql:42.7.11`, Spring Data JPA/Hibernate. Flyway 의존성은 있으나 **비활성**(`application.yml: flyway.enabled=false`) — 실제 스키마는 `hibernate.ddl-auto: update`가 만든다 (P0 감사 확인)
...
2. Flyway는 의존성만 있고 비활성 상태다(감사 확인). 스키마는 ddl-auto:update가 관리하므로, 신규 컬럼은 엔티티 필드 추가(기본값 포함)로 반영한다. Flyway 전환은 별도 과제로 뺀다.
...
└──────────┼─────────────────────┼─────────────────────────┼─────────────┘
   ...
   │  pgvector/pg18 │   │  (PG16, 포트 5437)     │  MERGE(Cypher)
...
- 신규 컴포넌트 `skaiworldwide/agensgraph:v2.16.0-bookworm` 는 compose에 별도 서비스로 고정 태깅.
...
```yaml
  agensgraph:
    image: skaiworldwide/agensgraph:v2.16.0-bookworm
...
## 15-B. P0 검증 결과 (2026-07-04)

- 이미지: Docker Hub에 `2.16.9` 태그는 없음. **`skaiworldwide/agensgraph:v2.16.0-bookworm`** 사용 — 내부 확인 결과 `PostgreSQL 16.9 (AgensGraph 2.16)`.
- 포트: 개발 머신에서 5435(v2.17.0)·5436(2.18-devel)을 이미 다른 AgensGraph 컨테이너가 점유 → 기본 포트를 **5437**로 변경 (env로 조정 가능).
- Cypher 검증: `CREATE GRAPH` / `VLABEL` / `ELABEL` / `MERGE`(멱등) / `MATCH` 패턴 / `CREATE PROPERTY INDEX` 전부 정상.
- 접속: 호스트 5437에 표준 PG 와이어 프로토콜로 접속 확인 (JDBC 경로 유효).
- 직렬화: vertex는 `Label[graphid]{json}`, edge는 `Label[graphid][start,end]{json}` 텍스트 형식 — GraphResponse 파서는 이 형식을 처리한다.
- 감사 반영: Flyway 비활성 확인 → P3의 컬럼 추가는 엔티티 필드(+기본값 `BASIC`)로 처리. 두 번째 DataSource는 JPA 자동구성을 건드리지 않도록 그래프 전용 풀을 자체 보유하는 컴포넌트로 만든다.
