# Development Guide (최종 업데이트: 2026-02-06)
...
const ExampleComponent = ({ projectId }) => {
  ...
  return <Box>{/* Material-UI 컴포넌트 사용 */}</Box>;
};
...
모든 작업은 **ICT** 프로젝트 JIRA 이슈와 연동되어 진행됩니다. 상세 가이드는 **[docs/JIRA_INTEGRATION.md](./JIRA_INTEGRATION.md)**를 참조하세요.
...
- **[E2E 테스트 가이드](./E2E_TESTING_GUIDE.md)** - E2E 테스트 작성 및 실행- **[JIRA 통합 가이드](./JIRA_INTEGRATION.md)** - JIRA 이슈 관리 및 협업 프로세스
