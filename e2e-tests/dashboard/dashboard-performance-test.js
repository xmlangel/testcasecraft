...
test.describe('대시보드 성능 테스트', () => {
  ...
  test('대시보드 API 동시 호출 성능 테스트', async () => {
    ...
    concurrentResponses.forEach((response, index) => {
      expect(response.status).toBe(200);
      console.log(`✅ ${apiEndpoints[index].split('/').pop()}: ${response.status}`);
    });

    // 동시 호출이 단일 호출보다 효율적인지 확인
    const totalSingleTime = singleCallResults.reduce((sum, result) => sum + result.responseTime, 0);
    const efficiency = ((totalSingleTime - totalConcurrentTime) / totalSingleTime) * 100;
  ...
  test('대시보드 차트 렌더링 성능 테스트', async ({ page }) => {
    ...
    try {
      ...
      const dashboardCharts = page.locator('svg').filter({ hasNot: page.locator('[aria-label*="devtools"]') });
  ...
  test('대시보드 메모리 사용량 테스트', async ({ page }) => {
    ...
    const initialMemoryMB = initialMemory.usedHeapSize ? (initialMemory.usedHeapSize / 1024 / 1024) : 0;
    ...
    const afterLoadMemoryMB = afterLoadMemory.usedHeapSize ? (afterLoadMemory.usedHeapSize / 1024 / 1024) : 0;
    ...
    const finalMemoryMB = finalMemory.usedHeapSize ? (finalMemory.usedHeapSize / 1024 / 1024) : 0;
  ...
  test('대시보드 대용량 데이터 처리 성능 테스트', async () => {
    ...
    for (const periodTest of largePeriodTests) {
      ...
      const startTime = Date.now();
      const response = await fetch(`http://localhost:8080/api/dashboard/projects/${projectId}/test-results-trend?days=${periodTest.days}`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Content-Type': 'application/json'
        }
      ...
      console.log(`📊 ${periodTest.name} 응답시간: ${responseTime}ms, 데이터 포인트: ${data.dataCount}개`);
    ...
    const concurrentLargeRequests = largePeriodTests.map(periodTest =>
      fetch(`http://localhost:8080/api/dashboard/projects/${projectId}/test-results-trend?days=${periodTest.days}`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Content-Type': 'application/json'
        }
  ...
  test('대시보드 반응형 성능 테스트', async ({ page }) => {
    ...
    for (const viewport of viewportSizes) {
      ...
      console.log(`📊 ${viewport.name} - 차트: ${svgCount}개, 위젯: ${paperCount}개`);
    }
