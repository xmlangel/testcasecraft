...
async function takeSuccessScreenshot(page, testName) {
  ...
  console.log(`📸 성공 스크린샷 저장: test-results/success-screenshots/${fileName}`);
}
...
test.describe('프로젝트 선택 및 대시보드 이동 E2E 테스트', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    page.on('console', msg => {
      if (msg.type() === 'error' || msg.text().includes('[Dashboard]') || msg.text().includes('Dashboard API')) {
    ...
    page.on('request', request => {
      if (request.url().includes('/api/dashboard/') || request.url().includes('/api/projects')) {
        console.log(`📤 API Request: ${request.url()} ${request.method()}`);
      }
    });
    
    page.on('response', response => {
      if (response.url().includes('/api/dashboard/') || response.url().includes('/api/projects')) {
  ...
  test('로그인 후 프로젝트 목록 표시 및 선택 테스트', async ({ page }) => {
    ...
    // 3. 대시보드 통계 카드 확인
    const statisticsCards = await page.locator('.MuiGrid-item', { hasText: /총 조직 수|총 프로젝트 수|총 테스트케이스/ });
    await expect(statisticsCards.first()).toBeVisible({ timeout: 10000 });
    console.log('✅ 대시보드 통계 카드 표시 확인');
    
    // 4. 프로젝트 선택 버튼 확인 (상단 네비게이션)
    const projectSelectionButton = await page.locator('button:has-text("프로젝트 선택")');
    ...
    // 6. 프로젝트 목록 페이지 로딩 확인
    await page.waitForSelector('h5:has-text("프로젝트 선택")', { timeout: 10000 });
  ...
  test('프로젝트 변경 시 대시보드 데이터 업데이트 테스트', async ({ page }) => {
    ...
    try {
      ...
      const selectButton = await page.locator('.MuiSelect-select, [role="combobox"], select').first();
      if (await selectButton.isVisible()) {
        await selectButton.click();
        console.log('✅ 프로젝트 선택 드롭다운 열기');
        
        // 옵션 목록 대기
        await page.waitForSelector('.MuiMenuItem-root, option', { timeout: 3000 });
  ...
  test('대시보드 API 호출 및 데이터 로딩 테스트', async ({ page }) => {
    ...
    page.on('response', response => {
      if (response.url().includes('/api/dashboard/')) {
        apiCallCount++;
        console.log(`📈 Dashboard API 호출 #${apiCallCount}: ${response.status()} ${response.url()}`);
    ...
    if (apiCallCount === 0) {
      console.log('⚠️ Dashboard API 호출이 없음 - 권한 문제 또는 토큰 이슈 가능성');
    }
  ...
  test('권한 오류 발생 시 사용자 안내 테스트', async ({ page }) => {
    ...
    // 콘솔 오류 모니터링
    page.on('console', msg => {
      if (msg.type() === 'error' && (msg.text().includes('403') || msg.text().includes('Forbidden'))) {
    ...
    const hasErrorMessage = await page.locator(':has-text("권한"), :has-text("접근"), :has-text("로그인")').count() > 0;
