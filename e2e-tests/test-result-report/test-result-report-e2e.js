...
test.describe('ICT-191: 테스트 결과 리포트 E2E 테스트', () => {
  ...
  test('3. 테스트 결과 테이블 데이터 로딩 테스트', async () => {
    ...
    for (const header of tableHeaders) {
      await expect(page.locator(`text=${header}`).first()).toBeVisible({ timeout: 10000 });
    }
  ...
  test('5. 테스트 결과 내보내기 기능 테스트', async () => {
    ...
    // Export 버튼 찾기
    const exportButton = page.locator('button', { hasText: /내보내기|Export|다운로드/i });
    ...
    if (await exportButton.isVisible().catch(() => false)) {
      ...
      // 내보내기 다이얼로그 확인
      const dialogExists = await page.locator('[role="dialog"], .MuiDialog-root').isVisible().catch(() => false);
      ...
      // 다이얼로그 닫기
      const closeButton = page.locator('button', { hasText: /취소|닫기|Close/i });
  ...
  test('7. 페이징 기능 테스트', async () => {
    ...
    if (!paginationFound) {
      console.log('ℹ️ 페이징 요소를 찾을 수 없음 (데이터가 적거나 다른 UI 패턴)');
    }
  ...
  test('10. 통합 시나리오 테스트', async () => {
    ...
    // 3. 통계 정보 확인
    const statsVisible = await page.locator('text=총').isVisible().catch(() => false);
    if (statsVisible) {
