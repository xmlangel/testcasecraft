...
test.describe('향상된 프로젝트 컨텍스트 관리 E2E 테스트', () => {
  ...
  async function selectProjectWithValidation(page, projectIndex = 0) {
    ...
    if (projectCount <= projectIndex) {
      throw new Error(`프로젝트 인덱스 ${projectIndex}가 사용 가능한 프로젝트 수 ${projectCount}를 초과합니다.`);
    }

    const targetProject = projects.nth(projectIndex);
    const projectTitle = await targetProject.locator('h6, h5, [class*="title"], .MuiTypography-h6, .MuiTypography-h5').first().textContent();
    
    console.log(`📁 프로젝트 선택: ${projectTitle} (인덱스: ${projectIndex})`);

    // 선택 전 컨텍스트 상태 수집
    const beforeState = await getContextState(page);
    console.log('📊 선택 전 컨텍스트:', JSON.stringify(beforeState.localStorage.selectedProject));
    ...
    console.log('📊 선택 후 컨텍스트:', JSON.stringify(afterState.localStorage.selectedProject));
    
    // 컨텍스트 변경 검증
    const contextChanged = beforeState.localStorage.selectedProject !== afterState.localStorage.selectedProject ||
  ...
  test('프로젝트 컨텍스트 동기화 및 일관성 테스트', async ({ page }, testInfo) => {
    ...
    console.log(`📁 첫 번째 프로젝트 선택 완료: ${firstSelection.projectTitle}`);
    ...
    if (projectCount > 1) {
      ...
      console.log(`📁 두 번째 프로젝트 선택 완료: ${secondSelection.projectTitle}`);
  ...
  test('프로젝트 전환 시 데이터 격리 및 독립성 테스트', async ({ page }, testInfo) => {
    ...
    if (projectCount >= 2) {
      ...
      const firstProjectData = await page.evaluate(() => {
        return {
          ...
          domElements: document.querySelectorAll('[data-testid*="project"], [class*="project"]').length,
          dataAttributes: Array.from(document.querySelectorAll('[data-project-id]')).map(el => el.getAttribute('data-project-id'))
        };
      });
      ...
      // 데이터 격리 검증
      const dataIsolated = firstProjectData.urlPath !== secondProjectData.urlPath ||
                          firstProjectData.pageContent !== secondProjectData.pageContent ||
                          JSON.stringify(firstProjectData.dataAttributes) !== JSON.stringify(secondProjectData.dataAttributes);
      ...
      const projectSpecificCalls = Array.from(apiCalls.keys()).filter(url => 
  ...
  test('프로젝트 컨텍스트 복원 및 회복력 테스트', async ({ page }, testInfo) => {
    ...
    const errorHandling = await page.evaluate(() => {
      const hasErrorAlert = document.querySelector('.MuiAlert-root[severity="error"], .error-message, [data-testid="error-alert"]');
      return {
    ...
    try {
      await page.waitForSelector('[data-testid="project-card"], .MuiCard-root', { timeout: 5000 });
      const projects = page.locator('[data-testid="project-card"], .MuiCard-root');
    ...
    console.log('🔄 최종 복원 상태:', {
      ...
      canAccessProjects: recoveredContext.url.includes('dashboard') || recoveredContext.url.includes('projects')
    });
  ...
  test('다중 탭 간 프로젝트 컨텍스트 동기화 테스트', async ({ page, context }, testInfo) => {
    console.log('📑 다중 탭 컨텍스트 동기화 테스트 시작...');

    // 첫 번째 탭에서 로그인 및 프로젝트 선택
    await loginAsAdmin(page);
    const firstProject = await selectProjectWithValidation(page, 0);
    console.log(`📁 첫 번째 탭에서 프로젝트 선택: ${firstProject.projectTitle}`);
    ...
    // localStorage 공유 확인
    const contextShared = tab1Context.localStorage.accessToken === tab2Context.localStorage.accessToken &&
                          tab1Context.localStorage.selectedProject === tab2Context.localStorage.selectedProject;
    ...
    const projects = page2.locator('[data-testid="project-card"], .MuiCard-root');
    const projectCount = await projects.count();
    
    if (projectCount > 1) {
      const secondProject = await selectProjectWithValidation(page2, 1);
      console.log(`📁 두 번째 탭에서 다른 프로젝트 선택: ${secondProject.projectTitle}`);
      ...
      console.log('📊 업데이트된 탭1 컨텍스트:', updatedTab1Context.localStorage.selectedProject);

      // 실시간 동기화 여부 확인
      const realtimeSync = updatedTab1Context.localStorage.selectedProject === tab2Context.localStorage.selectedProject;
      
      if (realtimeSync) {
        console.log('⚡ 실시간 탭 간 컨텍스트 동기화 확인');
      } else {
        console.log('⏳ 탭 간 컨텍스트 동기화가 지연되거나 수동 새로고침이 필요할 수 있음');
        ...
        console.log('🔄 새로고침 후 탭1 컨텍스트:', refreshedTab1Context.localStorage.selectedProject);
      }
    }
  ...
  test('프로젝트 컨텍스트 성능 및 최적화 테스트', async ({ page }, testInfo) => {
    ...
    if (projectCount >= 2) {
      ...
      if (switchingTimes.length > 0 && cacheTestTime < switchingTimes[0] * 0.8) {
