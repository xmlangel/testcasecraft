...
에이전트 entries 를 그대로 시드하지 말 것. **코드에 실제 존재하는 t() 키만** 시드한다 — 파일을 안 고치고 JSON 만 만든 에이전트의 고아 엔트리를 걸러낸다 (2026-06-06 사례: 262건 차단). 참조 구현: `_workspace/i18n-audit/merge_entries.py`. 코드에 있는데 entries 에 없는 키는 미번역 목록으로 뽑아 직접 번역한다.
