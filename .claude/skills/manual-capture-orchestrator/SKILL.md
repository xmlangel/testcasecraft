...
`_workspace/manual-capture/` 존재 여부와 사용자 요청 의도를 매칭해 실행 모드를 결정:

| 상황 | 분기 |
|------|------|
| `_workspace/manual-capture/` 미존재 | **초기 실행** — 모든 Phase 정상 진행 |
...
산출물 디렉토리 명명:
```
_workspace/manual-capture/run-2026-05-28/
_workspace/manual-capture/run-prev/  ← 직전 실행 보존
```
...
```bash
RUN_DIR="_workspace/manual-capture/run-$(date +%Y-%m-%d)"
mkdir -p "$RUN_DIR"
...
Agent({
  ...
  prompt: """
    ...
    입력:
    - 감사 리포트: _workspace/manual-capture/run-{date}/audit_report.txt
    - 캡처 로그:   _workspace/manual-capture/run-{date}/capture_and_audit.log
    - 신규 STEPS 추가 결과 (Phase 3 산출): _workspace/manual-capture/run-{date}/steps_added.md
    ...
    3. 라우트 제거된 경우 본문 정리
    4. 결과를 _workspace/manual-capture/run-{date}/manual_diff.md 에 보고
...
이전 산출물(`_workspace/manual-capture/run_prev/`)이 있을 때:
