-- RAG 스키마 (통합 인스턴스의 rag_db 에 적용)
-- pgvector 확장(CREATE EXTENSION)은 상위 01-init-rag.sh 가 슈퍼유저 권한으로 먼저 생성한다.
-- 이 파일은 rag_user 로 실행되어 테이블/인덱스/함수/트리거를 rag_user 소유로 만든다.
