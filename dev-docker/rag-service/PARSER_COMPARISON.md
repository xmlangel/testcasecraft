...
# Options: "upstage" (cloud API), "local" (PyPDF2/python-docx), "auto" (fallback)
...
| `local` | 로컬 강제 | 항상 PyPDF2/python-docx 사용 (무료) |
...
2. **텍스트 정규화**: PyPDF2는 단순 텍스트 추출로 일부 서식 손실
...
└── _parse_with_local()          # 로컬 파서 (PyPDF2/python-docx)
```
