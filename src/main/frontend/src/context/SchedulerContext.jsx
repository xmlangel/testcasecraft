...
export const SchedulerProvider = ({ children }) => {
  ...
  const fetchConfigs = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "스케줄 설정 조회에 실패했습니다.",
      );
      throw err;
    } finally {
      setLoading(false);
  ...
  const updateConfig = useCallback(async (taskKey, updateData) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message || "스케줄 설정 업데이트에 실패했습니다.";
  ...
  const toggleEnabled = useCallback(async (taskKey) => {
    ...
    try {
      const response = await axiosInstance.post(
        `${API_BASE_URL}/configs/${taskKey}/toggle`,
      );
      ...
      setError(
        err.response?.data?.message || "스케줄 상태 변경에 실패했습니다.",
      );
      throw err;
    } finally {
      setLoading(false);
  ...
  const executeNow = useCallback(
    async (taskKey) => {
      ...
      try {
        const response = await axiosInstance.post(
          `${API_BASE_URL}/configs/${taskKey}/execute`,
        );
