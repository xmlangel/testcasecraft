...
export const TestProvider = ({ children }) => {
  ...
  const fetchProjectTestCases = useCallback(
    async (projectId) => {
      if (!projectId) return;
      ...
      const promise = (async () => {
        try {
          ...
          debugLog(
            "TestContext",
            "fetchProjectTestCases 시작 - 프로젝트 ID:",
            projectId,
          );
          const baseUrl = await getApiBaseUrl();
          const res = await api(
            `${baseUrl}/api/testcases/project/${projectId}`,
          );
          if (!res.ok) throw new Error("Failed to fetch test cases");
          const data = await res.json();
          debugLog(
            "TestContext",
            "fetchProjectTestCases 완료 - 프로젝트 ID:",
            projectId,
            ", 테스트케이스 수:",
            data.length,
          );
  ...
  const updateTestCase = async (testCase) => {
    if (!testCase || !testCase.id) {
      console.error(
        "Error updating test case: Invalid testCase data",
        testCase,
      );
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errorData.message ||
            errorData.error ||
            `Failed to update test case (${res.status})`,
        );
      }
      const updated = await res.json();
      const payload =
        updated && updated.data && updated.data.id ? updated.data : testCase;

      setTestCases((prev) =>
        prev.map((tc) =>
          tc.id === payload.id
            ? { ...tc, ...payload, updatedAt: new Date().toISOString() }
            : tc,
        ),
      );
    } catch (error) {
  ...
  const deleteTestCase = async (id) => {
    try {
      ...
      if (!res.ok) {
        if (res.status === 404) {
          console.warn(
            "[TestContext] 테스트케이스가 이미 삭제되었거나 존재하지 않습니다:",
            id,
          );
        } else {
          let errorMsg = "Failed to delete test case";
          try {
            errorMsg = (await res.json()).message || errorMsg;
          } catch {}
      ...
      setTestPlans((prev) =>
        prev.map((plan) => ({
          ...
          testCaseIds: plan.testCaseIds
            ? plan.testCaseIds.filter((tid) => !idsToDelete.includes(tid))
            : [],
        })),
      );
    } catch (error) {
  ...
  const fetchTestPlans = useCallback(
    async (projectId) => {
      if (!projectId) return;
      ...
      const promise = (async () => {
        try {
          ...
          const baseUrl = await getApiBaseUrl();
          const res = await api(
            `${baseUrl}/api/test-plans/project/${projectId}`,
          );
  ...
  const updateTestPlan = async (testPlan) => {
    try {
      ...
      setTestPlans((prev) =>
        prev.map((p) =>
          p.id === testPlan.id
            ? { ...p, ...testPlan, updatedAt: new Date().toISOString() }
            : p,
        ),
      );
    }
  ...
  const deleteTestPlan = async (id) => {
    try {
      ...
      setTestExecutions((prev) =>
        prev.filter((exec) => exec.testPlanId !== id),
      );
    } catch (error) {
    ...
    async (projectId) => {
      ...
      const promise = (async () => {
        try {
          const baseUrl = await getApiBaseUrl();
          const res = await api(
            `${baseUrl}/api/test-executions/by-project/${projectId}`,
          );
  ...
  const addOrUpdateTestExecution = async (execution) => {
    ...
    if (execution.id) {
      setTestExecutions((prev) =>
        prev.map((exec) =>
          exec.id === saved.id
            ? { ...exec, ...saved, updatedAt: new Date().toISOString() }
            : exec,
        ),
      );
    } else {
  ...
  const updateTestExecution = (testExecution) => {
    setTestExecutions((prev) =>
      prev.map((exec) =>
        exec.id === testExecution.id
          ? { ...exec, ...testExecution, updatedAt: new Date().toISOString() }
          : exec,
      ),
    );
  };
  ...
  const restartTestExecution = async (id) => {
    try {
      ...
      setTestExecutions((prev) =>
        prev.map((exec) =>
          exec.id === updated.id
            ? { ...exec, ...updated, updatedAt: new Date().toISOString() }
            : exec,
        ),
      );
      return updated;
  ...
  const fetchRecentTestResults = async (limit = 10) => {
    try {
      const baseUrl = await getApiBaseUrl();
      const res = await api(
        `${baseUrl}/api/dashboard/recent-test-results?limit=${limit}`,
      );
  ...
  const fetchRecentTestResultsByProject = async (projectId, limit = 10) => {
    try {
      const baseUrl = await getApiBaseUrl();
      const res = await api(
        `${baseUrl}/api/dashboard/projects/${projectId}/recent-test-results?limit=${limit}`,
      );
      if (!res.ok)
        throw new Error("프로젝트별 최근 테스트 결과를 불러오지 못했습니다.");
      return await res.json();
    } catch (error) {
  ...
  const fetchRecentTestResultsByTestPlan = async (testPlanId, limit = 10) => {
    try {
      const baseUrl = await getApiBaseUrl();
      const res = await api(
        `${baseUrl}/api/dashboard/test-plans/${testPlanId}/recent-test-results?limit=${limit}`,
      );
      if (!res.ok)
        throw new Error(
          "테스트 플랜별 최근 테스트 결과를 불러오지 못했습니다.",
        );
      return await res.json();
    } catch (error) {
  ...
  const fetchOpenTestRunAssigneeResults = async (limit = 20) => {
    try {
      const baseUrl = await getApiBaseUrl();
      const res = await api(
        `${baseUrl}/api/dashboard/open-test-runs/assignee-results?limit=${limit}`,
      );
      if (!res.ok)
        throw new Error("오픈 테스트런 담당자별 결과를 불러오지 못했습니다.");
      return await res.json();
    } catch (error) {
  ...
  const fetchOpenTestRunAssigneeResultsByProject = async (
    projectId,
    limit = 20,
  ) => {
    try {
      const baseUrl = await getApiBaseUrl();
      const res = await api(
        `${baseUrl}/api/dashboard/projects/${projectId}/open-test-runs/assignee-results?limit=${limit}`,
      );
      if (!res.ok)
        throw new Error(
          "프로젝트별 오픈 테스트런 담당자별 결과를 불러오지 못했습니다.",
        );
      return await res.json();
    } catch (error) {
      console.error(
        "Error fetching open test run assignee results by project:",
        error,
      );
      throw error;
    }
