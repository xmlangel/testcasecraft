 ...
 */
export const exportTestResultToCSV = async (
  testResult,
  testSuites = [],
  allTestCases = [],
) => {
  try {
    ...
    csvContent += `테스트 실행 시간,${formatDateForCsv(
      testResult.uploadedAt,
    )}\n`;
    csvContent += `실행자,${escapeCsvText(
    ...
    csvContent += `Total Execution Time,${formatDurationForCsv(
      testResult.time || 0,
    )}\n`;
    ...
    if (testSuites && testSuites.length > 0) {
      ...
      csvContent +=
        "번호,스위트명,총 테스트,통과,실패,오류,스킵,성공률,실행시간,패키지명,상태\n";
      ...
      testSuites.forEach((suite, index) => {
        ...
        const suiteSuccessRate =
          suiteExecuted > 0
            ? ((suitePassed / suiteExecuted) * 100).toFixed(1)
            : 0;
        const suiteStatus =
          (suite.failures || 0) > 0 || (suite.errors || 0) > 0
            ? "FAILED"
            : "PASSED";
    ...
    if (allTestCases && allTestCases.length > 0) {
      ...
      csvContent +=
        "번호,테스트케이스명,클래스명,상태,실행시간,실패메시지,스택트레이스,SystemOut,SystemErr,사용자노트\n";
    ...
    if (passedCases.length === 0) {
      ...
      csvContent +=
        "번호,테스트케이스명,클래스명,상태,실행시간,SystemOut,SystemErr,사용자노트\n";
    ...
    const failedCases = allTestCases.filter(
    ...
    if (failedCases.length === 0) {
      ...
      csvContent +=
        "번호,테스트케이스명,클래스명,상태,실행시간,실패타입,실패메시지,스택트레이스\n";
    ...
    if (skippedCases.length > 0) {
      ...
      csvContent +=
        "번호,테스트케이스명,클래스명,상태,실행시간,스킵메시지,사용자노트\n";
