...
export const projectHelpers = {
  ...
  getProjectsByOrganization: (organizationId) => {
    ...
    return demoProjectsData.filter(
  ...
  getProjectStats: () => {
    ...
    const withOrganization = demoProjectsData.filter(
      (p) => p.organization,
    ).length;
    const independent = total - withOrganization;
    const totalTestCases = demoProjectsData.reduce(
      (sum, p) => sum + p.testCaseCount,
      0,
    );
    const totalMembers = demoProjectsData.reduce(
