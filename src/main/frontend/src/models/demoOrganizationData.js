...
export const organizationHelpers = {
  ...
  getOrganizationById: (organizationId) => {
    return demoOrganizationsData.organizations.find(
  ...
  // 전체 프로젝트 목록 조회 (조직 + 독립)
  getAllProjects: () => {
    const orgProjects = Object.values(
      demoOrganizationsData.organizationProjects,
    ).flat();
  ...
  getMemberActivityRanking: (organizationId = null, limit = 5) => {
    ...
    if (organizationId) {
      const org = demoOrganizationsData.organizations.find(
  ...
  getOverallStats: () => {
    ...
    const totalTestCases = allProjects.reduce(
      (sum, proj) => sum + proj.testCaseCount,
      0,
    );
    ...
    const totalCompleted = allStats.reduce(
      (sum, stat) => sum + stat.completedTests,
      0,
    );
    const totalFailed = allStats.reduce(
      (sum, stat) => sum + stat.failedTests,
      0,
    );
    const totalBlocked = allStats.reduce(
      (sum, stat) => sum + stat.blockedTests,
      0,
    );
    const totalNotRun =
      totalTestCases - totalCompleted - totalFailed - totalBlocked;
