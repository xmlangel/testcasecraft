...
class JunitVersionService {
  ...
  async createVersion(testResultId, originalFilePath, description = "") {
    try {
      ...
      const response = await fetch(`${baseURL}/${testResultId}/versions`, {
        method: "POST",
        headers: {
      ...
      throw new Error(error.message || "버전 생성 중 오류가 발생했습니다.");
    }
  }
  ...
  async getVersionHistory(testResultId) {
    try {
      const baseURL = await this.getBaseURL();
      const response = await fetch(`${baseURL}/${testResultId}/history`, {
        method: "GET",
      ...
      throw new Error(error.message || "히스토리 조회 중 오류가 발생했습니다.");
    }
  }
  ...
  async restoreVersion(testResultId, versionNumber, targetPath) {
    try {
      ...
      throw new Error(error.message || "버전 복원 중 오류가 발생했습니다.");
    }
  }
  ...
  async compareVersions(testResultId, version1, version2) {
    try {
      ...
      throw new Error(error.message || "버전 비교 중 오류가 발생했습니다.");
    }
  }
  ...
  async createBackup(testResultId, versionNumber = -1) {
    try {
      ...
      throw new Error(error.message || "백업 생성 중 오류가 발생했습니다.");
    }
  }
  ...
  async getStorageStatistics() {
    try {
      const baseURL = await this.getBaseURL();
      const response = await fetch(`${baseURL}/storage/statistics`, {
        method: "GET",
        headers: this.getAuthHeaders(),
      });
      ...
      throw new Error(error.message || "통계 조회 중 오류가 발생했습니다.");
    }
  }
  ...
  getOptimizationSuggestions(statistics) {
    ...
    if (
      statistics.totalStorageSize > 1024 * 1024 * 1024 &&
      !statistics.compressionEnabled
    ) {
      suggestions.push({
        type: "compression",
        priority: "high",
