...
class JiraService {
  ...
  async apiRequest(url, options = {}, subPath = "/api/jira") {
    ...
    try {
      ...
      console.error(
        `JIRA API 요청 실패 [${options.method || "GET"}] ${url}:`,
        error,
      );
      throw error;
    }
      ...
      if (error.message) {
        if (error.message.includes("400")) {
          enhancedError = new Error(
            "입력 데이터를 확인해주세요. 필수 필드가 누락되었거나 형식이 올바르지 않습니다.",
          );
          ...
          enhancedError = new Error(
            "서버 암호화 설정에 문제가 있습니다. 관리자에게 문의하세요.",
          );
        } else if (
  ...
  async getIssueDetails(issueKey) {
    try {
      ...
      const internalDetail = await this.apiRequest(
        `/issues/${issueKey}`,
        { method: "GET" },
        "/api/jira-status",
      );
      ...
      console.log(
        `Internal JIRA status not found for ${issueKey}, checking existence on JIRA server...`,
      );
  ...
  async getAllTestResultsByIssue(jiraIssueKey) {
    try {
      ...
      const apiUrl = await getDynamicApiUrl();
      const url = `${apiUrl}/api/jira-integration/all-test-results-by-issue?jiraIssueKey=${encodeURIComponent(
        jiraIssueKey,
      )}`;
