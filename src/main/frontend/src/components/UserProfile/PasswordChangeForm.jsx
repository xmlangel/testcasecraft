...
function PasswordChangeForm({ onSuccess, onError }) {
  ...
  const validatePassword = (password) => {
    ...
    if (password.length < 8) {
      errors.push(
        t("password.validation.minLength", "최소 8자 이상이어야 합니다"),
      );
    }

    if (password.length > 100) {
      errors.push(
        t("password.validation.maxLength", "최대 100자까지 입력 가능합니다"),
      );
    }
    ...
    if (field === "newPassword") {
      ...
      if (form.confirmPassword && value !== form.confirmPassword) {
        newErrors.confirmPassword = [
          t("password.validation.mismatch", "새 비밀번호와 일치하지 않습니다"),
        ];
  ...
  const handleSubmit = async (event) => {
    ...
    if (!form.newPassword.trim()) {
      errors.newPassword = [
        t("password.validation.newRequired", "새 비밀번호를 입력해주세요"),
      ];
    } else {
  ...
  const getPasswordRequirements = () => {
    ...
    const complexity =
      [hasLetter, hasDigit, hasSpecial].filter(Boolean).length >= 2;

    return [
      {
        text: t("password.requirements.length", "8-100자 길이"),
        valid: isLengthOk,
      },
      {
        text: t("password.requirements.letter", "영문 포함"),
        valid: hasLetter,
      },
      { text: t("password.requirements.digit", "숫자 포함"), valid: hasDigit },
      {
        text: t("password.requirements.special", "특수문자 포함"),
        valid: hasSpecial,
      },
      {
        text: t("password.requirements.combination", "2가지 이상 조합"),
        valid: complexity,
      },
    ];
  };
      ...
      <CardContent>
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ mt: 2 }}>
          ...
          <Box sx={{ mt: 3, display: "flex", justifyContent: "flex-end" }}>
            ...
            >
              {loading
                ? t("password.button.changing", "변경 중...")
                : t("password.button.change", "비밀번호 변경")}
            </Button>
          </Box>
        </Box>
      </CardContent>
