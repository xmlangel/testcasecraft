...
const JiraStatusSummaryCard = ({
  ...
  if (error) {
    return (
      <Card>
        <CardContent>
          <Alert severity="error">{t("jira.summary.error", { error })}</Alert>
        </CardContent>
      </Card>
    );
  }
  ...
  if (!jiraStatusData || jiraStatusData.length === 0) {
    return (
      <Card>
        <CardContent>
          <Alert severity="info">{t("jira.summary.noData")}</Alert>
        </CardContent>
      </Card>
    );
  }
      ...
      <CardContent>
        ...
        <Grid container spacing={2}>
          {filteredData.map((item, index) => (
            <Grid
              size={{ xs: 12, md: compact ? 12 : 6 }}
              key={item.jiraIssueKey || index}
            >
              <Card variant="outlined" sx={{ height: "100%" }}>
                <CardContent sx={{ p: 2 }}>
                  ...
                  <Box mb={2}>
                    ...
                    >
                      <Typography variant="body2" color="text.secondary">
                        {t("jira.summary.testResultsCount", {
                          count: item.linkedTestCount,
                        })}
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                  ...
                  {item.testResultDistribution && (
                    <Box mb={2}>
                      <Box display="flex" gap={1} flexWrap="wrap">
                        {Object.entries(item.testResultDistribution).map(
                          ([status, count]) => (
                            <Chip
                              key={status}
                              label={`${status}: ${count}`}
                  ...
                  <Box>
                    ...
                    <br />
                    <Typography variant="caption" color="text.secondary">
                      {t("jira.summary.executionTime")}{" "}
                      {formatDate(item.latestTestDate)}
                    </Typography>
                    {item.lastSyncAt && (
                      <>
                        <br />
          ...
          <Box mt={3} p={2} bgcolor="background.default" borderRadius={1}>
            ...
            <Grid container spacing={2}>
              <Grid size={{ xs: 3 }}>
                ...
                <Typography variant="h6">{jiraStatusData.length}</Typography>
              </Grid>
              <Grid size={{ xs: 3 }}>
                <Typography variant="body2" color="text.secondary">
