...
const JiraStatusDashboard = () => {
            ...
            <CardContent>
              ...
              <Box display="flex" gap={1} flexWrap="wrap">
                {statisticsData.statusDistribution &&
                  Object.entries(statisticsData.statusDistribution).map(
                    ([status, count]) => (
                      <Chip
                        key={status}
                        label={`${status}: ${count}`}
            ...
            <CardContent>
              ...
              <Box display="flex" gap={1} flexWrap="wrap">
                {statisticsData.priorityDistribution &&
                  Object.entries(statisticsData.priorityDistribution).map(
                    ([priority, count]) => (
                      <Chip
                        key={priority}
            ...
            <CardContent>
              ...
              <Box display="flex" gap={1} flexWrap="wrap" mb={2}>
                {statisticsData.syncStatusDistribution &&
                  Object.entries(statisticsData.syncStatusDistribution).map(
                    ([status, count]) => (
                      <Chip
                        key={status}
                        label={`${status}: ${count}`}
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Box>
            ...
            {statusLastUpdated && (
              ...
              >
                마지막 업데이트:{" "}
                {new Date(statusLastUpdated).toLocaleString("ko-KR")}
              </Typography>
            )}
          </Box>
      ...
      {/* 탭 내용 */}
      <Box>{renderTabPanel(tabValue)}</Box>
    </Container>
  );
};
