...
const Login = ({ onLoginSuccess }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setError(
        err.message ||
          t("login.error.general", "로그인 중 오류가 발생했습니다."),
      );
    }
    setLoading(false);
  };
  ...
  const handleRegister = async () => {
    ...
    if (form.password !== form.confirm) {
      setError(
        t("validation.password.mismatch", "비밀번호가 일치하지 않습니다."),
      );
      setLoading(false);
      return;
    }
    try {
      ...
      setInfo(
        t("register.success", "회원가입이 완료되었습니다. 로그인 해주세요."),
      );
      setMode("login");
      ...
      setError(
        err.message ||
          t("register.error.general", "회원가입 중 오류가 발생했습니다."),
      );
    }
    setLoading(false);
  };
          ...
          {mode === "register" && (
            <>
              ...
              <TextField
                ...
                disabled={loading}
                slotProps={{
                  htmlInput: { "data-testid": "register-name-input" },
                }}
              />
              <TextField
                ...
                disabled={loading}
                slotProps={{
                  htmlInput: { "data-testid": "register-email-input" },
                }}
              />
            </>
          )}
