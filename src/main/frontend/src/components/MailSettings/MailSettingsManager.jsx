...
const MailSettingsManager = () => {
  ...
  const loadSettings = async () => {
    try {
      ...
      setError(
        t("mail.message.loadError", "메일 설정을 불러오는데 실패했습니다."),
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSaveSettings = async (settingsData) => {
    try {
      ...
      setSuccess(
        t("mail.message.saveSuccess", "메일 설정이 성공적으로 저장되었습니다."),
      );
  ...
  const handleTestMail = async (testRecipient) => {
    try {
      ...
      setSuccess(
        t(
          "mail.message.testSuccess",
          "테스트 메일이 {email}로 발송되었습니다.",
        ).replace("{email}", testRecipient),
      );
  ...
  const handleDisableSettings = async () => {
    try {
      ...
      setSuccess(
        t("mail.message.disableSuccess", "메일 기능이 비활성화되었습니다."),
      );
      ...
      setError(
        t("mail.message.disableError", "메일 설정 비활성화에 실패했습니다."),
      );
    }
  };
        ...
        <Grid size={{ xs: 12, md: 8 }}>
          <Card>
                ...
                <List>
                    ...
                    <ListItemText
                      ...
                      secondary={
                        settings.mailEnabled
                          ? t("mail.status.activatedStatus", "활성화됨")
                          : t("mail.status.deactivatedStatus", "비활성화됨")
                      }
                    />
                  </ListItem>
                  ...
                  <ListItem>
                    ...
                    <ListItemText
                      ...
                      secondary={`${t("mail.smtp.auth", "인증")}: ${
                        settings.useAuth
                          ? t("mail.smtp.used", "사용")
                          : t("mail.smtp.notUsed", "미사용")
                      }, ${t("mail.smtp.tls", "TLS")}: ${
                        settings.useTLS
                          ? t("mail.smtp.used", "사용")
                          : t("mail.smtp.notUsed", "미사용")
            ...
            <CardActions>
              ...
              >
                {settings?.username
                  ? t("mail.button.modifySettings", "설정 수정")
                  : t("mail.button.newSettings", "새 설정")}
              </Button>
