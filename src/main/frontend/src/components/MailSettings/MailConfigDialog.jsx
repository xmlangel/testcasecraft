    ...
    if (open) {
      if (initialData) {
        setFormData({
          ...
          useAuth:
            initialData.useAuth !== undefined ? initialData.useAuth : true,
  ...
  const validateForm = () => {
    ...
    if (!formData.password) {
      ...
      newErrors.password = t(
        ...
        "앱 비밀번호는 8자 이상이어야 합니다.",
      );
    }

    if (!formData.fromName.trim()) {
      newErrors.fromName = t(
    ...
    >
      <DialogTitle sx={{ display: "flex", alignItems: "center" }}>
        ...
        {initialData?.username
          ? t("mail.config.title.edit", "메일 설정 수정")
          : t("mail.config.title.new", "새 메일 설정")}
      </DialogTitle>
      <DialogContent>
        <Alert severity="info" sx={{ mb: 3 }}>
          <Typography variant="body2">
            <strong>Gmail {t("mail.guide.requirements", "전용")}:</strong>{" "}
            {t(
        ...
        <Box sx={{ mt: 2 }}>
          ...
          <FormControlLabel
            ...
            label={
              <Box sx={{ display: "flex", alignItems: "center" }}>
                <Typography>
                  {t("mail.config.enableMail", "메일 기능 활성화")}
                </Typography>
                <Chip
                  label={
                    formData.mailEnabled
                      ? t("mail.status.active", "활성화")
                      : t("mail.status.inactive", "비활성화")
                  }
                  size="small"
          ...
          <TextField
            ...
            helperText={
              ...
              t(
                ...
                "16자리 Gmail 앱 비밀번호 (공백 없이)",
              )
            }
            margin="normal"
            placeholder={t(
              "mail.config.form.appPasswordPlaceholder",
              "Gmail 앱 비밀번호",
            )}
            required
            slotProps={{
          ...
          <TextField
            ...
            margin="normal"
            placeholder={t(
              "mail.config.form.senderNamePlaceholder",
              "TestCase Manager",
            )}
            required
            slotProps={{
            ...
            <>
              ...
              <TextField
                ...
                helperText={t(
                  ...
                  "설정 후 테스트 메일을 받을 이메일 주소",
                )}
                margin="normal"
                placeholder={t(
                  "mail.config.form.testRecipientPlaceholder",
                  "test@example.com",
                )}
                slotProps={{
                  input: {
      ...
      <DialogActions sx={{ p: 3 }}>
        ...
        >
          {saving
            ? t("mail.config.button.saving", "저장 중...")
            : t("mail.config.button.save", "저장")}
        </Button>
      </DialogActions>
    </Dialog>
  );
