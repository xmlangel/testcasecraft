  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        ...
        <Box sx={{ mb: 3 }}>
          <Typography variant="body2" color="text.secondary" gutterBottom>
            {t("rag.resumeAnalysis.status", "진행 상태:")}{" "}
            <strong>
              {status.status === "paused"
                ? t("rag.resumeAnalysis.paused", "일시정지")
                : t("rag.resumeAnalysis.processing", "진행 중")}
            </strong>
          </Typography>
          ...
          <Box sx={{ mt: 2 }}>
            ...
            >
              <Typography variant="body2" color="text.secondary">
                {t("rag.resumeAnalysis.progress", "분석 진행률")}
              </Typography>
              <Typography variant="body2" fontWeight="bold">
                {analyzedChunks} / {totalChunks}{" "}
                {t("rag.resumeAnalysis.chunks", "청크")} ({percentage}%)
              </Typography>
            </Box>
            <LinearProgress
              variant="determinate"
      ...
      <DialogActions sx={{ px: 3, pb: 2 }}>
        ...
        >
          {t("rag.resumeAnalysis.restart", "처음부터 시작")}
        </Button>
        <Button
          onClick={onResume}
          color="primary"
        ...
        >
          {t("rag.resumeAnalysis.resume", "이어서 하기")}
        </Button>
      </DialogActions>
    </Dialog>
  );
