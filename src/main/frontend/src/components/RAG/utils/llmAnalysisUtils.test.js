...
describe("getSummaryMarkdownStyles", () => {
  it("pre-wrap 을 마크다운 루트가 아닌 p/li 로만 한정한다 (공백 회귀 가드)", () => {
    ...
    // 한정된 규칙이 존재
    expect(styles["& .markdown-body p, & .markdown-body li"]).toEqual({
      whiteSpace: "pre-wrap",
    });

    // 루트(.markdown-body)에는 whiteSpace 가 없어야 함
    expect(styles["& .markdown-body"]?.whiteSpace).toBeUndefined();
  ...
  it("다크 모드에서도 동일한 한정 규칙을 유지한다", () => {
    ...
    expect(styles["& .markdown-body p, & .markdown-body li"].whiteSpace).toBe(
      "pre-wrap",
    );
  });
});
