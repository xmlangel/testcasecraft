...
import {
  formatDateArray,
  getProgressColor,
} from "./utils/documentFormatUtils.js";
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <Typography variant="h6">
            {t("rag.document.jobHistory.title", "작업 이력 - {fileName}", {
              fileName: selectedJobHistory?.fileName,
            })}
          </Typography>
        </Box>
          ...
          <TableContainer>
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell>
                    {t("rag.document.jobHistory.jobId", "작업 ID")}
                  </TableCell>
                  <TableCell>
                    {t("rag.document.jobHistory.llmProvider", "LLM 제공자")}
                  </TableCell>
                  <TableCell>
                    {t("rag.document.jobHistory.llmModel", "LLM 모델")}
                  </TableCell>
                  <TableCell>
                    {t("rag.document.jobHistory.status", "상태")}
                  </TableCell>
                  <TableCell align="center">
                    {t("rag.document.jobHistory.progress", "진행률")}
                  </TableCell>
                  <TableCell>
                    {t("rag.document.jobHistory.chunks", "청크")}
                  </TableCell>
                  <TableCell align="right">
                    {t("rag.document.jobHistory.cost", "비용 (USD)")}
                  </TableCell>
                  <TableCell align="right">
                    {t("rag.document.jobHistory.tokens", "토큰")}
                  </TableCell>
                  <TableCell>
                    {t("rag.document.jobHistory.startTime", "시작 시각")}
                  </TableCell>
                  <TableCell>
                    {t("rag.document.jobHistory.completedTime", "완료 시각")}
                  </TableCell>
                  <TableCell>
                    {t("rag.document.jobHistory.pausedTime", "일시정지 시각")}
                  </TableCell>
                  <TableCell>
                    {t("rag.document.jobHistory.errorMessage", "에러 메시지")}
                  </TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                    ...
                    <TableCell>
                      {job.errorMessage ? (
                        <Tooltip title={job.errorMessage}>
                          <Chip
                            label={t(
                              "rag.document.jobHistory.hasError",
                              "에러 있음",
                            )}
                            size="small"
                            color="error"
