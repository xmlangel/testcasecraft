...
export function useStreamingChat({
  ...
  const fallbackSimulationStateRef = useRef({
  ...
  const clearStreamingScheduler = useCallback(
    (shouldFlush = false) => {
  ...
  const simulateFallbackStreaming = useCallback(
    (targetMessageId, fullText, metadata = {}) => {
      if (!targetMessageId) {
      ...
      const chunkSize = Number.isFinite(metadata.chunkSize)
        ? metadata.chunkSize
        : 8;
      const interval = Number.isFinite(metadata.interval)
        ? metadata.interval
        : 24;
      ...
      const step = () => {
        const { cancelRequested, targetId } =
          fallbackSimulationStateRef.current;
