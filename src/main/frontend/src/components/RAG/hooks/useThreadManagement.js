...
export function useThreadManagement({
  ...
  const [isDeleteMessageConfirmOpen, setIsDeleteMessageConfirmOpen] =
    useState(false);
  ...
  const handleCreateThread = useCallback(async () => {
    ...
    if (!trimmedTitle) {
      setError(
        t("rag.chat.threadTitleRequired", "스레드 제목을 입력해주세요."),
      );
      return;
    }
    ...
    try {
      ...
      if (created?.id) {
        ...
        setSelectedCategoryIds(
          created?.categories?.map((category) => category.id) || [],
        );
  ...
  const handleManageThreadUpdate = useCallback(
    async (payload) => {
  ...
  const handleManageThreadDelete = useCallback(
    async (threadId) => {
