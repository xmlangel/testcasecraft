...
} from "../utils/llmAnalysisUtils.js";
...
export function useSummaryDialog({
    ...
    async (doc, page = 0) => {
      ...
      try {
        const response = await getLlmAnalysisResults(
          doc.id,
          offset,
          DOCUMENT_LIST_CONSTANTS.SUMMARY_PAGE_SIZE,
        );
   ...
   * 요약 페이지 변경
   */
  const handleSummaryPageChange = useCallback(
    (direction) => {
      ...
      const totalPages = summaryTotal
        ? Math.ceil(summaryTotal / DOCUMENT_LIST_CONSTANTS.SUMMARY_PAGE_SIZE)
        : null;
      if (
        direction === "next" &&
        totalPages &&
  ...
  const canGoNextSummary =
    summaryHasMore ||
    (summaryTotal
      ? (summaryPage + 1) * DOCUMENT_LIST_CONSTANTS.SUMMARY_PAGE_SIZE <
        summaryTotal
      : false);
