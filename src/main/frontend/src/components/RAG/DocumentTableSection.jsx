...
function DocumentTableSection({
  ...
  const getLlmAnalysisStatusChip = (docId) => {
    ...
    if (!llmState) {
      return (
        <Chip
          label={t("rag.document.list.loading", "로딩 중")}
          size="small"
          color="default"
        />
      );
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell>
                {t("rag.document.list.embeddingStatus", "임베딩")}
              </TableCell>
              <TableCell>{t("rag.document.list.chunks", "청크 수")}</TableCell>
              <TableCell>
                {t("rag.document.list.llmSummaryStatus", "LLM 요약 상태")}
              </TableCell>
              <TableCell align="center">
                {t("rag.document.list.summaryProgress", "요약 진행율")}
              </TableCell>
              <TableCell>
                {t("rag.document.list.analyzedChunks", "분석 청크")}
              </TableCell>
              <TableCell>
                {t("rag.document.list.uploadDate", "업로드 일시")}
              </TableCell>
              <TableCell align="center">
                {t("rag.document.list.actions", "작업")}
              </TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
                    ...
                    <TableCell align="center">
                      ...
                      {actionHandler("generateEmbeddings") && (
                        <IconButton
                          ...
                          color="secondary"
                          onClick={() =>
                            actionHandler("generateEmbeddings")(doc)
                          }
                          title={t(
                            "rag.document.generateEmbedding",
                            "임베딩 생성",
                          )}
                          disabled={
                            doc.analysisStatus !== "completed" ||
                            doc.metaData?.embedding_status === "completed" ||
                            doc.metaData?.embedding_status === "processing"
                          }
                        >
                      ...
                      {actionHandler("jobHistory") && (
                        <Tooltip
                          title={t("rag.document.jobHistory", "작업 이력 보기")}
                        >
                          <IconButton
                            size="small"
                            color="info"
                              ...
                              <Box>
                                ...
                                >
                                  ...
                                  <Box>
                                    ...
                                    <Typography variant="body2">
                                      {formatDateFromISO(llmState.startedAt)}
                                    </Typography>
                                  </Box>
                                  <Box>
                                    <Typography
                                    ...
                                    <Typography variant="body2">
                                      {formatDateFromISO(llmState.completedAt)}
                                    </Typography>
                                  </Box>
                                </Box>
                                    ...
                                    {onCancelJob &&
                                      (llmState.status === "processing" ||
                                        ...
                                        <Button
                                          ...
                                          startIcon={<StopIcon />}
                                          onClick={() =>
                                            onCancelJob(doc.id, doc.fileName)
                                          }
                                        >
                                          취소
                                        </Button>
