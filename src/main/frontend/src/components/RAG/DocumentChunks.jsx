...
function DocumentChunks({
  ...
  const { t } = useI18n();
  const { getDocumentChunks, getLlmAnalysisResults, getLlmAnalysisStatus } =
    useRAG();
  ...
  const [chunkSummaryLoadingIndex, setChunkSummaryLoadingIndex] =
    useState(null);
  ...
  const hasAnyLlmSummaries =
    (documentAnalysisStatus?.progress?.processedChunks || 0) > 0;
  ...
  const loadInitialChunks = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        // 순서를 유지하면서 중복 제거
        const orderedUniqueIndices = relatedChunkIndices.reduce(
          (acc, index) => {
  ...
  const handleViewLlmSummary = useCallback(
    async (chunk) => {
  ...
  useEffect(() => {
    if (
      !documentSummaryOpen ||
      !documentSummarySentinelRef.current ||
      !documentSummaryHasMore
    ) {
      return;
    }
    ...
    documentSummaryObserverRef.current.observe(
      documentSummarySentinelRef.current,
    );
    return () => {
  ...
  return (
    <>
        ...
        <DialogTitle>
          ...
          >
            ...
            <Box sx={{ display: "flex", gap: 1 }}>
              <Tooltip
                title={t(
                  "rag.chunks.viewCombinedSummary",
                  "LLM 분석 요약 보기",
                )}
              >
                <span>
          ...
          {!loading && !error && chunks.length > 0 && (
            <>
              ...
              >
                ...
                <Chip
                  label={`${t("rag.chunks.loaded", "로드됨")} ${chunks.length}${
                    isFilteredMode ? "" : ` / ${total}`
                  }`}
                  color="primary"
                  size="small"
                />
                  ...
                  return (
                    <React.Fragment key={chunk.id}>
                      ...
                      >
                        ...
                        {chunk.chunkMetadata &&
                          Object.keys(chunk.chunkMetadata).length > 0 && (
      ...
      >
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <Typography variant="h6">
              {t("rag.chunks.documentSummaryTitle", "LLM 분석 요약")} -{" "}
              {documentName}
            </Typography>
          </Box>
                ...
                <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  {documentSummaryEntries.map((entry, idx) => (
                    <Box
                      ...
                      }-${documentSummarySkip}-${idx}`}
                    >
                      <Chip
                        label={`${t("rag.chunks.chunkLabel", "청크")} ${
                    ...
                    documentSummaryEntries.length > 0 && (
                      ...
                      >
                        ...
                        {documentSummaryTotal
                          ? ` (${documentSummaryEntries.length}/${documentSummaryTotal})`
                          : ""}
                      </Typography>
                    )}
                </Box>
        ...
        <DialogActions>
          <Button onClick={handleCloseDocumentSummary}>
            {t("common.close", "닫기")}
          </Button>
        </DialogActions>
      </Dialog>
          ...
          {selectedSummary ? (
            <Box>
              ...
              <Box>
                ...
                >
                  {chunkSummaryLoadingIndex === selectedSummary.chunkIndex &&
