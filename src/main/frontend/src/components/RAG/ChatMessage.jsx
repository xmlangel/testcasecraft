...
function ChatMessage({
  ...
  const assistantMessageBg = isDarkMode
    ? alpha(theme.palette.common.white, 0.05)
    : theme.palette.grey[100];
  const inlineCodeBg = isDarkMode
    ? alpha(theme.palette.common.white, 0.15)
    : theme.palette.grey[200];
  const blockCodeBg = isDarkMode
    ? alpha(theme.palette.common.white, 0.1)
    : theme.palette.grey[200];
  const tableHeaderBg = isDarkMode
    ? alpha(theme.palette.common.white, 0.1)
    : theme.palette.grey[100];
  const tableRowAltBg = isDarkMode
    ? alpha(theme.palette.common.white, 0.05)
      ...
      const mappedData = {
        ...
        ...(tc.steps && tc.steps.length > 0
          ? tc.steps.reduce((acc, step, i) => {
              ...
              // 최대 20개 스텝까지 지원
              if (stepNum <= 20) {
                acc[`step${stepNum}_description`] =
                  step.action || step.description || "";
                acc[`step${stepNum}_expectedResult`] =
  ...
  // 청크 프리뷰에서 전체 문서 보기
  const handleViewDocumentFromChunk = useCallback(
    (doc) => {
      ...
      onDocumentClick({
        ...
        relatedChunkIndices:
          relatedChunkIndices.length > 0 ? relatedChunkIndices : undefined,
      });
    },
  ...
  const extractTestCaseInfo = (doc) => {
    ...
    const testCaseId = doc.testCaseId || metadata.testCaseId || fromFileName();
  ...
  return (
    ...
    >
              ...
              >
                ...
                {parsedTestCases.length > 0 && (
                  <Box sx={{ mt: 1, mb: 2 }}>
                    ...
                    >
                      {t(
                        "rag.chat.generatedTestCases",
                        "생성된 테스트 케이스 ({count})",
                        { count: parsedTestCases.length },
                      )}
                    </Typography>
                    <Box
                      sx={{
                    ...
                    >
                      ...
                      >
                        <thead style={{ backgroundColor: tableHeaderBg }}>
                          <tr>
                            <th
                              style={{ padding: "8px 4px", width: "40px" }}
                            ></th>
                            <th
                              style={{
                                padding: "8px 12px",
                            ...
                            >
                              {t("testcase.column.priority", "우선순위")}
                            </th>
                            <th
                              style={{
                                padding: "8px 12px",
                        ...
                        <tbody>
                          {parsedTestCases.map((tc, idx) => (
                            <React.Fragment key={idx}>
                              <tr
                                style={{
                                  borderBottom: expandedRows[idx]
                                    ? "none"
                                    : idx < parsedTestCases.length - 1
                                      ? `1px solid ${theme.palette.divider}`
                                      : "none",
                                  cursor: "pointer",
                ...
                <Box>
                  ...
                  {parsedTestCases.length > 0 && jsonContent && (
                    <Box sx={{ mt: 1 }}>
                      ...
                      >
                        {showJson
                          ? t("rag.chat.hideJson", "JSON 원본 숨기기")
                          : t("rag.chat.showJson", "JSON 원본 보기")}
                      </Button>
                ...
                return filteredDocs.length > 0 ? (
                  ...
                  >
                    ...
                    <Stack
                      ...
                      useFlexGap
                    >
                      {filteredDocs.map((doc, filteredIndex) => {
                        ...
                        const uniqueDocKey = `${message.id}-${filteredIndex}-${
                          doc.id || doc.chunkId || doc.chunkIndex || "doc"
                        }`;
                        ...
                        const metadata = doc.metadata || {};
                        const threadId =
                          metadata.threadId || metadata.thread_id;
                        const threadTitleFromList = threadId
                          ? ragThreads.find((thread) => thread.id === threadId)
                              ?.title
                          : null;
                        ...
                        const baseName = isTestCaseDoc
                          ? t(
                              "rag.chat.testCaseDocumentLabel",
                              "테스트케이스: {name}",
                              {
                          ...
                          : isConversationThread
                            ...
                            : doc.displayName ||
                              doc.title ||
                              doc.fileName ||
                              t("rag.chat.documentFallback", "문서 {index}", {
                        ...
                        const tooltipTitle = isTestCaseDoc
                          ...
                          : isConversationThread
                            ...
                            : t(
                                "rag.chat.documentTooltip",
                                "문서 상세 정보 보기",
                              );

                        // 출처 클릭 동작
                        const chipProps =
                          isTestCaseDoc && testCaseInfo?.url
                            ? {
                                // 테스트케이스: 새 탭으로 이동
                                component: "a",
              ...
              <Box sx={{ mt: 1 }}>
                {parsedTestCases.map((tc, index) => (
                  <Button
                    ...
                    sx={{ mr: 1, mb: 0.5 }}
                  >
                    {t("rag.testcase.addButton", "테스트케이스 추가")}:{" "}
                    {tc.name}
                  </Button>
                ))}
      ...
      <ChunkPreviewDialog
        ...
        onViewDocument={
          onDocumentClick ? handleViewDocumentFromChunk : undefined
        }
      />
    </Box>
  );
