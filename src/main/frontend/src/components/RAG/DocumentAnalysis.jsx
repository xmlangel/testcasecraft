...
function DocumentAnalysis({ document }) {
  ...
  const visibleConfigs = isAdmin
    ? activeConfigs
    : activeConfigs.filter((c) => c.isDefault);
  ...
  useEffect(() => {
    ...
    const defaultConfig =
      visibleConfigs.find((c) => c.isDefault) || visibleConfigs[0];
    const selectedStillVisible = visibleConfigs.some(
  ...
  useEffect(() => {
    if (selectedConfigId) {
      ...
      if (selectedConfig) {
        ...
        const providerValue =
          typeof selectedConfig.provider === "string"
  ...
  const handleEstimateCost = useCallback(async () => {
    ...
    try {
      ...
      // 에러 메시지 추출
      let errorMessage = t(
        "rag.analysis.error.costEstimate",
        "비용 추정에 실패했습니다.",
      );
  ...
  const handlePause = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err.message ||
          t("rag.analysis.error.pause", "일시정지에 실패했습니다."),
      );
    }
  ...
  const handleResume = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err.message || t("rag.analysis.error.resume", "재개에 실패했습니다."),
      );
    }
  ...
  const handleCancel = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err.message || t("rag.analysis.error.cancel", "취소에 실패했습니다."),
      );
    }
  ...
  return (
    <Box>
            ...
            <>
              ...
              {isAdmin && (
                <FormControl fullWidth sx={{ mb: 2 }}>
                  <InputLabel>
                    {t("rag.analysis.selectConfig", "LLM 설정 선택")}
                  </InputLabel>
                  <Select
                  ...
                  >
                    {visibleConfigs.map((llmConfig) => (
                      <MenuItem key={llmConfig.id} value={llmConfig.id}>
                        ...
                        {llmConfig.modelName})
                        {llmConfig.isDefault &&
                          ` ${t("rag.analysis.defaultBadge", "[기본]")}`}
                      </MenuItem>
                    ))}
                  </Select>
                </FormControl>
              ...
              {selectedConfigId && (
                ...
                >
                  ...
                  <Typography variant="body2">
                    <strong>{t("rag.analysis.provider", "제공자:")}</strong>{" "}
                    {config.llmProvider || "-"}
                  </Typography>
                  <Typography variant="body2">
                    <strong>{t("rag.analysis.model", "모델:")}</strong>{" "}
                    {config.llmModel || "-"}
                  </Typography>
                  <Typography variant="body2">
                    <strong>{t("rag.analysis.apiUrl", "API URL:")}</strong>{" "}
                    {config.llmBaseUrl ||
                      t("rag.analysis.defaultValue", "기본값")}
                  </Typography>
                </Box>
              )}
            </>
          ...
          >
            <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
              ...
              >
                {isAnalyzing && !isCompleted ? (
                  <>
                    {t("rag.analysis.processing", "처리 중:")}{" "}
                    <Chip
                      label={t("rag.analysis.chunkNumber", "{number}번 청크", {
                        number: status.progress?.processedChunks + 1,
                      })}
                      size="small"
                      color="primary"
                      variant="outlined"
                  ...
                  t("rag.analysis.completed", "완료: {count}개", {
                    count: status.progress?.processedChunks,
                  })
                )}
              </Typography>
              <Typography variant="caption" color="textSecondary">
                {t("rag.analysis.total", "/ 전체 {count} 청크", {
                  count: status.progress?.totalChunks,
                })}
              </Typography>
            </Box>
            <Typography
            ...
            >
              💰 {t("rag.analysis.cost", "비용:")}{" "}
              <strong>
                ${status.actualCostSoFar?.totalCostUsd?.toFixed(4) || "0.0000"}
              </strong>
            </Typography>
          </Box>
        </Box>
          ...
          <TableContainer component={Paper} variant="outlined">
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell>
                    {t("rag.analysis.chunkNumber.header", "청크 #")}
                  </TableCell>
                  <TableCell>
                    {t("rag.analysis.originalText", "원본 텍스트")}
                  </TableCell>
                  <TableCell>
                    {t("rag.analysis.llmResponse", "LLM 응답")}
                  </TableCell>
                  <TableCell align="right">
                    {t("rag.analysis.tokens", "토큰")}
                  </TableCell>
                  <TableCell align="right">
                    {t("rag.analysis.costHeader", "비용")}
                  </TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
      ...
      <CostWarningDialog
        ...
        loading={loading}
        selectedConfigName={
          visibleConfigs.find((c) => c.id === selectedConfigId)?.name
        }
      />
      {/* 배치 확인 다이얼로그 */}
      <BatchConfirmDialog
