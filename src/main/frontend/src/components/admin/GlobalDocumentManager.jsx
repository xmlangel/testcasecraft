...
const GlobalDocumentManager = ({ onSuccess }) => {
  ...
  const [loadingGlobalDocRequests, setLoadingGlobalDocRequests] =
    useState(false);
  ...
  const [chunksDialogState, setChunksDialogState] = useState({
    open: false,
    document: null,
  });
  ...
  const fetchSummaryPage = useCallback(
    async (doc, page = 0) => {
      ...
      try {
        const response = await getLlmAnalysisResults(
          doc.id,
          offset,
          SUMMARY_PAGE_SIZE,
        );
  ...
  const canGoNextSummary =
    summaryHasMore ||
    (summaryTotal
      ? (summaryPage + 1) * SUMMARY_PAGE_SIZE < summaryTotal
      : false);
    ...
    return {
      ...
      "& .wmde-markdown h3": {
        ...
        borderLeft: `4px solid ${
          isDarkMode ? "rgba(103, 232, 249, 0.5)" : "rgba(6, 182, 212, 0.5)"
        }`,
        paddingLeft: "12px",
      },
      "& .wmde-markdown hr": {
      ...
      await Promise.all(
        documents.map(async (doc) => {
          try {
            ...
            const totalChunks =
              status.progress?.totalChunks || doc.totalChunks || 0;
  ...
  const fetchGlobalDocuments = useCallback(async () => {
    ...
    if (isDebugEnabled("GlobalDocumentManager"))
      console.log(
        "[GlobalDocumentManager] Fetching documents - page:",
        page,
        "rowsPerPage:",
        rowsPerPage,
      );
    ...
    try {
      ...
      if (isDebugEnabled("GlobalDocumentManager"))
        console.log("[GlobalDocumentManager] API Response:", response);
      ...
      if (isDebugEnabled("GlobalDocumentManager"))
        console.log(
          "[GlobalDocumentManager] Set totalDocs to:",
          total,
          "docs.length:",
          docs.length,
        );
  ...
  const handleChangePage = useCallback(
    (event, newPage) => {
      if (isDebugEnabled("GlobalDocumentManager"))
        console.log(
          "[GlobalDocumentManager] Page changed from",
          page,
          "to",
          newPage,
        );
  ...
  const handleChangeRowsPerPage = useCallback(
    (event) => {
      ...
      if (isDebugEnabled("GlobalDocumentManager"))
        console.log(
          "[GlobalDocumentManager] RowsPerPage changed from",
          rowsPerPage,
          "to",
          newRowsPerPage,
        );
  ...
  const handleUploadGlobalDocument = async (event) => {
    ...
    try {
          ...
          if (onSuccess)
            onSuccess(
              t(
                ...
                '공통 문서 "{0}"이 업로드되고 분석 및 임베딩이 시작되었습니다',
              ).replace("{0}", file.name),
            );
      ...
      const errorMessage =
        ...
        err.message ||
        t("admin.globalDoc.message.uploadFailed", "공통 문서 업로드 실패");
      alert(
        t("admin.globalDoc.message.uploadFailed", "공통 문서 업로드 실패") +
          ": " +
          errorMessage,
  ...
  const handleApproveRequest = async (request) => {
    const note = window.prompt(
      t("admin.globalDoc.requests.approveNote", "승인 메모 (선택)"),
      "",
    );
    try {
      ...
      if (onSuccess)
        onSuccess(
          t("admin.globalDoc.requests.approved", "요청을 승인했습니다."),
        );
  ...
  const handleRejectRequest = async (request) => {
    const note = window.prompt(
      t("admin.globalDoc.requests.rejectNote", "거절 사유 (선택)"),
      "",
    );
    try {
      ...
      if (onSuccess)
        onSuccess(
          t("admin.globalDoc.requests.rejected", "요청을 거절했습니다."),
        );
      await fetchGlobalDocRequests();
    } catch (err) {
      console.error("Failed to reject global document request:", err);
      alert(
        t("admin.globalDoc.requests.rejectFailed", "요청 거절에 실패했습니다."),
      );
    }
  };
  ...
  const handleDeleteGlobalDocument = async (documentId, fileName) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        t("admin.globalDoc.message.deleteFailed", "공통 문서 삭제 실패");
      alert(
        t("admin.globalDoc.message.deleteFailed", "공통 문서 삭제 실패") +
          ": " +
          errorMessage,
    ...
    try {
      ...
      alert(
        t("admin.globalDoc.message.downloadFailed", "다운로드 실패") +
          ": " +
          (err.message ||
            t("admin.globalDoc.message.unknownError", "알 수 없는 오류")),
      );
    }
  };
  ...
  const handleSummaryPageChange = (direction) => {
    ...
    const totalPages = summaryTotal
      ? Math.ceil(summaryTotal / SUMMARY_PAGE_SIZE)
      : null;
    if (
      direction === "next" &&
      totalPages &&
  ...
  const handleViewJobHistory = async (doc) => {
    ...
    try {
      ...
      const filteredJobs =
        response.jobs?.filter((job) => job.documentId === doc.id) || [];
      setSelectedJobHistory({
        documentId: doc.id,
        fileName: doc.fileName,
        jobs: filteredJobs,
      ...
      {!isRagEnabled && (
        <Alert severity="error" sx={{ mb: 2 }}>
          ...
          <Typography variant="body2">
            ...
            <br />
            <strong>
              비활성화 상태에서 업로드 시 RAG 관련 처리가 정상적으로 동작하지
              않습니다.
            </strong>
          </Typography>
        </Alert>
      ...
      {isRagEnabled && (
        <Paper sx={{ p: 3 }}>
          ...
          <Alert severity="info" sx={{ mb: 2 }}>
            <Typography variant="body2">
              <strong>
                {t("admin.globalDoc.info.whatIsTitle", "📚 공통 문서란?")}
              </strong>
              <br />
              {t(
              ...
              <br />
              <strong>
                {t("admin.globalDoc.info.examplesTitle", "💡 활용 예시:")}
              </strong>
              <br />•{" "}
              {t(
              ...
              <br />
              <strong>
                {t("admin.globalDoc.info.techSpecsTitle", "⚙️ 기술 사양:")}
              </strong>
              <br />•{" "}
              {t(
          ...
          {loadingGlobalDocs ? (
            ...
            <DocumentTableSection
              ...
              actionHandlers={{
                ...
                delete: (doc) =>
                  handleDeleteGlobalDocument(doc.id, doc.fileName),
              }}
            />
          )}
          ...
          <Box sx={{ mt: 4 }}>
            ...
            {loadingGlobalDocRequests ? (
              ...
              <Alert severity="info" sx={{ mb: 0 }}>
                {t(
                  "admin.globalDoc.requests.empty",
                  "대기 중인 요청이 없습니다.",
                )}
              </Alert>
            ) : (
              <TableContainer>
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      <TableCell>
                        {t("rag.document.list.fileName", "파일명")}
                      </TableCell>
                      <TableCell>
                        {t("admin.globalDoc.requests.requestedBy", "요청자")}
                      </TableCell>
                      <TableCell>
                        {t("admin.globalDoc.requests.message", "요청 메모")}
                      </TableCell>
                      <TableCell>
                        {t("admin.globalDoc.requests.requestedAt", "요청 일시")}
                      </TableCell>
                  ...
                  <TableBody>
                    {globalDocRequests.map((request) => (
                      <TableRow key={request.id}>
                        ...
                        <TableCell>
                          {formatDateArray(request.createdAt)}
                        </TableCell>
                        <TableCell align="right">
                          <Stack
                            direction="row"
          ...
          {selectedSummary ? (
            <Box>
              <Box sx={{ display: "flex", gap: 2, mb: 3 }}>
                <Chip
                  label={t(
                    "rag.document.summary.totalChunks",
                    "총 {0}개 청크",
                  ).replace("{0}", selectedSummary.totalChunks || 0)}
                  size="small"
                  color="primary"
                />
                <Chip
                  label={t(
                    "rag.document.summary.analyzedChunks",
                    "분석 완료: {0}개",
                  ).replace("{0}", selectedSummary.analyzedChunks || 0)}
                  size="small"
                  color="success"
                />
                <Chip
                  label={t(
                    "rag.document.summary.progress",
                    "진행률: {0}%",
                  ).replace("{0}", selectedSummary.progress || 0)}
                  size="small"
      ...
      >
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <Typography variant="h6">
              {t("rag.document.jobHistory", "작업 이력")} -{" "}
              {selectedJobHistory?.fileName}
            </Typography>
          </Box>
              ...
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>
                      {t("rag.document.llmProvider", "LLM 제공자")}
                    </TableCell>
                    <TableCell>
                      {t("rag.document.llmModel", "LLM 모델")}
                    </TableCell>
                    <TableCell>{t("rag.document.status", "상태")}</TableCell>
                    <TableCell align="center">
                      {t("rag.document.summaryProgress", "진행률")}
                    </TableCell>
                    <TableCell>
                      {t("rag.document.list.chunks", "청크 수")}
                    </TableCell>
                    <TableCell align="right">
                      {t("rag.document.cost", "비용 (USD)")}
                    </TableCell>
                    <TableCell align="right">
                      {t("rag.document.tokens", "토큰")}
                    </TableCell>
                    <TableCell>
                      {t("rag.document.startedAt", "시작 시각")}
                    </TableCell>
                    <TableCell>
                      {t("rag.document.completedAt", "완료 시각")}
                    </TableCell>
                    <TableCell>
                      {t("rag.document.pausedAt", "일시정지 시각")}
                    </TableCell>
        ...
        <DialogActions>
          <Button onClick={handleCloseJobHistoryDialog}>
            {t("common.close", "닫기")}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      {analysisDialogState.document && (
        ...
        >
          ...
          >
            ...
            {analysisDialogState.document.fileName}
