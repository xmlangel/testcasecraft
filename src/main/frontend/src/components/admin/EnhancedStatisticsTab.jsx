...
export const EnhancedStatisticsTab = ({
  onLoadCategoryStats,
  onLoadLanguageStats,
  loading,
}) => {
  ...
  const loadCategoryTranslationStats = async () => {
    try {
      ...
      const response = await fetch(
        "/api/admin/translations/stats/category-completion",
        {
          headers: {
                  ...
                  <Grid size={{ xs: 12, sm: 6, md: 4 }} key={stat.languageCode}>
                    ...
                    >
                      ...
                      >
                        ...
                        <Chip
                          label={`${
                            stat.completionPercentage?.toFixed(1) || 0
                          }%`}
                          color={
                      ...
                      <Typography variant="caption" color="text.secondary">
                        {stat.translatedCount || 0} / {stat.totalCount || 0} 키
                        번역됨
                      </Typography>
                    </Box>
                  </Grid>
                    ...
                    <TableBody>
                      {categoryStats.map((category) => (
                        <TableRow key={category.category}>
                          ...
                          {languageStats.map((lang) => {
                            const langStat = category.languages.find(
                              (l) => l.languageCode === lang.languageCode,
                            );
                            const percentage =
                              langStat?.completionPercentage || 0;
                            const translatedKeys =
                              langStat?.translatedKeys || 0;
