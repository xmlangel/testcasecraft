...
const SchedulerConfigDialog = ({ open, onClose, config, onSuccess }) => {
  ...
  const validateCronExpression = (cron) => {
    if (!cron) {
      setCronError(
        t("scheduler.error.cronRequired", "Cron 표현식을 입력하세요"),
      );
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onSuccess?.(
        t("scheduler.dialog.updated", "스케줄 설정이 업데이트되었습니다."),
      );
      onClose();
    } catch (err) {
      const errorMessage =
  ...
  const formatMilliseconds = (ms) => {
    ...
    if (seconds < 60)
      return t("scheduler.time.seconds", "{seconds}초").replace(
        "{seconds}",
        seconds,
      );
    const minutes = Math.floor(seconds / 60);
    if (minutes < 60)
      return t("scheduler.time.minutes", "{minutes}분").replace(
        "{minutes}",
        minutes,
      );
    const hours = Math.floor(minutes / 60);
    if (hours < 24)
      return t("scheduler.time.hours", "{hours}시간").replace("{hours}", hours);
  ...
  const cronExamples = [
    {
      expression: "0 */5 * * * *",
      description: t("scheduler.cron.every5min", "매 5분마다"),
    },
    {
      expression: "0 0 * * * *",
      description: t("scheduler.cron.everyHour", "매 시간 정각"),
    },
    {
      expression: "0 0 0 * * *",
      description: t("scheduler.cron.midnight", "매일 자정"),
    },
    {
      expression: "0 0 1 * * *",
      description: t("scheduler.cron.daily1am", "매일 새벽 1시"),
    },
    {
      expression: "0 0 9 * * 1-5",
      description: t("scheduler.cron.weekdays9am", "평일 오전 9시"),
    },
    {
      expression: "0 0 9 * * 1",
      description: t("scheduler.cron.monday9am", "매주 월요일 오전 9시"),
    },
  ];
      ...
      <DialogContent dividers>
        ...
        <Box sx={{ mb: 3 }}>
          <Typography
            ...
            gutterBottom
          >
            {t("scheduler.dialog.taskKey", "작업 키:")}{" "}
            <Chip label={config?.taskKey} size="small" />
          </Typography>
          <Typography
            variant="body2"
            color="text.secondary"
            ...
            gutterBottom
          >
            {t("scheduler.dialog.scheduleType", "스케줄 타입:")}{" "}
            <Chip label={config?.scheduleType} size="small" color="primary" />
          </Typography>
          {config?.description && (
              ...
              <List dense>
                {cronExamples.map((example, index) => (
                  <ListItem key={index} disablePadding>
                    <ListItemButton
                      onClick={() =>
                        handleChange("cronExpression", example.expression)
                      }
                    >
                      <ListItemText
                        primary={
        ...
        {config?.scheduleType === "FIXED_RATE" && (
          <TextField
            ...
            onChange={(e) =>
              handleChange("fixedRateMs", parseInt(e.target.value))
            }
            helperText={t(
              "scheduler.dialog.currentValue",
              "현재 값: {value}",
            ).replace("{value}", formatMilliseconds(formData.fixedRateMs))}
            margin="normal"
            required
          />
        )}
        ...
        {config?.scheduleType === "FIXED_DELAY" && (
          <TextField
            ...
            onChange={(e) =>
              handleChange("fixedDelayMs", parseInt(e.target.value))
            }
            helperText={t(
              "scheduler.dialog.currentValue",
              "현재 값: {value}",
            ).replace("{value}", formatMilliseconds(formData.fixedDelayMs))}
            margin="normal"
            required
          />
        )}
        ...
        {config?.nextExecutionTime && (
          <Alert severity="info" sx={{ mt: 2 }}>
            {t(
              ...
              "{time}",
              new Date(config.nextExecutionTime).toLocaleString("ko-KR"),
            )}
          </Alert>
        )}
      </DialogContent>
