...
const TranslationManagement = () => {
  ...
  const [keyPagination, setKeyPagination] = useState({
    page: 0,
    size: 20,
    totalElements: 0,
    totalPages: 0,
  ...
  const [translationPagination, setTranslationPagination] = useState({
    page: 0,
    size: 10,
    totalElements: 0,
    totalPages: 0,
  ...
  const loadLanguages = async () => {
    try {
      ...
      setError(
        t("translation.languageLoadError", "언어 목록을 로드할 수 없습니다: ") +
          err.message,
      );
    } finally {
      setLoading(false);
  ...
  const loadTranslationKeys = async (page = keyPagination.page) => {
    try {
      ...
      if (keyFilters.isActive !== "")
        params.append("isActive", keyFilters.isActive);
      params.append("page", page);
      params.append("size", keyPagination.size);

      const data = await apiCall(
        `/api/admin/translations/keys?${params.toString()}`,
      );
      ...
      setError(
        t("translation.keyLoadError", "번역 키 목록을 로드할 수 없습니다: ") +
          err.message,
      );
    } finally {
      setLoading(false);
  ...
  const loadAvailableCategories = async () => {
    try {
      ...
      console.warn(
        t("translation.categoryLoadError", "카테고리 목록 로드 실패:"),
        err.message,
      );
    }
  };
  ...
  const loadStats = async () => {
    try {
      ...
      setError(
        t("translation.statisticsLoadError", "통계를 로드할 수 없습니다: ") +
          err.message,
      );
    } finally {
      setLoading(false);
  ...
  const handleCreateLanguage = async (formData) => {
    try {
      ...
      setError(
        t("translation.languageCreateError", "언어 생성 실패: ") + err.message,
      );
    }
  };
  ...
  const handleUpdateLanguage = async (id, formData) => {
    try {
      ...
      setError(
        t("translation.languageUpdateError", "언어 업데이트 실패: ") +
          err.message,
      );
    }
  };
  ...
  const handleDeleteLanguage = async (id) => {
    ...
    try {
      ...
      setError(
        t("translation.languageDeleteError", "언어 삭제 실패: ") + err.message,
      );
    }
  };
  ...
  const handleCreateTranslationKey = async (formData) => {
    try {
      ...
      setError(
        t("translation.keyCreateError", "번역 키 생성 실패: ") + err.message,
      );
    }
  };
  ...
  const handleUpdateTranslationKey = async (id, formData) => {
    try {
      ...
      setError(
        t("translation.keyUpdateError", "번역 키 업데이트 실패: ") +
          err.message,
      );
    }
  };
  ...
  const handleDeleteTranslationKey = async (id) => {
    ...
    try {
      ...
      setError(
        t("translation.keyDeleteError", "번역 키 삭제 실패: ") + err.message,
      );
    }
  };
  ...
  const handleCreateOrUpdateTranslation = async (formData) => {
    try {
      ...
      setError(
        t("translation.translationSaveError", "번역 저장 실패: ") + err.message,
      );
    }
  };
  ...
  const handleDeleteTranslation = async (id) => {
    ...
    try {
      ...
      setError(
        t("translation.translationDeleteError", "번역 삭제 실패: ") +
          err.message,
      );
    }
  };
  ...
  const handleClearCache = async () => {
    try {
      ...
      setError(
        t("translation.cacheRefreshError", "캐시 초기화 실패: ") + err.message,
      );
    }
  };
  ...
  const handleExportCsv = async (languageCode = null) => {
    try {
      ...
      const filename = languageCode
        ? `translations_${languageCode}.csv`
      ...
      setError(
        t("translation.csvExportError", "CSV 내보내기 실패: ") + err.message,
      );
    }
  };
  ...
  const handleImportCsv = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/admin/translations/import/csv`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (result.success) {
        ...
        if (result.errors && result.errors.length > 0) {
          ...
          if (result.errors.length > 5) {
            errorMessage +=
              `\n... ` +
              t("translation.csvImportMoreErrors", "그 외 {count}개 오류", {
      ...
      setError(
        t("translation.csvImportError", "CSV 가져오기 실패: ") + err.message,
      );
    }
  };
    ...
    <Box sx={{ width: "100%" }}>
      ...
      <Paper sx={{ width: "100%" }}>
        ...
        >
          <Tab
            icon={<LanguageIcon />}
            label={t("translation.tabs.languageManagement")}
          />
          <Tab icon={<KeyIcon />} label={t("translation.tabs.keyManagement")} />
          <Tab
            icon={<TranslateIcon />}
            label={t("translation.tabs.translationManagement")}
          />
          <Tab
            icon={<LanguageIcon />}
            label={t("translation.tabs.statistics")}
          />
        </Tabs>
        ...
        <Box sx={{ p: 3 }}>
          ...
          {tabValue === 0 && (
            <LanguageManagementTab
              languages={languages}
              onAdd={() =>
                setLanguageDialog({ open: true, mode: "create", data: null })
              }
              onEdit={(language) =>
                setLanguageDialog({ open: true, mode: "edit", data: language })
              }
          ...
          {tabValue === 1 && (
            <EnhancedTranslationKeyTab
              ...
              onAdd={() =>
                setKeyDialog({ open: true, mode: "create", data: null })
              }
              onEdit={(key) =>
                setKeyDialog({ open: true, mode: "edit", data: key })
              }
      ...
      <LanguageDialog
        ...
        onSave={
          languageDialog.mode === "create"
            ...
            : (formData) =>
                handleUpdateLanguage(languageDialog.data.id, formData)
        }
      />
      ...
      <TranslationKeyDialog
        ...
        onSave={
          keyDialog.mode === "create"
            ? handleCreateTranslationKey
            : (formData) =>
                handleUpdateTranslationKey(keyDialog.data.id, formData)
        }
      />
