...
const SchedulerManagement = () => {
  ...
  const { configs, loading, error, fetchConfigs, toggleEnabled, executeNow } =
    useScheduler();
  ...
  const handleToggleEnabled = async (taskKey) => {
    try {
      ...
      setSuccessMessage(
        t("scheduler.status.changed", "스케줄 상태가 변경되었습니다."),
      );
          ...
          <Tooltip title={t("scheduler.tooltip.execute", "즉시 실행")}>
            <span>
              <IconButton
                size="small"
                onClick={() =>
                  handleExecuteNow(params.row.taskKey, params.row.taskName)
                }
                color="success"
        ...
        >
          ...
          <Box sx={{ textAlign: "right" }}>
            <Typography
              ...
              display="block"
            >
              {t("scheduler.currentTime", "현재 시간 ({timezone})").replace(
                "{timezone}",
                user?.timeZone || user?.timezone || "Asia/Seoul",
              )}
            </Typography>
            <Chip
