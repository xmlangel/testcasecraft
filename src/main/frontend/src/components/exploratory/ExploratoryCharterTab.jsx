    ...
    <Stack spacing={2}>
      ...
      <Grid container spacing={2}>
        ...
        {filteredCharters.map((item) => {
          ...
          const missionSummary = item.mission ? item.mission.split(/\n\s*-{2,}\s*\n|\n\s*-{2,}\s*$/)[0].trim() : "";
      ...
      <Dialog open={charterDialogOpen} onClose={() => setCharterDialogOpen(false)} fullWidth maxWidth="lg" fullScreen>
        <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <Button
            ...
            color="info"
          >
            {showExamples ? t("common.hide", "숨기기") : t("exploratory.charter.guide.show", "작성 가이드 보기")}
          </Button>
        </DialogTitle>
          ...
          <Stack spacing={1.5} sx={{ mt: 0.5 }}>
            <Collapse in={showExamples}>
              <Alert severity="info" variant="outlined" sx={{ mb: 2 }}>
                ...
                <Typography variant="body2">
                  • {t("exploratory.charter.principles.specificity", "적정 수준의 구체성: 테스트 방향을 제시할 수 있을 정도")}
                </Typography>
                <Typography variant="body2">
                  • {t("exploratory.charter.principles.riskBased", "리스크 기반 접근: 고위험 영역에 집중 배치")}
                </Typography>
                <Typography variant="body2">
                  • {t("exploratory.charter.principles.focus", "한 번에 한 임무 집중: 세션 중 몰입 환경 확보")}
                </Typography>
                <Divider sx={{ my: 1 }} />
                <Typography variant="subtitle2" sx={{ mb: 0.5, fontWeight: 700 }}>
                  {t("exploratory.charter.examples.templateTitle", "작성 예시 (로그인 기능)")}
                </Typography>
                <Typography variant="body2">{t("exploratory.charter.examples.login.goal", "- 목표: 일반/특수 사용자 로그인 안정성 검증")}</Typography>
                <Typography variant="body2">{t("exploratory.charter.examples.login.resources", "- 자원: 테스트 계정, Postman, 개발자 도구")}</Typography>
                <Typography variant="body2">{t("exploratory.charter.examples.login.notes", "- 주의점: 토큰 유효성, 다국어 처리, 네트워크 지연")}</Typography>
              </Alert>
            </Collapse>
            <TextField
              fullWidth
            ...
            />
            <MarkdownFieldEditor
              label={t("exploratory.charter.dialog.mission", "미션 내용 (Markdown)")}
              value={charterForm.mission}
              placeholder={t("exploratory.charter.dialog.missionPlaceholder", "차터 내용을 마크다운으로 작성하세요.")}
              onChange={(val) => setCharterForm((prev) => ({ ...prev, mission: val }))}
              theme={theme}
              t={t}
