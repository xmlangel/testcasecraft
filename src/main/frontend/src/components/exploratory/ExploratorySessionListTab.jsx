...
function ExploratorySessionListTab({
  ...
  return (
    <Stack spacing={2}>
      ...
      {sessionsLoading && <Alert severity="info">{t("common.loading", "로딩 중...")}</Alert>}
      ...
      <List sx={{ bgcolor: "background.paper", borderRadius: 1 }}>
        {!sessionsLoading && filteredSessions.length === 0 && (
          <ListItem>
            <ListItemText primary={t("exploratory.session.empty", "조건에 맞는 세션이 없습니다.")} />
          </ListItem>
        )}
                ...
                primary={
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap" }}>
                    ...
                    {item.durationMin != null && (
                      <Chip size="small" label={t("exploratory.session.label.duration", "순수 실행 {minutes}분", { minutes: item.durationMin })} />
                    )}
                  </Box>
                }
                secondary={t(
