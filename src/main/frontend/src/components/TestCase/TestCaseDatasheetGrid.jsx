...
const TestCaseDatasheetGrid = ({
  ...
  const convertDataToGrid = useCallback(
    (testCases) => {
      ...
      return flattenedTestCases.map((testCase, index) => {
        const row = {
          ...
          parentFolder: testCase.parentId
            ? allData.find((item) => item.id === testCase.parentId)?.name || ""
            : "",
          name: testCase.name || "",
          ...
          isAutomated:
            testCase.isAutomated === true || testCase.isAutomated === "Y",
          executionType:
        ...
        for (let i = 0; i < maxSteps; i++) {
          const stepNum = i + 1;
          if (
            testCase[`step${stepNum}_description`] !== undefined ||
            testCase[`step${stepNum} _description`] !== undefined
          ) {
            // AI generated flat structure
            row[`step${stepNum} _description`] =
              testCase[`step${stepNum}_description`] ||
              testCase[`step${stepNum} _description`] ||
              "";
            row[`step${stepNum} _expected`] =
              testCase[`step${stepNum}_expectedResult`] ||
              testCase[`step${stepNum} _expectedResult`] ||
              "";
          } else {
            // Standard structure
  ...
  useEffect(() => {
    ...
    if (
      data &&
      data.length > 0 &&
      data.some((item) => item.__isAIGenerated === true)
    ) {
      setHasChanges(true);
    }
  ...
  const columns = useMemo(() => {
    const baseColumns = [
      ...
      {
        field: "parentFolder",
        headerName: t("testcase.spreadsheet.column.parentFolder", "상위폴더"),
        width: 120,
        editable: true,
      },
      {
        field: "name",
        headerName: t("testcase.spreadsheet.column.name", "이름"),
        width: 200,
        editable: true,
      },
      {
        field: "description",
        headerName: t("testcase.spreadsheet.column.description", "설명"),
        width: 250,
        editable: true,
      },
      {
        field: "preCondition",
        headerName: t("testcase.spreadsheet.column.preCondition", "사전조건"),
        width: 200,
        editable: true,
      },
      {
        field: "postCondition",
        headerName: t("testcase.spreadsheet.column.postCondition", "사후조건"),
        width: 200,
        editable: true,
      },
      {
        ...
        headerName: t(
          "testcase.spreadsheet.column.expectedResults",
          "예상결과",
        ),
        width: 200,
        editable: true,
      ...
      {
        field: "testTechnique",
        headerName: t("testcase.spreadsheet.column.testTechnique", "기법"),
        width: 150,
        editable: true,
      },
    ];
      ...
      stepColumns.push(
        {
          ...
          headerName: t("testcase.spreadsheet.column.step", "Step {number}", {
            number: i + 1,
          }),
          width: 200,
          editable: true,
        },
        {
          ...
          headerName: t(
            "testcase.spreadsheet.column.expected",
            "Expected {number}",
            { number: i + 1 },
          ),
          width: 200,
  ...
  const handleAddRow = useCallback(() => {
    ...
    const newRow = {
      ...
      displayOrder:
        gridRows.length > 0
          ? Math.max(...gridRows.map((r) => r.displayOrder || 0)) + 10
          : 10,
      type: "testcase",
  ...
  const findFolderIdByName = useCallback((folderName, allData) => {
    ...
    const folder = allData.find(
      (item) => item.type === "folder" && item.name === folderName.trim(),
    );
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const convertedTestCases = gridRows.map((row, index) => {
        ...
        if (row.parentFolder !== undefined) {
          if (!row.parentFolder || row.parentFolder.trim() === "") {
            ...
            const parentInGrid = gridRows.find(
              (r) => r.type === "folder" && r.name === row.parentFolder,
            );
            if (parentInGrid) {
        ...
        debugLog(
          "Spreadsheet",
          `Row ${index} (${row.name}): parentFolder = '${row.parentFolder}', parentId = '${parentId}'`,
        );

        return {
          id:
            row.id.startsWith("temp-") || row.id.startsWith("new-")
              ? null
              : row.id,
          name: row.name,
      ...
      // 유효성 검사 (이름 필수)
      const validTestCases = convertedTestCases.filter(
        (tc) => tc.name && tc.name.trim() !== "",
      );
      ...
      if (validTestCases.length === 0 && deletedIds.size === 0) {
        setSnackbarMessage(
          t(
            "testcase.spreadsheet.message.noChanges",
            "저장할 데이터가 없습니다.",
          ),
        );
  ...
  return (
    <Card sx={{ minHeight: 500 }}>
        ...
        {!readOnly && (
          <Alert severity="info" sx={{ mb: 2 }}>
            <Typography variant="body2">
              <strong>
                {t("testcase.advancedGrid.features.title", "고급 기능:")}
              </strong>{" "}
              {t(
              ...
              <br />
              <strong>
                {t("testcase.advancedGrid.multiSelect.title", "다중 선택:")}
              </strong>{" "}
              {t(
                "testcase.advancedGrid.multiSelect.range",
                "체크박스로 여러 행을 선택하여 일괄 삭제할 수 있습니다.",
              )}
              <br />
              <strong>
                {t("testcase.advancedGrid.tips.title", "팁:")}
              </strong>{" "}
              {t(
        ...
        >
          <Typography variant="caption" color="text.secondary">
            {t(
              ...
              "* MUI DataGrid 기반 고급 스프레드시트 • {count}개 스텝 • 효율적인 대량 편집 지원",
              { count: maxSteps },
            )}
          </Typography>
      ...
      >
        ...
        <DialogActions>
          <Button onClick={handleStepSettingsClose}>
