...
const IMPORT_FORMATS = [
  ...
  { value: "excel",         label: "Excel (.xlsx)", accept: ".xlsx,.xls", sampleExt: "xlsx" },
  { value: "json",          label: "JSON",          accept: ".json", sampleExt: "json" },
  { value: "google-sheets", label: "Google Sheets", accept: null, sampleExt: null },
];
...
export default function TestCaseImportExportDialog({ open, onClose, projectId, onImportComplete }) {
  ...
  const handleValidate = async () => {
    ...
    setImportError(null);
    try {
      const result = await validateImportFile(selectedFile, importFormat, projectId);
  ...
  const handleImport = async () => {
    if (importFormat === "google-sheets") {
      ...
      setImportError(null);
      try {
        const result = await importFromGoogleSheet(googleSheetUrl, googleSheetName, projectId);
        const count = Array.isArray(result) ? result.length : result.importedCount || 0;
    ...
    setImportError(null);
    try {
      const result = await importTestCases(selectedFile, importFormat, projectId);
  ...
  const handleExport = async () => {
    ...
    try {
      if (exportFormat === "google-sheets") {
        if (!exportSheetUrl) { setExportMessage({ type: "error", text: "Google Sheets URL을 입력하세요" }); return; }
        await exportToGoogleSheet(projectId, exportSheetUrl, exportSheetName);
        setExportMessage({ type: "success", text: "Google Sheets에 내보내기 완료!" });
      } else {
        const result = await exportTestCasesAs(projectId, exportFormat);
        if (result.success) {
          setExportMessage({ type: "success", text: "파일 다운로드가 시작되었습니다." });
        } else {
  ...
  const canImport = importFormat === "google-sheets"
        ...
        {activeTab === 0 && (
          <Box>
            ...
            <FormControl component="fieldset" sx={{ mb: 3 }}>
              ...
              <RadioGroup row value={importFormat} onChange={handleImportFormatChange}>
            ...
            <Box sx={{ mb: 3 }}>
              <Typography variant="body2" fontWeight={600} sx={{ mb: 1 }}>
                {importFormat !== "google-sheets" ? "3. 파일 업로드" : "2. Google Sheets 연결"}
              </Typography>
              ...
              {importFormat === "google-sheets" ? (
                ...
                <Box
                  ...
                  sx={{
                    ...
                    textAlign: "center",
                    cursor: "pointer",
                    bgcolor: isDragging ? "action.hover" : selectedFile ? "success.50" : "background.default",
                    transition: "all 0.2s",
                ...
                >
                  ...
                  {selectedFile ? (
                    ...
                    <Box>
                      <FileUploadIcon color="action" sx={{ fontSize: 40, mb: 1 }} />
            ...
            {/* 4. 검증 결과 */}
            {importFormat !== "google-sheets" && selectedFile && !importResult && (
              <Box sx={{ mb: 2 }}>
        ...
        {activeTab === 1 && (
          <Box>
            <FormControl component="fieldset" sx={{ mb: 3 }}>
              ...
              <RadioGroup row value={exportFormat} onChange={(e) => { setExportFormat(e.target.value); setExportMessage(null); }}>
              ...
              <TableBody>
                {errors.map((err, i) => (
                  <TableRow key={i}>
                    ...
                    <TableCell>
                      <Typography variant="caption" color="error">{err.message}</Typography>
                    </TableCell>
                    <TableCell>
                      <Typography variant="caption" color="text.secondary">
                        {err.value || "(빈 값)"}
              ...
              <TableBody>
                {previewData.map((row, i) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Typography variant="caption">{row.parentPath || "-"}</Typography>
                    </TableCell>
