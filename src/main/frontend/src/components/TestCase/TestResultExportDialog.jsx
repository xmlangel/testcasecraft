...
const TestResultExportDialog = ({
  ...
  const exportAsPdf = async (headers, data, fileName, summaryData = null) => {
    ...
    const drawHeaderSection = () => {
      ...
      pdf.setFontSize(18);
      pdf.text(
        t("testResult.export.pdf.title", "테스트 결과 고급 리포트"),
        margin,
        cursorY,
      );
      cursorY += 20;
      ...
      const projectLabel = `${t(
        ...
        activeProject?.name ||
        t("testResult.export.pdf.project.unknown", "미지정 프로젝트")
      }`;
      const generatedAtLabel = `${t(
    ...
    const drawSummarySection = () => {
      ...
      const periodText = (() => {
        if (summary.periodStart && summary.periodEnd) {
          return `${formatDate(
            summary.periodStart,
            "yyyy-MM-dd",
          )} ~ ${formatDate(summary.periodEnd, "yyyy-MM-dd")}`;
        }
        if (summary.periodStart || summary.periodEnd) {
      ...
      const cards = [
        {
          ...
          value: `${formatCountValue(summary.total)}${t(
            "testResult.export.pdf.summary.unit",
            "건",
          )}`,
          subValue: `${t(
            "testResult.export.pdf.summary.period",
            "기간",
          )}: ${periodText}`,
          bg: [235, 248, 255],
        },
        {
          ...
          subValue: t(
            "testResult.export.pdf.summary.executed",
            "실행 {count}건",
        ...
        },
        {
          ...
          subValue: t(
            "testResult.export.pdf.summary.passCount",
            "성공 {count}건",
        ...
        },
        {
          ...
          value: `${formatCountValue(summary.jiraLinked)}${t(
            "testResult.export.pdf.summary.unit",
            "건",
          )}`,
          subValue: t(
            "testResult.export.pdf.summary.jiraHint",
            "링크된 케이스 수",
          ),
          bg: [255, 243, 246],
        },
      ...
      breakdowns.forEach((item) => {
        ...
        pdf.setFontSize(12);
        pdf.text(
          `${formatCountValue(item.count)} (${formatPercentageValue(
            item.rate,
          )})`,
          cardX + 10,
          cursorY + 34,
    ...
    const drawListEntries = () => {
      if (!Array.isArray(data) || data.length === 0) {
        pdf.setFontSize(10);
        pdf.text(
          t("testResult.export.pdf.noData", "표시할 테스트 결과가 없습니다."),
          margin,
          cursorY + 12,
        );
        cursorY += 24;
      ...
      data.forEach((rowValues, index) => {
        ...
        const contentLinesCount = labelLines.reduce(
          (acc, lineArr) => acc + lineArr.length,
          0,
        );
        const blockHeight =
          contentLinesCount * lineHeight +
          labelLines.length * 4 +
          listPadding * 2 +
          20;
        ...
        pdf.text(
          t(
            "testResult.export.pdf.list.itemTitle",
            "테스트 결과 #{index}",
    ...
    pdf.setFontSize(12);
    pdf.text(
      t("testResult.export.pdf.detailTitle", "상세 테스트 결과"),
      margin,
      cursorY + 12,
    );
    cursorY += 20;
