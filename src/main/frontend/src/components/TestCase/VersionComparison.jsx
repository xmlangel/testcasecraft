    ...
    >
      <AccordionSummary expandIcon={<ExpandMoreIcon />}>
        <Typography variant="body2">
          {t("versionComparison.showDetails", "상세 내용 보기")}
        </Typography>
      </AccordionSummary>
...
const VersionComparison = ({ open, onClose, version1Id, version2Id }) => {
  ...
  const fetchComparisonData = async () => {
    ...
    try {
      // 버전 비교 API 호출
      const comparisonResponse = await api(
  ...
  const getChangeIcon = (changeType) => {
    switch (changeType) {
      case "ADDED":
        return { icon: <AddIcon />, color: "success" };
      case "REMOVED":
        return { icon: <RemoveIcon />, color: "error" };
      case "MODIFIED":
        return { icon: <EditIcon />, color: "warning" };
      default:
        return { icon: <EditIcon />, color: "default" };
    }
  };
      ...
      <Accordion key={fieldName} expanded>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          ...
          >
            ...
            <Chip
              size="small"
              label={t("versionComparison.changesCount", "{count}개 변경", {
          ...
          <TableContainer component={Paper} variant="outlined">
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell>
                    {t("versionComparison.changeType", "변경 유형")}
                  </TableCell>
                      ...
                      <Grid size={{ xs: 6 }}>
                        ...
                        <Box
                          sx={{
                            ...
                            mt: 1,
                            bgcolor:
                              stepChange.changeType === "ADDED"
                                ? "rgba(46, 125, 50, 0.1)"
                                : stepChange.changeType === "MODIFIED"
                                  ? "rgba(245, 124, 0, 0.1)"
                                  : (theme) =>
                                      theme.palette.mode === "dark"
                                        ? "rgba(255, 255, 255, 0.05)"
                        ...
                        >
                          ...
                          >
                            ...
                            {stepChange.newStep?.expectedResult || "-"}
                          </Typography>
                        </Box>
                      </Grid>
                      <Grid size={{ xs: 6 }}>
                        ...
                        >
                          ...
                          >
                            <strong>Expected:</strong>{" "}
                            {stepChange.oldStep?.expectedResult || "-"}
                          </Typography>
                        </Box>{" "}
                      </Grid>
                    </Grid>
      ...
      <DialogContent dividers sx={{ p: 0 }}>
        {loading && (
          <Box sx={{ display: "flex", justifyContent: "center", p: 3 }}>
            <CircularProgress />
            <Typography sx={{ ml: 2 }}>
              {t("versionComparison.comparing", "버전을 비교하고 있습니다...")}
            </Typography>
          </Box>
        )}
            ...
            <Box sx={{ mb: 2 }}>
              ...
              <Box sx={{ display: "flex", gap: 1, mb: 2 }}>
                {comparisonData.totalChanges > 0 ? (
                  <>
                    <Chip
                      label={t(
                        ...
                        "총 {count}개 변경",
                        { count: comparisonData.totalChanges },
                      )}
                      color="info"
                    />
                    {comparisonData.hasStepChanges && (
                      <Chip
                        label={t("versionComparison.stepChanges", "스텝 변경")}
                        color="warning"
                      />
                    )}
                    {comparisonData.hasFieldChanges && (
                      <Chip
                        label={t("versionComparison.fieldChanges", "필드 변경")}
                        color="primary"
                      />
                    )}
                  </>
                  ...
                  <Chip
                    label={t("versionComparison.noChanges", "변경 사항 없음")}
                    color="success"
                  />
                )}
              </Box>
