...
const TestResultAttachmentsView = ({
  ...
  const isTextFile = (attachment) => {
    ...
    const extension = attachment.fileExtension?.toLowerCase() || "";
    return (
      mimeType.includes("text") || ["txt", "log", "md"].includes(extension)
    );
  };
  ...
  const handlePreview = async (attachment) => {
    try {
      ...
      setError(
        t("attachments.error.previewError", "미리보기를 생성할 수 없습니다."),
      );
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await api(`/api/attachments/${selectedAttachment.id}`, {
        method: "DELETE",
      });

      // 목록에서 제거
      setAttachments((prev) =>
        prev.filter((att) => att.id !== selectedAttachment.id),
      );
      ...
      setError(
        t("attachments.error.deleteError", "파일 삭제 중 오류가 발생했습니다."),
      );
    }
  };
  ...
  const wrappedContent = compact ? (
    ...
    <Card variant="outlined">
      ...
      <CardContent sx={{ pt: showHeader ? 0 : 2 }}>{content}</CardContent>
    </Card>
  );
  ...
  return (
    <>
      ...
      >
        <DialogTitle>
          {t("attachments.delete.title", "첨부파일 삭제")}
        </DialogTitle>
        <DialogContent>
          <Typography>
