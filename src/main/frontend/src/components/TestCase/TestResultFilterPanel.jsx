...
const TestResultFilterPanel = ({
  ...
  const loadTestExecutions = async (testPlanId = null) => {
    try {
      const response = await testResultService.getTestExecutionsForFilter(
        projectId,
        testPlanId,
      );
      if (response.success && response.data) {
    ...
    <Paper sx={{ p: 2, mb: 2 }}>
      ...
      >
        ...
        <FormControl sx={{ minWidth: 200 }} disabled={loading || disabled}>
          <InputLabel>
            {t("testResult.filter.testPlan", "테스트 플랜")}
          </InputLabel>
          <Select
        ...
        <FormControl sx={{ minWidth: 200 }} disabled={loading || disabled}>
          <InputLabel>
            {t("testResult.filter.testExecution", "테스트 실행")}
          </InputLabel>
          <Select
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          <Button
            ...
            disabled={loading || disabled}
            startIcon={
              loading ? <CircularProgress size={16} /> : <FilterListIcon />
            }
          >
      ...
      {(selectedTestPlan || selectedTestExecution) && (
        <>
          ...
          >
            ...
            {selectedTestPlan && (
              <Chip
                label={`${t("testResult.filter.planPrefix", "플랜:")} ${
                  testPlans.find((p) => p.id === selectedTestPlan)?.name ||
                  selectedTestPlan
                }`}
                size="small"
                color="primary"
            ...
            {selectedTestExecution && (
              <Chip
                label={`${t("testResult.filter.executionPrefix", "실행:")} ${
                  testExecutions.find((e) => e.id === selectedTestExecution)
                    ?.name || selectedTestExecution
                }`}
                size="small"
                color="primary"
                variant="outlined"
