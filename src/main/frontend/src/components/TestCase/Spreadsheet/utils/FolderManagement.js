...
export const findFolderIdByName = (folderNameOrId, allData) => {
  if (
    !folderNameOrId ||
    (typeof folderNameOrId === "string" && !folderNameOrId.trim())
  )
    return null;

  const searchValue =
    typeof folderNameOrId === "string"
      ? folderNameOrId.trim()
  ...
  if (/^\d+$/.test(searchValue)) {
    ...
    if (!folder) {
      folder = allData.find(
        (item) => item.type === "folder" && item.displayOrder === numericValue,
      );
    }
  ...
  const folder = allData.find(
    (item) => item.type === "folder" && item.name === searchValue,
  );
...
export const sortFoldersByHierarchy = (folders, existingData) => {
  ...
  const addFolderWithParents = (folder, depth = 0) => {
    ...
    if (folder.parentFolderName && folder.parentFolderName.trim()) {
      ...
      const parentFolder = folders.find(
        (f) => f.name === folder.parentFolderName,
      );
      if (parentFolder && !visited.has(parentFolder.name)) {
        ...
        // 기존 데이터에서 부모 찾기
        const existingParent = existingData.find(
          (item) =>
  ...
  const rootFolders = folders.filter(
    (f) => !f.parentFolderName || f.parentFolderName.trim() === "",
  );
