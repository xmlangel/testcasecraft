...
const TestResultDetailTable = ({ projectId, onViewResult, dense = false }) => {
  ...
  const [columnVisibilityMenuAnchor, setColumnVisibilityMenuAnchor] =
    useState(null);
    ...
    async (filters = null) => {
      ...
      try {
        ...
        if (hasFilters) {
          ...
          // 기존 전체 데이터 로드 방식
          const apiUrl =
            buildUrl(API_ENDPOINTS.TEST_RESULTS.REPORT) +
        ...
        const tableData = testResults.map((result, index) => {
          ...
          const additionalJiraIds = jiraService.extractIssueKeys(
            result.notes || "",
          );
          const allJiraIds = jiraId
            ? [jiraId, ...additionalJiraIds.filter((id) => id !== jiraId)]
            : additionalJiraIds;
          ...
          return {
            ...
            folder:
              result.folderPath ||
              parentFolder?.name ||
              t("testResult.defaultValue.root", "루트"),
            testCase:
              result.testCaseName ||
              testCase?.name ||
              t(
            ...
            executor:
              result.executorName ||
              t("testResult.defaultValue.system", "시스템"),
            notes: result.notes || "",
            jiraId: jiraId,
            ...
            isAutomated:
              typeof testCase?.isAutomated === "boolean"
            ...
            linkedDocuments: Array.isArray(testCase?.linkedDocumentIds)
              ? testCase.linkedDocumentIds
              : [],
  ...
  const handleEditClick = (testResultId, executionId) => {
    const testResult = rows.find(
      (row) =>
        row.testCaseId === testResultId && row.executionId === executionId,
    );
    ...
    () => [
      ...
      },
      {
        ...
        renderCell: (params) => {
          ...
          const editStatusInfo = hasActiveEdit
            ? testResultEditService.getEditStatusInfo(hasActiveEdit.editStatus)
            : null;
              ...
              <Box sx={{ display: "flex", gap: 0.5 }}>
                ...
                <Tooltip title={t("testResult.button.edit", "편집")}>
                  <IconButton
                    ...
                    onClick={() =>
                      handleEditClick(
                        params.row.testCaseId,
                        params.row.executionId,
                      )
                    }
                    sx={{ p: 0.5 }}
                  >
                ...
                {onViewResult && (
                  <Tooltip
                    title={t("testResult.button.viewDetail", "상세보기")}
                  >
                    <IconButton
                      size="small"
                      onClick={() =>
      ...
      },
      {
        ...
        renderCell: (params) => {
          ...
          const resultKey = params.value
            ? `testResult.status.${params.value.toLowerCase()}`
            : "";
          const label = resultKey
            ? t(resultKey, getResultLabel(params.value))
            : "-";
      ...
      // ICT-275: 사전설정 컬럼
      {
        ...
        renderCell: (params) => (
          <Tooltip
            title={
              params.value ||
              t("testResult.tooltip.noPreCondition", "사전설정 없음")
            }
          >
            <Typography
              variant="body2"
      ...
      },
      {
        ...
        renderCell: (params) => (
          <Tooltip
            title={
              params.value ||
              t("testResult.tooltip.noPostCondition", "사후 조건 없음")
            }
          >
            <Typography
              variant="body2"
                ...
                >
                  ...
                  >
                    ...
                    {step.description && (
                      ...
                      >
                        <strong>
                          {t("testResult.steps.description", "설명")}:
                        </strong>{" "}
                        {step.description}
                      </Typography>
                    )}
      ...
      // ICT-275: 전체 예상결과 컬럼
      {
        ...
        renderCell: (params) => (
          <Tooltip
            title={
              params.value ||
              t("testResult.tooltip.noExpectedResults", "전체 예상결과 없음")
            }
          >
            <Typography
              variant="body2"
      ...
      },
      {
        ...
        renderCell: (params) => (
          <Tooltip
            title={
              params.value ||
              t("testResult.tooltip.noTestTechnique", "테스트 기법 없음")
            }
          >
            <Typography
              variant="body2"
      ...
      // ICT-362: 첨부파일 컬럼
      {
        ...
        renderCell: (params) => {
          ...
          return (
            ...
            >
              <Tooltip
                title={t("testResult.button.viewAttachments", "첨부파일 보기")}
              >
                <IconButton
                  size="small"
                  onClick={(e) => {
        ...
        renderCell: (params) => {
          ...
          return (
            <Tooltip title={tooltipContent} arrow placement="top">
              <Box sx={{ display: "flex", gap: 0.5, flexWrap: "wrap" }}>
                <Chip
                  label={t("testResult.column.linkedDocCount", "{count}건", {
          ...
          return (
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
              ...
              {params.row.hasMultipleJiraIds && (
                <Tooltip
                  title={t(
                    "testResult.tooltip.multipleJiraIds",
                    `총 ${params.row.jiraIds.length}개의 JIRA ID`,
                    { count: params.row.jiraIds.length },
                  )}
                >
                  <Chip
                    label={`+${params.row.jiraIds.length - 1}`}
                    size="small"
                    variant="outlined"
                  />
                </Tooltip>
    ...
    (params) => {
      ...
      steps.forEach((step) => {
        ...
        if (step.expectedResult) {
          const expectedResultLines = Math.ceil(
            step.expectedResult.length / 50,
          );
        ...
        >
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              {t(
                "testResult.count.results",
                "{count}개의 테스트 결과{filtered}",
                {
                  count: rows.length,
                  filtered: isFiltered
                    ? ` (${t("testResult.status.filtered", "필터링됨")})`
                    : "",
                },
              )}
            </Typography>
