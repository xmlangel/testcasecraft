...
const TestResultEditDialog = ({
  ...
  const [jiraValidation, setJiraValidation] = useState({
    status: null,
    message: null,
  });
        ...
        if (result.exists) {
          setJiraValidation({
            ...
            message: `✅ ${result.issueKey}: ${
              result.summary ||
              t("testResultEditDialog.jiraIssueExists", "이슈가 존재합니다")
            }`,
          });
        } else {
          setJiraValidation({
  ...
  const initializeEditDialog = async () => {
    ...
    try {
      ...
      if (
        activeEditData &&
        activeEditData.editStatus === "DRAFT" &&
        activeEditData.editedByUserId === user?.id
      ) {
        // 기존 DRAFT 편집본 로드
        setEditData({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editData.editedJiraIssueKey.trim()) {
        if (jiraValidation.status === "error") {
          setError(
            t(
              ...
              `JIRA 이슈 검증 실패: ${jiraValidation.message}`,
            ),
          );
          return;
        }
        ...
        if (
          !jiraValidation.status &&
          jiraService.isValidIssueKey(editData.editedJiraIssueKey.trim())
        ) {
          const result = await jiraService.checkIssueExists(
            editData.editedJiraIssueKey.trim(),
          );
          if (!result.exists) {
            setError(
              t(
      ...
      if (
        ...
        activeEdit.editedByUserId === user?.id
      ) {
        // 기존 DRAFT 편집본 수정
        ...
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, sm: 6 }}>
            ...
            <Typography variant="body1">
              {testCase?.name ||
                t("testResultEditDialog.unknown", "알 수 없음")}
            </Typography>
          </Grid>
          <Grid size={{ xs: 12, sm: 6 }}>
            <Typography variant="body2" color="text.secondary">
          ...
          <Grid size={{ xs: 12, sm: 6 }}>
            ...
            <Typography variant="body1">
              {testResult?.jiraIssueKey ||
                t("testResultEditDialog.noneAlt", "없음")}
            </Typography>
          </Grid>
        </Grid>
      </CardContent>
          ...
          <List dense>
            {editHistory.map((edit, index) => {
              ...
              return (
                <ListItem
                  key={edit.id}
                  divider={index < editHistory.length - 1}
                >
                  <ListItemIcon>
      ...
      <DialogTitle>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          {activeEdit && (
            <Chip
              label={
                testResultEditService.getEditStatusInfo(activeEdit.editStatus)
                  .label
              }
              size="small"
              color={
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <FormControl fullWidth>
                  <InputLabel>
                    {t("testResultEditDialog.testResultLabel", "테스트 결과")}
                  </InputLabel>
                  <Select
                  ...
                  >
                    <MenuItem value="PASS">
                      {t("testResultEditDialog.pass", "통과 (PASS)")}
                    </MenuItem>
                    <MenuItem value="FAIL">
                      {t("testResultEditDialog.fail", "실패 (FAIL)")}
                    </MenuItem>
                    <MenuItem value="BLOCKED">
                      {t("testResultEditDialog.blocked", "차단됨 (BLOCKED)")}
                    </MenuItem>
                    <MenuItem value="NOT_RUN">
                      {t("testResultEditDialog.notRun", "실행 안됨 (NOT_RUN)")}
                    </MenuItem>
                  </Select>
                </FormControl>
              </Grid>
                ...
                <TextField
                  ...
                  slotProps={{
                    input: {
                      ...
                      endAdornment:
                        jiraValidationLoading || jiraValidation.status ? (
                ...
                <FormControlLabel
                  control={
                    <Switch
                      ...
                      onChange={(e) =>
                        handleInputChange("saveAsDraft", e.target.checked)
                      }
                      disabled={loading}
                    />
                  }
                  label={
                    editData.saveAsDraft
                      ? t("testResultEditDialog.saveAsDraft", "임시저장")
                      : t("testResultEditDialog.requestApproval", "승인 요청")
                  }
                />
                <Typography
                  variant="caption"
