...
const DetailReportPresetManager = ({
  ...
  // 기본 프리셋 정의 (컴포넌트 내부에서 t() 필요)
  const getDefaultPresets = () => [
    {
      id: "all",
      name: t("preset.all.name", "전체 결과"),
      isDefault: true,
      filters: {
        searchText: "",
        testCaseName: "",
      ...
      description: t("preset.all.desc", "모든 테스트 결과 표시"),
    },
    {
      id: "failed_only",
      name: t("preset.failedOnly.name", "실패 케이스만"),
      isDefault: true,
      filters: {
        searchText: "",
        testCaseName: "",
      ...
      description: t("preset.failedOnly.desc", "실패한 테스트 케이스만 표시"),
    },
    {
      id: "recent_week",
      name: t("preset.recent7days.name", "최근 7일"),
      isDefault: true,
      filters: {
        searchText: "",
        testCaseName: "",
      ...
      description: t("preset.recent7days.desc", "최근 7일간의 테스트 결과"),
    },
    {
      id: "with_jira",
      name: t("preset.withJira.name", "JIRA 연동"),
      isDefault: true,
      filters: {
        searchText: "",
        testCaseName: "",
  ...
  const savePresets = (newPresets) => {
    try {
      ...
      setError(t("preset.saveError", "프리셋 저장 중 오류가 발생했습니다."));
    }
  };
  ...
  const handleSavePreset = () => {
    if (!presetName.trim()) {
      setError(t("preset.nameRequired", "프리셋 이름을 입력해주세요."));
      return;
    }
    ...
    if (existingPreset) {
      setError(t("preset.nameDuplicate", "이미 존재하는 프리셋 이름입니다."));
      return;
    }
    ...
    const newPreset = {
      ...
      description: t("preset.userDefined", "사용자 정의 프리셋"),
    ...
    <Box>
      ...
      >
        ...
        <Button
          ...
          size="small"
        >
          {t("preset.saveCurrentFilter", "현재 필터 저장")}
        </Button>
      </Box>
                ...
                <ListItemText
                  primary={
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                      ...
                      {preset.isDefault && (
                        <Chip
                          label={t("preset.defaultLabel", "기본")}
                          size="small"
                          variant="outlined"
                          color="primary"
                        />
      ...
      >
        ...
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
            label={t("preset.nameLabel", "프리셋 이름")}
            fullWidth
            variant="outlined"
            value={presetName}
      ...
      >
        ...
        <DialogActions>
          ...
          <Button onClick={handleEditPreset} variant="contained">
            {t("preset.editDialog.confirm", "수정")}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
