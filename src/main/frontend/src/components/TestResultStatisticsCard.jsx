...
import { CheckCircle, Cancel, Block, PauseCircle } from "@mui/icons-material";
 ...
 */
function TestResultStatisticsCard({
  statistics,
  loading = false,
  error = null,
  onResultClick,
      ...
      <Card sx={{ height: "100%", minHeight: 300 }}>
        <CardContent>
          ...
          <Typography color="error">
            {t("testResult.statistics.error", { error })}
          </Typography>
        </CardContent>
      </Card>
    ),
      ...
      <CardContent>
        ...
        <Grid container spacing={2} sx={{ mb: 2 }}>
          <Grid size={{ xs: 6 }}>
            ...
            >
              ...
              >
                {isLatestMode
                  ? t("testResult.statistics.latestSuccessRate", "최종 성공률")
              ...
              <Typography variant="h5" color="success.main" fontWeight="bold">
                <CountUp
                    ...
                    isLatestMode
                      ? statistics.latestSuccessRate || 0
                      : statistics.passRate || 0
                  }
                  duration={1}
                  suffix="%"
          ...
          <Grid size={{ xs: 6 }}>
            ...
            >
              ...
              >
                {isLatestMode
                  ? t("testResult.statistics.totalCaseCount", "전체 케이스")
        ...
        <Grid container spacing={1}>
          {statisticItems.map((item) => {
            const isClickable =
              onResultClick && clickableTypes.has(item.resultType);
            return (
              <Grid size={{ xs: 6 }} key={item.label}>
                <Box
                  onClick={
                    isClickable
                      ? () => onResultClick(item.resultType)
                      : undefined
                  }
                  sx={{
                    display: "flex",
