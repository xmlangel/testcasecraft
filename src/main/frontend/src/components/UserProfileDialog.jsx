...
function UserProfileDialog({
  ...
  const handleSave = async () => {
    if (!form.name.trim() || !form.email.trim()) {
      setError(
        t("profile.validation.allRequired", "이름과 이메일을 모두 입력하세요."),
      );
      return;
    }
    try {
      ...
      setSuccess(
        t("profile.success.updated", "정보가 성공적으로 변경되었습니다."),
      );
      ...
      setError(
        err.message ||
          t("profile.error.updateFailed", "정보 변경에 실패했습니다."),
      );
    }
  };
  ...
  const handleSendVerificationEmail = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        setError(
          result.message ||
            t("profile.email.failed", "이메일 발송에 실패했습니다."),
        );
        setSuccess("");
      }
    } catch (error) {
  ...
  const handleRefreshUserInfo = async () => {
    try {
      await fetchUserInfo();
      setSuccess(
        t("profile.refresh.success", "사용자 정보를 새로고침했습니다."),
      );
      setError("");
    } catch (error) {
      console.error("Failed to refresh user info:", error);
      setError(
        t("profile.refresh.error", "사용자 정보 새로고침에 실패했습니다."),
      );
      setSuccess("");
    }
  };
  ...
  const handleJiraSave = async (configData) => {
    try {
      ...
      setSuccess(
        t("profile.jira.success.saved", "JIRA 설정이 저장되었습니다."),
      );
  ...
  const handleDeleteJiraConfig = async () => {
    ...
    if (
      !window.confirm(
        t("profile.jira.confirm.delete", "JIRA 설정을 삭제하시겠습니까?"),
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setJiraConfig(null);
      setSuccess(
        t("profile.jira.success.deleted", "JIRA 설정이 삭제되었습니다."),
      );
    } catch (error) {
      console.error("JIRA 설정 삭제 실패:", error);
      setError(
        `${t("profile.jira.error.deleteFailed", "JIRA 설정 삭제 실패")}: ${
          error.message
        }`,
      );
    }
                ...
                {versionLoading ? (
                  ...
                  >
                    ...
                    >
                      ...
                      <Chip
                        label={versionInfo.backendVersion}
                        size="small"
                        variant="outlined"
                      />
                    </Box>
                    ...
                    >
                      ...
                      <Chip
                        label={versionInfo.frontendVersion}
                        size="small"
                        variant="outlined"
                      />
                    </Box>
                    ...
                    >
                      ...
                      <Chip
                        ...
                        variant="outlined"
                        color={
                          versionInfo.ragServiceVersion === "N/A"
                            ? "default"
                            : "primary"
                        }
                      />
            ...
            {tabValue === 2 && (
              <Box>
                ...
                <Typography
                  ...
                  sx={{ mt: 2 }}
                >
                  {t("language.current", "현재 언어")}:{" "}
                  <strong>
                    {currentLanguage === "ko"
                      ? t("language.korean", "한국어")
                ...
                <Typography
                  ...
                  sx={{ mt: 2 }}
                >
                  {t("timezone.current", "현재 시간대")}:{" "}
                  <strong>{form.timezone || "UTC"}</strong>
                </Typography>
              </Box>
            )}
