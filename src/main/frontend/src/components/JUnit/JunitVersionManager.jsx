...
const JunitVersionManager = ({ testResultId, onClose, open = false }) => {
  ...
  const handleCreateVersion = async (description = "") => {
    ...
    try {
      ...
      await junitVersionService.createVersion(
        testResultId,
        originalPath,
        description,
      );
  ...
  return (
    ...
    >
      <DialogTitle>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <Typography variant="h6">
            {t("junit.version.management", "버전 관리")}
          </Typography>
        ...
        <TabPanel value={tabValue} index={0}>
          <Box sx={{ display: "flex", gap: 2, mb: 3 }}>
            <Button
              ...
              onClick={() =>
                handleCreateVersion(
                  t("junit.version.manualCreate", "수동 버전 생성"),
                )
              }
              disabled={loading}
            >
          ...
          {versionHistory && versionHistory.versions.length > 0 ? (
            <List>
              {versionHistory.versions.map((version) => {
                const statusInfo =
                  junitVersionService.getVersionStatusInfo(version);
                ...
                return (
                  <ListItem key={version.versionId} divider>
                    ...
                    <ListItemText
                      primary={
                        ...
                        >
                          ...
                          {version.compressed && (
                            <Chip
                              label={t("junit.version.compressed", "압축됨")}
                              size="small"
                              color="info"
                              variant="outlined"
                            />
                      ...
                      secondary={
                        <Box>
                          <Typography variant="body2" color="text.secondary">
                            {version.description ||
                              t("junit.version.noDescription", "설명 없음")}
                          </Typography>
                          <Typography variant="caption" color="text.secondary">
                    ...
                    <ListItemSecondaryAction>
                      <Tooltip
                        title={t("junit.version.restoreTooltip", "복원")}
                      >
                        <IconButton
                          onClick={() => {
                            setSelectedVersion(version);
                            setRestorePath(
                              `/restore/${testResultId}_v${version.versionNumber}.xml`,
                            );
        ...
        <TabPanel value={tabValue} index={1}>
          {storageStats ? (
            <Grid container spacing={3}>
              <Grid size={{ xs: 12, md: 6 }}>
                <Card>
                  <CardContent>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {junitVersionService.formatFileSize(
                        storageStats.versionStorageSize,
                      )}
                    </Typography>
                  </CardContent>
                </Card>
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <Card>
                  <CardContent>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {junitVersionService.formatFileSize(
                        storageStats.backupStorageSize,
                      )}
                    </Typography>
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardContent>
                    ...
                    <Box sx={{ display: "flex", gap: 2 }}>
                      ...
                      <Chip
                        icon={
                          storageStats.autoBackupEnabled ? (
                            <CheckIcon />
                          ) : (
                            <ErrorIcon />
                          )
              ...
              {(() => {
                const suggestions =
                  junitVersionService.getOptimizationSuggestions(storageStats);
                return (
          ...
          {versionHistory &&
            (() => {
              const backupRecommendation =
                junitVersionService.shouldRecommendBackup(versionHistory);
              return (
                backupRecommendation.recommend && (
                  <Alert
                    severity={
                      backupRecommendation.urgency === "high"
                        ? "warning"
                        : "info"
                    }
                    sx={{ mb: 3 }}
                  >
                    <Typography variant="subtitle2">
                      {t("junit.version.backupRecommended", "백업 권장")}
                    </Typography>
                    <Typography variant="body2">
      ...
      >
        <DialogTitle>
          {t("junit.version.compareDialog", "버전 비교")}
        </DialogTitle>
        <DialogContent>
          <Grid container spacing={3} sx={{ mt: 1 }}>
            <Grid size={{ xs: 6 }}>
              <FormControl fullWidth>
                <InputLabel>
                  {t("junit.version.firstVersion", "첫 번째 버전")}
                </InputLabel>
                <Select
                ...
                >
                  {versionHistory?.versions.map((v) => (
                    <MenuItem key={v.versionId} value={v.versionNumber}>
                      {t("junit.version.versionNumber", "버전")}{" "}
                      {v.versionNumber}
                    </MenuItem>
                  ))}
                </Select>
            ...
            <Grid size={{ xs: 6 }}>
              <FormControl fullWidth>
                <InputLabel>
                  {t("junit.version.secondVersion", "두 번째 버전")}
                </InputLabel>
                <Select
              ...
              <Grid container spacing={2}>
                <Grid size={{ xs: 6 }}>
                  <Typography variant="caption">
                    {t("junit.version.sizeDiff", "크기 차이")}:
                  </Typography>
                  <Typography variant="body2">
                    {comparisonResult.sizeDifference > 0 ? "+" : ""}
                    {junitVersionService.formatFileSize(
                      Math.abs(comparisonResult.sizeDifference),
                    )}
                  </Typography>
                </Grid>
                <Grid size={{ xs: 6 }}>
                  <Typography variant="caption">
                    {t("junit.version.timeDiff", "시간 차이")}:
                  </Typography>
                  <Typography variant="body2">
      ...
      >
        <DialogTitle>
          {t("junit.version.restoreDialog", "버전 복원")}
        </DialogTitle>
        <DialogContent>
          {selectedVersion && (
            <>
      ...
      >
        <DialogTitle>
          {t("junit.version.backupCreateDialog", "백업 생성")}
        </DialogTitle>
        <DialogContent>
          <FormControl fullWidth sx={{ mt: 2 }}>
            <InputLabel>
              {t("junit.version.backupVersionLabel", "백업할 버전")}
            </InputLabel>
            <Select
              value={backupVersion}
            ...
            >
              <MenuItem value={-1}>
                {t("junit.version.latestVersion", "최신 버전")}
              </MenuItem>
              {versionHistory?.versions.map((v) => (
                <MenuItem key={v.versionId} value={v.versionNumber}>
