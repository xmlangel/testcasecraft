...
const JunitTestCaseEditor = ({
  ...
  const priorityOptions = [
    ...
    {
      value: "MEDIUM",
      label: t("junit.editor.priority.medium"),
      color: "warning",
    },
  ...
  const fetchPreviousNote = async () => {
    ...
    try {
      const result = await junitResultService.getPreviousTestCaseNote(
        testCase.id,
      );
  ...
  const userStatus = editForm.userStatus
    ? statusConfig[editForm.userStatus]
    : null;
        ...
        <Box display="flex" alignItems="center" justifyContent="space-between">
          <Box display="flex" alignItems="center" gap={2}>
            ...
            <Typography variant="h6">
              {t("junit.editor.title")}{" "}
              {readOnly
                ? t("junit.editor.viewMode")
                : t("junit.editor.editMode")}
            </Typography>
            <Chip
        ...
        <Grid container spacing={3}>
          ...
          <Grid size={{ xs: 12 }}>
            <TextField
              ...
              rows={3}
              label={t(
                "junit.editor.userDefinedDescription",
                "사용자 정의 설명",
              )}
              placeholder={t(
              ...
              onChange={(e) =>
                handleFormChange("userDescription", e.target.value)
              }
              disabled={readOnly}
              slotProps={{
                input: {
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <FormControl fullWidth>
              <InputLabel>
                {t("junit.editor.userDefinedStatus", "사용자 정의 상태")}
              </InputLabel>
              <Select
                value={editForm.userStatus || ""}
                onChange={(e) =>
                  handleFormChange("userStatus", e.target.value || null)
                }
                disabled={readOnly}
              ...
              >
                <MenuItem
                  value=""
                  data-testid="automation-case-status-option-original"
                >
                  <em>
                    {t("junit.editor.useOriginalStatus", "원본 상태 사용")}
                  </em>
                </MenuItem>
                {Object.entries(statusConfig).map(([key, config]) => (
                  <MenuItem
                    key={key}
                    value={key}
                    data-testid={`automation-case-status-option-${key}`}
                  >
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <FormControl fullWidth>
              <InputLabel>
                {t("junit.editor.priorityLabel", "우선순위")}
              </InputLabel>
              <Select
              ...
              >
                {priorityOptions.map((option) => (
                  <MenuItem
                    key={option.value}
                    value={option.value}
                    data-testid={`automation-case-priority-option-${option.value}`}
                  >
                    <Chip
                      label={option.label}
                      color={option.color}
                ...
                action={
                  <Button
                    ...
                    onClick={() => {
                      ...
                      const date = new Date(
                        previousNoteInfo.uploadedAt,
                      ).toLocaleString();
                      const copyText = `\n\n--- [${t(
                        "junit.testcase.previous",
                        "이전 테스트 케이스",
                      )}: ${execName}, ${date}] ---\n${
                  ...
                  >
                    {t(
                      "junit.testcase.previousNotes.copyBtn",
                      "현재 노트로 복사",
                    )}
                  </Button>
                }
