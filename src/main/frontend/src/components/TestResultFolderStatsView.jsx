...
const FolderTreeNode = ({
  ...
  const getStatusIcon = (rate) => {
    ...
    if (rate >= 60)
      return <CheckCircle sx={{ color: "#2196f3", fontSize: "1rem" }} />;
    if (rate >= 40)
      return <PlayCircleFilled sx={{ color: "#ffeb3b", fontSize: "1rem" }} />;
    ...
    data.forEach((item) => {
      ...
      const updateStats = (node) => {
        ...
        node.execution_count +=
          item.executionCount !== undefined
            ? item.executionCount
            : item.result && item.result !== "NOT_RUN"
              ? 1
              : 0;
      ...
      >
        ...
        >
          <Typography variant="h6">
            [{projectName || "Project"}]{" "}
            {t("testResult.folder.totalSuccessRate", "전체 성공률")}{" "}
            {folderTree.successRate.toFixed(1)}%
          </Typography>
          <AccountTree />
        </Box>
      </Paper>
          ...
          <Paper sx={{ p: 3, height: "600px" }}>
            ...
            <Box sx={{ mb: 4, textAlign: "center" }}>
              <Typography variant="body1" gutterBottom>
                {t("testResult.folder.successRate", "성공률")}:{" "}
            ...
            <List sx={{ width: "100%", maxWidth: 400, mx: "auto" }}>
              ...
              <ListItem sx={{ py: 1 }}>
                ...
                <ListItemText
                  primary={t("testResult.status.blocked", "차단")}
                />
                <Typography variant="body1" sx={{ fontWeight: 600 }}>
              ...
              <ListItem sx={{ py: 1 }}>
                <ListItemIcon>
                  <PlayCircleFilled sx={{ color: "text.disabled" }} />
                </ListItemIcon>
                <ListItemText
                  primary={t("testResult.status.notRun", "미실행")}
                />
                <Typography variant="body1" sx={{ fontWeight: 600 }}>
