...
function Dashboard() {
  ...
  const handleAccordionChange = (panel) => (event, isExpanded) => {
    ...
    localStorage.setItem(
      "testcase-manager-dashboard-accordion",
  ...
  const fetchAssigneeResults = async () => {
    ...
    try {
      ...
      if (activeProject && activeProject.id) {
        results = await fetchOpenTestRunAssigneeResultsByProject(
          activeProject.id,
          10,
        );
      } else {
    ...
    if (activeProject) {
      ...
      if (activeProject.testCaseCount !== undefined) {
        ...
        const projectTestCases = testCases.filter(
          (tc) => tc.projectId === activeProject.id,
        );
  ...
  const failRate =
    totalCases > 0 ? Math.round((lastResult.FAIL / totalCases) * 100) : 0;

  // ICT-135: 실제 API 데이터에서 차트 데이터 생성
  const testResultsHistory = dashboardData?.trend?.testResultsHistory || [];
  const openTestRunResults =
    dashboardData?.openTestRuns?.openTestRunResults || [];
  ...
  return (
    <Box sx={PAGE_CONTAINER_SX.main}>
        ...
        >
          ...
          >
            ...
            {![
              ...
              "PERMISSION_ERROR",
              "DATA_ERROR",
            ].includes(dashboardError.type) && "⚠️"}{" "}
          ...
          {dashboardError.userAction && (
            ...
            >
              {t("dashboard.error.solution", {
                action: dashboardError.userAction,
              })}
            </Typography>
          )}
      ...
      {activeProject && (
        <Accordion
          expanded={accordionExpanded.summary}
          onChange={handleAccordionChange("summary")}
          sx={{ mb: 2 }}
        >
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            <Typography variant="subtitle1" fontWeight="bold">
              {t("dashboard.sections.summary", "프로젝트 요약")}
            </Typography>
          </AccordionSummary>
          <AccordionDetails>
            <Paper
            ...
            >
              <Grid container spacing={2} alignItems="center">
                ...
                <Grid>
                  <Chip
                    label={t("dashboard.project.totalTestCases", {
                      count: totalCases,
                    })}
                    color="info"
                    size="small"
      ...
      >
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          <Typography variant="subtitle1" fontWeight="bold">
            {t("dashboard.sections.charts", "테스트 현황 및 트렌드")}
          </Typography>
        </AccordionSummary>
        <AccordionDetails>
                ...
                <Box sx={{ display: "flex", alignItems: "center" }}>
                  ...
                  <Box sx={{ ml: 2 }}>
                    <Box sx={{ display: "flex", alignItems: "baseline" }}>
                      ...
                      {failRate > 0 && (
                        <Chip
                          label={t("dashboard.status.failureRate", {
                            rate: failRate,
                          })}
                          color="error"
                          size="small"
              ...
              <StyledPaper>
                <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                  <Typography variant="subtitle1">
                    {t("dashboard.charts.testResultsTrend")}
                  </Typography>
                  <FormControl size="small" sx={{ minWidth: 120 }}>
                    <InputLabel>{t("dashboard.charts.last15Days")}</InputLabel>
                    <Select
                      label={t("dashboard.charts.last15Days")}
                      value={t("dashboard.charts.last15Days")}
                      disabled
                    >
                      <MenuItem value={t("dashboard.charts.last15Days")}>
                        {t("dashboard.charts.last15Days")}
                      </MenuItem>
                    </Select>
                  </FormControl>
                </Box>
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <LineChart data={testResultsHistory} isAnimationActive>
                      ...
                      <Line
                        ...
                        name={t("dashboard.status.blocked")}
                        strokeWidth={2}
                        dot={{ r: 4 }}
                        isAnimationActive
                      />
                      <Line
                        ...
                        name={t("dashboard.status.notrun")}
                        strokeWidth={2}
                        dot={{ r: 4 }}
                        isAnimationActive
                      />
      ...
      {/* 상세 분석 (하단 2개) */}
      <Accordion
        expanded={accordionExpanded.details}
        onChange={handleAccordionChange("details")}
      >
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          <Typography variant="subtitle1" fontWeight="bold">
            {t("dashboard.sections.details", "상세 분석")}
          </Typography>
        </AccordionSummary>
        <AccordionDetails>
          <Grid container spacing={3}>
