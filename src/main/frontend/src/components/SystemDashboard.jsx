...
const SystemDashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const totalTestCases = projects.reduce(
        (sum, project) => sum + (project.testCaseCount || 0),
        0,
      );
      const totalMembers = projects.reduce(
        (sum, project) => sum + (project.memberCount || 0),
        0,
      );
      ...
      projects.forEach((project) => {
        const orgName =
          ...
          t("systemDashboard.independentProject", "독립 프로젝트");
      ...
      const testResultStats = [
        {
          name: t("organization.dashboard.testResults.success", "Success"),
          value: Math.round(totalTestCases * 0.7),
          color: RESULT_COLORS.PASS,
        },
        {
          name: t("organization.dashboard.testResults.failure", "Failure"),
          value: Math.round(totalTestCases * 0.1),
          color: RESULT_COLORS.FAIL,
        },
        {
          name: t("organization.dashboard.testResults.blocked", "Blocked"),
          value: Math.round(totalTestCases * 0.05),
          color: RESULT_COLORS.BLOCKED,
        },
        {
          name: t("organization.dashboard.testResults.notRun", "Not Run"),
          value: Math.round(totalTestCases * 0.15),
  ...
  const usageTopPages = usageMetrics?.pages
    ? usageMetrics.pages.slice(0, 5)
    : [];
  ...
  const usageLastUpdatedLabel = usageGeneratedAt
    ? usageGeneratedAt.toLocaleTimeString("ko-KR", {
        hour: "2-digit",
        minute: "2-digit",
      })
    : null;
            ...
            <StyledDashboardPaper sx={{ height: "100%" }}>
              ...
              <List>
                {dashboardData.organizations.map((org, index) => (
                  <ListItem
                    key={org.id}
                    divider={index < dashboardData.organizations.length - 1}
                  >
                    <ListItemIcon>
                    ...
                    <ListItemText
                      primary={org.name}
                      secondary={`${t(
                        "organization.dashboard.list.projectCount",
                        {
                          count: projects.filter(
                            (p) => p.organization?.id === org.id,
                          ).length,
                        },
                      )} / ${t("organization.dashboard.list.memberCount", {
                        count: projects
                          .filter((p) => p.organization?.id === org.id)
                ...
                {dashboardData.testResultStats.map((stat, index) => (
                  <Box key={index} mb={2}>
                    ...
                    <LinearProgress
                      ...
                      value={
                        (stat.value /
                          dashboardData.testResultStats.reduce(
                            (sum, s) => sum + s.value,
                            0,
                          )) *
                        100
