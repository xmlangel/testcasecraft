...
const JiraIssueRedirect = () => {
  ...
  useEffect(() => {
    ...
    const fetchContextAndRedirect = async () => {
      if (!issueKey) {
        setError(
          t("common.errors.invalidIssueKey", "유효하지 않은 이슈 키입니다."),
        );
        setLoading(false);
        return;
      }
      ...
      try {
        const response = await api(
          `/api/jira-integration/latest-execution-context?issueKey=${issueKey}`,
        );
        ...
        if (Array.isArray(data)) {
          ...
          setError(
            t("common.errors.noDataFound", "데이터를 찾을 수 없습니다."),
          );
          setLoading(false);
        }
      } catch (err) {
  ...
  const handleRedirect = (exec) => {
    ...
    if (testCaseId) {
      ...
      navigate(
        `/projects/${projectId}/executions/${executionId}/testcases/${testCaseId}/result`,
      );
    } else {
  ...
  return (
    <Container maxWidth="md" sx={{ py: 6 }}>
      <Box sx={{ mb: 4, textAlign: "center" }}>
        ...
        <Typography variant="body1" color="text.secondary">
          <strong>{issueKey}</strong> 이슈와 연관된 테스트가 {executions.length}
          개 발견되었습니다.
          <br />
                  ...
                  >
                    <Stack direction="row" spacing={2}>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        <strong>Date:</strong>{" "}
                        {exec.startDate
                          ? new Date(exec.startDate).toLocaleDateString()
                          : "N/A"}
                      </Typography>
                    </Stack>
                    <Button
