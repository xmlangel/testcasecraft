...
const JiraStatusIndicator = ({
  ...
  const getStatusText = () => {
    ...
    if (!status.hasConfig)
      return t("jira.status.notConfigured", "설정되지 않음");
                ...
                <>
                  ...
                  >
                    <Typography variant="caption" color="text.secondary">
                      <strong>{t("jira.indicator.server", "서버")}:</strong>{" "}
                      {status.serverUrl}
                    </Typography>
                    <Typography variant="caption" color="text.secondary">
                      <strong>{t("jira.indicator.user", "사용자")}:</strong>{" "}
                      {status.username}
                    </Typography>
                    {status.lastTested && (
                      <Typography variant="caption" color="text.secondary">
                        <strong>
                          {t("jira.indicator.lastTested", "마지막 확인")}:
                        </strong>{" "}
                    ...
                    {lastRefresh && (
                      <Typography variant="caption" color="text.secondary">
                        <strong>
                          {t("jira.indicator.lastUpdate", "업데이트")}:
                        </strong>{" "}
  ...
  return (
    <Box>
      ...
      <Alert severity={getStatusColor()} icon={getStatusIcon()} sx={{ mb: 2 }}>
        <Typography variant="body2">{getDetailedStatusText()}</Typography>
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
            ...
            {status.lastTested && (
              <Typography variant="body2" color="text.secondary">
                ...
                />
                <strong>
                  {t("jira.indicator.lastTested", "마지막 테스트")}:
                </strong>{" "}
                {formatDate(status.lastTested)}
              </Typography>
            )}
            {lastRefresh && (
              <Typography variant="body2" color="text.secondary">
                <strong>
                  {t("jira.indicator.lastUpdate", "마지막 업데이트")}:
                </strong>{" "}
                {formatDate(lastRefresh)}
              </Typography>
            )}
