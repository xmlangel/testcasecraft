...
const JiraCommentDialog = ({
  ...
  const generateComment = () => {
    ...
    let generatedComment = `${statusIcon} **${t(
      "jira.comment.template.title",
    )}**\n\n`;
    generatedComment += `**${t("jira.comment.template.testCase")}** ${
      testCase.name
    }\n`;
    generatedComment += `**${t(
      "jira.comment.template.result",
    )}** ${statusText}\n`;
    generatedComment += `**${t(
      "jira.comment.template.executedAt",
    ...
    >
      ...
      <DialogContent dividers>
        <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
          ...
          {jiraStatus && (
            ...
            >
              {jiraStatus.hasConfig && jiraStatus.isConnected
                ? t("jira.comment.connectionStatus.connected", {
                    serverUrl: jiraStatus.serverUrl,
                  })
          ...
          {/* 성공 메시지 */}
          {success && (
            <Alert severity="success">{t("jira.comment.success.added")}</Alert>
          )}
            ...
            <Box>
              <Typography variant="subtitle2" gutterBottom>
                {linkedIssues.length > 0
                  ? t("jira.comment.detectedIssues.linked")
                  : t("jira.comment.detectedIssues.fromNotes")}
              </Typography>
          ...
          <Box sx={{ width: "100%" }}>
            ...
            >
              <Tab
                icon={<EditIcon />}
                iconPosition="start"
                label={t("jira.comment.tab.edit", "편집")}
              />
              <Tab
                icon={<PreviewIcon />}
                iconPosition="start"
                label={t("jira.comment.tab.preview", "미리보기")}
              />
            </Tabs>
            ...
            {activeTab === 0 ? (
              <TextField
                ...
                disabled={loading || success}
                helperText={t("jira.comment.field.comment.charCount", {
              ...
              <List dense>
                <ListItem>
                  ...
                  <ListItemText
                    ...
                    secondary={t("jira.comment.testInfo.result", {
                      result: getResultText(testResult.result),
                    })}
                  />
                </ListItem>
                {testResult.notes && (
      ...
      <DialogActions sx={{ px: 3, py: 2 }}>
        <Button onClick={onClose} disabled={loading}>
          {success
            ? t("jira.comment.button.close")
            : t("jira.comment.button.cancel")}
        </Button>
        ...
        {!success && (
          ...
          >
            {loading
              ? t("jira.comment.button.sending")
              : t("jira.comment.button.send")}
          </Button>
        )}
