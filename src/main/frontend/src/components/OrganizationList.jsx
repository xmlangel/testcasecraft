...
const OrganizationList = () => {
  ...
  const getErrorTitle = (errorCode) => {
    switch (errorCode) {
      case "ACCESS_DENIED":
        return t("organization.error.accessDenied");
      case "AUTHENTICATION_REQUIRED":
        return t("organization.error.authRequired");
      case "RESOURCE_NOT_FOUND":
        return t("organization.error.resourceNotFound");
      default:
        return t("organization.error.general");
    }
  };
  ...
  return (
    <Box sx={PAGE_CONTAINER_SX.main}>
      ...
      {error && (
        <Alert
          severity={
            errorDetails?.type === "ACCESS_DENIED" ? "warning" : "error"
          }
          sx={{ mb: 3 }}
        >
          ...
          {errorDetails?.timestamp && (
            <Typography
              ...
              sx={{ mt: 1, opacity: 0.7 }}
            >
              {t("organization.error.occurredAt", {
                date: new Date(errorDetails.timestamp).toLocaleString(),
              })}
            </Typography>
          )}
        </Alert>
      ...
      {organizations.length === 0 && !error ? (
        <Box textAlign="center" py={8}>
          ...
          <Typography variant="body2" color="text.disabled" mb={3}>
            {user?.role === "ADMIN"
              ? t("organization.messages.createHint")
              : t("organization.messages.joinHint")}
          </Typography>
          {user?.role === "ADMIN" && (
            <Button
              variant="contained"
      ...
      {error && errorDetails?.type === "ACCESS_DENIED" && (
        <Box textAlign="center" py={4}>
          <Typography variant="body1" color="text.secondary" mb={2}>
            {user?.role === "ADMIN"
              ? t("organization.messages.canCreateNew")
              : t("organization.messages.noAccessContact")}
          </Typography>
          {user?.role === "ADMIN" && (
            <Button
              variant="contained"
      ...
      >
        <DialogTitle>
          {editingOrg
            ? t("organization.dialog.edit.title")
      ...
      >
        ...
        <DialogContent>
          <Typography>
            ...
            {t("organization.dialog.delete.message")}
          </Typography>
