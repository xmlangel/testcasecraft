  ...
  const searchResults = useCallback(async () => {
    ...
    try {
      ...
      const response = await api(
        `/api/junit-results/projects/${activeProject.id}?size=50`,
      );
      if (response.ok) {
        const data = await response.json();
        ...
        if (searchTerm) {
          ...
          content = content.filter(
            (r) =>
              (r.testExecutionName &&
                r.testExecutionName.toLowerCase().includes(lowerTerm)) ||
                ...
                endAdornment: (
                  <InputAdornment position="end">
                    <Button
                      ...
                      disabled={loading}
                      data-testid="automation-search-button"
                    >
                      {t("common.search", "검색")}
                    </Button>
                  </InputAdornment>
        ...
        {loading ? (
          ...
          <List>
            {results.map((result) => {
              ...
              const linkedToOther =
                result.testPlanId && result.testPlanId !== testPlanId;

              return (
                <ListItem
                  key={result.id}
                  divider
                  data-testid={`automation-result-item-${result.id}`}
                >
                  <ListItemText
                    primary={
                      <Box
                    ...
                    secondary={
                      <>
                        ...
                        {result.testExecutionName &&
                          result.fileName !== result.testExecutionName && (
