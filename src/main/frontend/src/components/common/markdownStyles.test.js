...
describe("MARKDOWN_PREWRAP_SX", () => {
  it("pre-wrap 을 p/li 로만 한정한다", () => {
    expect(
      MARKDOWN_PREWRAP_SX["& .markdown-body p, & .markdown-body li"],
    ).toEqual({
      whiteSpace: "pre-wrap",
    });
  });

  it("마크다운 루트(.markdown-body)나 컨테이너에는 pre-wrap 을 적용하지 않는다", () => {
    expect(MARKDOWN_PREWRAP_SX["& .markdown-body"]).toBeUndefined();
