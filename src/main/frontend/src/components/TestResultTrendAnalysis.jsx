...
function TestResultTrendAnalysis() {
  ...
  const loadComparisonData = useCallback(async () => {
    ...
    try {
      if (comparisonMode === "testplan") {
        // 플랜별 비교 데이터 로드
        const comparisonResults = await getTestPlansComparison(
          selectedItems,
          days,
        );
        ...
        const assigneeResults = await getProjectAssigneeResults(
          activeProject.id,
          50,
        );
  ...
  const loadTrendData = useCallback(async () => {
    ...
    try {
      ...
      if (
        response.testResultsHistory &&
        response.testResultsHistory.length > 0
      ) {
        ...
        const transformedData = response.testResultsHistory.map((item) => ({
          ...
          total:
            ...
            (item.FAIL || 0) +
            (item.BLOCKED || 0) +
            (item.NOTRUN || item.notRun || 0) +
            (item.SKIPPED || 0),
          passRate:
            item.PASS && item.PASS + item.FAIL + item.BLOCKED + item.SKIPPED > 0
              ? Math.round(
                  (item.PASS /
                    (item.PASS + item.FAIL + item.BLOCKED + item.SKIPPED)) *
                    100,
                )
              : 0,
        }));
  ...
  const transformComparisonData = () => {
    ...
    const transformedData = trendData.testResultsHistory.map((item) => ({
      ...item,
    }));
    ...
    if (comparisonMode === "testplan" && Array.isArray(comparisonData)) {
      ...
      comparisonData.openTestRunResults
    ) {
      ...
      selectedItems.forEach((assigneeId) => {
        ...
        if (assigneeData) {
          transformedData.forEach((dateItem) => {
            // 실제 담당자 완료율 사용
            dateItem[`assignee_${assigneeId}_passRate`] =
              assigneeData.completionRate || 0;
          });
        }
      });
    }
          ...
          {payload.map((entry, index) => {
            ...
            return (
              ...
              >
                ...
                <strong>
                  {entry.value}
                  {t("testTrendAnalysis.tooltip.unit", "건")}
                </strong>
              </Typography>
            );
          })}
  ...
  const calculateSummaryStats = () => {
    if (
      !trendData?.testResultsHistory ||
      trendData.testResultsHistory.length === 0
    ) {
      return null;
    }
    ...
    const avgPassRate =
      data.reduce((sum, item) => sum + (item.passRate || 0), 0) / data.length;
    const avgCompleteRate =
      data.reduce((sum, item) => sum + (item.completeRate || 0), 0) /
      data.length;
    ...
    return {
      ...
      passRateChange: previous
        ? (latest.passRate || 0) - (previous.passRate || 0)
        : 0,
    };
  };
    ...
    return (
      ...
      >
        ...
        <Typography variant="body2" sx={{ ml: 2 }}>
          {t(
            "testTrendAnalysis.loading.trendData",
            "추이 데이터를 불러오는 중...",
          )}
        </Typography>
      </Box>
        ...
        >
          <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
            ...
            <FormControl size="small" sx={{ minWidth: 120 }}>
              <InputLabel>
                {t("testTrendAnalysis.period.label", "기간")}
              </InputLabel>
              <Select
                value={days}
              ...
              >
                <MenuItem value={7}>
                  {t("testTrendAnalysis.period.last7days", "최근 7일")}
                </MenuItem>
                <MenuItem value={15}>
                  {t("testTrendAnalysis.period.last15days", "최근 15일")}
                </MenuItem>
                <MenuItem value={30}>
                  {t("testTrendAnalysis.period.last30days", "최근 30일")}
                </MenuItem>
                <MenuItem value={60}>
                  {t("testTrendAnalysis.period.last60days", "최근 60일")}
                </MenuItem>
                <MenuItem value={90}>
                  {t("testTrendAnalysis.period.last90days", "최근 90일")}
                </MenuItem>
              </Select>
            </FormControl>
                ...
                <CardContent sx={{ textAlign: "center", py: 2 }}>
                  ...
                  >
                    <Typography variant="h4">
                      {summaryStats.passRateChange > 0 ? "+" : ""}
                      {summaryStats.passRateChange}%
                    </Typography>
                    <TrendingUpIcon
                      color={
                        summaryStats.passRateChange >= 0 ? "success" : "error"
                      }
                      sx={{
                        transform:
                          summaryStats.passRateChange < 0
        ...
        <CardContent>
          ...
          >
            <Typography variant="h6" gutterBottom>
              {comparisonMode === "overall"
                ? t(
                    "testTrendAnalysis.chart.overallTrend",
                    "테스트 결과 변화 추이",
                  )
                  ...
                  <LineChart data={transformComparisonData()}>
                    ...
                    {selectedItems.map((itemId, index) => {
                      ...
                      const displayName =
                        comparisonMode === "testplan"
                          ? `${t(
                              "testTrendAnalysis.legend.plan",
                              "Plan",
                            )} ${itemId}`
                          : `${t(
                              "testTrendAnalysis.legend.user",
                              "User",
                            )} ${itemId}`;
