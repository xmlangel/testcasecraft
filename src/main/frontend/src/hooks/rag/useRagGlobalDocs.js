...
export function useRagGlobalDocs(
  ...
  // ============ 전역 문서 등록 요청 생성 ============
  const createGlobalDocumentRequest = useCallback(
    async (documentId, reason = "") => {
      ensureRagAvailable("createGlobalDocumentRequest");

      try {
        const response = await api("/api/rag/global-documents/requests", {
          method: "POST",
          body: JSON.stringify({
  ...
  // ============ 전역 문서 등록 요청 목록 ============
  const listGlobalDocumentRequests = useCallback(
    async (status = null, page = 1, pageSize = 20) => {
  ...
  // ============ 전역 문서 요청 승인 ============
  const approveGlobalDocumentRequest = useCallback(
    async (requestId) => {
  ...
  // ============ 전역 문서 요청 거부 ============
  const rejectGlobalDocumentRequest = useCallback(
  ...
  // ============ 전역 문서 삭제 ============
  const deleteGlobalDocument = useCallback(
    async (documentId) => {
      ensureRagAvailable("deleteGlobalDocument");

      try {
        await api(`/api/rag/global-documents/${documentId}`, {
          method: "DELETE",
        });
      } catch (error) {
  ...
  const promoteDocumentToGlobal = useCallback(
    async (documentId, reason = null) => {
      ...
      try {
        ...
        dispatch({
          type: ActionTypes.SET_ERROR,
          payload:
            error.message || "문서를 공통 문서로 이동하는 데 실패했습니다.",
        });
        throw error;
      }
    },
  ...
  // ============ 공통 문서 등록 요청 (일반 사용자용) ============
  const requestPromoteDocument = useCallback(
    async (documentId, message = null) => {
