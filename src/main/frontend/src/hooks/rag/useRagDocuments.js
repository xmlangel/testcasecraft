...
export function useRagDocuments(
  ...
  const uploadDocument = useCallback(
    async (file, projectId, uploadedBy = null) => {
      ...
      dispatch({
        type: ActionTypes.ADD_UPLOADING_FILE,
      ...
      try {
        ...
        const response = await api(`/api/rag/documents/upload`, {
          method: "POST",
          headers: {
  ...
  const analyzeDocument = useCallback(
    async (documentId, parser = "pymupdf4llm") => {
      ...
      try {
        const url = `/api/rag/documents/${documentId}/analyze?parser=${encodeURIComponent(
          parser,
        )}`;
  ...
  // ============ 문서 분석 완료 대기 (폴링) ============
  const waitForDocumentAnalysis = useCallback(
    async (
      documentId,
      ...
      try {
        while (Date.now() - startedAt < timeoutMs) {
          ...
          try {
            ...
            const message =
              error?.message || "Document analysis status check failed.";
  ...
  // ============ 임베딩 생성 완료 대기 (폴링) ============
  const waitForEmbeddingGeneration = useCallback(
    async (
      documentId,
      { intervalMs = 2000, timeoutMs = 5 * 60 * 1000 } = {},
    ) => {
  ...
  const generateEmbeddings = useCallback(
    async (documentId, systemPrompt = null) => {
  ...
  const listDocuments = useCallback(
    async (projectId, page = 1, pageSize = 20) => {
      ...
      if (requestCache && requestCache.current.has(cacheKey)) {
        debugLog(
          "useRagDocuments",
          "Skipping duplicate listDocuments call:",
          cacheKey,
        );
      ...
      const promise = (async () => {
        ...
        try {
          const url = `/api/rag/documents?projectId=${encodeURIComponent(
            projectId,
  ...
  const getDocumentChunks = useCallback(
    async (documentId, page = 1, pageSize = 20) => {
