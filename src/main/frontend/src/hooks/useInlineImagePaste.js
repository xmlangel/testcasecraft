...
const useInlineImagePaste = ({
  ...
  const resetDialogState = useCallback(
    () => setImageDialogState(createDefaultDialogState()),
    [],
  );

  const insertPlaceholderAtSelection = useCallback(
    (fieldConfig, placeholder, selectionStart, selectionEnd) => {
      updateFieldValue(fieldConfig, (currentValue = "") => {
        const valueLength = currentValue.length;
        const startPosition = Math.max(
          0,
          Math.min(selectionStart ?? valueLength, valueLength),
        );
        const endPosition = Math.max(
          0,
          Math.min(selectionEnd ?? startPosition, valueLength),
        );
        return `${currentValue.slice(
          0,
          startPosition,
        )}${placeholder}${currentValue.slice(endPosition)}`;
      });
    },
    [updateFieldValue],
  );
  ...
  const replacePlaceholder = useCallback(
    (fieldConfig, placeholder, replacement) => {
      if (!placeholder) return;
      updateFieldValue(fieldConfig, (currentValue = "") =>
        currentValue.replace(placeholder, replacement),
      );
    },
    [updateFieldValue],
  );
  ...
  const uploadInlineImage = useCallback(
    async (file) => {
      ...
      formData.append(
        "description",
        t("testcase.inlineImage.description", "본문에 삽입된 이미지"),
      );
  ...
  const handleMarkdownPaste = useCallback(
    async (event, fieldConfig) => {
      ...
      const placeholderId = `inline-img-${Date.now()}-${Math.random()
        .toString(36)
        .slice(2, 8)}`;
      ...
      try {
        ...
        const imageMarkup = `<img src="${
          attachment.publicUrl
        }" alt="${sanitizeAltText(altText)}" data-attachment-id="${
          attachment.id
        }" style="${styleAttr}" />`;
  ...
  const handleInlineImageDialogClose = useCallback(async () => {
    ...
    if (imageDialogState.attachment?.id) {
      try {
        const response = await api(
          `/api/testcase-attachments/${imageDialogState.attachment.id}`,
          {
            method: "DELETE",
          },
        );
        ...
        if (!response.ok) {
          console.error(
            "Failed to delete uploaded image attachment:",
            imageDialogState.attachment.id,
          );
        }
      } catch (error) {
    ...
    if (imageDialogState.placeholder && imageDialogState.fieldConfig) {
      replacePlaceholder(
        imageDialogState.fieldConfig,
        imageDialogState.placeholder,
        "",
      );
    }
  ...
  const handleInlineImageInsert = useCallback(() => {
    const { attachment, placeholder, fieldConfig, altText, width, widthUnit } =
      imageDialogState;
    ...
    const validWidth =
      numericWidth && Number.isFinite(numericWidth) && numericWidth > 0;
