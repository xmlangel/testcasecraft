...
/** ICT-130: API 성능 모니터링을 위한 메트릭 설정 대시보드 API의 응답 시간과 호출 빈도를 추적합니다. */
@Configuration
public class MetricsConfig {
  ...
  public static class DashboardMetricsFilter extends OncePerRequestFilter {
    ...
    public DashboardMetricsFilter(MeterRegistry meterRegistry) {
      ...
      // 대시보드 API 응답 시간 타이머
      this.dashboardApiTimer =
          Timer.builder("dashboard.api.response.time")
              .description("Dashboard API response time")
              .tag("component", "dashboard");

      // 대시보드 API 호출 카운터
      this.dashboardApiCounter =
          Counter.builder("dashboard.api.requests")
    ...
    private String extractEndpoint(String requestURI) {
      ...
      return requestURI.replaceAll("/[a-fA-F0-9-]{36}", "/{id}").replaceAll("/\\d+", "/{id}");
    }
