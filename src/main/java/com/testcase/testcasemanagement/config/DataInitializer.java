...
public class DataInitializer {
  ...
  public CommandLineRunner initTestData(
    ...
    return args -> {
      ...
      Project project1 =
          createProject("QA팀 모바일 앱 테스트", "MOBILE-TEST", "iOS/Android 앱 테스트케이스 관리", 1);
      Project savedProject = projectRepository.save(project1);
      project1 = savedProject;
      System.out.println("✅ 프로젝트 생성 완료 (QA팀 모바일 앱 테스트)");

      // 3.1. 사용자-프로젝트 권한 부여
      ProjectUser adminProject1 =
          createProjectUser(project1, adminUser, ProjectRole.PROJECT_MANAGER);
      ...
      // 4.1. 폴더 5개 생성
      String[] folderNames = {"기본 기능 테스트", "고급 기능 테스트", "보안 테스트", "성능 테스트", "호환성 테스트"};
      ...
      allTestCases.addAll(
          createTestCasesForFolder(
              basicTests, savedFolders.get(0), project1, "iOS/Android", sequentialIdCounter));
      ...
      allTestCases.addAll(
          createTestCasesForFolder(
              advancedTests, savedFolders.get(1), project1, "고급 기능", sequentialIdCounter));
      ...
      allTestCases.addAll(
          createTestCasesForFolder(
              securityTests, savedFolders.get(2), project1, "보안", sequentialIdCounter));
      ...
      allTestCases.addAll(
          createTestCasesForFolder(
              performanceTests, savedFolders.get(3), project1, "성능", sequentialIdCounter));
      ...
      allTestCases.addAll(
          createTestCasesForFolder(
              compatibilityTests, savedFolders.get(4), project1, "호환성", sequentialIdCounter));
      ...
      List<String> allTestCaseIds =
          savedTestCases.stream().map(TestCase::getId).collect(Collectors.toList());
      ...
      // 플랜 2: 처음 50개만 포함
      List<String> halfTestCaseIds =
          savedTestCases.stream().limit(50).map(TestCase::getId).collect(Collectors.toList());
      ...
      savedTestPlans.forEach(
          tp ->
              System.out.println(
                  "   - " + tp.getName() + " (테스트케이스 수: " + tp.getTestCaseIds().size() + ")"));

      // 6. 테스트 실행 생성 - 100개 플랜 3개, 50개 플랜 1개
      System.out.println("▶️ 테스트 실행 생성 중...");

      // 100개 플랜 실행 3개
      TestExecution execution1 =
          createTestExecution(
              "전체 테스트 실행 #1 (완료)",
              project1,
      ...
      List<TestExecution> savedExecutions =
          testExecutionRepository.saveAll(List.of(execution1, execution2, execution3, execution4));
      System.out.println("✅ 테스트 실행 생성 완료: " + savedExecutions.size() + "개");
      savedExecutions.forEach(
          ex -> System.out.println("   - " + ex.getName() + " (상태: " + ex.getStatus() + ")"));
      ...
      List<TestCase> first50TestCases =
          savedTestCases.stream().limit(50).collect(Collectors.toList());

      List<TestResult> results4 =
          createTestResults(
              first50TestCases,
  ...
  private TestCase createTestCaseInFolder(
      String name, Project project, String description, TestCase parentFolder, int sequentialId) {
    TestCase testCase = new TestCase();
    testCase.setName(name);
    testCase.setType("testcase");
    testCase.setProject(project);
  ...
  private List<TestCase> createTestCasesForFolder(
      String[] testNames,
      TestCase folder,
      Project project,
      String prefix,
  ...
  private TestPlan createTestPlan(
      String name, Project project, String description, List<String> testCaseIds) {
  ...
  private TestExecution createTestExecution(
      String name, Project project, String testPlanId, String status) {
  ...
  private List<TestResult> createTestResults(
    ...
    for (int i = 0; i < testCases.size(); i++) {
      ...
      TestResult result =
          createTestResult(testCase.getId(), execution, resultStatus, notes, executor, executedAt);
