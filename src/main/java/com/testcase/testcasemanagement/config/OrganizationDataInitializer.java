...
/**
 * 조직 관리 시스템 테스트 데이터 초기화 @ConditionalOnProperty - 테스트 환경에서는 실행되지 않도록 설정 testcase.init.enabled=false
 * 설정 시 비활성화됨
 */
@Component
...
public class OrganizationDataInitializer implements CommandLineRunner {
          ...
          org -> {
            boolean isMember =
                organizationUserRepository.existsByOrganizationIdAndUserId(
                    org.getId(), finalAdminUser.getId());
  ...
  private void initializeOrganizationData() {
    ...
    if (organizationRepository.count() > 0
        && projectRepository.countByOrganizationIsNotNull() > 0) {
      System.out.println(
          "기존 조직 "
              + organizationRepository.count()
              + "개, 조직별 프로젝트 "
              + projectRepository.countByOrganizationIsNotNull()
              + "개 존재. 멤버십만 업데이트.");
      ensureAdminMembership();
      return;
    }
    ...
    User managerUser =
        createUserIfNotExists("manager", "manager@company.com", "이매니저", "manager123");
    User developerUser =
        createUserIfNotExists("developer", "developer@company.com", "박개발", "developer123");
  ...
  private void ensureAdminUserSetup(User adminUser) {
    ...
    List<OrganizationUser> existingMemberships =
        organizationUserRepository.findByUserId(adminUser.getId());
  ...
  private void ensureAdminMembership() {
    ...
    if (adminUser != null) {
      ...
      List<OrganizationUser> adminMemberships =
          organizationUserRepository.findByUserId(adminUser.getId());
      for (OrganizationUser membership : adminMemberships) {
        System.out.println(
            "  - 조직: "
                ...
                + ", 역할: "
                + membership.getRoleInOrganization());
      }
    } else {
  ...
  private void createOrganizationMember(
      Organization org, User user, OrganizationUser.OrganizationRole role) {
    // 중복 체크: 이미 존재하는 멤버십이면 생성하지 않음
    boolean exists =
        organizationUserRepository.existsByOrganizationIdAndUserId(org.getId(), user.getId());
    if (exists) {
