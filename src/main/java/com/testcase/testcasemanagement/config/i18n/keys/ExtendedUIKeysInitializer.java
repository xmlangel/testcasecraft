...
public class ExtendedUIKeysInitializer {
  ...
  private void initializeCommonUIKeys() {
    ...
    createTranslationKeyIfNotExists(
        "table.pagination.info", "table", "페이지 정보", "{total}개 중 {from}-{to}개 표시");

    // 검색 및 필터링 키들
    createTranslationKeyIfNotExists(
        "search.placeholder.global", "search", "전체 검색 플레이스홀더", "전체 콘텐츠 검색...");
    createTranslationKeyIfNotExists(
        "search.placeholder.testCase", "search", "테스트케이스 검색 플레이스홀더", "테스트케이스 검색...");
    createTranslationKeyIfNotExists(
        "search.placeholder.project", "search", "프로젝트 검색 플레이스홀더", "프로젝트 검색...");
    createTranslationKeyIfNotExists(
        "search.placeholder.user", "search", "사용자 검색 플레이스홀더", "사용자 검색...");
    ...
    createTranslationKeyIfNotExists(
        "search.results.noResults", "search", "검색 결과 없음", "검색 결과가 없습니다");
    ...
    createTranslationKeyIfNotExists(
        "navigation.menu.testCases", "navigation", "테스트케이스 메뉴", "테스트케이스");
    createTranslationKeyIfNotExists("navigation.menu.testPlans", "navigation", "테스트계획 메뉴", "테스트계획");
    createTranslationKeyIfNotExists(
        "navigation.menu.testExecution", "navigation", "테스트실행 메뉴", "테스트실행");
    ...
    createTranslationKeyIfNotExists(
        "navigation.breadcrumb.separator", "navigation", "브레드크럼 구분자", "/");
    createTranslationKeyIfNotExists("navigation.back.button", "navigation", "뒤로 가기 버튼", "뒤로");

    // 추가 네비게이션 키들
    createTranslationKeyIfNotExists(
        "navigation.menu.testExecutions", "navigation", "테스트 실행 메뉴", "테스트 실행");
    ...
    createTranslationKeyIfNotExists(
        "navigation.user.preferences", "navigation", "사용자 환경설정", "환경설정");
    ...
    // 추가 Validation 키들
    createTranslationKeyIfNotExists(
        "validation.email.invalid", "validation", "이메일 형식 오류", "올바른 이메일 형식이 아닙니다");
    createTranslationKeyIfNotExists(
        "validation.password.minLength", "validation", "비밀번호 최소 길이", "비밀번호는 최소 8자 이상이어야 합니다");
  }
  ...
  private void initializeDomainSpecificKeys() {
    ...
    createTranslationKeyIfNotExists(
        "testResult.statistics.totalCount", "testResult", "총 개수", "총 개수");
    createTranslationKeyIfNotExists(
    ...
    createTranslationKeyIfNotExists(
        "organization.member.role.admin", "organization", "관리자 역할", "관리자");
    createTranslationKeyIfNotExists(
        "organization.member.role.member", "organization", "멤버 역할", "멤버");
    createTranslationKeyIfNotExists(
        "organization.member.role.viewer", "organization", "뷰어 역할", "뷰어");
    createTranslationKeyIfNotExists(
        "organization.project.status.active", "organization", "활성 상태", "활성");
    createTranslationKeyIfNotExists(
        "organization.project.status.inactive", "organization", "비활성 상태", "비활성");
    createTranslationKeyIfNotExists(
        "organization.project.status.archived", "organization", "보관됨 상태", "보관됨");
    createTranslationKeyIfNotExists(
        "organization.dashboard.metrics.totalUsers", "organization", "전체 사용자 수", "전체 사용자 수");
    createTranslationKeyIfNotExists(
        "organization.dashboard.metrics.activeProjects", "organization", "활성 프로젝트 수", "활성 프로젝트 수");
    createTranslationKeyIfNotExists(
        "organization.dashboard.metrics.testCases", "organization", "테스트케이스 수", "테스트케이스 수");
    createTranslationKeyIfNotExists(
        "organization.dashboard.metrics.completedTests", "organization", "완료된 테스트 수", "완료된 테스트 수");
    createTranslationKeyIfNotExists(
        "organization.dashboard.stats.title", "organization", "조직 통계", "조직 통계");
    ...
    createTranslationKeyIfNotExists(
        "dashboard.chart.pieChart.title", "dashboard", "파이 차트 제목", "테스트 결과 파이 차트");
    ...
    createTranslationKeyIfNotExists(
        "dashboard.chart.pieChart.notRun", "dashboard", "미실행 차트", "미실행");
    createTranslationKeyIfNotExists(
        "dashboard.chart.barChart.title", "dashboard", "바 차트 제목", "월별 테스트 실행 추이");
    createTranslationKeyIfNotExists(
        "dashboard.chart.lineChart.title", "dashboard", "라인 차트 제목", "품질 추이");
    createTranslationKeyIfNotExists(
        "dashboard.chart.lineChart.passRate", "dashboard", "통과율 라인", "통과율");
    createTranslationKeyIfNotExists(
        "dashboard.chart.lineChart.failRate", "dashboard", "실패율 라인", "실패율");
    createTranslationKeyIfNotExists(
        "dashboard.chart.donutChart.title", "dashboard", "도넛 차트 제목", "우선순위별 테스트케이스 분포");
    createTranslationKeyIfNotExists(
        "dashboard.metrics.totalTestCases", "dashboard", "총 테스트케이스 메트릭", "총 테스트케이스");
    createTranslationKeyIfNotExists(
        "dashboard.metrics.executedTests", "dashboard", "실행된 테스트 메트릭", "실행된 테스트");
    createTranslationKeyIfNotExists(
        "dashboard.metrics.passedTests", "dashboard", "통과된 테스트 메트릭", "통과된 테스트");
    createTranslationKeyIfNotExists(
        "dashboard.metrics.failedTests", "dashboard", "실패된 테스트 메트릭", "실패된 테스트");
    createTranslationKeyIfNotExists("dashboard.metrics.passRate", "dashboard", "통과율 메트릭", "통과율");
    createTranslationKeyIfNotExists(
        "dashboard.widget.recentActivity", "dashboard", "최근 활동 위젯", "최근 활동");
    createTranslationKeyIfNotExists(
        "dashboard.widget.upcomingTests", "dashboard", "예정된 테스트 위젯", "예정된 테스트");
    createTranslationKeyIfNotExists(
        "dashboard.widget.criticalIssues", "dashboard", "중요 이슈 위젯", "중요 이슈");
    createTranslationKeyIfNotExists(
        "dashboard.widget.teamPerformance", "dashboard", "팀 성과 위젯", "팀 성과");
    createTranslationKeyIfNotExists(
        "dashboard.widget.projectStatus", "dashboard", "프로젝트 상태 위젯", "프로젝트 상태");
  ...
  private void initializeSystemKeys() {
    ...
    createTranslationKeyIfNotExists(
        "validation.confirm.password", "validation", "비밀번호 확인 검증", "비밀번호가 일치하지 않습니다");
    createTranslationKeyIfNotExists(
        "validation.date.invalid", "validation", "날짜 형식 검증", "올바른 날짜 형식이 아닙니다");
    createTranslationKeyIfNotExists(
        "validation.number.invalid", "validation", "숫자 형식 검증", "올바른 숫자 형식이 아닙니다");
    ...
    createTranslationKeyIfNotExists(
        "notification.email.testResult", "notification", "테스트 결과 이메일", "테스트 결과 알림");
    createTranslationKeyIfNotExists(
        "notification.email.projectUpdate", "notification", "프로젝트 업데이트 이메일", "프로젝트 업데이트 알림");
    createTranslationKeyIfNotExists(
        "notification.settings.enable", "notification", "알림 활성화", "알림 활성화");
    createTranslationKeyIfNotExists(
        "notification.settings.disable", "notification", "알림 비활성화", "알림 비활성화");
    createTranslationKeyIfNotExists("notification.markAsRead", "notification", "읽음 표시", "읽음으로 표시");
    createTranslationKeyIfNotExists(
        "notification.clearAll", "notification", "모든 알림 지우기", "모든 알림 지우기");
    createTranslationKeyIfNotExists(
        "notification.success.saved", "notification", "저장 성공 알림", "성공적으로 저장되었습니다");
    createTranslationKeyIfNotExists(
        "notification.success.deleted", "notification", "삭제 성공 알림", "성공적으로 삭제되었습니다");
    createTranslationKeyIfNotExists(
        "notification.success.updated", "notification", "수정 성공 알림", "성공적으로 수정되었습니다");
    createTranslationKeyIfNotExists(
        "notification.error.networkError", "notification", "네트워크 오류 알림", "네트워크 오류가 발생했습니다");
    createTranslationKeyIfNotExists(
        "notification.error.serverError", "notification", "서버 오류 알림", "서버 오류가 발생했습니다");
    createTranslationKeyIfNotExists(
        "notification.info.processing", "notification", "처리중 정보 알림", "처리 중입니다...");

    // 파일 관리 키들
    createTranslationKeyIfNotExists("file.upload.title", "file", "파일 업로드 제목", "파일 업로드");
    createTranslationKeyIfNotExists(
        "file.upload.description", "file", "파일 업로드 설명", "파일을 끌어다 놓거나 클릭하여 업로드하세요");
    ...
    createTranslationKeyIfNotExists(
        "export.options.includeAttachments", "export", "첨부파일 포함 옵션", "첨부파일 포함");
    createTranslationKeyIfNotExists("export.options.includeHistory", "export", "이력 포함 옵션", "이력 포함");
    createTranslationKeyIfNotExists(
        "export.progress.preparing", "export", "내보내기 준비", "내보내기 준비 중...");
    ...
    createTranslationKeyIfNotExists(
        "workflow.comment.placeholder", "workflow", "댓글 플레이스홀더", "댓글 추가...");
    ...
    createTranslationKeyIfNotExists(
        "action.user.changePassword", "action", "비밀번호 변경 액션", "비밀번호 변경");
    ...
    createTranslationKeyIfNotExists(
        "schedule.test.execution", "schedule", "테스트 실행 일정", "테스트 실행 일정");
    ...
    createTranslationKeyIfNotExists(
        "milestone.achievement.title", "milestone", "마일스톤 달성", "마일스톤 달성");

    // 커뮤니케이션 및 협업 키들
    createTranslationKeyIfNotExists("communication.chat.title", "communication", "채팅", "채팅");
    createTranslationKeyIfNotExists(
        "communication.message.send", "communication", "메시지 보내기", "보내기");
    createTranslationKeyIfNotExists(
        "communication.message.receive", "communication", "메시지 받기", "받기");
    ...
    createTranslationKeyIfNotExists(
        "responsive.breakpoint.desktop", "responsive", "데스크탑 화면", "데스크탑");
  ...
  private void initializeCollaborationAndSettingsKeys() {
    ...
    createTranslationKeyIfNotExists(
        "collaboration.review.request", "collaboration", "검토 요청", "검토 요청");
