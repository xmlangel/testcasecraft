...
public class AuthKeysInitializer {
        ...
        public void initialize() {
                ...
                createTranslationKeyIfNotExists("register.success", "register", "회원가입 성공 메시지",
                                "회원가입이 완료되었습니다. 로그인 해주세요.");
                createTranslationKeyIfNotExists("register.error.general", "register", "회원가입 일반 오류",
                                "회원가입 중 오류가 발생했습니다.");
                ...
                createTranslationKeyIfNotExists("profile.validation.emailRequired", "profile", "이메일 필수 입력",
                                "이메일을 입력하세요.");
                createTranslationKeyIfNotExists("profile.validation.allRequired", "profile", "모든 필드 필수",
                                "이름과 이메일을 모두 입력하세요.");
                createTranslationKeyIfNotExists("profile.success.updated", "profile", "정보 업데이트 성공",
                                "정보가 성공적으로 변경되었습니다.");
                createTranslationKeyIfNotExists("profile.error.updateFailed", "profile", "정보 업데이트 실패",
                                "정보 변경에 실패했습니다.");
                ...
                createTranslationKeyIfNotExists("password.validation.currentRequired", "password", "현재 비밀번호 필수",
                                "현재 비밀번호를 입력해주세요");
                createTranslationKeyIfNotExists("password.validation.newRequired", "password", "새 비밀번호 필수",
                                "새 비밀번호를 입력해주세요");
                createTranslationKeyIfNotExists("password.validation.confirmRequired", "password", "비밀번호 확인 필수",
                                "비밀번호 확인을 입력해주세요");
                createTranslationKeyIfNotExists("password.validation.mismatch", "password", "비밀번호 불일치",
                                "새 비밀번호와 일치하지 않습니다");
                createTranslationKeyIfNotExists("password.validation.sameAsCurrent", "password", "동일한 비밀번호",
                                "새 비밀번호는 현재 비밀번호와 달라야 합니다");
                createTranslationKeyIfNotExists("password.validation.minLength", "password", "최소 길이",
                                "최소 8자 이상이어야 합니다");
                createTranslationKeyIfNotExists("password.validation.maxLength", "password", "최대 길이",
                                "최대 100자까지 입력 가능합니다");
                ...
                createTranslationKeyIfNotExists("password.requirements.combination", "password", "조합 요구사항",
                                "2가지 이상 조합");
                ...
                createTranslationKeyIfNotExists("auth.session.expired.cause", "session", "세션 만료 원인",
                                "원인: {cause}");
                createTranslationKeyIfNotExists("auth.session.expired.default_help", "session", "세션 만료 기본 안내",
