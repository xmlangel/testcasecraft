...
public class CommonKeysInitializer {
  ...
  public void initialize() {
    ...
    createTranslationKeyIfNotExists(
        "validation.min_length", "validation", "최소 길이 검증", "최소 {0}글자 이상 입력하세요.");
    createTranslationKeyIfNotExists(
        "validation.max_length", "validation", "최대 길이 검증", "최대 {0}글자까지 입력 가능합니다.");
    createTranslationKeyIfNotExists(
        "validation.required.all", "validation", "모든 필드 필수 입력", "모든 필드를 입력해주세요.");
    createTranslationKeyIfNotExists(
        "validation.password.mismatch", "validation", "비밀번호 불일치", "비밀번호가 일치하지 않습니다.");
    ...
    createTranslationKeyIfNotExists(
        "language.helperText", "language", "언어 변경 도움말", "변경된 언어는 즉시 적용되며 자동으로 저장됩니다.");
    ...
    createTranslationKeyIfNotExists(
        "header.nav.organizationManagement", "header", "조직 관리 네비게이션", "조직 관리");
    createTranslationKeyIfNotExists(
        "header.nav.userManagement", "header", "사용자 관리 네비게이션", "사용자 관리");
    createTranslationKeyIfNotExists("header.nav.mailSettings", "header", "메일 설정 네비게이션", "메일 설정");
    createTranslationKeyIfNotExists(
        "header.nav.translationManagement", "header", "번역 관리 네비게이션", "번역 관리");
    createTranslationKeyIfNotExists("header.nav.llmConfig", "header", "LLM 설정 네비게이션", "LLM 설정");
    createTranslationKeyIfNotExists(
        "header.nav.schedulerManagement", "header", "스케줄러 관리 네비게이션", "스케줄러 관리");
    createTranslationKeyIfNotExists(
        "header.nav.projectSelection", "header", "프로젝트 선택 네비게이션", "프로젝트 선택");
    ...
    createTranslationKeyIfNotExists(
        "common.unauthorized.title", "common", "인증 실패 제목", "접근 권한이 없습니다");
    createTranslationKeyIfNotExists(
    ...
    createTranslationKeyIfNotExists(
        "admin.llmConfig.addConfig", "admin", "LLM 설정 추가 버튼", "LLM 설정 추가");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.editConfig", "admin", "LLM 설정 수정", "LLM 설정 수정");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.createConfig", "admin", "LLM 설정 생성", "LLM 설정 생성");
    ...
    createTranslationKeyIfNotExists(
        "admin.llmConfig.setAsDefault", "admin", "기본 설정으로 지정", "기본 설정으로 지정");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.noConfigs", "admin", "LLM 설정이 없습니다", "LLM 설정이 없습니다");

    // LLM 설정 메시지
    createTranslationKeyIfNotExists(
        "admin.llmConfig.message.allFieldsRequired", "admin", "모든 필수 필드 입력", "모든 필수 필드를 입력해주세요");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.message.connectionSuccess", "admin", "연결 성공", "연결 테스트 성공!");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.message.connectionFailed", "admin", "연결 실패", "연결 테스트 실패");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.message.invalidJson", "admin", "유효하지 않은 JSON", "템플릿이 유효한 JSON 형식이 아닙니다");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.message.confirmDelete", "admin", "삭제 확인", "정말 이 LLM 설정을 삭제하시겠습니까?");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.message.deleted", "admin", "삭제 완료", "LLM 설정이 삭제되었습니다");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.message.updated", "admin", "수정 완료", "LLM 설정이 수정되었습니다");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.message.created", "admin", "생성 완료", "LLM 설정이 생성되었습니다");
    createTranslationKeyIfNotExists(
    ...
    // LLM 탭 및 섹션
    createTranslationKeyIfNotExists(
        "admin.llmConfig.tab.configList", "admin", "LLM 설정 목록 탭", "LLM 설정 목록");
    createTranslationKeyIfNotExists("admin.llmConfig.tab.template", "admin", "기본 템플릿 탭", "기본 템플릿");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.template.title", "admin", "템플릿 제목", "테스트 케이스 생성 기본 템플릿");
    createTranslationKeyIfNotExists(
    ...
    createTranslationKeyIfNotExists(
        "admin.llmConfig.template.label", "admin", "기본 템플릿 JSON 라벨", "기본 템플릿 JSON:");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.template.download", "admin", "템플릿 다운로드", "다운로드");
    createTranslationKeyIfNotExists("admin.llmConfig.template.reset", "admin", "템플릿 초기화", "초기화");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.template.downloadJson", "admin", "JSON 다운로드", "JSON 다운로드");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.template.usageTitle", "admin", "사용 방법 제목", "사용 방법:");
    createTranslationKeyIfNotExists(
        "admin.llmConfig.template.usage1", "admin", "사용 방법 1", "1. LLM 설정 생성 시 이 템플릿이 자동으로 적용됩니다.");
    createTranslationKeyIfNotExists(
    ...
    // 공통 문서 등록 요청 키들
    createTranslationKeyIfNotExists(
        "admin.globalDoc.requests.title", "admin", "공통 문서 등록 요청 제목", "📨 공통 문서 등록 요청");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.requests.empty", "admin", "요청 없음 메시지", "대기 중인 요청이 없습니다.");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.requests.requestedBy", "admin", "요청자 컬럼", "요청자");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.requests.message", "admin", "요청 메모 컬럼", "요청 메모");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.requests.requestedAt", "admin", "요청 일시 컬럼", "요청 일시");
    ...
    // 공통 RAG 문서 관리
    createTranslationKeyIfNotExists(
        "admin.globalDoc.title", "admin", "공통 RAG 문서 관리", "공통 RAG 문서 관리");
    createTranslationKeyIfNotExists(
    ...
    // 공통 문서 정보 섹션
    createTranslationKeyIfNotExists(
        "admin.globalDoc.info.whatIsTitle", "admin", "공통 문서란 제목", "📚 공통 문서란?");
    createTranslationKeyIfNotExists(
    ...
    createTranslationKeyIfNotExists(
        "admin.globalDoc.info.examplesTitle", "admin", "활용 예시 제목", "💡 활용 예시:");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.info.example1", "admin", "활용 예시 1", "회사 공통 코딩 컨벤션 및 개발 가이드라인");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.info.example2", "admin", "활용 예시 2", "테스트 작성 표준 및 품질 관리 문서");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.info.example3", "admin", "활용 예시 3", "프로젝트 공통 참조 문서 (API 명세, 아키텍처 가이드 등)");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.info.example4", "admin", "활용 예시 4", "조직 전체의 모범 사례 및 학습 자료");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.info.techSpecsTitle", "admin", "기술 사양 제목", "⚙️ 기술 사양:");
    createTranslationKeyIfNotExists(
    ...
    createTranslationKeyIfNotExists(
        "admin.globalDoc.info.autoSearch", "admin", "자동 검색", "모든 프로젝트의 RAG Q&A에서 자동 검색됨");
    createTranslationKeyIfNotExists(
    ...
    // 문서 메시지
    createTranslationKeyIfNotExists(
        "admin.globalDoc.message.uploadSuccess", "admin", "업로드 성공", "공통 문서 \"{0}\"이 업로드되었습니다");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.message.uploadFailed", "admin", "업로드 실패", "공통 문서 업로드 실패");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.message.deleteSuccess", "admin", "삭제 성공", "공통 문서 \"{0}\"이 삭제되었습니다");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.message.deleteFailed", "admin", "삭제 실패", "공통 문서 삭제 실패");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.message.confirmDelete", "admin", "삭제 확인", "공통 문서 \"{0}\"을 삭제하시겠습니까?");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.message.confirmAnalyze", "admin", "분석 확인", "문서 \"{0}\"을 분석하시겠습니까?");
    createTranslationKeyIfNotExists(
    ...
    createTranslationKeyIfNotExists(
        "admin.globalDoc.message.analyzeStarted", "admin", "분석 시작", "문서 \"{0}\" 분석 시작됨");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.message.embeddingsStarted", "admin", "임베딩 시작", "문서 \"{0}\" 임베딩 생성 시작됨");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.message.analyzeFailed", "admin", "분석 실패", "분석 시작 실패");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.message.embeddingsFailed", "admin", "임베딩 실패", "임베딩 생성 실패");
    createTranslationKeyIfNotExists(
        "admin.globalDoc.message.downloadSuccess", "admin", "다운로드 성공", "문서 \"{0}\" 다운로드 완료");
    createTranslationKeyIfNotExists(
    ...
    createTranslationKeyIfNotExists(
        "admin.globalDoc.message.fileSizeLimit", "admin", "파일 크기 제한", "파일 크기는 50MB를 초과할 수 없습니다");
    createTranslationKeyIfNotExists(
