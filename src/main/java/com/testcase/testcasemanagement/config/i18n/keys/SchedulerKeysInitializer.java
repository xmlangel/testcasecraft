...
public class SchedulerKeysInitializer {
  ...
  public void initialize() {
    ...
    createTranslationKeyIfNotExists(
        "scheduler.currentTime", "scheduler", "현재 시간 라벨", "현재 시간 ({timezone})");
    createTranslationKeyIfNotExists("scheduler.refresh", "scheduler", "새로고침 버튼", "새로고침");
    createTranslationKeyIfNotExists(
        "scheduler.status.changed", "scheduler", "스케줄 상태 변경 성공 메시지", "스케줄 상태가 변경되었습니다.");
    createTranslationKeyIfNotExists(
        "scheduler.task.executed", "scheduler", "작업 실행 성공 메시지", "작업이 실행되었습니다.");
    createTranslationKeyIfNotExists(
    ...
    createTranslationKeyIfNotExists(
        "scheduler.column.scheduleExpression", "scheduler", "스케줄 표현식 컬럼", "스케줄 표현식");
    createTranslationKeyIfNotExists("scheduler.column.type", "scheduler", "타입 컬럼", "타입");
    createTranslationKeyIfNotExists(
        "scheduler.column.nextExecution", "scheduler", "다음 실행 컬럼", "다음 실행");
    createTranslationKeyIfNotExists(
        "scheduler.column.lastExecution", "scheduler", "마지막 실행 컬럼", "마지막 실행");
    ...
    createTranslationKeyIfNotExists(
        "scheduler.dialog.title", "scheduler", "스케줄 설정 다이얼로그 제목", "스케줄 설정 편집");
    createTranslationKeyIfNotExists("scheduler.dialog.taskKey", "scheduler", "작업 키 라벨", "작업 키:");
    createTranslationKeyIfNotExists(
        "scheduler.dialog.scheduleType", "scheduler", "스케줄 타입 라벨", "스케줄 타입:");
    createTranslationKeyIfNotExists("scheduler.dialog.description", "scheduler", "설명 라벨", "설명:");
    createTranslationKeyIfNotExists(
        "scheduler.dialog.cronExpression", "scheduler", "Cron 표현식 필드 라벨", "Cron 표현식");
    createTranslationKeyIfNotExists(
    ...
    createTranslationKeyIfNotExists(
        "scheduler.dialog.cronExamples", "scheduler", "Cron 표현식 예시 제목", "Cron 표현식 예시");
    createTranslationKeyIfNotExists(
        "scheduler.dialog.fixedRate", "scheduler", "Fixed Rate 필드 라벨", "Fixed Rate (밀리초)");
    createTranslationKeyIfNotExists(
        "scheduler.dialog.fixedDelay", "scheduler", "Fixed Delay 필드 라벨", "Fixed Delay (밀리초)");
    createTranslationKeyIfNotExists(
        "scheduler.dialog.currentValue", "scheduler", "현재 값 표시", "현재 값: {value}");
    createTranslationKeyIfNotExists("scheduler.dialog.enabled", "scheduler", "활성화 스위치 라벨", "활성화");
    createTranslationKeyIfNotExists(
        "scheduler.dialog.nextExecution", "scheduler", "다음 실행 예정 메시지", "다음 실행 예정: {time}");
    ...
    createTranslationKeyIfNotExists(
        "scheduler.dialog.updated", "scheduler", "설정 업데이트 성공 메시지", "스케줄 설정이 업데이트되었습니다.");

    // Cron 예시
    createTranslationKeyIfNotExists(
        "scheduler.cron.every5min", "scheduler", "매 5분마다 Cron 예시", "매 5분마다");
    createTranslationKeyIfNotExists(
        "scheduler.cron.everyHour", "scheduler", "매 시간 정각 Cron 예시", "매 시간 정각");
    createTranslationKeyIfNotExists(
        "scheduler.cron.midnight", "scheduler", "매일 자정 Cron 예시", "매일 자정");
    createTranslationKeyIfNotExists(
        "scheduler.cron.daily1am", "scheduler", "매일 새벽 1시 Cron 예시", "매일 새벽 1시");
    createTranslationKeyIfNotExists(
        "scheduler.cron.weekdays9am", "scheduler", "평일 오전 9시 Cron 예시", "평일 오전 9시");
    createTranslationKeyIfNotExists(
        "scheduler.cron.monday9am", "scheduler", "매주 월요일 오전 9시 Cron 예시", "매주 월요일 오전 9시");

    // 에러 메시지
    createTranslationKeyIfNotExists(
        "scheduler.error.cronRequired", "scheduler", "Cron 표현식 필수 에러", "Cron 표현식을 입력하세요");
    createTranslationKeyIfNotExists(
