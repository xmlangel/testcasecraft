...
/** ICT-134: 대시보드 모니터링을 위한 스케줄링 설정 RefreshTokenService의 @Scheduled 메서드와 대시보드 모니터링 스케줄링이 동작하도록 설정 */
@Configuration
@EnableScheduling
public class SchedulingConfig {
  ...
  @Autowired private com.testcase.testcasemanagement.service.RagService ragService;
  ...
  @Autowired
  private com.testcase.testcasemanagement.service.TestCaseFileStorageService
      testCaseFileStorageService;

  /** 대시보드 성능 메트릭을 로그에 기록 DynamicSchedulerService에서 호출됨 (taskKey: performance-metrics) */
  ...
  /** 시스템 헬스 임계값 확인 DynamicSchedulerService에서 호출됨 (taskKey: system-health-check) */
  ...
  /** 전체 캐시 상태를 확인하고 필요시 경고 로그 출력 DynamicSchedulerService에서 호출됨 (taskKey: cache-monitoring) */
  ...
  /** 일일 성능 리포트 생성 DynamicSchedulerService에서 호출됨 (taskKey: daily-performance-report) */
  ...
  /** 주간 성능 요약 리포트 생성 DynamicSchedulerService에서 호출됨 (taskKey: weekly-performance-report) */
  ...
  /** RAG 고아 문서 정리 DynamicSchedulerService에서 호출됨 (taskKey: rag-cleanup) */
    ...
    try {
      ...
      for (var doc : ragDocuments.getDocuments()) {
        if (doc.getFileName() != null && doc.getFileName().startsWith("testcase_")) {
          ...
          String testCaseId = doc.getFileName().replace("testcase_", "").replace(".txt", "");
          ...
          if (!exists) {
            ...
            try {
              ...
              deletedCount++;
              logger.info(
                  "고아 RAG 문서 삭제 완료: documentId={}, fileName={}", doc.getId(), doc.getFileName());
            } catch (Exception e) {
              logger.error(
                  "고아 RAG 문서 삭제 실패: documentId={}, fileName={}", doc.getId(), doc.getFileName(), e);
            }
          }
        }
      }
      ...
      logger.info(
          "📋 RAG 고아 문서 정리 완료 - 총 문서: {}, 고아 문서: {}, 삭제 완료: {}",
          ragDocuments.getDocuments().size(),
          totalOrphaned,
          deletedCount);
  ...
  /**
   * 미사용 첨부파일 정리 7일 이상 사용되지 않은 첨부파일을 자동으로 삭제 DynamicSchedulerService에서 호출됨 (taskKey:
   * attachment-cleanup)
   */
  public void cleanupUnusedAttachments() {
    try {
      ...
      logger.info(
          "🗑️ 미사용 첨부파일 정리 완료 - 삭제: {}, 실패: {}, 확보 공간: {} MB (기준: {}일 이전 생성)",
  ...
  @Autowired private com.testcase.testcasemanagement.service.LlmConfigService llmConfigService;

  @Autowired private com.testcase.testcasemanagement.service.LlmTemplateService llmTemplateService;

  /**
   * RAG 문서 자동 LLM 청크 분석 등록 날짜순으로 미분석 문서를 순차적으로 분석 DynamicSchedulerService에서 호출됨 (taskKey:
   * rag-auto-analysis)
   */
  public void autoAnalyzeRagDocumentsWithLlm() {
    ...
    try {
      ...
      logger.info(
          "기본 LLM Config 사용: id={}, provider={}, model={}",
          llmConfigId,
      ...
      logger.info(
          "기본 LLM 템플릿 사용: maxTokens={}, temperature={}, batchSize={}",
          template.getMaxTokens(),
          template.getTemperature(),
          template.getChunkBatchSize());

      // 2. RAG 문서 목록 조회 (날짜순 정렬은 FastAPI에서 자동으로 created_at 기준)
      var ragDocuments = ragService.listDocuments(null, 1, 1000);
      if (ragDocuments == null
          || ragDocuments.getDocuments() == null
          || ragDocuments.getDocuments().isEmpty()) {
      ...
      for (var doc : ragDocuments.getDocuments()) {
        try {
          ...
          var analysisRequest =
              com.testcase.testcasemanagement.dto.rag.RagLlmAnalysisRequest.builder()
          ...
          logger.error("문서 '{}' LLM 분석 중 오류 발생: {}", doc.getFileName(), e.getMessage(), e);
          errorCount++;
        }
      }

      logger.info(
          "🤖 RAG 문서 자동 LLM 청크 분석 완료 - 총 문서: {}, 분석 시작: {}, Skip: {}, 오류: {}",
          ragDocuments.getDocuments().size(),
          processedCount,
          skippedCount,
          errorCount);
