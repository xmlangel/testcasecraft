 ...
 * 가용성 확인 결과
 *
 * <p>모델별 판정과 함께 계정 한도 상태를 담는다. 한도 개념이 없는 제공자(NVIDIA)에서는 {@code accountLimit} 이 항상 null 이다. 한도 상태를
 * 별도 필드로 두는 이유는, 판정 사유 문자열에 묻어 두면 화면이 그것을 문자열로 뒤져야 하고 그 방식은 문구를 고칠 때마다 깨지기 때문이다.
 *
...
public class LlmModelProbeResponse {
   ...
   *
   * <p>이 값이 없으면 요청한 모델이 결과에서 조용히 사라진다. 45개를 확인 요청했는데 상한이 40이면 5개가 결과에 없고, 화면은 전부 확인된 것으로 보인다. 확인되지
   * 않은 모델을 나중에 골라 채팅하면 실패하고 원인을 되짚기 어렵다.
   */
