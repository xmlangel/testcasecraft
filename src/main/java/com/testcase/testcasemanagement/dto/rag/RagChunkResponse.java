 ...
 *
 * <p>FastAPI ChunkResponse 스키마와 매핑되는 DTO 문서의 텍스트 청크 조회 응답에 사용
 */
@Data
@Builder
@NoArgsConstructor
...
public class RagChunkResponse {
  ...
  /** 청크 ID Frontend: id (unchanged) FastAPI: id (unchanged) */
  @JsonProperty("id")
  private UUID id;

  /** 문서 ID Frontend: documentId (camelCase) FastAPI: document_id (snake_case) - JsonAlias로 매핑 */
  @JsonProperty("documentId")
  @com.fasterxml.jackson.annotation.JsonAlias({"document_id"})
  private UUID documentId;

  /**
   * 청크 인덱스 (순서) Frontend: chunkIndex (camelCase) FastAPI: chunk_index (snake_case) - JsonAlias로 매핑
   */
  @JsonProperty("chunkIndex")
  @com.fasterxml.jackson.annotation.JsonAlias({"chunk_index"})
  private Integer chunkIndex;

  /** 청크 텍스트 내용 Frontend: chunkText (camelCase) FastAPI: chunk_text (snake_case) - JsonAlias로 매핑 */
  @JsonProperty("chunkText")
  @com.fasterxml.jackson.annotation.JsonAlias({"chunk_text"})
  private String chunkText;

  /**
   * 청크 메타데이터 Frontend: chunkMetadata (camelCase) FastAPI: chunk_metadata (snake_case) - JsonAlias로
   * 매핑
   */
