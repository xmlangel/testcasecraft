...
/** ICT-208: 표준화된 API 응답 래퍼 클래스 모든 API 응답에 일관된 형식을 제공합니다. */
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class ApiResponse<T> {
  ...
  public static <T> ApiResponse<T> success(T data, Object metadata) {
    return ApiResponse.<T>builder().success(true).data(data).metadata(metadata).build();
  }
  ...
  public static <T> ApiResponse<T> error(String errorMessage) {
    return ApiResponse.<T>builder().success(false).errorMessage(errorMessage).build();
  }
