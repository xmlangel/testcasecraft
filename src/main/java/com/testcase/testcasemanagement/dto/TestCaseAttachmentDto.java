...
public class TestCaseAttachmentDto {
  ...
  public static TestCaseAttachmentDto fromEntity(TestCaseAttachment entity) {
    ...
    dto.setUploadedByName(
        entity.getUploadedBy() != null ? entity.getUploadedBy().getName() : "알 수 없음");
    ...
    dto.setDownloadUrl(
        entity.isDownloadable()
            ? "/api/testcase-attachments/" + entity.getId() + "/download"
            : null);
    if (entity.getPublicAccessToken() != null && !entity.getPublicAccessToken().isBlank()) {
      dto.setPublicUrl(
          "/api/testcase-attachments/public/"
              + entity.getId()
              + "?token="
              + entity.getPublicAccessToken());
    }
