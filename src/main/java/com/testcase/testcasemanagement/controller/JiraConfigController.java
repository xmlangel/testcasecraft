...
public class JiraConfigController {
    ...
    public ResponseEntity<JiraConfigDto> updateConfig(
            @Parameter(description = "수정할 설정 ID", required = true)
            @PathVariable @NotBlank String configId,
            @Parameter(description = "수정할 JIRA 설정 정보", required = true)
            @Valid @RequestBody JiraConfigDto configDto) {
    ...
    public ResponseEntity<JiraConfigDto.ConnectionStatusDto> testConnection(
            @Parameter(description = "연결 테스트 정보", required = true)
            @Valid @RequestBody JiraConfigDto.TestConnectionDto testConfig) {
    ...
    public ResponseEntity<Map<String, Object>> deleteConfig(
            @Parameter(description = "삭제할 설정 ID", required = true)
            @PathVariable @NotBlank String configId) {
    ...
    public ResponseEntity<Map<String, Object>> activateConfig(
            @Parameter(description = "활성화할 설정 ID", required = true)
            @PathVariable @NotBlank String configId) {
    ...
    public ResponseEntity<Map<String, Object>> addTestResultComment(
            @Parameter(description = "코멘트 추가 요청", required = true)
        ...
        try {
            ...
            log.error("JIRA 코멘트 추가 중 오류", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(Map.of("success", false, "message", "코멘트 추가 중 오류 발생"));
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> getConnectionStatus() {
        try {
            ...
            if (config.isPresent()) {
                ...
                status.put("isConnected", configDto.getConnectionVerified() != null && configDto.getConnectionVerified());
    ...
    public ResponseEntity<Map<String, Object>> searchIssues(
            @Parameter(description = "이슈 검색 요청", required = true)
