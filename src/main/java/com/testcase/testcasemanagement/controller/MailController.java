        ...
        @ApiResponse(
            ...
            content =
                @Content(
                    ...
                    examples = @ExampleObject(value = "메일이 성공적으로 발송되었습니다."))),
        @ApiResponse(
            responseCode = "500",
            ...
            content =
                @Content(
                    mediaType = "text/plain",
                    examples = @ExampleObject(value = "메일 발송 실패: [오류 메시지]")))
      })
  @PostMapping("/send")
      ...
      if (mailDto.getFrom() != null && !mailDto.getFrom().isEmpty()) {
        ...
        if (mailDto.isHtml()) {
          mailService.sendHtmlMail(
              mailDto.getFrom(), mailDto.getTo(), mailDto.getSubject(), mailDto.getText());
        } else {
