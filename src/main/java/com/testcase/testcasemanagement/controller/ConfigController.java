...
/** 프론트엔드 설정 정보를 제공하는 컨트롤러 런타임 환경변수를 프론트엔드에 전달 */
...
public class ConfigController {
  ...
  @Value(
      "${frontend.api.base-url:#{environment['REACT_APP_API_BASE_URL'] ?:"
          + " 'http://localhost:8080'}}")
  ...
  @Value("${frontend.show-exploratory-session-tab:false}")
  private boolean showExploratorySessionTab;
  ...
  /**
   * 프론트엔드 설정 정보 반환 요청에서 동적으로 서버 URL을 구성하여 반환
   *
  ...
  public ResponseEntity<Map<String, Object>> getConfig(HttpServletRequest request) {
    ...
    config.put("showExploratorySessionTab", showExploratorySessionTab);
  ...
  /**
   * API 기본 URL만 반환 (간단한 형태) 요청에서 동적으로 서버 URL을 구성하여 반환
   *
   * @param request HTTP 요청 객체
   * @return API 기본 URL
   */
  ...
  /**
   * 환경 정보 반환 요청에서 동적으로 서버 URL을 구성하여 반환
   *
   * @param request HTTP 요청 객체
   * @return 환경 정보
   */
  ...
  public ResponseEntity<Map<String, Object>> getUploadLimits() {
    ...
    String maxRequestSize =
        environment.getProperty("spring.servlet.multipart.max-request-size", "100MB");

    // JUnit 관련 설정
    String junitMaxSize =
        environment.getProperty("junit.file.max-size", "104857600"); // 100MB in bytes
  ...
  /**
   * HTTP 요청에서 동적으로 API 기본 URL을 구성 X-Forwarded-* 헤더를 고려하여 프록시 환경에서도 올바른 URL 반환
   *
   * @param request HTTP 요청 객체
