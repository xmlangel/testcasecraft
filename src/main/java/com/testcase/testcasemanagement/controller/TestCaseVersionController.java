 ...
 *
 * <p>테스트케이스 버전 관리를 위한 REST API 엔드포인트 제공
 */
@Slf4j
@RestController
...
public class TestCaseVersionController {
  ...
  public ResponseEntity<ApiResponse<List<TestCaseVersionDto>>> getCurrentVersionsByProject(
    ...
    try {
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
          .body(
              ApiResponse.<List<TestCaseVersionDto>>error("프로젝트 현재 버전들 조회 실패: " + e.getMessage()));
    }
  }
  ...
  public ResponseEntity<ApiResponse<TestCaseVersionDto>> createManualVersion(
    ...
    try {
      ...
      TestCaseVersionDto version =
          versionService.createManualVersion(testCaseId, versionLabel, versionDescription);
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> compareVersions(
    ...
    try {
      ...
      log.warn(
          "버전 비교 요청 오류: versionId1={}, versionId2={}, error={}",
          versionId1,
          versionId2,
          e.getMessage());
      return ResponseEntity.badRequest()
      ...
      log.error(
          "버전 비교 실패: versionId1={}, versionId2={}, error={}",
          versionId1,
          versionId2,
          e.getMessage(),
          e);
  ...
  public ResponseEntity<ApiResponse<Integer>> cleanupOldVersions(
    ...
    try {
      ...
      return ResponseEntity.ok(
          ApiResponse.success(deletedCount, "오래된 버전 정리 완료: " + deletedCount + "개 버전 삭제"));
    } catch (Exception e) {
      log.error(
          "오래된 버전 정리 실패: testCaseId={}, keepCount={}, error={}",
          testCaseId,
          keepCount,
          e.getMessage(),
          e);
  ...
  public ResponseEntity<ApiResponse<Integer>> cleanupDraftVersions(
    ...
    try {
      ...
      return ResponseEntity.ok(
          ApiResponse.success(deletedCount, "임시 버전 정리 완료: " + deletedCount + "개 버전 삭제"));
    } catch (Exception e) {
