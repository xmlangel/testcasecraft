...
@Tag(
    ...
    **📊 사용자 활동 추적 시스템 API**
    ...
    시스템 내 모든 사용자 활동을 실시간으로 추적하고 분석할 수 있는 API 모음입니다.
    
    **🔐 보안 요구사항:**
    • **JWT 인증**: 모든 API 호출 시 Bearer Token 필수
    • **권한별 접근 제어**: 본인 활동 조회 또는 관리자 권한 필요
    • **개인정보 보호**: IP 주소, 사용자 에이전트 등 민감 정보 적절히 제한
    ...
    **🎯 활동 타입:**
    • 인증: LOGIN, LOGOUT, LOGIN_FAILED, PASSWORD_CHANGE
    • 프로젝트: PROJECT_CREATE, PROJECT_ACCESS, PROJECT_UPDATE
    • 테스트: TESTCASE_CREATE, TESTCASE_VIEW, TESTCASE_UPDATE
    • 조직: ORGANIZATION_CREATE, ORGANIZATION_ACCESS
    • 시스템: ADMIN_ACCESS, SYSTEM_CONFIG
    """
)
@RestController
...
public class UserActivityController {
    ...
    public ResponseEntity<Page<UserActivityDto.Response>> searchActivities(
            @Parameter(description = "검색 키워드", example = "로그인")
    ...
    public ResponseEntity<List<UserActivityDto.Response>> getAnomalousActivities(
            @Parameter(description = "사용자 ID (선택사항, 관리자는 전체 조회 가능)")
    ...
    public ResponseEntity<UserActivityDto.Response> getActivityDetail(
            @Parameter(description = "활동 ID", required = true)
    ...
    private UserActivityDto.StatisticsResponse convertToStatisticsDto(Map<String, Object> statistics, String userId, LocalDateTime startDate, LocalDateTime endDate) {
