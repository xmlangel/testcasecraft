...
public class JunitVersionController {
  ...
  @Autowired private JunitVersionControlService versionControlService;
  ...
  public ResponseEntity<Map<String, Object>> restoreVersion(
    ...
    try {
      FileRestoreResult result =
          versionControlService.restoreVersion(
              testResultId, versionNumber, request.getTargetPath());
      ...
      if (versionNumber == -1 && !history.getVersions().isEmpty()) {
        ...
        targetVersion =
            history.getVersions().stream()
                ...
                .findFirst()
                .orElseThrow(
                    () -> new VersionControlException("Version not found: " + versionNumber));
      }
