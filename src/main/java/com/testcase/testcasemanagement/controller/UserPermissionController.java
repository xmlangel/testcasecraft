...
/** 사용자 권한 및 역할 관리 API 컨트롤러 ICT-33: 사용자 권한 및 역할 관리 강화 */
...
public class UserPermissionController {
  ...
  @Autowired private CsvPermissionService csvPermissionService;

  @Autowired private PermissionConflictService permissionConflictService;
  ...
  public ResponseEntity<Map<String, String>> addUserToOrganization(
      @PathVariable String organizationId, @RequestBody Map<String, String> request) {
  ...
  public ResponseEntity<List<UserPermissionDto>> getOrganizationMembers(
      @PathVariable String organizationId) {
  ...
  public ResponseEntity<List<PermissionConflictDto>> validatePermissionChanges(
    ...
    List<PermissionConflictDto> conflicts =
        userPermissionService.validatePermissionChanges(changes);
  ...
  public ResponseEntity<Map<String, Object>> comprehensiveValidateChanges(
    ...
    List<PermissionConflictDto> conflicts =
        permissionConflictService.comprehensiveConflictCheck(changes);

    // 충돌 유형별 분류
    Map<String, List<PermissionConflictDto>> conflictsByType =
        conflicts.stream().collect(Collectors.groupingBy(PermissionConflictDto::getConflictType));

    long autoResolvableCount =
        conflicts.stream().filter(PermissionConflictDto::isCanAutoResolve).count();
  ...
  public ResponseEntity<Map<String, Object>> autoResolveConflicts(
    ...
    List<BulkPermissionChangeDto> resolvedChanges =
        permissionConflictService.autoResolveConflicts(changes, conflicts);

    // 해결 후 재검증
    List<PermissionConflictDto> remainingConflicts =
        permissionConflictService.comprehensiveConflictCheck(resolvedChanges);
  ...
  public ResponseEntity<Map<String, String>> handleSecurityException(SecurityException e) {
    return ResponseEntity.status(403).body(Map.of("error", "권한 부족", "message", e.getMessage()));
  }
  ...
  public ResponseEntity<Map<String, Object>> uploadCsvFile(
    ...
    try {
      ...
      return ResponseEntity.status(500)
          .body(Map.of("error", "파일 처리 중 오류가 발생했습니다.", "message", e.getMessage()));
    }
  }
  ...
  public ResponseEntity<String> downloadCsvTemplate() {
    ...
    return ResponseEntity.ok()
        .header(
            ...
            "attachment; filename=\"permission_changes_template.csv\"")
  ...
  public ResponseEntity<Map<String, String>> executeCsvChanges(
    ...
    try {
      ...
      return ResponseEntity.ok(
          Map.of(
              "message",
              "CSV 기반 대량 권한 변경이 성공적으로 완료되었습니다.",
              "processedCount",
              String.valueOf(validatedChanges.size())));
    } catch (Exception e) {
      return ResponseEntity.status(500)
  ...
  public ResponseEntity<Map<String, String>> handleRuntimeException(RuntimeException e) {
    return ResponseEntity.status(400).body(Map.of("error", "요청 처리 실패", "message", e.getMessage()));
  }
}
