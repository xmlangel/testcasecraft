...
/** ICT-189: JIRA 상태 통합 및 중복 제거 컨트롤러 테스트 결과의 JIRA ID들을 통합하여 중복을 제거하고 현재 상태를 조회하는 API */
@RestController
...
public class JiraStatusController {
  ...
  @Operation(
      summary = "프로젝트 JIRA 상태 요약 조회",
      description = "프로젝트의 모든 테스트 결과에서 JIRA ID를 추출하여 중복 제거 후 현재 상태를 조회합니다.")
  @ApiResponses({
  ...
  public ResponseEntity<List<JiraStatusSummaryDto>> getProjectJiraStatusSummary(
    ...
    try {
      List<JiraStatusSummaryDto> summary =
          jiraStatusAggregationService.getProjectJiraStatusSummary(userId, projectId);
  ...
  public ResponseEntity<JiraStatusSummaryDto> getJiraStatusDetail(
      @Parameter(hidden = true) java.security.Principal principal,
      @Parameter(description = "JIRA 이슈 키 (예: PRJ-123)", required = true) @PathVariable
          String jiraId) {
    ...
    try {
      ...
      JiraStatusSummaryDto detail =
          jiraStatusAggregationService.getJiraStatusDetail(userId, jiraId);
  ...
  public ResponseEntity<List<JiraStatusSummaryDto>> refreshProjectJiraStatus(
    ...
    try {
      List<JiraStatusSummaryDto> refreshedSummary =
          jiraStatusAggregationService.refreshProjectJiraStatus(userId, projectId);

      log.info(
          "프로젝트 JIRA 상태 강제 새로고침 완료: projectId={}, 요약 개수={}", projectId, refreshedSummary.size());
  ...
  @Operation(
      summary = "여러 프로젝트의 JIRA 상태 요약 배치 조회",
      description = "여러 프로젝트의 JIRA 상태를 한 번에 조회합니다. 성능 최적화를 위해 병렬 처리됩니다.")
  @ApiResponses({
  ...
  public ResponseEntity<?> getBatchProjectJiraStatusSummary(
      @Parameter(hidden = true) java.security.Principal principal,
      @Parameter(description = "프로젝트 ID 목록", required = true) @RequestBody
    ...
    try {
      ...
      // 병렬 스트림을 사용한 배치 처리
      var batchResult =
          projectIds.parallelStream()
              .collect(
                  java.util.stream.Collectors.toMap(
                      projectId -> projectId,
                      projectId -> {
                        try {
                          return jiraStatusAggregationService.getProjectJiraStatusSummary(
                              userId, projectId);
                        } catch (Exception e) {
      ...
      int totalSummaries = batchResult.values().stream().mapToInt(List::size).sum();

      log.info(
          "배치 프로젝트 JIRA 상태 요약 조회 완료: 프로젝트 수={}, 총 요약 개수={}", projectIds.size(), totalSummaries);
  ...
  public ResponseEntity<?> getJiraStatusStatistics(
    ...
    try {
      ...
      if (projectId != null && !projectId.trim().isEmpty()) {
        summaries =
            jiraStatusAggregationService.getProjectJiraStatusSummary(userId, projectId.trim());
      } else {
        // 전체 프로젝트 통계는 향후 구현
  ...
  public ResponseEntity<?> getBatchJiraIssueSummaries(
    ...
    String userId = resolveUserId(principal);
    log.info(
        "배치 JIRA 이슈 상태 조회 요청: 사용자={}, 개수={}",
        userId,
        jiraIssueKeys != null ? jiraIssueKeys.size() : 0);
    ...
    try {
      List<JiraStatusSummaryDto> summaries =
          jiraStatusAggregationService.getBatchJiraStatusSummary(userId, jiraIssueKeys);
  ...
  @Operation(
      summary = "JIRA 상태 데이터베이스 동기화",
      description = "JIRA API에서 최신 상태를 조회하여 데이터베이스에 저장합니다. 이후 조회 시 DB 값을 사용하여 성능이 향상됩니다.")
  @ApiResponses({
  ...
  public ResponseEntity<?> syncJiraStatusToDatabase(
      @Parameter(hidden = true) java.security.Principal principal,
      @Parameter(description = "JIRA 이슈 키 목록", required = true) @RequestBody
          List<String> jiraIssueKeys) {

    String userId = resolveUserId(principal);
    log.info(
        "JIRA 상태 데이터베이스 동기화 요청: 사용자={}, 개수={}",
        userId,
        jiraIssueKeys != null ? jiraIssueKeys.size() : 0);
   ...
   * @return 통계 정보
   */
  private java.util.Map<String, Object> calculateJiraStatusStatistics(
      List<JiraStatusSummaryDto> summaries) {
    ...
    var priorityDistribution =
        summaries.stream()
            .collect(
                java.util.stream.Collectors.groupingBy(
                    JiraStatusSummaryDto::getPriority, java.util.stream.Collectors.counting()));
    statistics.put("priorityDistribution", priorityDistribution);

    // 동기화 상태별 분포
    var syncStatusDistribution =
        summaries.stream()
            .collect(
                java.util.stream.Collectors.groupingBy(
                    JiraStatusSummaryDto::getSyncStatus, java.util.stream.Collectors.counting()));
    statistics.put("syncStatusDistribution", syncStatusDistribution);

    // 연결된 테스트 총 수
    long totalLinkedTests =
        summaries.stream()
    ...
    double averageSuccessRate =
        summaries.stream().mapToDouble(JiraStatusSummaryDto::getSuccessRate).average().orElse(0.0);
    statistics.put("averageSuccessRate", Math.round(averageSuccessRate * 100.0) / 100.0);

    // 실패한 테스트가 있는 이슈 수
    long issuesWithFailures = summaries.stream().mapToLong(s -> s.hasFailedTests() ? 1 : 0).sum();
    statistics.put("issuesWithFailures", issuesWithFailures);

    // 모든 테스트가 통과한 이슈 수
    long issuesAllPassed = summaries.stream().mapToLong(s -> s.allTestsPassed() ? 1 : 0).sum();
    statistics.put("issuesAllPassed", issuesAllPassed);

    // 활성 이슈 수 (완료되지 않은 이슈)
    long activeIssues = summaries.stream().mapToLong(s -> s.isActiveIssue() ? 1 : 0).sum();
