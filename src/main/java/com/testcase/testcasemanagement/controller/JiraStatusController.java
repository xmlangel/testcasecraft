...
public class JiraStatusController {
    ...
    public ResponseEntity<List<JiraStatusSummaryDto>> getProjectJiraStatusSummary(
        ...
        log.info("프로젝트 JIRA 상태 요약 조회 요청: projectId={}", projectId);
        
        try {
            List<JiraStatusSummaryDto> summary = jiraStatusAggregationService.getProjectJiraStatusSummary(projectId);
    ...
    @Operation(
        summary = "JIRA 이슈 상세 상태 조회",
        description = "특정 JIRA 이슈의 상세 상태와 연결된 테스트 결과 정보를 조회합니다."
    )
    @ApiResponses({
    ...
    public ResponseEntity<JiraStatusSummaryDto> getJiraStatusDetail(
            @Parameter(description = "JIRA 이슈 키 (예: PRJ-123)", required = true)
            @PathVariable String jiraId) {
        
        log.info("JIRA 이슈 상세 상태 조회 요청: jiraId={}", jiraId);
        ...
        try {
            ...
            JiraStatusSummaryDto detail = jiraStatusAggregationService.getJiraStatusDetail(jiraId);
    ...
    @Operation(
        summary = "프로젝트 JIRA 상태 강제 새로고침",
        description = "캐시를 무시하고 프로젝트의 JIRA 상태를 강제로 새로고침합니다."
    )
    @ApiResponses({
    ...
    public ResponseEntity<List<JiraStatusSummaryDto>> refreshProjectJiraStatus(
        ...
        log.info("프로젝트 JIRA 상태 강제 새로고침 요청: projectId={}", projectId);
        
        try {
            List<JiraStatusSummaryDto> refreshedSummary = jiraStatusAggregationService.refreshProjectJiraStatus(projectId);
    ...
    public ResponseEntity<?> getBatchProjectJiraStatusSummary(
            @Parameter(description = "프로젝트 ID 목록", required = true)
            @RequestBody List<String> projectIds) {
        
        log.info("배치 프로젝트 JIRA 상태 요약 조회 요청: projectIds={}", projectIds);
        ...
        try {
            ...
            var batchResult = projectIds.parallelStream()
                .collect(java.util.stream.Collectors.toMap(
                    ...
                    projectId -> {
                        try {
                            return jiraStatusAggregationService.getProjectJiraStatusSummary(projectId);
                        } catch (Exception e) {
    ...
    @Operation(
        summary = "JIRA 상태 통계 조회",
        description = "전체 또는 특정 프로젝트의 JIRA 상태별 통계 정보를 조회합니다."
    )
    @ApiResponses({
    ...
    public ResponseEntity<?> getJiraStatusStatistics(
            @Parameter(description = "프로젝트 ID (선택적)")
            @RequestParam(required = false) String projectId) {
        
        log.info("JIRA 상태 통계 조회 요청: projectId={}", projectId);
        
        try {
            List<JiraStatusSummaryDto> summaries;
            
            if (projectId != null && !projectId.trim().isEmpty()) {
                summaries = jiraStatusAggregationService.getProjectJiraStatusSummary(projectId.trim());
            } else {
                // 전체 프로젝트 통계는 향후 구현
