...
/** 조직 관리 컨트롤러 Spring Security @PreAuthorize를 통한 메서드 레벨 보안 적용 */
...
public class OrganizationController {
  ...
  public ResponseEntity<Organization> createOrganization(
    ...
    Organization organization =
        organizationService.createOrganization(request.getName(), request.getDescription());
    return ResponseEntity.status(HttpStatus.CREATED).body(organization);
  }

  /** 사용자가 접근 가능한 조직 목록 조회 권한: 인증된 사용자만 가능 */
  @Operation(summary = "접근 가능한 조직 조회", description = "사용자가 속하거나 접근 가능한 모든 조직 목록을 조회합니다.")
  @GetMapping
  @PreAuthorize(
      "hasRole('USER') or hasRole('ADMIN') or hasRole('TESTER') or hasAuthority('ROLE_USER') or"
          + " hasAuthority('ROLE_ADMIN') or hasAuthority('ROLE_TESTER')")
  ...
  public ResponseEntity<Organization> updateOrganization(
      @PathVariable String organizationId, @RequestBody UpdateOrganizationRequest request) {
    Organization organization =
        organizationService.updateOrganization(
            organizationId, request.getName(), request.getDescription());
    return ResponseEntity.ok(organization);
  }

  /**
   * 조직 삭제 권한: 조직 소유자 또는 시스템 관리자
   *
  ...
  @PreAuthorize(
      "@organizationSecurityService.canDeleteOrganization(#organizationId, authentication.name)")
  ...
  /** 조직에 멤버 초대 권한: 조직 관리자 이상 또는 시스템 관리자 */
  @Operation(summary = "조직 멤버 초대", description = "조직에 새로운 멤버를 초대합니다.")
  @PostMapping("/{organizationId}/members")
  @PreAuthorize(
      "@organizationSecurityService.canManageOrganization(#organizationId, authentication.name)")
  public ResponseEntity<OrganizationUser> inviteMember(
      @PathVariable String organizationId, @RequestBody InviteMemberRequest request) {
    OrganizationUser member =
        organizationService.inviteMember(organizationId, request.getUsername(), request.getRole());
    return ResponseEntity.status(HttpStatus.CREATED).body(member);
  }

  /** 조직에서 멤버 제거 권한: 조직 관리자 이상 또는 시스템 관리자 (자기 자신은 항상 가능) */
  ...
  /** 멤버 역할 변경 권한: 조직 관리자 이상 또는 시스템 관리자 */
  @Operation(summary = "조직 멤버 역할 변경", description = "조직 멤버의 역할을 변경합니다.")
  @PutMapping("/{organizationId}/members/{userId}/role")
  @PreAuthorize(
      "@organizationSecurityService.canManageOrganization(#organizationId, authentication.name)")
  public ResponseEntity<OrganizationUser> updateMemberRole(
      @PathVariable String organizationId,
      @PathVariable String userId,
      @RequestBody UpdateMemberRoleRequest request) {
    OrganizationUser member =
        organizationService.updateMemberRole(organizationId, userId, request.getRole());
    return ResponseEntity.ok(member);
  }

  /** 조직 소유권 이전 권한: 조직 소유자 또는 시스템 관리자 */
  @Operation(summary = "조직 소유권 이전", description = "조직의 소유권을 다른 사용자에게 이전합니다.")
  @PostMapping("/{organizationId}/transfer-ownership")
  @PreAuthorize(
      "@organizationSecurityService.isOrganizationOwner(#organizationId, authentication.name) or"
          + " hasRole('ADMIN') or hasAuthority('ROLE_ADMIN')")
  public ResponseEntity<Organization> transferOwnership(
      @PathVariable String organizationId, @RequestBody TransferOwnershipRequest request) {
    Organization organization =
        organizationService.transferOwnership(organizationId, request.getNewOwnerUserId());
    return ResponseEntity.ok(organization);
  }

  /** 조직 멤버 목록 조회 권한: 조직 멤버 또는 시스템 관리자 */
  @Operation(summary = "조직 멤버 조회", description = "조직에 속한 모든 멤버 목록을 조회합니다.")
  @GetMapping("/{organizationId}/members")
  @PreAuthorize(
      "@organizationSecurityService.canAccessOrganization(#organizationId, authentication.name)")
  public ResponseEntity<List<OrganizationUser>> getOrganizationMembers(
      @PathVariable String organizationId) {
    List<OrganizationUser> members = organizationService.getOrganizationMembers(organizationId);
    return ResponseEntity.ok(members);
  }

  /** 조직의 프로젝트 목록 조회 권한: 조직 멤버 또는 시스템 관리자 */
  ...
  /** 조직의 그룹 목록 조회 권한: 조직 멤버 또는 시스템 관리자 */
