...
public class TestCaseAttachmentController {
  ...
  public ResponseEntity<?> uploadFile(
    ...
    try {
      ...
      TestCaseAttachmentDto attachmentDto =
          fileStorageService.uploadFile(testCaseId, file, currentUser, description);

      log.info(
          "테스트케이스 파일 업로드 성공: {} by {} (테스트케이스 ID: {})",
      ...
      response.put(
          "message", i18nService.getTranslation("attachment.success.upload", DEFAULT_LANG));
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
          .body(
              createErrorResponse(
                  i18nService.getTranslation("attachment.error.upload.io", DEFAULT_LANG)));
  ...
  public ResponseEntity<?> getAllAttachments(
    ...
    try {
      ...
      log.info(
          "전체 첨부파일 목록 조회 by {} - 총 {}개 (activeOnly: {})",
  ...
  public ResponseEntity<?> getAttachmentsByTestCase(
      @PathVariable String testCaseId, @AuthenticationPrincipal UserDetails userDetails) {

    try {
      List<TestCaseAttachmentDto> attachments =
          fileStorageService.getAttachmentsByTestCaseId(testCaseId);
  ...
  public ResponseEntity<?> getAttachmentInfo(
    ...
    try {
      ...
      return ResponseEntity.status(HttpStatus.NOT_FOUND)
          .body(
              createErrorResponse(
                  i18nService.getTranslation("attachment.error.notfound", DEFAULT_LANG)));
  ...
  public ResponseEntity<?> deleteAttachment(
    ...
    try {
      ...
      response.put("success", true);
      response.put(
          "message", i18nService.getTranslation("attachment.success.delete", DEFAULT_LANG));
  ...
  public ResponseEntity<?> getMyAttachments(@AuthenticationPrincipal UserDetails userDetails) {
    ...
    try {
      ...
      List<TestCaseAttachmentDto> attachments =
          fileStorageService.getAttachmentsByUser(currentUser.getId());
  ...
  public ResponseEntity<?> markAttachmentAsUsed(
    ...
    try {
      ...
      response.put("success", true);
      response.put(
          "message", i18nService.getTranslation("attachment.success.markused", DEFAULT_LANG));
  ...
  public ResponseEntity<?> cleanupUnusedAttachments(
      @Parameter(description = "생성일 기준 일수 (0=모든 미사용 파일, 기본값: 7일)")
          @RequestParam(value = "daysOld", defaultValue = "7")
          int daysOld,
      @AuthenticationPrincipal UserDetails userDetails) {
    ...
    try {
      ...
      TestCaseFileStorageService.CleanupResult result =
          fileStorageService.cleanupUnusedAttachments(daysOld);
      ...
      log.info(
          "미사용 첨부파일 정리 API 호출 by {} - 삭제: {}, 실패: {}, 확보 공간: {}",
          userDetails.getUsername(),
