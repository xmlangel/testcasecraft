...
public class AuthController {
    ...
    try {
      ...
      response.put(
          ...
          Map.of(
              ...
              "timezone",
              userEntity.getTimezone() != null ? userEntity.getTimezone() : "UTC"));
  ...
  public ResponseEntity<?> updateMyInfo(
      Authentication authentication, @RequestBody Map<String, String> updateRequest) {
    // 인증 정보가 없는 경우
  ...
  public ResponseEntity<?> refreshToken(@RequestBody Map<String, String> request) {
    try {
      ...
      Optional<String> newAccessTokenOpt =
          refreshTokenService.refreshAccessToken(refreshTokenValue);
      ...
      return ResponseEntity.status(500)
          .body(Map.of("message", "토큰 갱신 중 오류가 발생했습니다: " + e.getMessage()));
    }
  }
  ...
  public ResponseEntity<?> logout(
    ...
    try {
      ...
      return ResponseEntity.status(500)
          .body(Map.of("message", "로그아웃 중 오류가 발생했습니다: " + e.getMessage()));
    }
  }
  ...
  public ResponseEntity<?> logoutAll(Authentication authentication) {
    try {
      ...
      return ResponseEntity.ok(Map.of("message", "모든 디바이스에서 로그아웃이 완료되었습니다"));
  ...
  public ResponseEntity<?> changeMyPassword(
      Authentication authentication, @RequestBody UserDto.ChangePasswordRequest passwordRequest) {
    try {
      // 인증 확인
      if (authentication == null) {
        return ResponseEntity.status(401)
      ...
      if (passwordRequest.getCurrentPassword() == null
          || passwordRequest.getCurrentPassword().trim().isEmpty()) {
        return ResponseEntity.badRequest()
            .body(
                Map.of(
      ...
      if (passwordRequest.getNewPassword() == null
          || passwordRequest.getNewPassword().trim().isEmpty()) {
        return ResponseEntity.badRequest()
            .body(
                Map.of(
      ...
      return ResponseEntity.badRequest()
          .body(Map.of("errorCode", "VALIDATION_ERROR", "message", e.getMessage()));
    } catch (com.testcase.testcasemanagement.exception.ResourceNotFoundException e) {
      return ResponseEntity.status(404)
          .body(Map.of("errorCode", "USER_NOT_FOUND", "message", e.getMessage()));
    } catch (Exception e) {
      return ResponseEntity.status(500)
          .body(
              Map.of(
    ...
    try {
      ...
      if (accessToken != null) {
        try {
          ...
          result.put("accessToken", Map.of("valid", false, "error", e.getMessage()));
        }
      }
      ...
      return ResponseEntity.status(500)
          .body(Map.of("message", "토큰 검증 중 오류가 발생했습니다: " + e.getMessage()));
    }
  }
  ...
  public ResponseEntity<?> updatePreferredLanguage(
    ...
    try {
      ...
      if (languageCode == null || languageCode.trim().isEmpty()) {
        return ResponseEntity.badRequest().body(Map.of("message", "언어 코드가 필요합니다"));
      }
      ...
      return ResponseEntity.status(500)
          .body(Map.of("message", "언어 설정 업데이트 중 오류가 발생했습니다: " + e.getMessage()));
    }
  }
  ...
  public ResponseEntity<?> sendMyVerificationEmail(
      Authentication authentication, jakarta.servlet.http.HttpServletRequest request) {
    try {
      if (authentication == null) {
      ...
      EmailVerificationDto.CommonResponse result =
          emailVerificationService.createVerificationToken(user.getId(), user.getEmail(), baseUrl);
      if (result.isSuccess()) {
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
          .body(Map.of("success", false, "message", "이메일 발송 중 오류가 발생했습니다: " + e.getMessage()));
    }
  }
}
