...
/** JIRA 모니터링 시스템 REST API 컨트롤러 ICT-165: 모니터링 데이터 조회 및 관리 */
...
public class JiraMonitoringController {
  ...
  public ResponseEntity<?> getSystemHealth() {
    try {
      ...
      return ResponseEntity.internalServerError()
          .body(
              Map.of(
                  "success", false, "message", "헬스 체크 중 서버 오류가 발생했습니다.", "error", e.getMessage()));
    }
  }
  ...
  @Operation(
      summary = "시스템 메트릭 조회",
      description = "JIRA 시스템의 주요 성과 지표(성공률, 응답시간 등)를 조회합니다. (관리자 전용)")
  @GetMapping("/metrics")
  @PreAuthorize("hasRole('ADMIN')")
  public ResponseEntity<?> getSystemMetrics() {
    try {
      ...
      return ResponseEntity.internalServerError()
          .body(
              Map.of(
                  "success", false, "message", "메트릭 조회 중 서버 오류가 발생했습니다.", "error", e.getMessage()));
    }
  }
  ...
  @Operation(
      summary = "시스템 리포트 생성",
      description = "JIRA 시스템 상태에 대한 상세 텍스트 리포트를 생성하여 다운로드합니다. (관리자 전용)")
  ...
  public ResponseEntity<String> generateSystemReport() {
    try {
      ...
      return ResponseEntity.ok()
          .header(
              "Content-Disposition",
              "attachment; filename=\"jira-system-report-"
  ...
  /** JSON 형식 시스템 리포트 */
  @Operation(
      summary = "시스템 리포트 조회 (JSON)",
      description = "JIRA 시스템 상태에 대한 상세 리포트를 JSON 형식으로 조회합니다. (관리자 전용)")
    ...
    try {
      ...
      return ResponseEntity.ok(
          Map.of(
              ...
              true,
              "message",
              "모든 JIRA 모니터링 메트릭이 성공적으로 리셋되었습니다.",
              "timestamp",
              LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)));
      ...
      return ResponseEntity.internalServerError()
          .body(
              Map.of(
                  "success", false, "message", "메트릭 리셋 중 서버 오류가 발생했습니다.", "error", e.getMessage()));
    }
  }
  ...
  public ResponseEntity<?> getMonitoringSummary() {
    try {
      ...
      return ResponseEntity.ok(
          Map.of(
              ...
              "summary",
                  Map.of(
                      "systemStatus",
                      metrics.isSystemHealthy() ? "정상" : "점검 중",
                      "healthStatus",
                      health.getStatus().getCode(),
                      "totalApiCalls",
      ...
      return ResponseEntity.internalServerError()
          .body(Map.of("success", false, "message", "모니터링 요약 조회 중 서버 오류가 발생했습니다."));
    }
  }
    ...
    try {
      ...
      return ResponseEntity.ok(
          Map.of(
              ...
              LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME),
              "status",
              metrics.isSystemHealthy() ? "UP" : "WARNING",
              "monitoringEnabled",
              metrics.isMonitoringEnabled()));
