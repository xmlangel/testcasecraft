...
public class AdminMailSettingsController {
  ...
  @Operation(summary = "메일 설정 조회", description = "현재 시스템의 메일 설정을 조회합니다. 관리자만 접근 가능합니다.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  @Operation(summary = "메일 설정 저장", description = "시스템의 메일 설정을 저장합니다. Gmail만 지원하며, 앱 비밀번호가 필요합니다.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  public ResponseEntity<?> saveMailSettings(
      @Parameter(description = "메일 설정 정보", required = true) @Valid @RequestBody
    ...
    try {
      ...
      MailSettingsDto savedSettings =
          mailSettingsService.saveSettings(mailSettingsDto, principal.getName());

      return ResponseEntity.ok(
          Map.of("message", "메일 설정이 성공적으로 저장되었습니다.", "settings", savedSettings));
      ...
      return ResponseEntity.internalServerError().body(Map.of("message", "메일 설정 저장 중 오류가 발생했습니다."));
    }
  }

  @Operation(summary = "메일 설정 업데이트", description = "기존 메일 설정을 업데이트합니다.")
  @ApiResponses(
      value = {
        @ApiResponse(responseCode = "200", description = "메일 설정 업데이트 성공"),
        @ApiResponse(responseCode = "400", description = "잘못된 요청 데이터")
      })
  @PutMapping
  public ResponseEntity<?> updateMailSettings(
      @Valid @RequestBody MailSettingsDto mailSettingsDto, Principal principal) {

    try {
      log.info("메일 설정 업데이트 요청 - 사용자: {}", principal.getName());

      MailSettingsDto updatedSettings =
          mailSettingsService.updateSettings(mailSettingsDto, principal.getName());

      return ResponseEntity.ok(
          Map.of("message", "메일 설정이 성공적으로 업데이트되었습니다.", "settings", updatedSettings));
      ...
      return ResponseEntity.internalServerError()
          .body(Map.of("message", "메일 설정 업데이트 중 오류가 발생했습니다."));
    }
  }

  @Operation(summary = "메일 발송 테스트", description = "현재 설정으로 테스트 메일을 발송합니다.")
  @ApiResponses(
      value = {
        @ApiResponse(responseCode = "200", description = "테스트 메일 발송 성공"),
        @ApiResponse(responseCode = "400", description = "메일 설정이 없거나 비활성화됨"),
        @ApiResponse(responseCode = "500", description = "메일 발송 실패")
      })
  @PostMapping("/test")
  public ResponseEntity<?> testMailSettings(
      @Parameter(description = "테스트 메일 수신자", required = true) @RequestParam String testRecipient,
      Principal principal) {
    ...
    try {
      ...
      return ResponseEntity.ok(
          Map.of("message", "테스트 메일이 성공적으로 발송되었습니다.", "recipient", testRecipient));
      ...
      return ResponseEntity.badRequest()
          .body(
              Map.of(
                  "message",
                  "메일 서버 인증에 실패했습니다. (Authentication failed)\n\n"
      ...
      return ResponseEntity.internalServerError()
          .body(Map.of("message", "테스트 메일 발송 중 오류가 발생했습니다: " + e.getMessage()));
    }
  }

  @Operation(summary = "메일 기능 비활성화", description = "메일 기능을 비활성화합니다.")
  @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "메일 기능 비활성화 성공")})
  @PostMapping("/disable")
  public ResponseEntity<?> disableMailSettings(Principal principal) {
    try {
      ...
      if (success) {
        return ResponseEntity.ok(Map.of("message", "메일 기능이 성공적으로 비활성화되었습니다."));
      } else {
        return ResponseEntity.badRequest().body(Map.of("message", "비활성화할 메일 설정이 없습니다."));
      }

    } catch (Exception e) {
      log.error("메일 설정 비활성화 실패", e);
      return ResponseEntity.internalServerError()
          .body(Map.of("message", "메일 설정 비활성화 중 오류가 발생했습니다."));
    }
  }

  @Operation(summary = "Gmail 설정 가이드 조회", description = "Gmail 앱 비밀번호 설정 방법에 대한 가이드를 제공합니다.")
  @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "설정 가이드 조회 성공")})
  @GetMapping("/guide")
