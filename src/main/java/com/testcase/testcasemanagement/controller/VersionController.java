...
/**
 * 애플리케이션 버전 정보 API 컨트롤러 Backend, Frontend, RAG Service의 버전 정보를 제공
 *
 * <p>버전 정보 소스: - Backend: build.gradle의 version (BuildProperties를 통해 자동 주입) - Frontend:
 * package.json의 version (빌드 시점에 복사된 파일에서 읽음) - RAG Service: /health 엔드포인트에서 실시간 조회
 */
@Slf4j
@RestController
...
public class VersionController {
  ...
  /**
   * 전체 애플리케이션 버전 정보 조회 GET /api/version
   *
   * @return 버전 정보 DTO (Backend, Frontend, RAG Service)
   */
  @Operation(
      summary = "전체 애플리케이션 버전 정보 조회",
      description = "Backend, Frontend, RAG Service의 버전 정보를 조회합니다.")
  @GetMapping
  ...
  /**
   * Frontend 버전 정보 읽기 package.json 파일에서 version 필드를 추출
   *
  ...
  /**
   * RAG Service 버전 정보 조회 RAG Service의 /health 엔드포인트에서 버전 정보를 가져옴
   *
  ...
  /** RAG Service Health Response DTO FastAPI의 /health 엔드포인트 응답 매핑 */
  @lombok.Data
