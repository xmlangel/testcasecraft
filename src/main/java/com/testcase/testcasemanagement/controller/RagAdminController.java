 ...
 * RAG 시스템 관리용 컨트롤러
 *
 * <p>목적: 삭제되지 않은 고아(orphan) RAG 문서 정리
 */
@Slf4j
@RestController
...
public class RagAdminController {
   ...
   * 고아 RAG 문서 찾기 및 삭제
   *
   * <p>DB에는 존재하지 않지만 RAG에는 존재하는 문서들을 찾아서 삭제
   */
  ...
  public ResponseEntity<?> cleanupOrphanedDocuments(
    ...
    try {
      ...
      if (ragDocuments == null || ragDocuments.getDocuments() == null) {
        return ResponseEntity.ok(Map.of("message", "RAG 문서를 찾을 수 없습니다.", "deletedCount", 0));
      }
      ...
      for (var doc : ragDocuments.getDocuments()) {
        if (doc.getFileName() != null && doc.getFileName().startsWith("testcase_")) {
          ...
          if (!exists) {
            log.info(
                "고아 RAG 문서 발견: documentId={}, fileName={}, testCaseId={}",
                doc.getId(),
                doc.getFileName(),
                testCaseId);
      ...
      return ResponseEntity.internalServerError()
          .body(Map.of("error", "정리 작업 실패: " + e.getMessage()));
    }
  }
}
