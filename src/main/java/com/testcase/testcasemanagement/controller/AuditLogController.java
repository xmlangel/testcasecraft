...
/** 감사 로그 조회 컨트롤러 Spring Security @PreAuthorize를 통한 메서드 레벨 보안 적용 */
...
public class AuditLogController {
  ...
  /** 최근 감사 로그 조회 (전체 - 시스템 관리자만) 권한: 시스템 관리자만 가능 */
  ...
  /** 특정 엔티티의 감사 로그 조회 권한: 해당 엔티티에 접근 권한이 있는 사용자 */
  ...
  public ResponseEntity<List<AuditLog>> getEntityLogs(
    ...
    Pageable pageable = PageRequest.of(0, Math.min(limit, 1000));
    List<AuditLog> logs = auditLogRepository.findRecentLogsByEntity(entityType, entityId, pageable);
    return ResponseEntity.ok(logs);
  }

  /** 조직 관련 감사 로그 조회 권한: 조직 멤버 또는 시스템 관리자 */
  ...
  /** 프로젝트 관련 감사 로그 조회 권한: 프로젝트 멤버 또는 시스템 관리자 */
  ...
  public ResponseEntity<List<AuditLog>> getProjectLogs(
      @PathVariable String projectId, @RequestParam(defaultValue = "50") int limit) {
  ...
  /** 그룹 관련 감사 로그 조회 권한: 그룹 멤버 또는 상위 조직/프로젝트 멤버 또는 시스템 관리자 */
  ...
  public ResponseEntity<List<AuditLog>> getGroupLogs(
      @PathVariable String groupId, @RequestParam(defaultValue = "50") int limit) {
  ...
  /** 현재 사용자의 감사 로그 조회 권한: 인증된 사용자 (자기 자신의 로그만) */
  ...
  /** 특정 사용자의 감사 로그 조회 권한: 시스템 관리자만 가능 */
  ...
  public ResponseEntity<List<AuditLog>> getUserLogs(
      @PathVariable String userId, @RequestParam(defaultValue = "50") int limit) {
  ...
  /** 액션별 감사 로그 통계 조회 권한: 시스템 관리자만 가능 */
  ...
  /** 엔티티 타입별 감사 로그 통계 조회 권한: 시스템 관리자만 가능 */
  ...
  /** 특정 기간의 감사 로그 조회 권한: 시스템 관리자만 가능 */
