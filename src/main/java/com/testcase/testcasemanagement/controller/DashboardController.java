...
public class DashboardController {
  ...
  /** ICT-135: 대시보드 차트 데이터 조회 API 추가 프론트엔드에서 fake 데이터 대신 실제 백엔드 데이터를 사용하도록 지원 */

  /** 프로젝트별 테스트케이스 결과 요약 조회 (파이차트용) 최근 실행 결과를 기반으로 PASS, FAIL, BLOCKED, SKIPPED, NOTRUN 통계 제공 */
  ...
  public ResponseEntity<Map<String, Object>> getProjectTestResultsSummary(
      @PathVariable String projectId, Authentication authentication, HttpServletRequest request) {

    logger.info("대시보드 테스트 결과 요약 조회 - 프로젝트: {}, 사용자: {}", projectId, authentication.getName());
        ...
        () -> {
          try {
            ...
            int completedCases =
                statistics.getPASS()
                    + statistics.getFAIL()
                    + statistics.getBLOCKED()
                    + statistics.getSKIPPED();
            int completeRate =
                statistics.getTotalCases() > 0
                    ? Math.round((float) completedCases / statistics.getTotalCases() * 100)
                    : 0;

            response.put("completeRate", completeRate);
            response.put(
                "lastUpdated", LocalDateTime.now().format(DateTimeFormatter.ofPattern("M/d/yyyy")));

            logger.debug(
                "테스트 결과 요약 조회 성공 - 총 케이스: {}, 완료율: {}%", statistics.getTotalCases(), completeRate);
  ...
  /** 오픈 테스트런 결과 조회 (바차트용) 현재 진행 중인 테스트 실행의 담당자별 결과 통계 제공 */
  ...
  public ResponseEntity<Map<String, Object>> getOpenTestRunResults(
    ...
    logger.info(
        "대시보드 오픈 테스트런 결과 조회 - 프로젝트: {}, 제한: {}, 사용자: {}",
        projectId,
        limit,
    ...
    return monitoringService.measureDashboardApiTime(
        ...
        () -> {
          try {
            List<OpenTestRunAssigneeResultDto> assigneeResults =
                dashboardService.getOpenTestRunAssigneeResultsByProject(projectId, limit);

            // 바차트에 맞는 형태로 데이터 변환
            List<Map<String, Object>> chartData =
                assigneeResults.stream()
                    .map(
                        result -> {
  ...
  /** 프로젝트 대시보드 종합 정보 조회 한 번의 호출로 대시보드에 필요한 모든 기본 정보 제공 */
  ...
  public ResponseEntity<Map<String, Object>> getProjectDashboardOverview(
    ...
    logger.info("대시보드 종합 정보 조회 - 프로젝트: {}, 사용자: {}", projectId, authentication.getName());
  ...
  /**
   * 테스트 플랜 목록 조회 ICT-202: Hibernate 프록시 직렬화 문제 해결을 위해 DTO 사용 ICT-332: 시스템 관리자만 접근 가능하도록 권한 제한
   *
   * @return 테스트 플랜 목록
   */
  ...
  /** TestPlan을 안전하게 DTO로 변환하는 헬퍼 메소드 Hibernate 프록시 문제를 방지하기 위해 안전한 방식으로 변환 */
  ...
  public ResponseEntity<List<RecentTestResultDto>> getRecentTestResultsByProject(
    ...
    List<RecentTestResultDto> results =
        dashboardService.getRecentTestResultsByProject(projectId, limit);
    return ResponseEntity.ok(results);
  }
  ...
  public ResponseEntity<List<RecentTestResultDto>> getRecentTestResultsByTestPlan(
    ...
    List<RecentTestResultDto> results =
        dashboardService.getRecentTestResultsByTestPlan(testPlanId, limit);
    return ResponseEntity.ok(results);
  }
  ...
  public ResponseEntity<List<OpenTestRunAssigneeResultDto>> getOpenTestRunAssigneeResults(
    ...
    List<OpenTestRunAssigneeResultDto> results =
        dashboardService.getOpenTestRunAssigneeResults(limit);
    return ResponseEntity.ok(results);
  }
  ...
  /**
   * ICT-135: 프로젝트별 테스트 결과 추이 조회 (라인차트용) 대시보드 차트에서 사용할 수 있도록 응답 형식을 Map으로 변경
   *
   * @param projectId 프로젝트 ID
  ...
  public ResponseEntity<Map<String, Object>> getTestResultsTrend(
    ...
    logger.info(
        "대시보드 테스트 결과 추이 조회 - 프로젝트: {}, 기간: {}일, 사용자: {}",
        projectId,
        days,
        authentication.getName());
        ...
        () -> {
          try {
            ...
            LocalDateTime startDate =
                endDate.minusDays(days).withHour(0).withMinute(0).withSecond(0);

            // 기존 서비스 메소드를 사용하여 추이 데이터 조회
            List<TestResultsTrendDto> trendResults =
                dashboardService.getTestResultsTrend(projectId, startDate, endDate);
  ...
  @PreAuthorize("@projectSecurityService.canAccessProject(#projectId, authentication.name)")
  public ResponseEntity<TestCaseStatisticsDto> getTestCaseStatistics(
      @PathVariable String projectId) {
    TestCaseStatisticsDto statistics = dashboardService.getTestCaseStatistics(projectId);
  ...
  public ResponseEntity<List<TestExecutionProgressDto>> getInProgressTestExecutionsByProject(
    ...
    List<TestExecutionProgressDto> results =
        dashboardService.getInProgressTestExecutionsByProject(projectId, limit);
    return ResponseEntity.ok(results);
  }

  /**
   * ICT-129: 프로젝트 전체 통계 조회 대시보드의 전체 통계 섹션을 위한 종합 통계 정보를 제공합니다.
   *
   * @param projectId 프로젝트 ID
