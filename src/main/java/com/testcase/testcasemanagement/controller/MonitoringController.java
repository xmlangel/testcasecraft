...
/** ICT-134: 대시보드 모니터링 및 헬스체크 컨트롤러 시스템 상태, 성능 메트릭, 헬스체크 엔드포인트를 제공합니다. */
@RestController
...
public class MonitoringController implements HealthIndicator {
  ...
  @Autowired private PageVisitMetricsService pageVisitMetricsService;

  /** 대시보드 헬스체크 엔드포인트 대시보드 관련 시스템 상태를 종합적으로 확인합니다. */
  ...
  public ResponseEntity<Map<String, Object>> dashboardHealthCheck() {
    ...
    try {
      ...
      memoryInfo.put(
          "memoryUsagePercentage", String.format("%.1f%%", (double) usedMemory / maxMemory * 100));
      ...
      if (cacheHitRate < 0.7) {
        warnings.put(
            "cacheHitRate", "Low cache hit rate: " + String.format("%.1f%%", cacheHitRate * 100));
      }
  ...
  public ResponseEntity<Map<String, Object>> getDashboardMetrics() {
    ...
    try {
      ...
      try {
        double projectHits =
            meterRegistry.counter("cache.gets", "cache", "project", "result", "hit").count();
        double projectMisses =
            meterRegistry.counter("cache.gets", "cache", "project", "result", "miss").count();
        double projectHitRate =
            (projectHits + projectMisses) > 0
      ...
      try {
        double testCaseHits =
            meterRegistry.counter("cache.gets", "cache", "testCase", "result", "hit").count();
        double testCaseMisses =
            meterRegistry.counter("cache.gets", "cache", "testCase", "result", "miss").count();
        double testCaseHitRate =
            (testCaseHits + testCaseMisses) > 0
  ...
  public ResponseEntity<Void> recordPageVisit(
      @Valid @RequestBody PageVisitRecordRequest request, HttpServletRequest httpServletRequest) {
    String clientIp = extractClientIp(httpServletRequest);
    boolean recorded =
        pageVisitMetricsService.recordVisit(
            request.getPagePath(), request.getVisitorId(), clientIp);
