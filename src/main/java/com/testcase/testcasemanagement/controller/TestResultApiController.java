...
/** ICT-208: 테스트 결과 조회 및 통계 API 통합 컨트롤러 기존 분산된 API들을 통합하고 표준화된 형식으로 제공합니다. */
@Slf4j
@RestController
...
public class TestResultApiController {
  ...
  public ResponseEntity<ApiResponse<List<TestResultReportDto>>> searchTestResults(
    ...
    try {
      ...
      // 페이징 처리된 결과 조회
      Page<TestResultReportDto> resultPage =
  ...
  public ResponseEntity<ApiResponse<Object>> getTestResultDetail(
      ...
      @Parameter(description = "결과 타입 (testResult|junitResult)")
          @RequestParam(defaultValue = "testResult")
          String resultType) {
    ...
    try {
      ...
      Map<String, Object> metadata =
          Map.of("resultType", resultType, "retrievedAt", LocalDateTime.now());
      ...
      return ResponseEntity.status(500)
          .body(
              ApiResponse.error("DETAIL_FETCH_FAILED", "테스트 결과 상세 조회에 실패했습니다: " + e.getMessage()));
    }
  }
  ...
  public ResponseEntity<ApiResponse<TestResultStatisticsDto>> getBasicStatistics(
      ...
      @Parameter(description = "테스트 실행 ID") @RequestParam(required = false)
    ...
    try {
      List<String> testPlanIds =
          testPlanId != null && !testPlanId.isEmpty() ? List.of(testPlanId) : null;
      TestResultStatisticsDto statistics =
          testResultReportService.getTestResultStatistics(projectId, testPlanIds, testExecutionId);

      Map<String, Object> metadata =
          Map.of("statisticsType", "basic", "generatedAt", LocalDateTime.now());
      ...
      return ResponseEntity.status(500)
          .body(
              ApiResponse.error("BASIC_STATISTICS_FAILED", "기본 통계 조회에 실패했습니다: " + e.getMessage()));
    }
  }
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> getDashboardChartData(
    ...
    try {
      ...
      Map<String, Object> metadata =
          Map.of(
              "chartTypes",
              chartTypes != null ? chartTypes : List.of("pieChart", "trendChart", "barChart"),
              "periodDays",
              days,
              "generatedAt",
              LocalDateTime.now());
  ...
  public ResponseEntity<byte[]> exportTestResults(@RequestBody TestResultQueryDto query) {
    ...
    try {
      ...
      String filename =
          "test_results_"
              ...
              + "."
              + (filter.getExportFormat() != null
                  ? filter.getExportFormat().toLowerCase()
                  : "xlsx");
    ...
    try {
      Map<String, Object> healthInfo =
          Map.of(
              ...
              "supportedFeatures",
                  List.of(
                      "테스트 결과 검색 및 필터링", "종합 통계 및 분석", "대시보드 차트 데이터", "다양한 형식 내보내기", "실시간 통계 업데이트"),
              "endpoints",
                  Map.of(
  ...
  private Object getTrendChartData(String projectId, int days) {
    ...
    return Map.of("description", "최근 " + days + "일 테스트 결과 추이", "period", days, "data", "트렌드 데이터");
  }
  ...
  public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getTestPlansForFilter(
    ...
    try {
      ...
      List<Map<String, Object>> planOptions =
          testPlanEntities.stream()
              .map(
                  plan ->
      ...
      return ResponseEntity.status(500)
          .body(
              ApiResponse.error(
                  "TEST_PLANS_FETCH_FAILED", "테스트 플랜 목록 조회에 실패했습니다: " + e.getMessage()));
    }
  }
  ...
  public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getTestExecutionsForFilter(
    ...
    try {
      ...
      List<Map<String, Object>> executionOptions =
              ...
              .map(
                  execution ->
                      Map.<String, Object>of(
                          ...
                          execution.getName(),
                          "testPlanId",
                          execution.getTestPlanId() != null ? execution.getTestPlanId() : "",
                          "status",
                          execution.getStatus(),
                          "startDate",
                          execution.getStartDate() != null
                              ...
                              : "",
                          "endDate",
                          execution.getEndDate() != null ? execution.getEndDate().toString() : ""))
              .collect(Collectors.toList());
      ...
      Map<String, Object> metadata =
          Map.of(
              ...
              projectId,
              "testPlanId",
              testPlanId != null ? testPlanId : "all",
              "executionCount",
              executionOptions.size(),
              "retrievedAt",
              LocalDateTime.now());
      ...
      return ResponseEntity.status(500)
          .body(
              ApiResponse.error(
                  "TEST_EXECUTIONS_FETCH_FAILED", "테스트 실행 목록 조회에 실패했습니다: " + e.getMessage()));
    }
  }
  ...
  public ResponseEntity<ApiResponse<List<TestResultReportDto>>> getFilteredTestResults(
    ...
    try {
      // 필터 생성
      TestResultFilterDto filter =
          TestResultFilterDto.builder().projectId(projectId).page(page).size(size).build();
      ...
      Map<String, Object> metadata =
          Map.of(
              ...
              testPlanId != null ? testPlanId : "all",
              "testExecutionId",
              testExecutionId != null ? testExecutionId : "all",
              "totalResults",
      ...
      return ResponseEntity.status(500)
          .body(
              ApiResponse.error(
                  "FILTERED_RESULTS_FETCH_FAILED", "필터링된 테스트 결과 조회에 실패했습니다: " + e.getMessage()));
    }
  }
  ...
  public ResponseEntity<List<Map<String, Object>>> getComparisonStatisticsByPlan(
    ...
    try {
      List<Map<String, Object>> statistics =
          statisticsService.getComparisonStatisticsByPlan(projectId);
  ...
  public ResponseEntity<List<Map<String, Object>>> getComparisonStatisticsByExecutor(
    ...
    try {
      List<Map<String, Object>> statistics =
          statisticsService.getComparisonStatisticsByExecutor(projectId);
      return ResponseEntity.ok(statistics);
    } catch (Exception e) {
