 ...
 * RAG API 클라이언트 컨트롤러
 *
 * <p>Spring Boot Backend에서 RAG FastAPI 서비스를 호출하기 위한 REST 엔드포인트를 제공합니다.
 */
...
public class RagController {
   ...
   * 문서 업로드 엔드포인트
   *
   * <p>POST /api/rag/documents/upload
   */
  ...
  public ResponseEntity<RagDocumentResponse> uploadDocument(
    ...
    log.info(
        "REST API: Upload document request - file={}, projectId={}",
   ...
   * 문서 분석 엔드포인트
   *
   * <p>POST /api/rag/documents/{documentId}/analyze
   */
   ...
   * 임베딩 생성 엔드포인트
   *
   * <p>POST /api/rag/embeddings/generate
   */
   ...
   *
   * <p>POST /api/rag/search/advanced
   */
  ...
  public ResponseEntity<RagSearchResponse> searchAdvanced(
    ...
    log.info(
        "REST API: Advanced search request - query={}, method={}",
   ...
   * 문서 조회 엔드포인트
   *
   * <p>GET /api/rag/documents/{documentId}
   */
  ...
  public ResponseEntity<RagDocumentListResponse> listDocuments(
    ...
    log.info(
        "REST API: List documents request - projectId={}, page={}, size={}", projectId, page, size);
   ...
   * 문서 삭제 엔드포인트
   *
   * <p>DELETE /api/rag/documents/{documentId}
   */
   ...
   * 문서 다운로드 엔드포인트
   *
   * <p>GET /api/rag/documents/{documentId}/download
   */
   ...
   *
   * <p>GET /api/rag/documents/{documentId}/chunks?skip=0&limit=50
   */
  ...
  public ResponseEntity<RagChunkListResponse> getDocumentChunks(
    ...
    log.info(
        "REST API: Get document chunks request - documentId={}, skip={}, limit={}",
        documentId,
        skip,
        limit);
   ...
   *
   * <p>POST /api/rag/testcases/vectorize-all?projectId={projectId}
   *
   * <p>전체 또는 특정 프로젝트의 모든 TestCase를 RAG 시스템에 벡터화하여 등록합니다. 관리자만 접근 가능합니다.
   */
  @Operation(
  ...
  public ResponseEntity<Map<String, Object>> vectorizeAllTestCases(
    ...
    try {
      ...
      log.info(
          "ICT-388 REST API: Vectorize all testcases completed - totalCount={}, successCount={},"
              + " failureCount={}",
   ...
   * 공통 문서 목록 조회 엔드포인트
   *
   * <p>GET /api/rag/global-documents
   */
  ...
  /**
   * 공통 문서 삭제 엔드포인트 관리자만 삭제 가능
   *
   * <p>DELETE /api/rag/global-documents/{documentId}
   */
  ...
  public ResponseEntity<RagGlobalDocumentRequestDto> requestGlobalDocument(
    ...
    log.info(
        "REST API: Create global document request - documentId={}, requestedBy={}",
        documentId,
        authentication.getName());
    try {
      RagGlobalDocumentRequestDto response =
   ...
   * LLM 분석 비용 추정
   *
   * <p>POST /api/rag/documents/{documentId}/estimate-cost
   */
  ...
  public ResponseEntity<RagCostEstimateResponse> estimateAnalysisCost(
    ...
    log.info(
        "REST API: Estimate LLM analysis cost - documentId={}, llmProvider={}, llmModel={}",
        documentId,
   ...
   * 문서 LLM 순차 분석 시작
   *
   * <p>POST /api/rag/documents/{documentId}/analyze-with-llm
   */
  ...
  public ResponseEntity<RagLlmAnalysisResponse> analyzeDocumentWithLlm(
    ...
    log.info(
        "REST API: Start LLM analysis - documentId={}, llmProvider={}, llmModel={}",
        documentId,
        request.getLlmProvider(),
        request.getLlmModel());
   ...
   * LLM 분석 진행 상황 조회
   *
   * <p>GET /api/rag/documents/{documentId}/llm-analysis-status
   */
  @Operation(summary = "LLM 분석 상태 조회", description = "진행 중인 LLM 분석 작업의 상태를 조회합니다.")
  @GetMapping("/documents/{documentId}/llm-analysis-status")
  @PreAuthorize(
      "@projectSecurityService.canAccessDocumentProject(#documentId, authentication.name)")
  public ResponseEntity<RagLlmAnalysisStatusResponse> getLlmAnalysisStatus(
      @PathVariable UUID documentId) {
   ...
   * LLM 분석 결과 조회
   *
   * <p>GET /api/rag/documents/{documentId}/llm-analysis-results
   */
  ...
  public ResponseEntity<RagLlmAnalysisResultsResponse> getLlmAnalysisResults(
    ...
    log.info(
        "REST API: Get LLM analysis results - documentId={}, skip={}, limit={}",
        documentId,
        skip,
        limit);
   ...
   * LLM 분석 일시정지
   *
   * <p>POST /api/rag/documents/{documentId}/pause-analysis
   */
   ...
   * LLM 분석 재개
   *
   * <p>POST /api/rag/documents/{documentId}/resume-analysis
   */
   ...
   * LLM 분석 취소
   *
   * <p>POST /api/rag/documents/{documentId}/cancel-analysis
   */
   ...
   * LLM 분석 작업 목록 조회
   *
   * <p>GET /api/rag/llm-analysis/jobs?projectId=...&status=...&page=1&size=20
   */
  ...
  public ResponseEntity<RagLlmAnalysisJobListResponse> listLlmAnalysisJobs(
    ...
    log.info(
        "REST API: List LLM analysis jobs - projectId={}, status={}, page={}, size={}",
        projectId,
        status,
        page,
        size);
   ...
   * 분석 요약 생성
   *
   * <p>POST /api/rag/analysis-summaries
   */
  ...
  public ResponseEntity<java.util.List<RagAnalysisSummaryResponse>> listAnalysisSummaries(
    ...
    log.info(
        "REST API: List analysis summaries - documentId={}, userId={}, isPublic={}, skip={},"
            + " limit={}",
        documentId,
        userId,
        isPublic,
   ...
   * 분석 요약 상세 조회
   *
   * <p>GET /api/rag/analysis-summaries/{summaryId}
   */
