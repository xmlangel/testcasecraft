...
public class TestPlanController {
  ...
  @PostMapping
  public ResponseEntity<TestPlanDto> createTestPlan(@RequestBody @Valid TestPlanDto dto) {
    TestPlan createdPlan = testPlanService.createTestPlan(dto);

    return ResponseEntity.status(HttpStatus.CREATED).body(testPlanMapper.toDto(createdPlan));
  }

  @Operation(summary = "테스트 계획 수정", description = "기존 테스트 계획을 수정합니다.")
  @PutMapping("/{id}")
  public ResponseEntity<TestPlanDto> updateTestPlan(
      @PathVariable String id, @RequestBody @Valid TestPlanDto dto) {
  ...
  public ResponseEntity<List<TestPlanDto>> getTestPlansByProject(@PathVariable String projectId) {
    List<TestPlan> plans = testPlanService.getTestPlansByProject(projectId);
    List<TestPlanDto> dtos = plans.stream().map(testPlanMapper::toDto).collect(Collectors.toList());
