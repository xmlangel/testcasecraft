...
@Tag(
    ...
    **🛡️ 관리자 전용 사용자 관리 API**
    ...
    시스템 관리자가 사용자 계정을 종합적으로 관리할 수 있는 API 모음입니다.
    ...
    **⚠️ 공통 에러 응답:**
    ...
    • **404 Not Found**: 요청한 사용자 존재하지 않음
    • **400 Bad Request**: 잘못된 요청 데이터
    • **500 Internal Server Error**: 서버 내부 오류
    
    **🚀 성능 특성:**
    • 평균 응답 시간: 100-300ms
    • 동시 처리: 최대 100 req/sec
    • 캐시 활용: 통계 및 목록 조회 최적화
    
    **📱 사용 방법:**
    1. JWT 토큰 발급 (/api/auth/login)
    2. Authorization 헤더에 "Bearer {token}" 설정
    3. ADMIN 권한 확인 후 API 호출
    """
)
@RestController
...
public class UserManagementController {
    ...
    public ResponseEntity<UserDto.Response> activateUser(
            @Parameter(description = "활성화할 사용자 ID", required = true, example = "user-123")
            @PathVariable String userId,
            Authentication authentication) {
    ...
    public ResponseEntity<UserDto.Response> deactivateUser(
            @Parameter(description = "비활성화할 사용자 ID", required = true, example = "user-123")
            @PathVariable String userId,
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
