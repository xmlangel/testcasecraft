...
@Tag(
    ...
    **🚀 테스트 프로젝트 종합 관리 API**
    ...
    테스트 케이스 관리 시스템의 핵심 기능인 프로젝트 생성, 관리, 협업을 위한 API 모음입니다.
    
    **🔐 공통 보안 요구사항:**
    • **JWT 인증**: 모든 API 호출 시 Bearer Token 필수
    • **역할 기반 권한**: 각 API별 최소 권한 원칙 적용
    • **프로젝트 멤버십**: 프로젝트별 접근 권한 확인
    ...
    **🏢 조직 연동:**
    • 조직 소속 프로젝트와 독립 프로젝트 구분 관리
    • 조직 권한 상속 및 프로젝트 고유 권한 병행
    • 프로젝트 조직 간 이전 기능
    
    **⚠️ 공통 에러 응답:**
    • **401 Unauthorized**: JWT 토큰 없음 또는 만료
    • **403 Forbidden**: 프로젝트/조직 접근 권한 없음
    • **404 Not Found**: 요청한 프로젝트 존재하지 않음
    • **400 Bad Request**: 잘못된 요청 데이터
    • **500 Internal Server Error**: 서버 내부 오류
    
    **🚀 성능 특성:**
    • 평균 응답 시간: 50-200ms
    • 동시 처리: 최대 200 req/sec
    • 대용량 프로젝트 지원: 10만+ 테스트 케이스
    
    **📱 사용 방법:**
    1. JWT 토큰 발급 (/api/auth/login)
    2. Authorization 헤더에 "Bearer {token}" 설정
    3. 프로젝트 권한 확인 후 API 호출
    """
)
@RestController
...
public class ProjectController {
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(
            ...
            content = @Content(
                mediaType = "application/json",
                examples = @ExampleObject(
                    value = "{\"error\": \"요청한 리소스를 찾을 수 없습니다\"}"
                )
            )
        ),
        @ApiResponse(responseCode = "401", description = "인증되지 않은 사용자"),
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(
            ...
            content = @Content(
                mediaType = "application/json",
                examples = @ExampleObject(
                    value = "{\"error\": \"이름은 100자 이내로 입력해주세요\"}"
                )
            )
        ),
    ...
    public ResponseEntity<ProjectUser> inviteProjectMember(
            ...
            @Parameter(
                description = "초대할 사용자의 사용자명",
                required = true,
                example = "developer1"
            )
            @RequestParam String username,
            @Parameter(
                description = "부여할 프로젝트 내 역할",
                required = true,
                example = "DEVELOPER"
            )
