...
/** ICT-209: 테스트 결과 편집 컨트롤러 테스트 결과 편집 기능을 위한 REST API 엔드포인트 */
...
public class TestResultEditController {
  ...
  public ResponseEntity<Page<TestResultEditDto>> getEdits(
    ...
    log.debug(
        "Getting edits with filters - projectId: {}, executionId: {}, page: {}",
        projectId,
        testExecutionId,
        page);

    try {
      TestResultEditDto.EditFilterDto filter =
          TestResultEditDto.EditFilterDto.builder()
              .projectId(projectId)
  ...
  public ResponseEntity<List<TestResultEditDto>> getEditHistory(@PathVariable String testResultId) {
  ...
  public ResponseEntity<TestResultEditDto> getActiveEdit(@PathVariable String testResultId) {
