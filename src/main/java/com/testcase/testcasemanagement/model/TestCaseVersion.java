 ...
 *
 * <p>테스트케이스의 모든 버전을 저장하고 관리하는 엔티티입니다. 테스트케이스가 수정될 때마다 자동으로 새로운 버전이 생성됩니다.
 */
@Getter
@Setter
@NoArgsConstructor
...
public class TestCaseVersion {
  ...
  /** 현재 활성 버전 여부 (테스트 실행 시 사용되는 버전) 각 테스트케이스마다 하나의 버전만 true */
  ...
  /** 테스트 스텝 (JSON 형태로 저장) TestStep 리스트를 JSON으로 직렬화하여 저장 */
