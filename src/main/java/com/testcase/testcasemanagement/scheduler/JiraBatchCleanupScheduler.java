...
/** JIRA 배치 처리 정리 스케줄러 ICT-165: 배치 처리 통계 및 리소스 정리 */
@Component
...
public class JiraBatchCleanupScheduler {
  ...
  /** 오래된 배치 작업 통계 정리 스케줄러 매일 새벽 3시에 실행 */
    ...
    try {
      ...
      if (!stats.isEmpty()) {
        ...
        long completedOperations =
            stats.values().stream().mapToLong(stat -> stat.isCompleted() ? 1 : 0).sum();

        long runningOperations = totalOperations - completedOperations;

        if (runningOperations > 10) {
          log.warn("⚠️  많은 수의 배치 작업이 실행 중입니다: 실행중={}, 총작업={}", runningOperations, totalOperations);
        }
  ...
  /** 메모리 사용량 기반 긴급 정리 매 10분마다 실행하여 메모리 상태 확인 */
  ...
  public void emergencyMemoryCleanup() {
    try {
      ...
      if (memoryUsage > 80) {
        ...
        log.info("배치 통계 정리 후 메모리 사용률: {:.2f}% → {:.2f}%", memoryUsage, newMemoryUsage);
      }
