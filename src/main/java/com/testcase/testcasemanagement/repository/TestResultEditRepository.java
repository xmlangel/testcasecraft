...
public interface TestResultEditRepository extends JpaRepository<TestResultEdit, String> {
  ...
  @Query(
      "SELECT e FROM TestResultEdit e WHERE e.originalTestResult.id = :testResultId ORDER BY"
          + " e.editVersion DESC, e.createdAt DESC")
  ...
  @Query(
      "SELECT e FROM TestResultEdit e "
          ...
          + "ORDER BY e.createdAt DESC")
  Page<TestResultEdit> findEditsByExecutionId(
      @Param("executionId") String executionId, Pageable pageable);

  /** 편집 상태별 편집본 조회 */
  @Query("SELECT e FROM TestResultEdit e WHERE e.editStatus = :status ORDER BY e.createdAt DESC")
  Page<TestResultEdit> findEditsByStatus(
      @Param("status") TestResultEdit.EditStatus status, Pageable pageable);

  /** 특정 기간 내 편집본 조회 */
  @Query(
      "SELECT e FROM TestResultEdit e WHERE e.createdAt BETWEEN :fromDate AND :toDate ORDER BY"
          + " e.createdAt DESC")
  Page<TestResultEdit> findEditsByDateRange(
  ...
  @Query(
      "SELECT COALESCE(MAX(e.editVersion), 0) + 1 FROM TestResultEdit e WHERE"
          + " e.originalTestResult.id = :testResultId")
  Integer getNextEditVersion(@Param("testResultId") String testResultId);

  /** 특정 테스트 결과의 기존 활성 편집본 비활성화 */
  @Modifying
  @Query(
      "UPDATE TestResultEdit e SET e.isActive = false WHERE e.originalTestResult.id = :testResultId"
          + " AND e.isActive = true")
  ...
  @Query(
      "SELECT e FROM TestResultEdit e "
          ...
          + "ORDER BY e.createdAt DESC")
  Page<TestResultEdit> findEditsByFilter(
  ...
  Page<TestResultEdit> findPendingEditsForApprover(
      @Param("approverId") String approverId, Pageable pageable);
  ...
  @Query(
      "SELECT e FROM TestResultEdit e "
          ...
          + "ORDER BY e.createdAt DESC")
  List<TestResultEdit> findRecentEdits(
