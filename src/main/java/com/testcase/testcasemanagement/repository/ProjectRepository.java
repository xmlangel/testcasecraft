...
public interface ProjectRepository extends JpaRepository<Project, String> {
  ...
  @Query(
      "SELECT p FROM Project p LEFT JOIN FETCH p.organization WHERE p.organization.id ="
          + " :organizationId")
  ...
  // 조직 내에서 프로젝트명으로 검색
  @Query(
      "SELECT p FROM Project p WHERE p.organization.id = :organizationId AND LOWER(p.name) LIKE"
          + " LOWER(CONCAT('%', :name, '%'))")
  List<Project> findByOrganizationIdAndNameContaining(
  ...
  Optional<Project> findByOrganizationIdAndCode(
      @Param("organizationId") String organizationId, @Param("code") String code);
  ...
  @Query(
      "SELECT DISTINCT p FROM Project p LEFT JOIN FETCH p.organization "
  ...
  @Query(
      "SELECT DISTINCT pu.project FROM ProjectUser pu WHERE pu.user.id = :userId AND"
          + " pu.roleInProject = :role")
  List<Project> findProjectsByUserIdAndRole(
  ...
  @Query(
      "SELECT DISTINCT pu.project FROM ProjectUser pu WHERE pu.user.id = :userId AND"
          + " pu.roleInProject IN ('PROJECT_MANAGER', 'LEAD_DEVELOPER')")
  ...
  // 조직 내 프로젝트 검색
  @Query(
      "SELECT p FROM Project p WHERE p.organization.id = :organizationId AND "
          + "(LOWER(p.name) LIKE LOWER(CONCAT('%', :keyword, '%')) OR "
          + "LOWER(p.description) LIKE LOWER(CONCAT('%', :keyword, '%')) OR "
          + "LOWER(p.code) LIKE LOWER(CONCAT('%', :keyword, '%')))")
  List<Project> searchByKeywordInOrganization(
  ...
  // 조직의 최근 프로젝트들 조회
  @Query(
      "SELECT p FROM Project p WHERE p.organization.id = :organizationId ORDER BY p.createdAt DESC")
  List<Project> findRecentProjectsByOrganization(
