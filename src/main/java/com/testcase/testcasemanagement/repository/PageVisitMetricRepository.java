...
public interface PageVisitMetricRepository extends JpaRepository<PageVisitMetric, String> {
  ...
  @Query(
      "SELECT pvm FROM PageVisitMetric pvm WHERE pvm.visitDate BETWEEN :startDate AND :endDate"
          + " ORDER BY pvm.visitDate DESC, pvm.dailyCount DESC")
  List<PageVisitMetric> findByVisitDateBetween(
  ...
  @Query(
      "SELECT pvm FROM PageVisitMetric pvm WHERE pvm.pagePath = :pagePath ORDER BY pvm.visitDate"
          + " DESC")
