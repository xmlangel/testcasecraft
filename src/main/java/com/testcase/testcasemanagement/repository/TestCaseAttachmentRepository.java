...
public interface TestCaseAttachmentRepository extends JpaRepository<TestCaseAttachment, String> {
  ...
  @Query(
      "SELECT a FROM TestCaseAttachment a WHERE a.testCase.id = :testCaseId AND a.status = 'ACTIVE'"
  ...
  /** 저장된 파일명으로 첨부파일 조회 */
  Optional<TestCaseAttachment> findByStoredFileNameAndStatus(
      String storedFileName, TestCaseAttachment.AttachmentStatus status);

  /** 테스트케이스별 첨부파일 개수 조회 */
  @Query(
      "SELECT COUNT(a) FROM TestCaseAttachment a WHERE a.testCase.id = :testCaseId AND a.status ="
          + " 'ACTIVE'")
  long countActiveByTestCaseId(@Param("testCaseId") String testCaseId);

  /** 사용자가 업로드한 첨부파일 목록 조회 */
  @Query(
      "SELECT a FROM TestCaseAttachment a WHERE a.uploadedBy.id = :userId AND a.status = 'ACTIVE'"
          + " ORDER BY a.createdAt DESC")
  List<TestCaseAttachment> findActiveByUploadedById(@Param("userId") String userId);

  /** 파일 크기가 큰 첨부파일 목록 조회 (정리용) */
  @Query(
      "SELECT a FROM TestCaseAttachment a WHERE a.fileSize > :minSize AND a.status = 'ACTIVE' ORDER"
          + " BY a.fileSize DESC")
  List<TestCaseAttachment> findLargeFiles(@Param("minSize") Long minSize);

  /** 특정 기간 이전의 첨부파일 목록 조회 (정리용) */
  @Query(
      "SELECT a FROM TestCaseAttachment a WHERE a.createdAt < :beforeDate AND a.status = 'ACTIVE'")
  ...
  @Query(
      "SELECT a FROM TestCaseAttachment a WHERE "
          ...
          + "AND a.status = 'ACTIVE' "
          + "ORDER BY a.createdAt ASC")
  List<TestCaseAttachment> findUnusedFilesBeforeDate(
