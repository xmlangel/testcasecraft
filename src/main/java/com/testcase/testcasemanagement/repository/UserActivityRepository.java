...
public interface UserActivityRepository extends JpaRepository<UserActivity, String> {
  ...
  // 사용자별 시간 범위 활동 조회
  List<UserActivity> findByUserIdAndTimestampBetween(
  ...
  @Query(
      "SELECT ua FROM UserActivity ua WHERE ua.user.id = :userId AND ua.riskScore >= :minRiskScore")
  List<UserActivity> findUserHighRiskActivities(
      @Param("userId") String userId, @Param("minRiskScore") Integer minRiskScore);

  // 특정 엔티티 관련 활동 조회
  List<UserActivity> findByTargetEntityTypeAndTargetEntityId(
      String targetEntityType, String targetEntityId);

  // 최근 로그인 조회
  @Query(
      "SELECT ua FROM UserActivity ua WHERE ua.user.id = :userId AND ua.activityType = 'LOGIN'"
          + " ORDER BY ua.timestamp DESC")
  List<UserActivity> findRecentLogins(@Param("userId") String userId, Pageable pageable);

  // 최근 로그아웃 조회
  @Query(
      "SELECT ua FROM UserActivity ua WHERE ua.user.id = :userId AND ua.activityType = 'LOGOUT'"
          + " ORDER BY ua.timestamp DESC")
  List<UserActivity> findRecentLogouts(@Param("userId") String userId, Pageable pageable);

  // 마지막 로그인 시간 조회
  @Query(
      "SELECT ua FROM UserActivity ua WHERE ua.user.id = :userId AND ua.activityType = 'LOGIN' AND"
          + " ua.isSuccessful = true ORDER BY ua.timestamp DESC")
  Optional<UserActivity> findLastSuccessfulLogin(@Param("userId") String userId);

  // 활동 통계 - 활동 타입별 카운트
  @Query(
      "SELECT ua.activityType, COUNT(ua) FROM UserActivity ua WHERE ua.user.id = :userId GROUP BY"
          + " ua.activityType")
  List<Object[]> getUserActivityTypeStatistics(@Param("userId") String userId);

  // 활동 통계 - 카테고리별 카운트
  @Query(
      "SELECT ua.activityCategory, COUNT(ua) FROM UserActivity ua WHERE ua.user.id = :userId GROUP"
          + " BY ua.activityCategory")
  List<Object[]> getUserActivityCategoryStatistics(@Param("userId") String userId);

  // 일별 활동 통계
  @Query(
      "SELECT DATE(ua.timestamp), COUNT(ua) FROM UserActivity ua "
  ...
  List<Object[]> getUserDailyActivityStatistics(
  ...
  // 시간대별 활동 통계
  @Query(
      "SELECT HOUR(ua.timestamp), COUNT(ua) FROM UserActivity ua "
          + "WHERE ua.user.id = :userId AND ua.timestamp BETWEEN :startDate AND :endDate "
          + "GROUP BY HOUR(ua.timestamp) ORDER BY HOUR(ua.timestamp)")
  List<Object[]> getUserHourlyActivityStatistics(
      @Param("userId") String userId,
      @Param("startDate") LocalDateTime startDate,
      @Param("endDate") LocalDateTime endDate);
  ...
  @Query(
      "SELECT COUNT(DISTINCT ua.sessionId) FROM UserActivity ua "
          + "WHERE ua.user.id = :userId AND ua.timestamp BETWEEN :startDate AND :endDate")
  long countDistinctSessionsByUserAndPeriod(
      @Param("userId") String userId,
      @Param("startDate") LocalDateTime startDate,
      @Param("endDate") LocalDateTime endDate);
  ...
  @Query(
      "SELECT DISTINCT ua.sessionId FROM UserActivity ua "
  ...
  // 전체 활동 검색
  @Query(
      "SELECT ua FROM UserActivity ua WHERE "
  ...
  // 특정 기간 사용자 활동 수 카운트
  @Query(
      "SELECT COUNT(ua) FROM UserActivity ua WHERE ua.user.id = :userId AND ua.timestamp BETWEEN"
