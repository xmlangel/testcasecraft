...
public interface ProjectUserRepository extends JpaRepository<ProjectUser, String> {
  ...
  // 사용자의 프로젝트별 역할 조회
  @Query(
      "SELECT pu.roleInProject FROM ProjectUser pu WHERE pu.project.id = :projectId AND pu.user.id"
          + " = :userId")
  ...
  // 역할별 멤버 수 조회
  @Query(
      "SELECT COUNT(pu) FROM ProjectUser pu WHERE pu.project.id = :projectId AND pu.roleInProject ="
          + " :role")
  long countByProjectIdAndRole(
      @Param("projectId") String projectId, @Param("role") ProjectRole role);

  // 프로젝트의 총 멤버 수 조회
  long countByProjectId(String projectId);

  @Query(
      "SELECT pu.project.id, COUNT(pu) FROM ProjectUser pu WHERE pu.project.id IN :projectIds GROUP"
          + " BY pu.project.id")
  ...
  // 최근 참여한 멤버들 조회
  @Query(
      "SELECT pu FROM ProjectUser pu WHERE pu.project.id = :projectId ORDER BY pu.createdAt DESC")
  List<ProjectUser> findRecentMembersByProjectId(
  ...
  @Query(
      "SELECT COUNT(pu) > 0 FROM ProjectUser pu WHERE pu.project.id = :projectId AND pu.user.id ="
          + " :userId AND pu.roleInProject IN ('PROJECT_MANAGER', 'LEAD_DEVELOPER')")
