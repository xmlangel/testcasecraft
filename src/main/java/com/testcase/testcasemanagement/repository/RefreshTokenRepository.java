...
public interface RefreshTokenRepository extends JpaRepository<RefreshToken, String> {
  ...
  @Query(
      "SELECT rt FROM RefreshToken rt WHERE rt.userId = :userId AND rt.isRevoked = false AND"
          + " rt.expiryDate > :now")
  ...
  @Transactional
  @Query(
      "UPDATE RefreshToken rt SET rt.isRevoked = true, rt.updatedAt = :now WHERE rt.userId ="
          + " :userId")
  ...
  @Transactional
  @Query(
      "UPDATE RefreshToken rt SET rt.isRevoked = true, rt.updatedAt = :now WHERE rt.token = :token")
  void revokeByToken(@Param("token") String token, @Param("now") LocalDateTime now);

  /** 사용자의 활성 토큰 개수 조회 */
  @Query(
      "SELECT COUNT(rt) FROM RefreshToken rt WHERE rt.userId = :userId AND rt.isRevoked = false AND"
          + " rt.expiryDate > :now")
