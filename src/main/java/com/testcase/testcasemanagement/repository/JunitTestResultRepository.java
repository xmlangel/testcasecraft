...
public interface JunitTestResultRepository extends JpaRepository<JunitTestResult, String> {
  ...
  /** 프로젝트별 완료된 테스트 결과 조회 */
  List<JunitTestResult> findByProjectIdAndStatusOrderByUploadedAtDesc(
      String projectId, JunitProcessStatus status);
  ...
  @Query(
      "SELECT jtr FROM JunitTestResult jtr WHERE jtr.uploadedAt BETWEEN :startDate AND :endDate"
          + " ORDER BY jtr.uploadedAt DESC")
  List<JunitTestResult> findByUploadedAtBetween(
      @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

  /** 프로젝트별 테스트 결과 통계 */
  @Query(
      "SELECT COUNT(jtr), SUM(jtr.totalTests), SUM(jtr.failures), SUM(jtr.errors) "
          + "FROM JunitTestResult jtr WHERE jtr.projectId = :projectId AND jtr.status = :status")
  Object[] getProjectStatistics(
      @Param("projectId") String projectId, @Param("status") JunitProcessStatus status);

  /** 체크섬으로 중복 파일 확인 */
  Optional<JunitTestResult> findByProjectIdAndFileChecksum(String projectId, String checksum);

  /** 실패한 처리 상태의 테스트 결과 조회 (재처리 대상) */
  List<JunitTestResult> findByStatusAndUploadedAtBefore(
      JunitProcessStatus status, LocalDateTime before);

  /** 프로젝트별 최근 테스트 결과 조회 (제한된 개수) */
  @Query(
      "SELECT jtr FROM JunitTestResult jtr WHERE jtr.projectId = :projectId AND jtr.status ="
          + " :status ORDER BY jtr.uploadedAt DESC")
  List<JunitTestResult> findRecentByProject(
  ...
  /** 파일명으로 검색 */
  @Query(
      "SELECT jtr FROM JunitTestResult jtr WHERE jtr.projectId = :projectId "
          + "AND (jtr.fileName LIKE %:searchTerm% OR jtr.testExecutionName LIKE %:searchTerm%) "
          + "ORDER BY jtr.uploadedAt DESC")
  Page<JunitTestResult> searchByProjectAndFileName(
      @Param("projectId") String projectId,
