...
public interface TestExecutionRepository extends JpaRepository<TestExecution, String> {
  @Query(
      "SELECT te FROM TestExecution te WHERE te.testPlanId = :testPlanId ORDER BY te.startDate"
          + " DESC")
  List<TestExecution> findByTestPlanId(@Param("testPlanId") String testPlanId);

  @Query(
      "SELECT te FROM TestExecution te WHERE te.testPlanId IN :testPlanIds ORDER BY te.startDate"
          + " DESC")
  ...
  @Query(
      "SELECT DISTINCT te FROM TestExecution te "
  ...
  @Query(
      "SELECT te.project.id, COUNT(te) FROM TestExecution te WHERE te.project.id IN :projectIds"
  ...
  @Query(
      "SELECT te FROM TestExecution te "
          + "WHERE te.project.id = :projectId "
          + "ORDER BY te.createdAt DESC")
  org.springframework.data.domain.Page<TestExecution> findPageByProjectId(
      @Param("projectId") String projectId, org.springframework.data.domain.Pageable pageable);
  ...
  @Query(
      "SELECT te FROM TestExecution te "
          ...
          + "ORDER BY te.createdAt DESC")
  List<TestExecution> findByProjectIdAndNameContainingIgnoreCase(
  ...
  @Query(
      "SELECT te FROM TestExecution te "
          ...
          + "AND LOWER(te.name) LIKE LOWER(CONCAT('%', :name, '%')) "
          + "ORDER BY te.createdAt DESC")
  org.springframework.data.domain.Page<TestExecution>
      findPageByProjectIdAndNameContainingIgnoreCase(
   ...
   */
  @Query(
      "SELECT te FROM TestExecution te WHERE te.status = 'INPROGRESS' ORDER BY te.startDate DESC")
   ...
   */
  @Query(
      "SELECT te FROM TestExecution te WHERE te.project.id = :projectId AND te.status ="
          + " 'INPROGRESS' ORDER BY te.startDate DESC")
