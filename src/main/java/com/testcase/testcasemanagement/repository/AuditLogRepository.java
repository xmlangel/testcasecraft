...
public interface AuditLogRepository extends JpaRepository<AuditLog, String> {
  ...
  // 특정 엔티티의 최근 로그들 조회
  @Query(
      "SELECT al FROM AuditLog al WHERE al.entityType = :entityType AND al.entityId = :entityId"
          + " ORDER BY al.timestamp DESC")
  List<AuditLog> findRecentLogsByEntity(
  ...
  List<AuditLog> findRecentLogsByUser(
      @Param("userId") String userId, org.springframework.data.domain.Pageable pageable);

  // 조직 관련 로그들 조회
  @Query(
      "SELECT al FROM AuditLog al WHERE al.entityType = 'ORGANIZATION' OR (al.entityType ="
          + " 'ORGANIZATION_USER' AND al.details LIKE CONCAT('%organizationId:', :organizationId,"
          + " '%'))")
  List<AuditLog> findOrganizationLogs(@Param("organizationId") String organizationId);

  // 프로젝트 관련 로그들 조회
  @Query(
      "SELECT al FROM AuditLog al WHERE al.entityType = 'PROJECT' OR (al.entityType ="
          + " 'PROJECT_USER' AND al.details LIKE CONCAT('%projectId:', :projectId, '%'))")
  List<AuditLog> findProjectLogs(@Param("projectId") String projectId);

  // 그룹 관련 로그들 조회
  @Query(
      "SELECT al FROM AuditLog al WHERE al.entityType = 'GROUP' OR (al.entityType = 'GROUP_MEMBER'"
          + " AND al.details LIKE CONCAT('%groupId:', :groupId, '%'))")
  List<AuditLog> findGroupLogs(@Param("groupId") String groupId);

  // 사용자 관련 모든 로그들 조회 (사용자가 수행한 것 + 사용자에 대한 것)
  @Query(
      "SELECT al FROM AuditLog al WHERE al.performedBy.id = :userId OR "
  ...
  // 사용자별 활동 통계 조회
  @Query(
      "SELECT al.performedBy.username, COUNT(al) FROM AuditLog al GROUP BY al.performedBy.username"
          + " ORDER BY COUNT(al) DESC")
  List<Object[]> getUserActivityStatistics();

  // 일별 활동 통계 조회
  @Query(
      "SELECT DATE(al.timestamp), COUNT(al) FROM AuditLog al "
          + "WHERE al.timestamp BETWEEN :startDate AND :endDate "
          + "GROUP BY DATE(al.timestamp) ORDER BY DATE(al.timestamp)")
  List<Object[]> getDailyActivityStatistics(
      @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

  // 특정 기간 동안의 특정 액션 수 조회
  @Query(
      "SELECT COUNT(al) FROM AuditLog al WHERE al.action = :action AND al.timestamp BETWEEN"
          + " :startDate AND :endDate")
  long countByActionAndPeriod(
  ...
  // 특정 사용자의 특정 액션 수 조회
  @Query(
      "SELECT COUNT(al) FROM AuditLog al WHERE al.performedBy.id = :userId AND al.action = :action")
  long countByUserAndAction(@Param("userId") String userId, @Param("action") String action);

  // 키워드로 로그 검색
  @Query(
      "SELECT al FROM AuditLog al WHERE "
