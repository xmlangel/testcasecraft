...
public interface DailyVisitSummaryRepository extends JpaRepository<DailyVisitSummary, String> {
  ...
  /** 특정 기간 내의 방문 요약 조회 */
  @Query(
      "SELECT dvs FROM DailyVisitSummary dvs WHERE dvs.visitDate BETWEEN :startDate AND :endDate"
          + " ORDER BY dvs.visitDate DESC")
  List<DailyVisitSummary> findByVisitDateBetween(
