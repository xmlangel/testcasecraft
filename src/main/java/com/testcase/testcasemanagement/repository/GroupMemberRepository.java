...
public interface GroupMemberRepository extends JpaRepository<GroupMember, String> {
  ...
  @Query(
      "SELECT gm.roleInGroup FROM GroupMember gm WHERE gm.group.id = :groupId AND gm.user.id ="
          + " :userId")
  Optional<GroupRole> findRoleByGroupIdAndUserId(
  ...
  List<GroupMember> findRecentMembersByGroupId(
      @Param("groupId") String groupId, org.springframework.data.domain.Pageable pageable);

  // 사용자가 특정 그룹에서 리더십 역할을 가지는지 확인
  @Query(
      "SELECT COUNT(gm) > 0 FROM GroupMember gm WHERE gm.group.id = :groupId AND gm.user.id ="
          + " :userId AND gm.roleInGroup IN ('LEADER', 'CO_LEADER')")
  boolean hasLeadershipRole(@Param("groupId") String groupId, @Param("userId") String userId);

  // 사용자가 특정 그룹의 리더인지 확인
  @Query(
      "SELECT COUNT(gm) > 0 FROM GroupMember gm WHERE gm.group.id = :groupId AND gm.user.id ="
          + " :userId AND gm.roleInGroup = 'LEADER'")
  ...
  @Query(
      "SELECT gm FROM GroupMember gm WHERE gm.user.id = :userId AND gm.group.organization.id ="
          + " :organizationId")
  List<GroupMember> findByUserIdAndOrganizationId(
      @Param("userId") String userId, @Param("organizationId") String organizationId);

  // 프로젝트 내에서 사용자가 속한 그룹 멤버십들 조회
  @Query(
      "SELECT gm FROM GroupMember gm WHERE gm.user.id = :userId AND gm.group.project.id ="
          + " :projectId")
  List<GroupMember> findByUserIdAndProjectId(
      @Param("userId") String userId, @Param("projectId") String projectId);

  // 사용자가 조직의 어떤 그룹에라도 속해있는지 확인
  @Query(
      "SELECT COUNT(gm) > 0 FROM GroupMember gm WHERE gm.user.id = :userId AND"
  ...
  @Query(
      "SELECT COUNT(gm) > 0 FROM GroupMember gm WHERE gm.user.id = :userId AND gm.group.project.id"
          + " = :projectId")
