...
public interface TranslationKeyRepository extends JpaRepository<TranslationKey, String> {
  ...
  List<TranslationKey> findByKeyNameContainingIgnoreCaseAndIsActive(
      String keyword, Boolean isActive);

  // 키 이름에 키워드 포함하며 카테고리로 필터링
  List<TranslationKey> findByKeyNameContainingIgnoreCaseAndCategory(
      String keyword, String category);

  // 키 이름에 키워드 포함하며 카테고리와 활성 상태로 필터링
  List<TranslationKey> findByKeyNameContainingIgnoreCaseAndCategoryAndIsActive(
  ...
  @Query(
      "SELECT tk FROM TranslationKey tk WHERE "
  ...
  List<TranslationKey> searchByKeywordAndFilters(
  ...
  // 모든 카테고리 목록 조회
  @Query(
      "SELECT DISTINCT tk.category FROM TranslationKey tk WHERE tk.category IS NOT NULL AND"
          + " tk.isActive = true")
  List<String> findAllCategories();

  // 특정 언어에 번역이 없는 키들 조회
  @Query(
      "SELECT tk FROM TranslationKey tk WHERE tk.isActive = true AND tk.id NOT IN (SELECT DISTINCT"
          + " t.translationKey.id FROM Translation t WHERE t.language.code = :languageCode AND"
          + " t.isActive = true)")
  List<TranslationKey> findKeysWithoutTranslation(@Param("languageCode") String languageCode);

  // 특정 언어에 번역이 있는 키들 조회
  @Query(
      "SELECT tk FROM TranslationKey tk WHERE tk.isActive = true AND tk.id IN (SELECT DISTINCT"
          + " t.translationKey.id FROM Translation t WHERE t.language.code = :languageCode AND"
          + " t.isActive = true)")
  ...
  // 카테고리별 키 개수 조회
  @Query(
      "SELECT tk.category, COUNT(tk) FROM TranslationKey tk WHERE tk.isActive = true GROUP BY"
          + " tk.category")
  ...
  @Query(
      "SELECT tk FROM TranslationKey tk WHERE "
          ...
          + "AND (:category IS NULL OR tk.category = :category) "
          + "AND (:isActive IS NULL OR tk.isActive = :isActive)")
  Page<TranslationKey> searchByKeywordAndFilters(
      @Param("keyword") String keyword,
      @Param("category") String category,
      @Param("isActive") Boolean isActive,
      Pageable pageable);
  ...
  // 카테고리와 활성 상태별 페이지네이션
  Page<TranslationKey> findByCategoryAndIsActive(
