...
public interface DisplayIdHistoryRepository extends JpaRepository<DisplayIdHistory, String> {
  ...
  /**
   * 이전 DisplayID로 최신 매핑 조회 프로젝트 코드가 여러 번 변경된 경우를 고려하여 가장 최신 이력을 반환
   *
   * @param oldDisplayId 이전 DisplayID
   * @return 최신 DisplayID 히스토리 (Optional)
   */
  @Query(
      "SELECT h FROM DisplayIdHistory h WHERE h.oldDisplayId = :oldDisplayId "
   ...
   */
  @Query(
      "SELECT h FROM DisplayIdHistory h WHERE h.testCase.id = :testCaseId "
