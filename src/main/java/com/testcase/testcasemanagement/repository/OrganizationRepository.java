...
public interface OrganizationRepository extends JpaRepository<Organization, String> {
  ...
  long countMembersByOrganizationIdAndRole(
  ...
  @Query(
      "SELECT DISTINCT ou.organization FROM OrganizationUser ou WHERE ou.user.id = :userId AND"
          + " ou.roleInOrganization = :role")
  List<Organization> findOrganizationsByUserIdAndRole(
  ...
  // 생성일 범위로 조직 검색
  @Query(
      "SELECT o FROM Organization o WHERE o.createdAt BETWEEN :startDate AND :endDate ORDER BY"
          + " o.createdAt DESC")
  List<Organization> findByCreatedAtBetween(
      @Param("startDate") java.time.LocalDateTime startDate,
      @Param("endDate") java.time.LocalDateTime endDate);

  // 조직과 멤버 정보를 함께 조회 (fetch join)
  @Query(
      "SELECT o FROM Organization o LEFT JOIN FETCH o.organizationUsers ou LEFT JOIN FETCH ou.user"
  ...
  @Query(
      "SELECT DISTINCT o FROM Organization o LEFT JOIN FETCH o.organizationUsers ou LEFT JOIN FETCH"
          + " ou.user ORDER BY o.createdAt DESC")
  List<Organization> findAllWithMembers();

  // 사용자가 속한 조직들과 멤버 정보를 함께 조회 (fetch join)
  @Query(
      "SELECT DISTINCT o FROM Organization o LEFT JOIN FETCH o.organizationUsers ou LEFT JOIN FETCH"
          + " ou.user WHERE o.id IN (SELECT ou2.organization.id FROM OrganizationUser ou2 WHERE"
          + " ou2.user.id = :userId) ORDER BY o.createdAt DESC")
