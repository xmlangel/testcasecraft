...
/** LLM 설정 Repository LLM 설정 정보를 관리하는 JPA Repository 인터페이스 */
@Repository
public interface LlmConfigRepository extends JpaRepository<LlmConfig, String> {

  /** 기본 설정 조회 시스템에서 기본으로 사용할 LLM 설정을 반환합니다. */
  ...
  /** 연결 상태가 검증된 설정들 조회 */
  @Query(
      "SELECT l FROM LlmConfig l WHERE l.connectionVerified = true AND l.isActive = true ORDER BY"
          + " l.createdAt DESC")
   ...
   */
  @Query(
      "SELECT l FROM LlmConfig l WHERE l.isActive = true AND "
  ...
  /** 연결 실패한 설정들 조회 */
  @Query(
      "SELECT l FROM LlmConfig l WHERE l.connectionVerified = false AND l.isActive = true ORDER BY"
          + " l.lastConnectionTest DESC")
  ...
  /** 모든 기본 설정 플래그를 false로 변경 새로운 기본 설정을 지정하기 전에 실행합니다. */
  @Modifying
