...
public interface UserRepository extends JpaRepository<User, String> {
  ...
  // 조직의 멤버들 조회
  @Query(
      "SELECT DISTINCT ou.user FROM OrganizationUser ou WHERE ou.organization.id = :organizationId")
  List<User> findUsersByOrganizationId(@Param("organizationId") String organizationId);

  // 조직의 특정 역할 멤버들 조회
  @Query(
      "SELECT DISTINCT ou.user FROM OrganizationUser ou WHERE ou.organization.id = :organizationId"
          + " AND ou.roleInOrganization = :role")
  ...
  // 프로젝트의 특정 역할 멤버들 조회
  @Query(
      "SELECT DISTINCT pu.user FROM ProjectUser pu WHERE pu.project.id = :projectId AND"
          + " pu.roleInProject = :role")
  List<User> findUsersByProjectIdAndRole(
  ...
  // 그룹의 특정 역할 멤버들 조회
  @Query(
      "SELECT DISTINCT gm.user FROM GroupMember gm WHERE gm.group.id = :groupId AND gm.roleInGroup"
          + " = :role")
  List<User> findUsersByGroupIdAndRole(
      @Param("groupId") String groupId,
      @Param("role") com.testcase.testcasemanagement.model.GroupMember.GroupRole role);

  // 조직에 속하지 않은 사용자들 조회
  @Query(
      "SELECT u FROM User u WHERE u.id NOT IN (SELECT DISTINCT ou.user.id FROM OrganizationUser"
          + " ou)")
  ...
  @Query(
      "SELECT u FROM User u WHERE u.id NOT IN (SELECT ou.user.id FROM OrganizationUser ou WHERE"
  ...
  @Query(
      "SELECT u FROM User u WHERE u.id NOT IN (SELECT pu.user.id FROM ProjectUser pu WHERE"
          + " pu.project.id = :projectId)")
  List<User> findUsersNotInProject(@Param("projectId") String projectId);

  // 특정 그룹에 속하지 않은 사용자들 조회
  @Query(
      "SELECT u FROM User u WHERE u.id NOT IN (SELECT gm.user.id FROM GroupMember gm WHERE"
          + " gm.group.id = :groupId)")
  ...
  @Query(
      "SELECT DISTINCT al.performedBy FROM AuditLog al WHERE al.timestamp >= :since ORDER BY"
          + " al.timestamp DESC")
  ...
  List<User> searchByKeywordAndActiveStatus(
      @Param("keyword") String keyword, @Param("isActive") Boolean isActive);
