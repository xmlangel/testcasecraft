...
public interface GroupRepository extends JpaRepository<Group, String> {
  ...
  @Query(
      "SELECT g FROM Group g WHERE g.organization.id = :organizationId OR g.project.id ="
          + " :projectId")
  List<Group> findByOrganizationIdOrProjectId(
      @Param("organizationId") String organizationId, @Param("projectId") String projectId);

  // 그룹명으로 조직 내 검색
  @Query(
      "SELECT g FROM Group g WHERE g.organization.id = :organizationId AND LOWER(g.name) LIKE"
          + " LOWER(CONCAT('%', :name, '%'))")
  List<Group> findByOrganizationIdAndNameContaining(
      @Param("organizationId") String organizationId, @Param("name") String name);

  // 그룹명으로 프로젝트 내 검색
  @Query(
      "SELECT g FROM Group g WHERE g.project.id = :projectId AND LOWER(g.name) LIKE"
          + " LOWER(CONCAT('%', :name, '%'))")
  List<Group> findByProjectIdAndNameContaining(
  ...
  @Query(
      "SELECT COUNT(gm) FROM GroupMember gm WHERE gm.group.id = :groupId AND gm.roleInGroup ="
          + " :role")
  long countMembersByGroupIdAndRole(
  ...
  @Query(
      "SELECT DISTINCT gm.group FROM GroupMember gm WHERE gm.user.id = :userId AND gm.roleInGroup ="
          + " :role")
  List<Group> findGroupsByUserIdAndRole(
  ...
  @Query(
      "SELECT DISTINCT gm.group FROM GroupMember gm WHERE gm.user.id = :userId AND"
          + " gm.group.organization.id = :organizationId")
  List<Group> findGroupsByUserIdAndOrganizationId(
      @Param("userId") String userId, @Param("organizationId") String organizationId);

  // 프로젝트 내에서 사용자가 속한 그룹들 조회
  @Query(
      "SELECT DISTINCT gm.group FROM GroupMember gm WHERE gm.user.id = :userId AND"
          + " gm.group.project.id = :projectId")
  List<Group> findGroupsByUserIdAndProjectId(
  ...
  // 조직 내 그룹 검색
  @Query(
      "SELECT g FROM Group g WHERE g.organization.id = :organizationId AND "
          + "(LOWER(g.name) LIKE LOWER(CONCAT('%', :keyword, '%')) OR "
          + "LOWER(g.description) LIKE LOWER(CONCAT('%', :keyword, '%')))")
  List<Group> searchByKeywordInOrganization(
      @Param("organizationId") String organizationId, @Param("keyword") String keyword);

  // 프로젝트 내 그룹 검색
  @Query(
      "SELECT g FROM Group g WHERE g.project.id = :projectId AND "
          + "(LOWER(g.name) LIKE LOWER(CONCAT('%', :keyword, '%')) OR "
          + "LOWER(g.description) LIKE LOWER(CONCAT('%', :keyword, '%')))")
  List<Group> searchByKeywordInProject(
