...
public interface OrganizationUserRepository extends JpaRepository<OrganizationUser, String> {
  ...
  // 특정 역할로 조직 멤버 조회
  List<OrganizationUser> findByOrganizationIdAndRoleInOrganization(
      String organizationId, OrganizationRole role);
  ...
  // 사용자의 조직별 역할 조회
  @Query(
      "SELECT ou.roleInOrganization FROM OrganizationUser ou WHERE ou.organization.id ="
          + " :organizationId AND ou.user.id = :userId")
  Optional<OrganizationRole> findRoleByOrganizationIdAndUserId(
  ...
  // 역할별 멤버 수 조회
  @Query(
      "SELECT COUNT(ou) FROM OrganizationUser ou WHERE ou.organization.id = :organizationId AND"
          + " ou.roleInOrganization = :role")
  long countByOrganizationIdAndRole(
      @Param("organizationId") String organizationId, @Param("role") OrganizationRole role);

  // 조직의 총 멤버 수 조회
  long countByOrganizationId(String organizationId);

  @Query(
      "SELECT ou.organization.id, COUNT(ou) FROM OrganizationUser ou WHERE ou.organization.id IN"
          + " :organizationIds GROUP BY ou.organization.id")
  ...
  // 최근 가입한 멤버들 조회
  @Query(
      "SELECT ou FROM OrganizationUser ou WHERE ou.organization.id = :organizationId ORDER BY"
          + " ou.createdAt DESC")
  List<OrganizationUser> findRecentMembersByOrganizationId(
      @Param("organizationId") String organizationId,
      org.springframework.data.domain.Pageable pageable);

  // 사용자가 특정 조직에서 관리 권한을 가지는지 확인
  @Query(
      "SELECT COUNT(ou) > 0 FROM OrganizationUser ou WHERE ou.organization.id = :organizationId AND"
          + " ou.user.id = :userId AND ou.roleInOrganization IN ('OWNER', 'ADMIN')")
