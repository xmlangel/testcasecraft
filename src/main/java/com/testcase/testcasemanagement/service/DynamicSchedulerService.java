...
/** 동적 스케줄링 서비스 런타임에 스케줄 작업을 등록, 변경, 취소할 수 있는 핵심 서비스 */
@Service
public class DynamicSchedulerService {
  ...
  public void initializeSchedules() {
    ...
    for (SchedulerConfig config : configs) {
      try {
        scheduleTask(config);
        logger.info(
            "스케줄 등록 완료: taskKey={}, type={}, expression={}",
            config.getTaskKey(),
            config.getScheduleType(),
            getScheduleExpression(config));
      } catch (Exception e) {
        logger.error("스케줄 등록 실패: taskKey={}, error={}", config.getTaskKey(), e.getMessage(), e);
      }
    }
  ...
  public void scheduleTask(SchedulerConfig config) {
    ...
    try {
      ...
      logger.info(
          "스케줄 작업 등록 완료: taskKey={}, type={}, expression={}",
          config.getTaskKey(),
          config.getScheduleType(),
          getScheduleExpression(config));

    } catch (Exception e) {
      logger.error("스케줄 작업 등록 실패: taskKey={}, error={}", config.getTaskKey(), e.getMessage());
