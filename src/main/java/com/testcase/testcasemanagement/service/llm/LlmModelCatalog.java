 ...
 *       확인을 아껴야 한다.
 *   <li><b>NVIDIA</b>: 응답에 가격도 모달리티도 없어 ID 패턴으로 추정할 수밖에 없다. 게다가 목록의 상당수가 계정에 없어 404 를 낸다(실측: 77개 중
 *       25개만 사용 가능). 확인이 곧 유일한 판정 수단이고, 확인에 한도 제약이 사실상 없다.
 * </ul>
 *
...
public interface LlmModelCatalog {
   ...
   * OpenRouter 는 2분 30초(40개 / 동시 4 × 15초), NVIDIA 는 6분(120개 / 동시 10 × 30초)이 걸린다. 결과를 기다려 돌려주면 그동안
   * 서블릿 스레드 하나가 묶이므로, 구독은 호출한 쪽이 하도록 미룬다.
   *
   * @param apiKey 제공자 API Key
   ...
   * 진행 상황을 알리며 확인한다.
   *
   * <p>백그라운드 작업이 진행률을 화면에 보여 주려면 모델 하나가 끝날 때마다 알아야 한다. 콜백은 판정이 나올 때마다 한 번 불리고, 건너뛴 모델도 센다. 여러 스레드에서
   * 동시에 불리므로 받는 쪽이 스스로 동기화해야 한다.
   *
   ...
   * 전수 확인을 기본으로 권할지 여부.
   *
   * <p>{@code true} 면 목록만으로는 쓸 수 있는 모델을 알 수 없고 확인에 한도 부담도 없다는 뜻이다(NVIDIA). {@code false} 면 확인이 한도를
   * 태우므로 고른 모델 하나만 확인하는 것이 낫다는 뜻이다(OpenRouter).
   */
