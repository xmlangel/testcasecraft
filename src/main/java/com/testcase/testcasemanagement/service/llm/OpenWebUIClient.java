...
public class OpenWebUIClient implements LlmClient {
  ...
  private static final String CHAT_COMPLETIONS_PATH =
      LlmApiUrlNormalizer.chatCompletionsPathOf(LlmConfig.LlmProvider.OPENWEBUI);
    ...
    try {
      ...
      if (e.getStatusCode().value() == 401 || e.getStatusCode().value() == 403) {
        throw new LlmClientException(
            "OpenWebUI API 인증에 실패했습니다 (401/403). 등록된 API Key가 올바르고 만료되지 않았는지 확인해 주세요.", e);
      }
      throw new LlmClientException(
          "OpenWebUI API 호출 실패 (상태코드: " + e.getStatusCode() + "): "
              + "[호출 주소: "
  ...
  public void chatStream(
    ...
    try {
      ...
      log.error("❌ OpenWebUI API 스트리밍 응답 에러 (상태코드: {})", e.getStatusCode(), e);
      if (e.getStatusCode().value() == 401 || e.getStatusCode().value() == 403) {
        throw new LlmClientException(
            "OpenWebUI API 스트리밍 인증에 실패했습니다 (401/403). 등록된 API Key가 올바르고 만료되지 않았는지 확인해 주세요.", e);
      }
      throw new LlmClientException(
          "Failed to call OpenWebUI API stream (상태코드: "
              + e.getStatusCode()
              + "): "
              + "[호출 주소: "
              + LlmApiUrlNormalizer.resolveEndpoint(
