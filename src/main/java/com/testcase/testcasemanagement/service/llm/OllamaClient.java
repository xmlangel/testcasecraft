 ...
 * Ollama API 클라이언트
 *
 * <p>Ollama의 /v1/chat/completions 엔드포인트와 통신 (OpenAI 호환 API)
 */
@Service
...
public class OllamaClient implements LlmClient {
  ...
  private static final String CHAT_COMPLETIONS_PATH =
      LlmApiUrlNormalizer.chatCompletionsPathOf(LlmConfig.LlmProvider.OLLAMA);
  ...
  public LlmResponse chat(
    ...
    try {
      ...
      WebClient.Builder builder = webClientBuilder.baseUrl(
      ...
      log.error("❌ Ollama API 응답 에러 (상태코드: {})", e.getStatusCode(), e);
      if (e.getStatusCode().value() == 401 || e.getStatusCode().value() == 403) {
        throw new LlmClientException(
            "Ollama API 인증에 실패했습니다 (401/403). 등록된 API Key가 올바르고 만료되지 않았는지 확인해 주세요.", e);
      }
      throw new LlmClientException(
          "Ollama API 호출 실패 (상태코드: " + e.getStatusCode() + "): "
              + "[호출 주소: "
  ...
  public void chatStream(
    ...
    try {
      log.info(
          "📤 Ollama API 스트리밍 호출: model={}, messages={}", config.getModelName(), messages.size());
      ...
      log.error("❌ Ollama API 스트리밍 응답 에러 (상태코드: {})", e.getStatusCode(), e);
      if (e.getStatusCode().value() == 401 || e.getStatusCode().value() == 403) {
        throw new LlmClientException(
            "Ollama API 스트리밍 인증에 실패했습니다 (401/403). 등록된 API Key가 올바르고 만료되지 않았는지 확인해 주세요.", e);
      }
      throw new LlmClientException(
          "Failed to call Ollama API stream (상태코드: "
              + e.getStatusCode()
              + "): "
              + "[호출 주소: "
              + LlmApiUrlNormalizer.resolveEndpoint(
