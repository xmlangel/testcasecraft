 ...
 * OpenAI API 클라이언트
 *
 * <p>OpenAI의 /v1/chat/completions 엔드포인트와 통신
 */
@Service
...
public class OpenAIClient implements LlmClient {
  ...
  public LlmResponse chat(
    ...
    try {
      log.info("📤 OpenAI API 호출: model={}, messages={}", config.getModelName(), messages.size());
      ...
      if (response == null) {
        throw new LlmClientException("OpenAI API returned null response");
      }
      ...
      log.info("✅ OpenAI API 응답 성공: tokens={}", tokensUsed);

      return new LlmResponse(content, tokensUsed, config.getModelName());

    } catch (org.springframework.web.reactive.function.client.WebClientResponseException e) {
      log.error("❌ OpenAI API 응답 에러 (상태코드: {})", e.getStatusCode(), e);
  ...
  public void chatStream(
    ...
    try {
      log.info(
          "📤 OpenAI API 스트리밍 호출: model={}, messages={}", config.getModelName(), messages.size());
      ...
      log.error("❌ OpenAI API 스트리밍 응답 에러 (상태코드: {})", e.getStatusCode(), e);
      if (e.getStatusCode().value() == 401 || e.getStatusCode().value() == 403) {
        throw new LlmClientException(
            "OpenAI API 스트리밍 인증에 실패했습니다 (401/403). 등록된 API Key가 올바르고 만료되지 않았는지 확인해 주세요.", e);
      }
      throw new LlmClientException(
      ...
      throw new LlmClientException("Failed to call OpenAI API stream: " + e.getMessage(), e);
    }
  }
