 ...
 *
 * <p>OpenRouter의 /api/v1/chat/completions 엔드포인트와 통신 OpenAI 호환 API를 사용하므로 OpenAIClient와 유사한 구조
 */
@Service
...
public class OpenRouterClient implements LlmClient {
  ...
  public LlmResponse chat(
    ...
    try {
      log.info(
          "📤 OpenRouter API 호출: model={}, messages={}", config.getModelName(), messages.size());
      ...
      log.info("✅ OpenRouter API 응답 성공: tokens={}", tokensUsed);

      return new LlmResponse(content, tokensUsed, config.getModelName());

    } catch (org.springframework.web.reactive.function.client.WebClientResponseException e) {
      log.error("❌ OpenRouter API 응답 에러 (상태코드: {})", e.getStatusCode(), e);
  ...
  public void chatStream(
    ...
    try {
      log.info(
          "📤 OpenRouter API 스트리밍 호출: model={}, messages={}",
          config.getModelName(),
          messages.size());
      ...
      responseFlux
          ...
          .doOnError(
              error -> {
                log.error("❌ OpenRouter API 스트리밍 실패", error);
              })
          .doOnComplete(
              () -> {
                log.info("✅ OpenRouter API 스트리밍 완료");
                if (!completionSent.getAndSet(true)) {
                  callback.onChunk("", true);
                }
              })
      ...
      log.error("❌ OpenRouter API 스트리밍 응답 에러 (상태코드: {})", e.getStatusCode(), e);
      if (e.getStatusCode().value() == 401 || e.getStatusCode().value() == 403) {
        throw new LlmClientException(
            "OpenRouter API 스트리밍 인증에 실패했습니다 (401/403). 등록된 API Key가 올바르고 만료되지 않았는지 확인해 주세요.", e);
      }
      throw new LlmClientException(
      ...
      log.error("❌ OpenRouter API 스트리밍 호출 실패", e);
