 ...
 *
 * OpenRouter의 /api/v1/chat/completions 엔드포인트와 통신
 * OpenAI 호환 API를 사용하므로 OpenAIClient와 유사한 구조
 */
@Service
...
public class OpenRouterClient implements LlmClient {
    ...
    public LlmResponse chat(LlmConfig config, List<RagChatMessage> messages,
        ...
        try {
            log.info("📤 OpenRouter API 호출: model={}, messages={}", config.getModelName(), messages.size());
    ...
    public void chatStream(LlmConfig config, List<RagChatMessage> messages,
        ...
        try {
            log.info("📤 OpenRouter API 스트리밍 호출: model={}, messages={}", config.getModelName(), messages.size());
