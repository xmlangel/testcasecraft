...
/** ICT-208: 테스트 결과 고급 통계 서비스 다양한 통계 분석 및 인사이트를 제공합니다. */
@Slf4j
@Service
...
public class TestResultStatisticsService {
  ...
  public TestResultSummaryDto generateComprehensiveStatistics(TestResultQueryDto query) {
    ...
    try {
      ...
      // 상세 통계 수집
      Map<String, TestResultSummaryDto.ResultByPriority> priorityStats =
          calculateStatisticsByPriority(query);
      List<TestResultSummaryDto.ResultByAssignee> assigneeStats =
          calculateStatisticsByAssignee(query);
      ...
      // 통계 기간 설정
      TestResultSummaryDto.StatisticsPeriod period =
          createStatisticsPeriod(query.getStartDate(), query.getEndDate(), query.getRecentDays());
  ...
  /** 우선순위별 통계 계산 */
  private Map<String, TestResultSummaryDto.ResultByPriority> calculateStatisticsByPriority(
      TestResultQueryDto query) {
    // 우선순위별 그룹화 로직 구현
    ...
    for (String priority : priorities) {
      // 각 우선순위별 통계 계산
      TestResultSummaryDto.ResultByPriority stat =
          TestResultSummaryDto.ResultByPriority.builder()
              .priority(priority)
              .totalCount(0L)
              .passedCount(0L)
              .failedCount(0L)
  ...
  /** 담당자별 통계 계산 */
  private List<TestResultSummaryDto.ResultByAssignee> calculateStatisticsByAssignee(
      TestResultQueryDto query) {
    ...
    try {
      ...
      if (query.getProjectId() != null) {
        List<Map<String, Object>> rawStats =
            testResultRepository.findExecutorStatisticsByProject(query.getProjectId());
        ...
        for (Map<String, Object> stat : rawStats) {
          ...
          TestResultSummaryDto.ResultByAssignee assigneeStat =
              TestResultSummaryDto.ResultByAssignee.builder()
  ...
  /** 일별 통계 계산 */
  private List<TestResultSummaryDto.DailyTestResult> calculateDailyStatistics(
      TestResultQueryDto query) {
    ...
    while (!current.isAfter(endDate)) {
      ...
      // 해당 일의 테스트 결과 통계 계산
      TestResultSummaryDto.DailyTestResult dailyStat =
          TestResultSummaryDto.DailyTestResult.builder()
              .date(dayStart)
              .testsExecuted(0L)
              .testsPassed(0L)
              .testsFailed(0L)
  ...
  /** 자주 실패하는 테스트 케이스 식별 */
  private List<TestResultSummaryDto.FrequentFailure> identifyFrequentFailures(
      TestResultQueryDto query) {
  ...
  public List<Map<String, Object>> getComparisonStatisticsByExecutor(String projectId) {
    ...
    try {
      ...
      List<Map<String, Object>> executorStats =
          testResultRepository.findStatisticsByExecutor(projectId);
