...
/** 권한 충돌 검증 및 해결 서비스 ICT-33: 사용자 권한 및 역할 관리 강화 */
@Service
...
public class PermissionConflictService {
  ...
  /** 포괄적인 권한 충돌 검증 */
  public List<PermissionConflictDto> comprehensiveConflictCheck(
  ...
  /** 역할 계층 충돌 검증 */
  private List<PermissionConflictDto> roleHierarchyConflictCheck(
      List<BulkPermissionChangeDto> changes) {
    List<PermissionConflictDto> conflicts = new ArrayList<>();
  ...
  /** 조직-프로젝트 관계 충돌 검증 */
  private List<PermissionConflictDto> organizationProjectConflictCheck(
      List<BulkPermissionChangeDto> changes) {
    List<PermissionConflictDto> conflicts = new ArrayList<>();
  ...
  /** 시스템 정책 충돌 검증 */
  private List<PermissionConflictDto> systemPolicyConflictCheck(
      List<BulkPermissionChangeDto> changes) {
    List<PermissionConflictDto> conflicts = new ArrayList<>();
  ...
  /** 배치 내 충돌 검증 */
  private List<PermissionConflictDto> batchInternalConflictCheck(
      List<BulkPermissionChangeDto> changes) {
    List<PermissionConflictDto> conflicts = new ArrayList<>();

    // 같은 사용자-리소스 조합에 대한 중복 변경 확인
    Map<String, List<BulkPermissionChangeDto>> groupedChanges =
        changes.stream()
            .collect(
                Collectors.groupingBy(change -> change.getUserId() + ":" + change.getResourceId()));
  ...
  private List<PermissionConflictDto> checkRoleElevationAuthority(BulkPermissionChangeDto change) {
    ...
    if (change.getResourceType().equals("ORGANIZATION")) {
      OrganizationRole currentRole =
          change.getCurrentRole() != null
              ? OrganizationRole.valueOf(change.getCurrentRole())
              : null;
  ...
  private List<PermissionConflictDto> checkPermissionAuthority(BulkPermissionChangeDto change) {
    ...
    if (change.getResourceType().equals("ORGANIZATION")) {
      if (!organizationSecurityService.canManageOrganization(change.getResourceId())) {
        conflicts.add(
            createConflict(
                "INSUFFICIENT_PERMISSION", "조직 관리 권한이 없습니다.", change, "조직 관리자에게 요청하세요.", false));
      }
    }
  ...
  /** 충돌 객체 생성 헬퍼 메서드 */
  private PermissionConflictDto createConflict(
      String conflictType,
      String message,
    ...
    conflict.setConflictDetails(
        String.format(
            "Change: %s, Resource: %s, Role: %s → %s",
            change.getChangeType(),
            change.getResourceType(),
            change.getCurrentRole(),
            change.getNewRole()));
  ...
  public List<BulkPermissionChangeDto> autoResolveConflicts(
      List<BulkPermissionChangeDto> changes, List<PermissionConflictDto> conflicts) {

    List<BulkPermissionChangeDto> resolvedChanges = new ArrayList<>();
    Set<String> conflictedChangeIds =
        conflicts.stream()
  ...
  /** 자동 해결 로직 적용 */
  private BulkPermissionChangeDto applyAutoResolution(
      BulkPermissionChangeDto change, List<PermissionConflictDto> conflicts) {
    // 중복 멤버십의 경우 ADD를 CHANGE로 변경
    if (change.getChangeType().equals("ADD_ORG_MEMBER")
        || change.getChangeType().equals("ADD_PROJECT_MEMBER")) {

      String newChangeType =
          change.getChangeType().replace("ADD_", "CHANGE_").replace("_MEMBER", "_ROLE");
      change.setChangeType(newChangeType);

      // 현재 역할 조회 및 설정
      if (change.getResourceType().equals("ORGANIZATION")) {
        Optional<OrganizationRole> currentRole =
            organizationUserRepository.findRoleByOrganizationIdAndUserId(
                change.getResourceId(), change.getUserId());
        if (currentRole.isPresent()) {
          change.setCurrentRole(currentRole.get().toString());
        }
      } else if (change.getResourceType().equals("PROJECT")) {
        Optional<ProjectRole> currentRole =
            projectUserRepository.findRoleByProjectIdAndUserId(
                change.getResourceId(), change.getUserId());
        if (currentRole.isPresent()) {
          change.setCurrentRole(currentRole.get().toString());
        }
