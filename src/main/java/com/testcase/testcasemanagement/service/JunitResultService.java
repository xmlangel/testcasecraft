...
/** ICT-203: JUnit 테스트 결과 메인 서비스 파일 업로드, 파싱, 저장의 전체 프로세스를 관리 */
@Service
@Transactional
public class JunitResultService {
  ...
  @Autowired private JunitTestResultRepository testResultRepository;

  @Autowired private JunitTestSuiteRepository testSuiteRepository;
  ...
  @Autowired private JunitXmlParserService xmlParserService;
   ...
   */
  public JunitTestResult uploadAndProcessJunitXml(
      MultipartFile file,
      String projectId,
      String username,
    ...
    logger.info(
        "JUnit XML 파일 업로드 및 처리 시작 - 파일: {}, 프로젝트: {}, 사용자: {}",
    ...
    try {
      ...
      Optional<JunitTestResult> existingResult =
          testResultRepository.findByProjectIdAndFileChecksum(
              projectId, storageResult.getChecksum());
      ...
      logger.info(
          "JUnit XML 파일 처리 완료 - ID: {}, 총 테스트: {}", testResult.getId(), testResult.getTotalTests());
  ...
  private void processXmlFile(JunitTestResult testResult, String filePath, User user)
    ...
    try {
      ...
      try (InputStream inputStream = fileStorageService.loadFileAsInputStream(filePath)) {
        ...
        // XML 파싱
        JunitTestResult parsedResult =
            xmlParserService.parseJunitXml(
        ...
        logger.info(
            "XML 파싱 완료 - 테스트 스위트: {}, 총 테스트: {}",
            testResult.getTestSuites().size(),
  ...
  public List<JunitTestSuite> getTestSuitesByResult(String testResultId) {
    List<JunitTestSuite> suites =
        testSuiteRepository.findByJunitTestResult_IdOrderByName(testResultId);
    ...
    for (JunitTestSuite suite : suites) {
      ...
      if (suite.getTests() != actualCount) {
        logger.debug(
            "Suite {} count mismatch corrected: metadata={}, db={}",
            suite.getName(),
            suite.getTests(),
            actualCount);
   ...
   * @throws JunitProcessingException 처리 중 오류 발생 시
   */
  public JunitTestResult createInitialTestResult(
      MultipartFile file,
      String projectId,
      String username,
      String executionName,
      String description)
      throws JunitProcessingException {
    logger.info("비동기 처리용 초기 테스트 결과 생성 - 파일: {}, 프로젝트: {}", file.getOriginalFilename(), projectId);
    ...
    try {
      ...
      throw new JunitProcessingException(
          "Failed to create initial test result: " + e.getMessage(), e);
    }
  }

  /** 테스트 케이스 편집 */
  public JunitTestCase updateTestCase(
      String testCaseId,
      String userTitle,
      String userDescription,
      String userNotes,
      JunitTestStatus userStatus,
      String tags,
      String priority,
      String userId) {
  ...
  public Map<String, Object> getProjectSummaryStatistics(String projectId) {
    ...
    try {
      ...
      List<JunitTestResult> testResults =
          testResultRepository.findByProjectIdOrderByUploadedAtDesc(projectId);
      ...
      List<JunitTestResult> recentResults =
          testResults.stream()
              .limit(5) // 최근 5개 결과
      ...
      // 상태별 통계
      Map<JunitProcessStatus, Long> statusCounts =
          testResults.stream()
      ...
      logger.debug(
          "프로젝트 JUnit 요약 통계 조회 완료: {} - 결과 {}개, 최근 성공률 {}%",
  ...
  public Map<String, Map<String, Object>> getBatchProjectSummaryStatistics(
    ...
    logger.debug("배치 프로젝트 JUnit 요약 통계 조회 완료: {}/{} 성공", batchSummary.size(), projectIds.size());
  ...
  public Optional<JunitTestCase> getTestCaseById(String testCaseId) {
    ...
    try {
      ...
      if (testCase.isPresent()) {
        logger.debug("테스트 케이스 조회 성공 - ID: {}, 이름: {}", testCaseId, testCase.get().getName());
      } else {
  ...
  public Map<String, Object> getPreviousTestCaseNote(String testCaseId) {
    try {
      ...
      List<JunitTestCase> prevCases =
          testCaseRepository.findPreviousTestCasesWithNotes(
              projectId,
      ...
      if (prevCases != null && !prevCases.isEmpty()) {
        ...
        result.put(
            "executionName",
            prevResult.getTestExecutionName() != null
                ? prevResult.getTestExecutionName()
  ...
  public JunitTestResult linkTestPlan(String testResultId, String testPlanId) {
    JunitTestResult testResult =
        testResultRepository
            ...
            .orElseThrow(
                () -> new IllegalArgumentException("Test result not found: " + testResultId));
    ...
    if (testPlanId != null) {
      TestPlan testPlan =
          testPlanRepository
              .findById(testPlanId)
              .orElseThrow(
                  () -> new IllegalArgumentException("Test plan not found: " + testPlanId));
  ...
  public String getTestPlanName(String testPlanId) {
    if (testPlanId == null) return null;
    return testPlanRepository.findById(testPlanId).map(TestPlan::getName).orElse(null);
  }

  /** 특정 JUnit 결과에서 노트가 있는 테스트 케이스 수 조회 */
  @Transactional(readOnly = true)
  public int getNotesCountByTestResult(String testResultId) {
    if (testResultId == null) return 0;
    try {
  ...
  public List<Map<String, Object>> getComparisonStatisticsByPlan(String projectId) {
    ...
    try {
      List<Map<String, Object>> planStats =
          testResultRepository.findStatisticsByTestPlan(projectId);

      return planStats.stream()
          .map(
              stat -> {
                Map<String, Object> result = new HashMap<>();
  ...
  public List<Map<String, Object>> getComparisonStatisticsByExecutor(String projectId) {
    ...
    try {
      List<Map<String, Object>> executorStats =
          testResultRepository.findStatisticsByUploader(projectId);

      return executorStats.stream()
          .map(
              stat -> {
                Map<String, Object> result = new HashMap<>();
