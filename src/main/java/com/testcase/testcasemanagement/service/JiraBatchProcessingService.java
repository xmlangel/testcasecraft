...
/** JIRA API 배치 처리 최적화 서비스 ICT-165: 성능 향상을 위한 배치 처리 구현 */
@Service
@Slf4j
@ConditionalOnProperty(
    name = "jira.batch-processing.enabled",
    havingValue = "true",
...
public class JiraBatchProcessingService {
  ...
  public JiraBatchProcessingService(
  ...
  public void init() {
    ...
    log.info(
        "JIRA 배치 처리 서비스 초기화 완료 - 스레드: {}, 배치크기: {}, 타임아웃: {}s",
        maxThreads,
        batchSize,
        timeoutSeconds);
  }
  ...
  public BatchOperationResult batchAddComments(String userId, List<BatchCommentRequest> requests) {
    ...
    try {
      ...
      for (List<BatchCommentRequest> batch : batches) {
        Future<List<BatchCommentResult>> future =
      ...
      log.info(
          "배치 코멘트 작업 완료: operationId={}, 성공={}, 실패={}, 소요시간={}ms",
          operationId,
          successCount,
          failureCount,
  ...
  public BatchOperationResult batchGetProjects(List<String> userIds) {
    ...
    try {
      ...
      for (List<String> batch : batches) {
        Future<List<BatchProjectResult>> future =
            executorService.submit(() -> processBatchProjects(batch, stats));
        futures.add(future);
      }
  ...
  public BatchOperationResult batchTestConnections(List<String> configIds) {
    ...
    try {
      ...
      for (List<JiraConfig> batch : batches) {
        Future<List<BatchConnectionTestResult>> future =
            executorService.submit(() -> processBatchConnectionTests(batch, stats));
        futures.add(future);
      }
      ...
      for (Future<List<BatchConnectionTestResult>> future : futures) {
        try {
          List<BatchConnectionTestResult> batchResults =
  ...
  private List<BatchCommentResult> processBatchComments(
      JiraConfig config,
      String apiToken,
        ...
        .map(
            request -> {
              try {
                boolean success =
                    addCommentWithRetry(
                        config, apiToken, request.getIssueKey(), request.getComment());
                stats.recordSuccess();
  ...
  private List<BatchProjectResult> processBatchProjects(
      List<String> userIds, BatchOperationStats stats) {
  ...
  private List<BatchConnectionTestResult> processBatchConnectionTests(
      List<JiraConfig> configs, BatchOperationStats stats) {
        ...
        .map(
            config -> {
              try {
                ...
                return new BatchConnectionTestResult(
                    config.getId(), status.getIsConnected(), status);
              } catch (Exception e) {
                stats.recordError();
  ...
  private boolean addCommentWithRetry(
      JiraConfig config, String apiToken, String issueKey, String comment) {
  ...
  public static class BatchConnectionTestResult {
    ...
    public BatchConnectionTestResult(
        String configId, boolean success, JiraConfigDto.ConnectionStatusDto status) {
    ...
    public JiraConfigDto.ConnectionStatusDto getStatus() {
      return status;
    }
  }
  ...
  public static class BatchOperationResult {
    ...
    public static BatchOperationResult success(
        String operationId, Object results, int successCount, int failureCount) {
