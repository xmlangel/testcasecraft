...
public class TestCaseService {
  ...
  public TestCase saveTestCase(TestCaseDto testCaseDto) {
    ...
    Project project =
        projectRepository
            .findById(testCaseDto.getProjectId())
    ...
    if (testCaseDto.getParentId() != null && !testCaseDto.getParentId().isEmpty()) {
      boolean isParentValid =
          testCaseRepository
              .findById(testCaseDto.getParentId())
              .map(
                  parent -> {
    ...
    try {
      TestCaseVersionEvent event =
          new TestCaseVersionEvent(this, savedEntity.getId(), "CREATE", "초기 테스트케이스 생성");
   ...
   * @return 삭제 결과
   */
  public com.testcase.testcasemanagement.dto.TestCaseBulkOperationDto batchDeleteTestCases(
      List<String> ids) {
    com.testcase.testcasemanagement.dto.TestCaseBulkOperationDto result =
        new com.testcase.testcasemanagement.dto.TestCaseBulkOperationDto();

    result.setOperationType(
        com.testcase.testcasemanagement.dto.TestCaseBulkOperationDto.OperationType.DELETE);
  ...
  public TestCase updateTestCase(String id, TestCaseDto testCaseDto) {
    ...
    String normalizedParentId =
        (testCaseDto.getParentId() == null || testCaseDto.getParentId().trim().isEmpty())
            ? null
    ...
    log.debug(
        "중복 검증 스킵 - projectId: {}, name: '{}', parentId: '{}', type: {}, excludeId: {}",
    ...
    if ("folder".equals(entity.getType())) {
      ...
      List<String> addedTags =
          newTags.stream().filter(tag -> !oldTags.contains(tag)).collect(Collectors.toList());
    ...
    if (parentChanged) {
      ...
      Integer maxOrder =
          testCaseRepository.findByParentIdOrderByDisplayOrder(newParentId).stream()
    ...
    try {
      ...
      if (fullErrorMessage.contains("UKL7WIR8HGJNYHVRMU717NSRTYY")
        ...
        log.warn(
            ...
            e.getMessage());
        Integer maxOrder =
            testCaseRepository.findByParentIdOrderByDisplayOrder(entity.getParentId()).stream()
    ...
    // 저장 후 수정자 정보 확인 로그
    log.info(
        "테스트케이스 업데이트 완료: id={}, name={}, updatedBy={}, createdBy={}",
        updatedEntity.getId(),
    ...
    try {
      ...
      TestCaseVersionEvent event =
          new TestCaseVersionEvent(this, updatedEntity.getId(), "UPDATE", changeSummary);
      eventPublisher.publishEvent(event);
      log.info("ICT-349: 테스트케이스 수정 이벤트 발행: {}", updatedEntity.getId());
    } catch (Exception e) {
      log.error(
          "ICT-349: 테스트케이스 수정 이벤트 발행 실패: {}, error: {}", updatedEntity.getId(), e.getMessage());
    }
  ...
  /** 테스트케이스의 전체 폴더 경로를 구성합니다. 예: "폴더A >> 하위폴더1 >> 하위폴더2" pathCache를 전달받아 중복 조회를 방지합니다. */
  ...
  public List<TestCase> importFromCsv(InputStream is, String projectId, CsvMappingConfig config) {
    ...
    if (config.getFieldMappings() == null || config.getFieldMappings().isEmpty())
      throw new CsvImportException(
          "No field mappings", Collections.singletonList(Map.of("error", "No field mappings")));
    ...
    if (!errors.isEmpty()) throw new CsvImportException("CSV import failed", errors);
  ...
  public List<TestCase> importFromExcel(InputStream is, String projectId, CsvMappingConfig config) {
    ...
    if (config.getFieldMappings() == null || config.getFieldMappings().isEmpty()) {
      throw new CsvImportException(
          "필드 매핑 정보가 필요합니다", Collections.singletonList(Map.of("error", "No field mappings")));
    }
    ...
    for (int i = 0; i < rows.size(); i++) {
      ...
      try {
        ...
        Optional<TestCase> orderDup =
            testCaseRepository.findByParentIdAndDisplayOrder(
                tc.getParentId(), tc.getDisplayOrder());
  ...
  @Transactional
  public List<TestCase> importFromGoogleSheet(
      String spreadsheetId, String sheetName, String projectId, CsvMappingConfig config)
    ...
    if (sheetValues == null || sheetValues.isEmpty()) {
      throw new ResourceNotValidException(
          "Google Sheet is empty", Map.of("entity", "sheet", "sheetName", sheetName));
    }
  ...
  private String getOrCreateImportFolder(Project project) {
    ...
    Optional<TestCase> importFolderOpt =
        testCaseRepository.findByProjectIdAndType(projectId, "folder").stream()
  ...
  /** 테스트케이스와 각 스텝을 구글 시트로 내보낸다. 각 테스트케이스의 스텝이 여러 개면, 동일한 테스트케이스 정보에 step 정보만 다르게 여러 행이 생성된다. */
  @Transactional
  ...
  /**
   * 배치 저장 메서드 ICT-373: 스프레드시트 일괄 저장 배치 처리 최적화
   *
   * <p>JPA의 saveAll()과 flush()를 사용하여 bulk insert/update 최적화
   *
    ...
    try {
      ...
      for (int i = 0; i < testCaseDtos.size(); i++) {
        ...
        try {
          ...
          if (!isNewEntity) {
            ...
            entity =
                testCaseRepository
                    .findById(dto.getId())
                    .orElseThrow(
                        () -> new IllegalArgumentException("테스트케이스를 찾을 수 없습니다: " + dto.getId()));
            ...
            entity.setUpdatedBy(currentUser);
            log.info(
                "배치 저장 - 기존 테스트케이스 수정자 정보 설정: id={}, name={}, updatedBy={}",
                entity.getId(),
                dto.getName(),
                currentUser);
            ...
            entity.setUpdatedBy(currentUser);
            log.info(
                "배치 저장 - 신규 테스트케이스 작성자 정보 설정: name={}, createdBy={}, updatedBy={}",
                dto.getName(),
                currentUser,
                currentUser);
            ...
            if (entity.getSequentialId() == null) {
              ...
              if (!projectMaxSeqIdMap.containsKey(projectId)) {
                Integer maxSequentialId =
                    testCaseRepository.findMaxSequentialIdByProjectId(projectId);
                projectMaxSeqIdMap.put(projectId, maxSequentialId == null ? 0 : maxSequentialId);
                log.info(
                    "배치 저장 - 프로젝트 {}의 현재 최대 순차 ID: {}",
                    projectId,
              ...
              log.info(
                  "배치 저장 - 새 테스트케이스에 순차 ID 할당: {} (프로젝트: {}, 배치 내 {}번째)",
                  newSeqId,
                  projectId,
                  i + 1);
            }
            ...
            if (entity.getDisplayId() == null || entity.getDisplayId().trim().isEmpty()) {
              ...
              entity.setDisplayId(generatedDisplayId);
              log.info(
                  "배치 저장 - 새 테스트케이스에 Display ID 할당: {} (프로젝트: {})",
                  generatedDisplayId,
          ...
          errors.add(
              com.testcase.testcasemanagement.dto.BatchSaveResult.BatchError.builder()
                  .index(i)
      ...
      if (!testCaseEntities.isEmpty()) {
        ...
        for (int i = 0; i < savedEntities.size(); i++) {
          ...
          savedTestCases.add(
              com.testcase.testcasemanagement.mapper.TestCaseMapper.toDto(savedEntity));
          ...
          try {
            ...
            log.debug(
                "ICT-373: 배치 저장 버전 이벤트 발행 - testCaseId={}, type={}",
                savedEntity.getId(),
                versionType);
          } catch (Exception e) {
            log.error(
                "ICT-373: 배치 저장 버전 이벤트 발행 실패: testCaseId={}, error={}",
                savedEntity.getId(),
                e.getMessage());
          }
          ...
          log.info(
              "ICT-373: 배치 저장 - RAG 벡터화 호출 시작: testCaseId={}, type={}, name={}",
              savedEntity.getId(),
              savedEntity.getType(),
              savedEntity.getName());
          vectorizeTestCaseToRAG(savedEntity);
  ...
  /** DTO에서 Entity로 업데이트 ICT-373: 배치 처리를 위한 헬퍼 메서드 */
  private void updateEntityFromDto(
      TestCase entity, com.testcase.testcasemanagement.dto.TestCaseDto dto) {
    ...
    if (dto.getSteps() != null) {
      ...
      entity
          .getSteps()
          .addAll(com.testcase.testcasemanagement.mapper.TestCaseMapper.toEntity(dto).getSteps());
    }
  ...
  private String formatTestCaseForRAG(TestCase testCase) {
    ...
    if (testCase.getIsAutomated() != null) {
      content
          .append("자동화 여부: ")
          .append(Boolean.TRUE.equals(testCase.getIsAutomated()) ? "Y" : "N")
          .append("\n\n");
    }
  ...
  /**
   * ICT-388: TestCase를 RAG 시스템에 벡터화하여 등록 비동기적으로 처리하여 CRUD 작업에 영향을 주지 않음
   *
   * @param testCase 등록할 TestCase
   */
  private void vectorizeTestCaseToRAG(TestCase testCase) {
    log.info(
        "vectorizeTestCaseToRAG 호출됨: testCaseId={}, type={}, name={}",
        testCase.getId(),
        testCase.getType(),
        testCase.getName());
    ...
    try {
      ...
      UUID projectId =
          testCase.getProject() != null ? UUID.fromString(testCase.getProject().getId()) : null;
      String currentUser = getCurrentUsername();

      log.info(
          "RAG 벡터화 준비 완료: testCaseId={}, projectId={}, contentLength={}, user={}",
          testCase.getId(),
          projectId,
          testCaseContent.length(),
          currentUser);

      // ICT-388: @Async로 비동기 실행되므로 즉시 반환됨 (백그라운드에서 처리)
      ragService.vectorizeTestCase(
          testCase.getId(), testCase.getName(), testCaseContent, projectId, currentUser);

      log.info(
          "ICT-388: TestCase RAG 벡터화 시작 (백그라운드 처리): testCaseId={}, name={}",
          testCase.getId(),
          testCase.getName());
      ...
      log.error(
          "ICT-388: TestCase RAG 벡터화 시작 실패: testCaseId={}, error={}",
          testCase.getId(),
          e.getMessage(),
          e);
    }
  ...
  /**
   * ICT-388: RAG 시스템에서 TestCase 삭제 비동기적으로 처리하여 CRUD 작업에 영향을 주지 않음
   *
  ...
  private void deleteTestCaseFromRAG(String testCaseId) {
    try {
      ...
      // RAG 연동 실패는 로그만 남기고 CRUD 작업에는 영향을 주지 않음
      log.error(
          "ICT-388: TestCase RAG 삭제 실패: testCaseId={}, error={}", testCaseId, e.getMessage(), e);
    }
  }
  ...
  public Map<String, Object> vectorizeAllTestCases(String projectId) {
    ...
    log.info(
        "ICT-388: 기존 TestCase 일괄 벡터화 완료 - 총: {}, 성공: {}, 실패: {}",
        testCases.size(),
        successCount,
        failureCount);
  ...
  public Optional<TestCase> findByDisplayIdWithRedirect(String displayId, String projectId) {
    ...
    Optional<com.testcase.testcasemanagement.model.DisplayIdHistory> history =
        displayIdHistoryRepository.findLatestByOldDisplayId(displayId);
  ...
  private static final String[] STANDARD_EXPORT_HEADERS = {
    "type", "name", "parentPath", "description", "preCondition", "postCondition",
    "priority", "executionType", "isAutomated", "tags"
  };
  ...
  public String generateSampleJson() {
    ...
    tags1.add("smoke"); tags1.add("login");
    tc1.set("tags", tags1);
    ArrayNode steps1 = mapper.createArrayNode();
    ObjectNode s1a = mapper.createObjectNode(); s1a.put("stepNumber",1); s1a.put("description","ID/PW 입력"); s1a.put("expectedResult","입력 완료");
    ObjectNode s1b = mapper.createObjectNode(); s1b.put("stepNumber",2); s1b.put("description","로그인 버튼 클릭"); s1b.put("expectedResult","대시보드 이동");
    steps1.add(s1a); steps1.add(s1b);
    ...
    tags2.add("login"); tags2.add("negative");
    tc2.set("tags", tags2);
    ArrayNode steps2 = mapper.createArrayNode();
    ObjectNode s2a = mapper.createObjectNode(); s2a.put("stepNumber",1); s2a.put("description","ID 입력 후 잘못된 PW 입력"); s2a.put("expectedResult","입력 완료");
    ObjectNode s2b = mapper.createObjectNode(); s2b.put("stepNumber",2); s2b.put("description","로그인 버튼 클릭"); s2b.put("expectedResult","오류 메시지 출력");
    steps2.add(s2a); steps2.add(s2b);
  ...
  private List<String[]> buildSampleDataRows() {
    return Arrays.asList(
      new String[]{"folder","로그인 테스트","","","","","MEDIUM","Manual","false","","","","",""},
      new String[]{"testcase","정상 로그인","로그인 테스트","유효한 자격증명으로 로그인","계정이 존재함","대시보드 표시","HIGH","Manual","false","smoke;login","ID/PW 입력","입력 완료","로그인 버튼 클릭","대시보드 이동"},
      new String[]{"testcase","잘못된 비밀번호","로그인 테스트","잘못된 PW로 로그인 시도","계정이 존재함","오류 메시지 표시","MEDIUM","Manual","false","login;negative","ID 입력 후 잘못된 PW 입력","입력 완료","로그인 버튼 클릭","오류 메시지 출력"}
    );
  }
  ...
  private List<Map<String, String>> parseStandardCsv(InputStream is) {
    ...
    try (CSVReader reader = new CSVReader(new InputStreamReader(is, StandardCharsets.UTF_8))) {
      ...
      throw new CsvImportException("CSV 파싱 오류: " + e.getMessage(),
          Collections.singletonList(Map.of("error", e.getMessage())));
    }
    return rows;
  }
  ...
  private List<Map<String, String>> parseExcelRaw(InputStream is) {
    ...
    try (Workbook workbook = new XSSFWorkbook(is)) {
      ...
      throw new CsvImportException("Excel 파싱 오류: " + e.getMessage(),
          Collections.singletonList(Map.of("error", e.getMessage())));
    }
    return rows;
  }
  ...
  public List<TestCase> importFromJson(InputStream is, String projectId) {
    ...
    try {
      ...
      if (testCasesNode == null || !testCasesNode.isArray()) {
        throw new CsvImportException("JSON 형식 오류: testCases 배열이 없습니다",
      ...
      throw new CsvImportException("JSON 처리 오류: " + e.getMessage(),
          Collections.singletonList(Map.of("error", e.getMessage())));
    }
  }
  ...
  private List<TestCase> processStandardRows(List<Map<String, String>> rows, Project project) {
    ...
    testCaseRepository.findByProjectIdAndType(projectId, "folder")
    ...
    for (int i = 0; i < rows.size(); i++) {
      ...
      try {
        ...
        if (name.isEmpty()) { errors.add(Map.of("row", i+2, "message", "폴더명이 비어있습니다")); continue; }
    ...
    for (int i = 0; i < rows.size(); i++) {
      ...
      try {
        String name = row.getOrDefault("name", "").trim();
        if (name.isEmpty()) { errors.add(Map.of("row", i+2, "message", "이름이 비어있습니다")); continue; }
  ...
  public ImportValidationResultDto validateImport(InputStream is, String format, String projectId) {
    ...
    try {
      switch (format.toLowerCase()) {
        case "csv":   rows = parseStandardCsv(is); break;
        case "excel": rows = parseExcelRaw(is); break;
        case "json":  rows = parseJsonToRowMaps(is); break;
      ...
      result.setTotalRows(0); result.setValidRows(0); result.setInvalidRows(0);
    ...
    for (int i = 0; i < rows.size(); i++) {
      ...
      if (name.isEmpty()) {
        errors.add(new ImportValidationResultDto.ValidationError(rowNum, "name", "이름은 필수 항목입니다", name));
        rowValid = false;
      }
      if (!type.isEmpty() && !VALID_TYPES.contains(type)) {
        errors.add(new ImportValidationResultDto.ValidationError(rowNum, "type", "유효하지 않은 타입 (folder 또는 testcase)", type));
        rowValid = false;
      }
      if (!priority.isEmpty() && !VALID_PRIORITIES.contains(priority)) {
        errors.add(new ImportValidationResultDto.ValidationError(rowNum, "priority", "유효하지 않은 우선순위 (HIGH, MEDIUM, LOW)", priority));
        rowValid = false;
      }
  ...
  private List<Map<String, String>> parseJsonToRowMaps(InputStream is) {
    ...
    try {
      ...
      throw new CsvImportException("JSON 파싱 오류: " + e.getMessage(),
          Collections.singletonList(Map.of("error", e.getMessage())));
    }
    return rows;
  }
  ...
  public byte[] exportToCsvBytes(String projectId) throws IOException {
    ...
    int maxSteps = testCases.stream().mapToInt(tc -> tc.getSteps() == null ? 0 : tc.getSteps().size()).max().orElse(0);
  ...
  public String exportToJsonString(String projectId) {
    ...
    root.put("version", "1.0");
    root.put("exportedAt", LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));
    root.put("projectCode", projectOpt.map(p -> p.getCode() != null ? p.getCode() : p.getName()).orElse("EXPORT"));
    ...
    for (TestCase tc : testCases) {
      ...
      String parentPath = tc.getParentId() != null ? idToName.getOrDefault(tc.getParentId(), "") : "";
      if (parentPath.isEmpty()) node.putNull("parentPath"); else node.put("parentPath", parentPath);
  ...
  @Transactional
  public void exportToGoogleSheetByProject(String projectId, String spreadsheetId, String sheetName)
      throws IOException, GeneralSecurityException {
    List<TestCase> testCases = getTestCasesByProjectId(projectId);
    Map<String, String> idToName = buildIdToNameMap(testCases);
    int maxSteps = testCases.stream().mapToInt(tc -> tc.getSteps() == null ? 0 : tc.getSteps().size()).max().orElse(2);
    ...
    sheetsService.spreadsheets().values()
        .clear(spreadsheetId, sheetName, new ClearValuesRequest()).execute();
    ValueRange body = new ValueRange().setValues(values);
    sheetsService.spreadsheets().values()
