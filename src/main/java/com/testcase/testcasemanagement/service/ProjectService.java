...
public class ProjectService {
  ...
  @Autowired
  private com.testcase.testcasemanagement.repository.RagChatThreadRepository
      ragChatThreadRepository;

  @Autowired
  private com.testcase.testcasemanagement.repository.DisplayIdHistoryRepository
  ...
  public Project getProject(String projectId) {
    ...
    return projectRepository
        .findByIdWithOrganization(projectId)
  ...
  public ProjectUser inviteMember(String projectId, String username, ProjectUser.ProjectRole role) {
    ...
    // 이미 멤버인지 확인
    Optional<ProjectUser> existingMember =
        projectUserRepository.findByProjectIdAndUserId(projectId, invitedUser.getId());
  ...
  /** 멤버 역할 변경 */
  public ProjectUser updateMemberRole(
      String projectId, String targetUserId, ProjectUser.ProjectRole newRole) {
    String currentUsername = securityContextUtil.getCurrentUsername();
    if (currentUsername == null) {
      throw new AccessDeniedException("인증이 필요합니다.");
    }
  ...
  public Project saveProject(Project project) {
    ...
    if (project.getId() == null || project.getId().trim().isEmpty()) {
      ...
      return createProjectWithCode(
          project.getName(), project.getCode(), project.getDescription(), organizationId);
    } else {
      // 기존 프로젝트 업데이트
  ...
  /** 코드를 포함한 프로젝트 생성 */
  public Project createProjectWithCode(
      String name, String code, String description, String organizationId) {
    String currentUsername = securityContextUtil.getCurrentUsername();
    if (currentUsername == null) {
      throw new AccessDeniedException("인증이 필요합니다.");
    }
  ...
  public Project deleteProject(String id, boolean force) {
    ...
    if (force) {
      ...
      try {
        ...
        int ragThreadCategoriesDeleted =
            entityManager
                .createNativeQuery(
      ...
      if (testCaseCount > 0 || testPlanCount > 0 || testExecutionCount > 0) {
        throw new DataIntegrityViolationException(
            String.format(
                "프로젝트에 연관 데이터가 존재합니다. (테스트케이스: %d개, 테스트플랜: %d개, 테스트실행: %d개) 강제 삭제를 사용하세요.",
  ...
  private void updateAllTestCaseDisplayIds(String projectId, String oldCode, String newCode) {
    List<com.testcase.testcasemanagement.model.TestCase> testCases =
    ...
    for (com.testcase.testcasemanagement.model.TestCase testCase : testCases) {
      if (testCase.getDisplayId() != null && testCase.getDisplayId().startsWith(oldCode + "-")) {
        ...
        String newDisplayId =
            oldDisplayId.replaceFirst("^" + java.util.regex.Pattern.quote(oldCode), newCode);
        ...
        com.testcase.testcasemanagement.model.DisplayIdHistory history =
            new com.testcase.testcasemanagement.model.DisplayIdHistory();
