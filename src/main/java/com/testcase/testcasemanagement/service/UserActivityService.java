...
public class UserActivityService {
  ...
  public UserActivity logActivityFromRequest(
      String userId,
      String activityType,
      String activityCategory,
      String targetEntityType,
  ...
  public Page<UserActivity> getUserActivities(String userId, int page, int size) {
    ...
    if (!securityContextUtil.isSystemAdmin()
        && !securityContextUtil.getCurrentUserId().equals(userId)) {
  ...
  public Map<String, Object> getUserActivityStatistics(
    ...
    statistics.put(
        "totalActivities",
        userActivityRepository.countByUserIdAndPeriod(userId, startDate, endDate));
    statistics.put(
        "distinctSessions",
        userActivityRepository.countDistinctSessionsByUserAndPeriod(userId, startDate, endDate));
    ...
    List<Object[]> dailyStats =
        userActivityRepository.getUserDailyActivityStatistics(userId, startDate, endDate);
    statistics.put("dailyActivityStatistics", dailyStats);

    // 시간대별 활동 통계
    List<Object[]> hourlyStats =
        userActivityRepository.getUserHourlyActivityStatistics(userId, startDate, endDate);
    ...
    Double averageSessionDuration =
        userActivityRepository.getAverageSessionDuration(userId, startDate, endDate);
    statistics.put(
        "averageSessionDurationMinutes",
        averageSessionDuration != null ? averageSessionDuration / 60000 : 0);
  ...
  private void detectAnomalies(UserActivity activity) {
    ...
    if (ActivityType.LOGIN_FAILED.equals(activity.getActivityType())) {
      ...
      List<UserActivity> recentFailures =
          userActivityRepository
              .findByUserIdAndActivityType(activity.getUser().getId(), ActivityType.LOGIN_FAILED)
              .stream()
  ...
  /** 사용자 활동 접근 권한 확인 Spring Security @PreAuthorize에서 사용 */
  public boolean canAccessUserActivities(String targetUserId, String currentUsername) {
    try {
      ...
      logger.error(
          "사용자 활동 접근 권한 확인 실패: targetUserId={}, currentUsername={}",
          targetUserId,
          currentUsername,
          e);
      return false;
