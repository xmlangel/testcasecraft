...
public class RagGlobalDocumentRequestService {
  ...
  @Transactional
  public RagGlobalDocumentRequestDto createRequest(
      UUID documentId, String requestedBy, String message) {
    checkRagEnabled();
    log.info(
        "Creating global document request: documentId={}, requestedBy={}", documentId, requestedBy);

    var document =
        Optional.ofNullable(ragService.getDocument(documentId))
    ...
    repository
        .findTopByDocumentIdAndStatusOrderByCreatedAtDesc(
            documentId, RagGlobalDocumentRequestStatus.PENDING)
        .ifPresent(
            existing -> {
              throw new IllegalStateException("이미 처리 대기 중인 요청이 있습니다.");
            });

    RagGlobalDocumentRequest entity =
        RagGlobalDocumentRequest.builder()
  ...
  @Transactional
  public RagGlobalDocumentRequestDto approveRequest(
      String requestId, String processedBy, String note) {
    RagGlobalDocumentRequest request =
        repository
            .findById(requestId)
  ...
  @Transactional
  public RagGlobalDocumentRequestDto rejectRequest(
      String requestId, String processedBy, String note) {
    RagGlobalDocumentRequest request =
        repository
            .findById(requestId)
