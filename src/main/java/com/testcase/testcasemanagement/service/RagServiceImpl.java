 ...
 *
 * <p>WebClient를 사용하여 FastAPI RAG 서비스와 통신합니다.
 */
@Slf4j
@Service
public class RagServiceImpl implements RagService {

  /** 글로벌 문서를 위한 특수 프로젝트 ID 모든 프로젝트에서 접근 가능한 공통 문서를 식별하는 데 사용됩니다. */
  ...
  public RagDocumentResponse analyzeDocument(UUID documentId, String parser) {
    ...
    try {
      ...
      if (response != null) {
        log.info(
            "Document analysis request accepted: documentId={}, status={}",
            documentId,
  ...
  public RagDocumentResponse generateEmbeddings(UUID documentId) {
    ...
    try {
      ...
      RagEmbeddingRequest request = RagEmbeddingRequest.builder().documentId(documentId).build();

      // POST /api/v1/embeddings/generate 호출
      RagEmbeddingResponse embeddingResponse =
          ragWebClient
              .post()
          ...
          RagDocumentResponse.builder()
              ...
              .message(
                  embeddingResponse != null
                      ? embeddingResponse.getMessage()
                      : "Embedding generation started")
              .build();

      log.info(
          "Embedding generation request accepted: documentId={}, message={}",
          documentId,
          embeddingResponse != null ? embeddingResponse.getMessage() : "N/A");
      return response;
    } catch (Exception e) {
  ...
  public RagSearchResponse searchAdvanced(RagAdvancedSearchRequest request) {
    checkRagEnabled();
    log.info(
        "Advanced search in RAG API: query={}, method={}",
    ...
    try {
      ...
      log.error(
          "Failed to perform advanced search in RAG API: query={}, method={}",
          request.getQueryText(),
          request.getSearchMethod(),
          e);
  ...
  public RagDocumentResponse getDocument(UUID documentId) {
    ...
    try {
      ...
      log.info(
          "Document retrieved successfully: documentId={}, fileName={}",
          documentId,
  ...
  public RagDocumentListResponse listDocuments(UUID projectId, Integer page, Integer size) {
    checkRagEnabled();
    log.info(
        "Listing documents from RAG API: projectId={}, page={}, size={}", projectId, page, size);
  ...
  public RagChunkListResponse getDocumentChunks(UUID documentId, Integer skip, Integer limit) {
    checkRagEnabled();
    log.info(
        "Getting document chunks from RAG API: documentId={}, skip={}, limit={}",
        documentId,
        skip,
        limit);
  ...
  /**
   * Wait until document analysis completes or fails by polling the RAG API. 문서 분석이 완료되거나 실패할 때까지
   * RAG API를 폴링하여 대기합니다.
   */
  private boolean waitForAnalysisCompletion(
      UUID documentId, Duration timeout, Duration pollInterval) {
      ...
      try {
        ...
        if (document != null && document.getAnalysisStatus() != null) {
          ...
          if ("completed".equals(status)) {
            log.info(
                "Document analysis completed: documentId={}, totalChunks={}",
                documentId,
        ...
        log.warn(
            "Failed to fetch document status while waiting for analysis: documentId={}, reason={}",
            documentId,
            e.getMessage());
      } catch (Exception e) {
        log.warn(
            "Unexpected error while waiting for analysis: documentId={}, reason={}",
            documentId,
            e.getMessage());
      }
  ...
  /**
   * Wait until embedding generation completes or fails by polling the RAG API. 임베딩 생성이 완료되거나 실패할
   * 때까지 RAG API를 폴링하여 대기합니다.
   */
  private boolean waitForEmbeddingCompletion(
      UUID documentId, Duration timeout, Duration pollInterval) {
    long deadline = System.currentTimeMillis() + timeout.toMillis();
    ...
    while (System.currentTimeMillis() < deadline) {
      try {
        ...
        log.warn(
            "Failed to fetch embedding status while waiting: documentId={}, reason={}",
            documentId,
            e.getMessage());
      } catch (Exception e) {
        log.warn(
            "Unexpected error while waiting for embedding: documentId={}, reason={}",
            documentId,
            e.getMessage());
      }
  ...
  public void vectorizeTestCase(
    ...
    try {
      ...
      try {
        ...
        analysisCompleted =
            waitForAnalysisCompletion(documentId, Duration.ofMinutes(10), Duration.ofSeconds(2));
      ...
      if (!analysisCompleted) {
        log.warn(
            "Skipping embedding generation because analysis did not complete: documentId={}",
            documentId);
        return;
      }
      ...
      try {
        ...
        boolean embeddingsCompleted =
            waitForEmbeddingCompletion(documentId, Duration.ofMinutes(10), Duration.ofSeconds(2));
  ...
  public void deleteTestCaseFromRAG(String testCaseId) {
    ...
    try {
      ...
      if (documents != null && documents.getDocuments() != null) {
        ...
        List<RagDocumentResponse> matchingDocs =
            documents.getDocuments().stream()
                .filter(
                    doc -> doc.getFileName() != null && doc.getFileName().equals(searchFileName))
                .collect(java.util.stream.Collectors.toList());

        log.info(
            "Found {} matching TestCase documents to delete: testCaseId={}",
            matchingDocs.size(),
            testCaseId);
        ...
        for (RagDocumentResponse doc : matchingDocs) {
          try {
            ...
            log.info(
                "TestCase deleted from RAG successfully: testCaseId={}, documentId={}",
                testCaseId,
                doc.getId());
          } catch (Exception e) {
  ...
  public void deleteConversationMessage(UUID messageId) {
    ...
    try {
      ragWebClient
          ...
          .uri(
              uriBuilder ->
                  uriBuilder.path("/api/v1/conversations/messages/{messageId}").build(messageId))
          .retrieve()
          .onStatus(
              status -> status.is4xxClientError(),
              clientResponse ->
  ...
  public RagDocumentListResponse listGlobalDocuments(Integer page, Integer size) {
    log.info(
        "Listing global documents from RAG API: globalProjectId={}, page={}, size={}",
        GLOBAL_PROJECT_ID,
        page,
        size);
    ...
    try {
      ...
      if (response != null) {
        log.info(
            "Global documents retrieved successfully: total={}",
  ...
  @Override
  public RagDocumentResponse moveDocumentToGlobal(
      UUID documentId, String requestedBy, String reason) {
    log.info(
        "Moving document to global project: documentId={}, requestedBy={}",
        documentId,
        requestedBy);

    try {
      RagDocumentMoveRequest moveRequest =
          RagDocumentMoveRequest.builder()
  ...
  @Override
  public RagCostEstimateResponse estimateAnalysisCost(
      UUID documentId, RagCostEstimateRequest request) {
    log.info(
        "Estimating LLM analysis cost: documentId={}, llmConfigId={}, llmProvider={}, llmModel={}",
        documentId,
    ...
    try {
      ...
      log.info(
          "Sending cost estimate request with provider={}, model={}, baseUrl={}",
          enrichedRequest.getLlmProvider(),
          enrichedRequest.getLlmModel(),
          enrichedRequest.getLlmBaseUrl());

      RagCostEstimateResponse response =
          ragWebClient
              .post()
      ...
      log.info(
          ...
          documentId,
          response != null && response.getCostBreakdown() != null
   ...
   *
   * <p>비용 추정은 실제 LLM API를 호출하지 않으므로 llm_api_key와 llm_base_url은 불필요
   */
  private RagCostEstimateRequest enrichCostEstimateRequest(RagCostEstimateRequest request) {
    ...
    try {
      ...
      log.info(
          "Enriching cost estimate request with LLM Config: id={}, provider={}, model={}",
          configId,
      ...
      log.warn("Failed to load LLM Config {}, using request as-is: {}", configId, e.getMessage());
      return request;
    }
  }
  ...
  public RagLlmAnalysisResponse analyzeDocumentWithLlm(
    ...
    try {
      ...
      log.info(
          "Sending LLM analysis request: llmConfigId={}, provider={}, model={}, baseUrl={}",
          enrichedRequest.getLlmConfigId(),
          enrichedRequest.getLlmProvider(),
          enrichedRequest.getLlmModel(),
          enrichedRequest.getLlmBaseUrl());

      RagLlmAnalysisResponse response =
          ragWebClient
              .post()
      ...
      log.info(
          ...
          documentId,
          response != null ? response.getJobId() : "N/A",
  ...
  private RagLlmAnalysisRequest enrichLlmAnalysisRequest(RagLlmAnalysisRequest request) {
    ...
    try {
      ...
      log.info(
          "Enriching request with LLM Config: id={}, provider={}, model={}",
          configId,
          llmConfig.getProvider(),
          llmConfig.getModelName());
  ...
  public RagLlmAnalysisStatusResponse getLlmAnalysisStatus(UUID documentId) {
    ...
    try {
      ...
      RagLlmAnalysisStatusResponse response =
          ragWebClient
              .get()
              ...
              .exchangeToMono(
                  clientResponse -> {
                    ...
                    if (statusCode.value() == HttpStatus.NOT_FOUND.value()) {
                      log.info(
                          "No existing LLM analysis job for document {}. Returning default"
                              + " not_started status.",
                          documentId);
                    ...
                    if (statusCode.is4xxClientError()) {
                      return clientResponse
                          .bodyToMono(String.class)
                          .flatMap(
                              error -> Mono.error(new RuntimeException("분석 상태 조회 실패: " + error)));
                    }
                    ...
                    if (statusCode.is5xxServerError()) {
                      return clientResponse
                          .bodyToMono(String.class)
                          .flatMap(
                              error -> Mono.error(new RuntimeException("RAG API 서버 에러: " + error)));
                    }
      ...
      if (response != null
          && response.getLlmConfigId() != null
          && !response.getLlmConfigId().isEmpty()) {
        try {
          llmConfigRepository
              .findById(response.getLlmConfigId())
              .ifPresent(
                  llmConfig -> {
                    response.setLlmProvider(llmConfig.getProvider().name());
                    response.setLlmModel(llmConfig.getModelName());
                    log.debug(
                        "Enriched LLM status response with provider={} and model={}",
      ...
      log.debug(
          "LLM analysis status retrieved: documentId={}, status={}, progress={}%",
          documentId,
          response != null ? response.getStatus() : "N/A",
  ...
  private RagLlmAnalysisStatusResponse buildNotStartedStatus(UUID documentId) {
    return RagLlmAnalysisStatusResponse.builder()
        ...
        .status("not_started")
        .progress(
            RagLlmAnalysisStatusResponse.ProgressInfo.builder()
                .totalChunks(0)
                .processedChunks(0)
                .percentage(0.0)
                .build())
  ...
  @Override
  public RagLlmAnalysisResultsResponse getLlmAnalysisResults(
      UUID documentId, Integer skip, Integer limit) {
    log.info(
        "Fetching LLM analysis results: documentId={}, skip={}, limit={}", documentId, skip, limit);
    ...
    try {
      ...
      log.info(
          ...
          documentId,
          response != null ? response.getTotal() : "N/A");
      return response;
    } catch (Exception e) {
  ...
  public RagLlmAnalysisStatusResponse pauseAnalysis(UUID documentId) {
    ...
    try {
      ...
      log.info(
          ...
          documentId,
          response != null && response.getProgress() != null
              ? response.getProgress().getProcessedChunks()
              : "N/A");
      return response;
    } catch (Exception e) {
    ...
    try {
      ...
      if (statusResponse.getLlmConfigId() != null && !statusResponse.getLlmConfigId().isEmpty()) {
        try {
          LlmConfig llmConfig =
              llmConfigRepository
                  .findById(statusResponse.getLlmConfigId())
                  .orElseThrow(
                      () ->
          ...
          log.info(
              "Resume with LLM Config: id={}, provider={}, model={}",
              llmConfig.getId(),
              llmConfig.getProvider(),
              llmConfig.getModelName());
          ...
          log.warn(
              "Failed to load LLM Config {}, resuming without API key: {}",
              statusResponse.getLlmConfigId(),
              e.getMessage());
        }
      }
  ...
  @Override
  public RagLlmAnalysisJobListResponse listLlmAnalysisJobs(
      UUID projectId, String status, Integer page, Integer size) {
    log.info(
        "Listing LLM analysis jobs: projectId={}, status={}, page={}, size={}",
        projectId,
        status,
        page,
        size);
  ...
  public RagAnalysisSummaryResponse createAnalysisSummary(RagAnalysisSummaryRequest request) {
    ...
    try {
      RagAnalysisSummaryResponse response =
          ragWebClient
              .post()
  ...
  public RagAnalysisSummaryResponse getAnalysisSummary(UUID summaryId) {
    ...
    try {
      ...
      log.info(
          ...
          summaryId,
          response != null ? response.getTitle() : "N/A");
      return response;
    } catch (Exception e) {
  ...
  public List<RagAnalysisSummaryResponse> listAnalysisSummaries(
    ...
    log.info(
        "Listing analysis summaries: documentId={}, userId={}, isPublic={}, skip={}, limit={}",
        documentId,
        userId,
        isPublic,
        skip,
    ...
    try {
      ...
      List<RagAnalysisSummaryResponse> response =
          ragWebClient
              .get()
              .uri(
                  uriBuilder -> {
  ...
  @Override
  public RagAnalysisSummaryResponse updateAnalysisSummary(
      UUID summaryId, RagAnalysisSummaryRequest request) {
