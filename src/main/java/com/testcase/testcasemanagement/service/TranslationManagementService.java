...
public class TranslationManagementService {
  ...
  public Language createLanguage(
      String code, String name, String nativeName, Boolean isDefault, Integer sortOrder) {
    ...
    Language language =
        new Language(code, name, nativeName, isDefault != null && isDefault, sortOrder);
  ...
  @Transactional
  public TranslationKey updateTranslationKey(
      String keyId, String category, String description, String defaultValue, Boolean isActive) {
    TranslationKey translationKey =
  ...
  public List<TranslationKey> searchTranslationKeys(
      String keyword, String category, Boolean isActive) {
    if (keyword != null && !keyword.trim().isEmpty()) {
      String searchKeyword = "%" + keyword.trim().toLowerCase() + "%";
      if (category != null && !category.isEmpty()) {
        if (isActive != null) {
          return translationKeyRepository.findByKeyNameContainingIgnoreCaseAndCategoryAndIsActive(
              keyword, category, isActive);
        } else {
          return translationKeyRepository.findByKeyNameContainingIgnoreCaseAndCategory(
              keyword, category);
        }
      } else {
        if (isActive != null) {
          return translationKeyRepository.findByKeyNameContainingIgnoreCaseAndIsActive(
              keyword, isActive);
        } else {
  ...
  /** 페이지네이션을 지원하는 번역 키 검색 (번역 존재 여부 포함) N+1 쿼리 문제를 해결하기 위해 최적화됨 */
  @Transactional(readOnly = true)
  public Map<String, Object> searchTranslationKeysWithPagination(
      String keyword, String category, Boolean isActive, int page, int size) {
    Pageable pageable = PageRequest.of(page, size, Sort.by("keyName").ascending());

    Page<TranslationKey> result =
        translationKeyRepository.searchByKeywordAndFilters(
    ...
    List<String> keyNames =
        result.getContent().stream().map(TranslationKey::getKeyName).collect(Collectors.toList());
  ...
  public List<Map<String, Object>> getCategoryTranslationCompletionStats() {
    ...
    for (Object[] row : rawStats) {
      ...
      categoryStatsMap.computeIfAbsent(
          category,
          k -> {
      ...
      languageStat.put(
          "completionPercentage", totalKeys > 0 ? (double) translatedKeys / totalKeys * 100 : 0.0);

      @SuppressWarnings("unchecked")
      List<Map<String, Object>> languages =
          (List<Map<String, Object>>) categoryStatsMap.get(category).get("languages");
  ...
  public List<Map<String, Object>> getCategoryCompletionStatsByLanguage(String languageCode) {
    List<Object[]> rawStats =
        translationRepository.getCategoryCompletionStatsByLanguage(languageCode);
        ...
        .map(
            row -> {
              ...
              stat.put(
                  ...
                  totalKeys > 0 ? (double) translatedKeys / totalKeys * 100 : 0.0);
  ...
  /** 페이지네이션을 지원하는 번역 검색 */
  @Transactional(readOnly = true)
  public Map<String, Object> searchTranslationsWithPagination(
      String languageCode, String keyName, Boolean isActive, int page, int size) {
    Pageable pageable =
        PageRequest.of(
            page,
            size,
            Sort.by("translationKey.keyName")
                .ascending()
                .and(Sort.by("language.code").ascending()));

    Page<Translation> result =
        translationRepository.searchTranslationsWithPagination(
  ...
  public Translation createOrUpdateTranslation(
      String keyName, String languageCode, String value, String context, String updatedBy) {
    // 입력 값 유효성 검사
    ...
    try {
      ...
      log.debug(
          "번역 저장 완료: ID={}, Key={}, Lang={}", savedTranslation.getId(), keyName, languageCode);
      return savedTranslation;

    } catch (Exception e) {
      log.error(
          "번역 저장 중 데이터베이스 오류: {} - {} = '{}', Error: {}",
          keyName,
          languageCode,
          value,
          e.getMessage());
  ...
  @Transactional
  public Map<String, Object> batchCreateTranslations(
      List<Map<String, String>> translations, String updatedBy) {
    int successCount = 0;
    int failureCount = 0;
  ...
  public Map<String, Object> getTranslationProgress(String languageCode) {
    ...
    List<TranslationKey> missingKeys =
        translationRepository.findMissingTranslationKeys(languageCode);
  ...
  @Transactional
  public Map<String, Object> importTranslationsFromCsv(
      InputStream csvInputStream, boolean overwrite, String updatedBy) {
    Map<String, Object> result = new HashMap<>();
    List<String> errors = new ArrayList<>();
    int successCount = 0;
    int skippedCount = 0;
    ...
    try (BufferedReader reader =
        new BufferedReader(new InputStreamReader(csvInputStream, "UTF-8"))) {
      String line;
                  ...
                  .orElseGet(
                      () -> {
                        TranslationKey newKey =
                            new TranslationKey(keyName, null, "CSV에서 가져온 키", null);
                        return translationKeyRepository.save(newKey);
                      });

          // 기존 번역 확인
          Optional<Translation> existingTranslation =
              translationRepository.findByKeyNameAndLanguageCode(keyName, languageCode);
      ...
      result.put(
          "message",
          String.format("처리 완료: %d개 성공, %d개 스킵, %d개 오류", successCount, skippedCount, errorCount));
