 ...
 * 바뀌었다. 토큰도 8,552 에서 2,994 로 줄었다. 게다가 답변이 더 좋아졌다. 요약문을 거치면 원본의 표 구조가 사라지는데, 답변 LLM 이 행을 직접 읽으면 스스로
 * 표로 정리한다.
 *
 * <p>그래서 임계를 높여 원본을 그대로 넘기는 쪽을 기본으로 삼았다. 프롬프트가 무한정 커지지 않는 것은 {@code RagSqlExecutor} 가 조회 자체에 상한을 두기
 * 때문이다. 두 값은 함께 정해야 하며 한쪽만 올리면 프롬프트가 커진다.
 */
@Service
