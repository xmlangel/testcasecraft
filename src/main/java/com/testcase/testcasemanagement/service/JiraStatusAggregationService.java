...
/** ICT-189: JIRA 상태 통합 및 중복 제거 서비스 테스트 결과에서 JIRA ID들을 추출, 중복 제거하고 현재 상태를 조회하는 서비스 */
@Service
@Slf4j
...
public class JiraStatusAggregationService {
  ...
  private final com.testcase.testcasemanagement.repository.JiraConfigRepository
  ...
  public JiraStatusSummaryDto getJiraStatusDetail(String userId, String jiraId) {
    ...
    try {
      ...
      List<TestResult> testResults =
          testResultRepository.findByJiraIssueKeyOrderByExecutedAtDesc(jiraId);
  ...
  private JiraStatusSummaryDto createJiraStatusSummary(
      String userId, String projectId, String jiraId) {
    ...
    try {
      ...
      var configEntity =
          jiraConfigRepository
              ...
              .orElseThrow(
                  () -> new IllegalStateException("JIRA 설정을 찾을 수 없습니다: " + config.getId()));
      String decryptedToken = encryptionUtil.decrypt(configEntity.getEncryptedApiToken());

      JsonNode issueInfo =
          jiraApiService.getIssueInfo(
              config.getServerUrl(), config.getUsername(), decryptedToken, jiraId);
    ...
    try {
      ...
      for (int i = 0; i < jiraIdList.size(); i += batchSize) {
        ...
        List<JsonNode> issues =
            jiraApiService.searchIssues(
                config.getServerUrl(), config.getUsername(), decryptedToken, jql, batchSize);
  ...
  /**
   * 활성화된 JIRA 설정 조회 헬퍼 메서드 1. 지정된 사용자의 활성 설정 조회 2. 연결 상태 확인 및 필요 시 자동 재연결
   *
   * @param userId 사용자 ID
   * @return 활성화된 JIRA 설정
   */
  private Optional<com.testcase.testcasemanagement.dto.JiraConfigDto> getActiveJiraConfig(
    ...
    if (!Boolean.TRUE.equals(config.getConnectionVerified())) {
      ...
      try {
        ...
        var testConfig =
            com.testcase.testcasemanagement.dto.JiraConfigDto.TestConnectionDto.builder()
  ...
  private Set<String> extractUniqueJiraIds(String projectId) {
    ...
    List<TestResult> testResults =
        testResultRepository.findByProjectIdAndJiraIssueKeyIsNotNull(projectId);

    Set<String> uniqueJiraIds =
        testResults.stream()
    ...
    log.debug(
        "고유 JIRA ID 추출 완료: 총 테스트 결과={}, 고유 JIRA ID={}", testResults.size(), uniqueJiraIds.size());
   ...
   * @return 요약 정보 리스트
   */
  public List<JiraStatusSummaryDto> getBatchJiraStatusSummary(
      String userId, List<String> jiraIssueKeys) {
    ...
    LinkedHashSet<String> normalizedKeys =
        sanitizedKeys.stream()
      ...
      if (issueInfo == null) {
        ...
        summary =
            buildSummaryFromData(
                originalKeyMap.getOrDefault(issueKey, issueKey), issueInfo, results, jiraBaseUrl);
      }
  ...
  /**
   * JIRA 상태를 데이터베이스에 동기화 API에서 조회한 JIRA 상태를 TestResult 엔티티에 저장
   *
  ...
  public int syncJiraStatusToDatabase(String userId, List<String> jiraIssueKeys) {
    ...
    log.info(
        "JIRA 상태 데이터베이스 동기화 시작: 사용자={}, 개수={} - {}", userId, jiraIssueKeys.size(), jiraIssueKeys);
    ...
    try {
      ...
      Set<String> normalizedKeys =
          sanitizedKeys.stream().map(String::toUpperCase).collect(Collectors.toSet());
      ...
      for (TestResult result : relatedResults) {
        ...
        if (currentStatus != null && !currentStatus.isEmpty()) {
          ...
          updateCount++;
          log.info(
              "JIRA 상태 업데이트: {} -> {} (테스트 결과 ID: {})",
              result.getJiraIssueKey(),
              currentStatus,
              result.getId());
        } else {
  ...
  private String calculateOverallSyncStatus(List<TestResult> testResults) {
    ...
    long syncedCount =
        testResults.stream()
            .mapToLong(
                tr ->
                    tr.getJiraSyncStatus() != null && tr.getJiraSyncStatus().name().equals("SYNCED")
                        ? 1
                        : 0)
            .sum();
    ...
    long inProgressCount =
        testResults.stream()
            .mapToLong(
                tr ->
                    tr.getJiraSyncStatus() != null
                            && tr.getJiraSyncStatus().name().equals("IN_PROGRESS")
                        ? 1
                        : 0)
            .sum();
    ...
    long failedCount =
        testResults.stream()
            .mapToLong(
                tr ->
                    tr.getJiraSyncStatus() != null && tr.getJiraSyncStatus().name().equals("FAILED")
                        ? 1
                        : 0)
            .sum();
  ...
  private JiraStatusSummaryDto buildSummaryFromData(
      String jiraId, JsonNode issueInfo, List<TestResult> testResults, String jiraBaseUrl) {
    ...
    String jiraUrl =
        jiraBaseUrl != null ? jiraApiService.generateIssueUrl(jiraBaseUrl, jiraId) : null;

    String projectId = null;
    String projectName = null;
    if (latestTest.getTestExecution() != null
        && latestTest.getTestExecution().getProject() != null) {
   ...
   * @return 실패 상태의 요약 객체
   */
  private JiraStatusSummaryDto createFailedSummary(
      String projectId, String jiraId, String errorMessage) {
    return JiraStatusSummaryDto.builder()
        .jiraIssueKey(jiraId)
        .currentStatus("확인 불가")
        .issueType("정보 없음")
