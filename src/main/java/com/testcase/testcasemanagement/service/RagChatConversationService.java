...
public class RagChatConversationService {
  ...
  public RagChatThread ensureThread(Project project, RagChatRequest request, String username) {
    if (request.getThreadId() != null) {
      return threadRepository
          .findByIdAndProject_Id(request.getThreadId(), project.getId())
          .map(
              existing -> {
                ...
                if (!CollectionUtils.isEmpty(request.getCategoryIds())) {
                  Set<RagChatCategory> categories =
                      resolveCategories(project, request.getCategoryIds());
          ...
          .orElseThrow(
              () -> new IllegalArgumentException("채팅 스레드를 찾을 수 없습니다: " + request.getThreadId()));
    }

    String inferredTitle =
        Optional.ofNullable(request.getThreadTitle())
            .filter(StringUtils::hasText)
            .orElseGet(() -> inferThreadTitle(request.getMessage()));

    RagChatThread thread =
        RagChatThread.builder()
            .project(project)
            .title(inferredTitle)
            .description(null)
            .createdBy(username)
    ...
    log.info(
        "Created new chat thread: id={}, project={}, title={}",
        saved.getId(),
        project.getId(),
        inferredTitle);
    return saved;
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "채팅 스레드를 찾을 수 없습니다: " + threadId));
    ...
    RagChatThread thread =
        threadRepository
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "채팅 스레드를 찾을 수 없습니다: " + request.getThreadId()));
    ...
    for (RagChatMessage message : messages) {
      ...
      if (StringUtils.hasText(embeddingMessageId)) {
        try {
          ...
          log.warn(
              "Failed to delete conversation embedding for message: messageId={}, embeddingId={}",
              message.getId(),
              embeddingMessageId,
              e);
        }
  ...
  private Set<RagChatCategory> resolveCategories(Project project, List<String> categoryIds) {
    ...
    Map<String, RagChatCategory> filtered =
        categories.stream()
  ...
  public RagChatMessage persistUserMessage(RagChatThread thread, String content, String username) {
    RagChatMessage message =
        RagChatMessage.builder()
            .thread(thread)
  ...
  public RagChatMessage persistAssistantMessage(
    ...
    try {
      RagConversationMessageIndexRequest indexRequest =
          RagConversationMessageIndexRequest.builder()
      ...
      RagConversationMessageIndexResponse response =
          ragService.indexConversationMessage(indexRequest);
  ...
  private Map<String, Object> buildEmbeddingMetadata(
      RagChatThread thread, List<RagChatContext> contexts, Map<String, Object> metadata) {
    ...
    embeddingMetadata.put(
        "categories", thread.getCategories().stream().map(RagChatCategory::getId).toList());
  ...
  public List<RagChatThreadDTO> getThreads(String projectId) {
    ...
    List<RagChatThreadDTO> dtos = entities.stream().map(this::toThreadDTO).toList();
  ...
  public RagChatThreadDTO toThreadDTO(RagChatThread thread) {
    LocalDateTime lastMessageAt =
        messageRepository
            .findFirstByThread_IdOrderByCreatedAtDesc(thread.getId())
    ...
    return RagChatThreadDTO.builder()
        ...
        .categories(thread.getCategories().stream().map(this::toCategoryDTO).toList())
        .build();
  }
  ...
  @Transactional
  public RagChatMessageDTO editAssistantMessage(
      RagChatMessageEditRequest request, String username) {
    RagChatMessage message =
        messageRepository
            .findById(request.getMessageId())
            .orElseThrow(
                () -> new IllegalArgumentException("메시지를 찾을 수 없습니다: " + request.getMessageId()));
    ...
    try {
      String question =
          Optional.ofNullable(message.getParentMessageId())
          ...
          RagConversationMessageIndexRequest.builder()
              ...
              .metadata(
                  buildEmbeddingMetadata(
                      message.getThread(), Collections.emptyList(), request.getMetadata()))
              .build();
      ...
      log.error(
          "Failed to re-index assistant message after edit: messageId={}", message.getId(), e);
    ...
    RagChatMessage message =
        messageRepository
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "메시지를 찾을 수 없습니다: " + messageId));
  ...
  public List<RagChatCategoryDTO> getCategories(String projectId) {
    return categoryRepository.findByProject_IdOrderByNameAsc(projectId).stream()
  ...
  public RagChatCategoryDTO createCategory(RagChatCategoryCreateRequest request, String username) {
    ...
    RagChatCategory category =
        RagChatCategory.builder()
            .project(project)
  ...
  public RagChatCategoryDTO updateCategory(RagChatCategoryUpdateRequest request, String username) {
    RagChatCategory category =
        categoryRepository
            ...
            .orElseThrow(
                () -> new IllegalArgumentException("카테고리를 찾을 수 없습니다: " + request.getCategoryId()));
