...
/** ICT-209: 테스트 결과 편집 서비스 원본 데이터 보존과 편집 이력 추적을 담당 */
@Slf4j
@Service
...
public class TestResultEditService {
  ...
  /** 새로운 편집본 생성 */
  public TestResultEditDto createEdit(
      TestResultEditDto.CreateEditRequestDto request, String currentUserId) {
    log.info(
        "Creating edit for test result: {} by user: {}",
        request.getOriginalTestResultId(),
        currentUserId);

    // 원본 테스트 결과 조회
    TestResult originalTestResult =
        testResultRepository
            .findById(request.getOriginalTestResultId())
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        "테스트 결과를 찾을 수 없습니다: " + request.getOriginalTestResultId()));
  ...
  /** 편집본 수정 (DRAFT 상태만 가능) */
  public TestResultEditDto updateEdit(
      String editId, TestResultEditDto.CreateEditRequestDto request, String currentUserId) {
  ...
  /** 편집본 승인/거부 */
  public TestResultEditDto processEditApproval(
      TestResultEditDto.EditApprovalRequestDto request, String approverId) {
    log.info("Processing edit approval: {} by approver: {}", request.getEditId(), approverId);

    TestResultEdit edit =
        editRepository
            .findById(request.getEditId())
            .orElseThrow(
                () -> new IllegalArgumentException("편집본을 찾을 수 없습니다: " + request.getEditId()));
  ...
  public Page<TestResultEditDto> getEdits(TestResultEditDto.EditFilterDto filter) {
    ...
    Page<TestResultEdit> edits =
        editRepository.findEditsByFilter(
            filter.getProjectId(),
  ...
  public List<TestResultEditDto> getEditHistory(String testResultId) {
    ...
    return edits.stream().map(this::convertToDto).collect(Collectors.toList());
  }
  ...
  public Optional<TestResultEditDto> getActiveEdit(String testResultId) {
    return editRepository.findActiveEditByTestResultId(testResultId).map(this::convertToDto);
  }
  ...
  private String createOriginalSnapshot(TestResult testResult) {
    try {
      TestResultEditDto.OriginalTestResultDto snapshot =
          TestResultEditDto.OriginalTestResultDto.builder()
              .id(testResult.getId())
  ...
  private TestResultEditDto convertToDto(TestResultEdit edit) {
    ...
    return TestResultEditDto.builder()
        ...
        .approvedByUserName(
            edit.getApprovedBy() != null ? edit.getApprovedBy().getUsername() : null)
  ...
  private TestResultEditDto.OriginalTestResultDto buildOriginalDataDto(TestResult testResult) {
    return TestResultEditDto.OriginalTestResultDto.builder()
        ...
        .executedAt(testResult.getExecutedAt())
        .executorName(
            testResult.getExecutedBy() != null ? testResult.getExecutedBy().getUsername() : null)
        .build();
  }
}
