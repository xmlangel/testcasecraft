 ...
 *
 * <p>FastAPI RAG 서비스와 통신하여 다음 기능을 제공합니다: - 문서 업로드 및 분석 - 임베딩 생성 - 유사도 검색 (벡터 검색)
 */
public interface RagService {
  ...
  /**
   * TestCase를 RAG 시스템에 벡터화하여 등록 ICT-388: @Async로 비동기 처리되므로 반환값 없음 (void)
   *
   ...
   * @return 인덱싱 결과
   */
  RagConversationMessageIndexResponse indexConversationMessage(
  ...
  /**
   * 공통 문서 업로드 (모든 프로젝트에서 접근 가능) projectId를 null로 설정하여 업로드
   *
   * @param file 업로드할 파일
   * @param uploadedBy 업로더 사용자명
   * @return 업로드된 문서 정보
  ...
  /**
   * LLM 분석 비용 추정 분석 시작 전 예상 토큰 사용량 및 비용 계산
   *
   * @param documentId 분석할 문서 ID
  ...
  /**
   * 문서의 모든 청크를 LLM으로 순차 분석 백그라운드 작업으로 실행되며, 상태 조회 API로 진행 상황 확인
   *
   * @param documentId 분석할 문서 ID
  ...
  /**
   * LLM 분석 진행 상황 조회 처리된 청크 수, 진행률, 비용 정보 반환
   *
   * @param documentId 문서 ID
  ...
  /**
   * LLM 분석 결과 조회 (페이지네이션) 청크별 분석 결과, LLM 응답, 토큰 사용량 반환
   *
   * @param documentId 문서 ID
  ...
  /**
   * LLM 분석 일시정지 현재 배치 처리 후 분석 중단 (재개 가능)
   *
   * @param documentId 문서 ID
  ...
  /**
   * LLM 분석 취소 분석 완전 중단 (재개 불가, 지금까지 결과는 보존)
   *
   * @param documentId 문서 ID
  ...
  /**
   * LLM 분석 작업 목록 조회 프로젝트별 또는 전체 LLM 분석 작업 목록 조회
   *
   ...
   * @return LLM 분석 작업 목록
   */
  RagLlmAnalysisJobListResponse listLlmAnalysisJobs(
      UUID projectId, String status, Integer page, Integer size);

  // ==================== 분석 요약 CRUD ====================

  /**
   * 분석 요약 생성 사용자가 분석 결과를 정리하여 저장
   *
   ...
   * @return 수정된 요약 정보
   */
  RagAnalysisSummaryResponse updateAnalysisSummary(
