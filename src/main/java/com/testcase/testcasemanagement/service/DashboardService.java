...
public class DashboardService {
  ...
  public List<RecentTestResultDto> getRecentTestResults(int limit) {
    return monitoringService.measureDashboardApiTime(
        "getRecentTestResults",
        () -> {
          try {
            logger.debug("getRecentTestResults with limit: {}", limit);

            return monitoringService.measureQueryTime(
                "findRecentTestResults",
                () -> {
            ...
            throw new DashboardException.DataRetrievalException(
                "최근 테스트 결과 조회에 실패했습니다: " + e.getMessage(), "RecentTestResults");
          }
        });
  }
  ...
  public List<RecentTestResultDto> getRecentTestResultsByProject(String projectId, int limit) {
    return monitoringService.measureDashboardApiTime(
        "getRecentTestResultsByProject",
        () -> {
          try {
            return monitoringService.measureQueryTime(
                "findRecentTestResultsByProject",
                () -> {
                  Pageable pageable = PageRequest.of(0, limit);
            ...
            logger.error(
                "getRecentTestResultsByProject 실행 중 오류 발생 - projectId: {}, error: {}",
                projectId,
                e.getMessage());
            throw new DashboardException.DataRetrievalException(
                "프로젝트별 최근 테스트 결과 조회에 실패했습니다: " + e.getMessage(), "ProjectRecentTestResults");
          }
        });
  }
  ...
  public List<RecentTestResultDto> getRecentTestResultsByTestPlan(String testPlanId, int limit) {
    Pageable pageable = PageRequest.of(0, limit);
    List<TestResult> results =
        testResultRepository.findRecentTestResultsByTestPlan(testPlanId, pageable);
   ...
   * @return 오픈 테스트런 담당자별 결과 목록
   */
  public List<OpenTestRunAssigneeResultDto> getOpenTestRunAssigneeResultsByProject(
   ...
   */
  private List<OpenTestRunAssigneeResultDto> processAssigneeResults(
      List<TestResult> allResults, int limit) {
    // 테스트 실행별로 그룹화
    Map<String, List<TestResult>> resultsByExecution =
        allResults.stream().collect(Collectors.groupingBy(tr -> tr.getTestExecution().getId()));
    ...
    for (Map.Entry<String, List<TestResult>> entry : resultsByExecution.entrySet()) {
      ...
      Map<String, List<TestResult>> resultsByAssignee =
      ...
      for (Map.Entry<String, List<TestResult>> assigneeEntry : resultsByAssignee.entrySet()) {
        ...
        double completionRate =
            totalTestCases > 0 ? (double) completedTestCases / totalTestCases * 100 : 0.0;
        double passRate =
            completedTestCases > 0 ? (double) passedTestCases / completedTestCases * 100 : 0.0;
    ...
    // 제한된 수만큼 반환
    return assigneeResults.stream().limit(limit).collect(Collectors.toList());
  }
  ...
  /**
   * 프로젝트의 테스트케이스 결과 추이 조회 ICT-265: 중복 데이터 제거 로직 추가 - 동일한 executionId + testCaseId 조합에서 최신 결과만 사용
   *
   * @param projectId 프로젝트 ID
   ...
   */
  public List<TestResultsTrendDto> getTestResultsTrend(
    ...
    List<TestResult> filteredByDateResults =
        allResults.stream()
            .filter(
                result -> {
                  if (result.getExecutedAt() == null) return false;
                  return !result.getExecutedAt().isBefore(startDate)
                      && !result.getExecutedAt().isAfter(endDate);
                })
            .collect(Collectors.toList());

    System.out.println(
        "ICT-265 Debug - getTestResultsTrend 기간 필터 후 개수: " + filteredByDateResults.size());

    // ICT-265: ICT-263과 동일한 중복 제거 로직 적용 (통계 계산용)
    Map<String, TestResult> latestResultsMap =
        filteredByDateResults.stream()
            .collect(
                Collectors.toMap(
    ...
    for (Map.Entry<String, Map<String, Integer>> entry : groupedData.entrySet()) {
      ...
      TestResultsTrendDto trendDto =
          new TestResultsTrendDto(date, pass, fail, blocked, skipped, notRun, completeRate, notRun);
  ...
  /**
   * 프로젝트의 테스트케이스 상태별 통계 조회 ICT-265: 중복 데이터 제거 로직 추가 - 동일한 executionId + testCaseId 조합에서 최신 결과만 사용
   *
   * @param projectId 프로젝트 ID
   ...
   * @return 진행 중인 테스트 실행 진행률 목록
   */
  public List<TestExecutionProgressDto> getInProgressTestExecutionsByProject(
      String projectId, int limit) {
    List<TestExecution> executions =
        testExecutionRepository.findInProgressExecutionsByProject(projectId);
  ...
  /**
   * ICT-129: 프로젝트 전체 통계 조회 ICT-265: 중복 데이터 제거 로직 추가 - Repository 쿼리에서 이미 중복 제거 적용됨 성능 최적화된 방식으로
   * 프로젝트의 종합 통계 정보를 제공합니다.
   *
   * @param projectId 프로젝트 ID
  ...
  public ProjectStatisticsDto getProjectStatistics(String projectId) {
    ...
    Map<String, Object> executionStats =
        testResultRepository.findProjectExecutionStatistics(projectId);

    // 3. 실행 상태 통계 조회
    Map<String, Object> statusStats =
        testResultRepository.findProjectExecutionStatusStatistics(projectId);
    ...
    Map<String, Object> priorityStats =
        testResultRepository.findProjectPriorityStatistics(projectId);
    ...
    Integer yesterdayExecutions =
        testResultRepository.countExecutionsByDateRange(projectId, yesterdayStart, yesterdayEnd);
    Integer todayExecutions =
        testResultRepository.countExecutionsByDateRange(projectId, todayStart, now);
    ...
    Integer lastWeekExecutions =
        testResultRepository.countExecutionsByDateRange(projectId, lastWeekStart, lastWeekEnd);
    Integer thisWeekExecutions =
        testResultRepository.countExecutionsByDateRange(projectId, thisWeekStart, now);

    // 4. 최근 7일/30일 평균 통과율 계산
    LocalDateTime last7DaysStart = now.minusDays(7);
    LocalDateTime last30DaysStart = now.minusDays(30);

    Double averagePassRate7Days =
        testResultRepository.calculateAveragePassRateByPeriod(projectId, last7DaysStart, now);
    Double averagePassRate30Days =
        testResultRepository.calculateAveragePassRateByPeriod(projectId, last30DaysStart, now);

    // 5. 최근 7일 중요 실패 수 조회
    Integer criticalFailures7Days =
        testResultRepository.countCriticalFailuresByPeriod(projectId, last7DaysStart, now);
    ...
    if (yesterdayExecutions != null && yesterdayExecutions > 0) {
      dailyChangeRate =
          ((double) (todayExecutions - yesterdayExecutions) / yesterdayExecutions) * 100;
    }
    ...
    if (lastWeekExecutions != null && lastWeekExecutions > 0) {
      weeklyChangeRate =
          ((double) (thisWeekExecutions - lastWeekExecutions) / lastWeekExecutions) * 100;
    }
    ...
    if (executedCases != null && executedCases > 0) {
      ...
      logger.info(
          "ICT-265 Debug - 통과율 계산: {}개 중 {}개 통과 = {}%",
          executedCases, passedCases, dto.getPassRate());
    } else {
      dto.setPassRate(0.0);
  ...
  private ProjectStatisticsDto convertToProjectStatisticsDto(
      Map<String, Object> data, String projectId) {
  ...
  /** 대시보드 성능 메트릭을 정기적으로 로그에 기록하는 메서드 스케줄링을 통해 주기적으로 호출됩니다. */
