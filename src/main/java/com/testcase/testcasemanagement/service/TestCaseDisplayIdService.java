 ...
 *
 * <p>프로젝트코드-넘버 형식의 Display ID를 생성하고 관리합니다. 예: PROJ-001, TEST-042, WEB-123
 */
@Slf4j
@Service
...
public class TestCaseDisplayIdService {
  ...
  /**
   * 프로젝트에서 코드를 추출합니다. 1. 프로젝트의 code 필드 사용 2. code가 없으면 name의 첫 4글자 대문자로 사용
   *
   * @param project 프로젝트
  ...
  public void updateDisplayId(TestCase testCase) {
    ...
    if (newDisplayId != null && !newDisplayId.equals(testCase.getDisplayId())) {
      log.info(
          "TestCase Display ID 업데이트: {} -> {} (TestCase ID: {})",
          testCase.getDisplayId(),
          newDisplayId,
          testCase.getId());
