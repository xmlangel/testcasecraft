...
/** 사용자 권한 및 역할 관리를 위한 통합 서비스 ICT-33: 사용자 권한 및 역할 관리 강화 */
@Service
@Transactional
public class UserPermissionService {
  ...
  public void addUserToOrganization(String userId, String organizationId, OrganizationRole role) {
    ...
    // 감사 로그 기록
    logPermissionChange(
        "ORGANIZATION_MEMBER_ADDED", userId, organizationId, null, role.toString(), "조직에 사용자 추가");
  }
  ...
  public void addUserToProject(String userId, String projectId, ProjectRole role) {
    ...
    // 감사 로그 기록
    logPermissionChange(
        "PROJECT_MEMBER_ADDED", userId, projectId, null, role.toString(), "프로젝트에 사용자 추가");
  }

  /** 조직 내 사용자 역할 변경 */
  public void changeOrganizationRole(
      String userId, String organizationId, OrganizationRole newRole) {
    // 권한 확인
    if (!organizationSecurityService.canManageOrganization(organizationId)) {
      throw new RuntimeException("조직 관리 권한이 없습니다.");
    }
  ...
  public void removeUserFromOrganization(String userId, String organizationId) {
    ...
    // 감사 로그 기록
    logPermissionChange(
        "ORGANIZATION_MEMBER_REMOVED", userId, organizationId, oldRole, null, "조직에서 사용자 제거");
  }
  ...
  public void removeUserFromProject(String userId, String projectId) {
    ...
    // 감사 로그 기록
    logPermissionChange(
        "PROJECT_MEMBER_REMOVED", userId, projectId, oldRole, null, "프로젝트에서 사용자 제거");
  }
  ...
  public void processBulkPermissionChanges(List<BulkPermissionChangeDto> changes) {
    ...
    // 대량 변경 완료 로그
    logPermissionChange(
        "BULK_CHANGE_COMPLETED", null, null, null, null, "대량 권한 변경 완료: " + changes.size() + "건 처리");
  }

  /** 권한 변경 시 충돌 검증 */
  @Transactional(readOnly = true)
  public List<PermissionConflictDto> validatePermissionChanges(
  ...
  private void logPermissionChange(
    ...
    log.setDetails(
        details + " | Resource: " + resourceId + " | Old: " + oldValue + " | New: " + newValue);
