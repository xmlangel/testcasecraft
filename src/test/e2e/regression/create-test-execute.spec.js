...
test.describe('테스트실행', () => {
    ...
    test('테스트 실행에서 첨부파일이 정상적으로 표시되는지 확인한다', async ({ page, projectListPage, testCasePage, testPlanPage, testExecutionPage }, testInfo) => {
        ...
        console.log(`🚀 [Worker ${workerIndex}] 테스트 환경 구축 및 실행 테스트 시작...`);
        ...
        await testExecutionPage.createNewExecution(executionName, testPlanName, true);
        ...
        // 📋 Step 7: 첨부파일 섹션 확인
        const attachmentSection = page.locator('text=파일 첨부').or(page.locator('text=첨부파일')).first();
    ...
    test('생성된 테스트 실행을 삭제한다', async ({ page, projectListPage, testExecutionPage }, testInfo) => {
        const workerIndex = testInfo.workerIndex;
        const timestamp = Date.now();
