...
async function runValidationVisualIndicatorTest() {
  console.log(
    "🧪 [ICT-344] 스프레드시트 오류 행 시각적 표시 기능 E2E 테스트 시작",
  );
  ...
  try {
    ...
    await page.waitForSelector("text=테스트케이스", {
      timeout: CONFIG.waitTimeout,
    });
    ...
    await page.screenshot({
      path: "spreadsheet-validation-debug.png",
      fullPage: true,
    });
    ...
    for (const selector of validateSelectors) {
      try {
        ...
        if (count > 0) {
          ...
          console.log(
            `✅ 검증 버튼 발견 (선택자: ${selector}), 개수: ${count}개`,
          );
          break;
        }
      } catch (e) {
    ...
    const validationDialog = await page.locator(
      '[role="dialog"]:has(text("데이터 검증"))',
    );
    ...
    if (errorAlerts > 0) {
      const firstError = await page
        .locator(".MuiAlert-errorStandard")
        .first()
        .textContent();
    ...
    // 오류가 있는 셀의 배경색 확인 시도
    const styledCells = await page
      .locator('.Spreadsheet td[style*="background"]')
      .count();
    ...
    // 성공 메시지 확인
    const successSnackbar = await page
      .locator(".MuiAlert-successStandard")
      .first();
    ...
    console.log(
      "🎉 [ICT-344] 스프레드시트 오류 행 시각적 표시 기능 E2E 테스트 완료",
    );

    return {
      success: true,
      message:
        "ICT-344 테스트 성공: 검증 기능 및 시각적 표시 기능이 정상 작동함",
      details: {
    ...
    return {
      ...
      error: error.message,
      details:
        "ICT-344 스프레드시트 오류 행 시각적 표시 기능 테스트 중 오류 발생",
    };
  } finally {
    if (browser) {
      await browser.close();
