...
test.describe('프로젝트 관리', () => {
    ...
    test('독립 프로젝트를 생성하고 삭제할 수 있어야 한다', async ({ page, loginPage, projectListPage }, testInfo) => {
        ...
        console.log(`🚀 [Worker ${workerIndex}] 독립 프로젝트 생성 테스트 시작: ${projectName}`);
        ...
        // 📋 생성: 조직 없이 생성
        await projectListPage.createProject(projectName, projectCode, '', description);
    ...
    test('조직 프로젝트를 생성하고 삭제할 수 있어야 한다', async ({ page, loginPage, projectListPage }, testInfo) => {
        const workerIndex = testInfo.workerIndex;
        const timestamp = Date.now();
        ...
        console.log(`🚀 [Worker ${workerIndex}] 조직 프로젝트 생성 테스트 시작: ${projectName}`);
        ...
        await page.waitForSelector(`[data-testid="project-name"]:has-text("${projectName}")`, { timeout: 10000 });
        console.log(`✅ 프로젝트 생성 확인: ${projectName}`);
