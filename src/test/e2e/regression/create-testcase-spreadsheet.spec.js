...
test.describe('스프레드시트를 이용한 테스트 케이스 생성 테스트', () => {
  ...
  test('스프레드시트를 통해 새로운 테스트 케이스와 폴더를 생성한다', async ({ page, projectListPage, testCasePage }) => {
    ...
    await testCasePage.fillSpreadsheetCell(-1, 7, '스프레드시트를 통해 생성된 TC 설명');
    ...
    // 저장 성공 메시지 대기
    const successLocator = page.locator('text=/배치 저장 완료|개의 테스트케이스가 저장되었습니다/');
