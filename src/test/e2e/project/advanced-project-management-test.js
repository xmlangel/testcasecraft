...
test.describe("고급 프로젝트 관리 기능 E2E 테스트", () => {
          ...
          for (const optionSelector of actionOptions) {
            try {
              ...
              if (await option.isVisible({ timeout: 1000 })) {
                ...
                // 안전한 작업만 테스트 (상태 변경)
                if (
                  optionSelector.includes("상태") ||
                  optionSelector.includes("Status")
                ) {
                  await option.click();
  ...
  test("프로젝트 정렬 및 필터링 고급 기능 테스트", async ({
    page,
  }, testInfo) => {
        ...
        if (await filterElement.isVisible({ timeout: 3000 })) {
          if (selector.includes("input")) {
            ...
            const filteredProjects = await page
              .locator('[data-testid="project-card"], .MuiCard-root')
              .count();
      ...
      try {
        ...
        if (await advancedButton.isVisible({ timeout: 2000 })) {
          ...
          const closeButton = page
            .locator(
              'button:has-text("닫기"), button:has-text("Close"), .MuiDialog-root button[aria-label*="close"]',
            )
            .first();
  ...
  test("프로젝트 내보내기 및 가져오기 기능 테스트", async ({
    page,
  }, testInfo) => {
      ...
      try {
        ...
        if (await importButton.isVisible({ timeout: 3000 })) {
          ...
          if (uploadAreaFound) {
            ...
            // 지원되는 파일 형식 정보 확인
            const supportedFormats = await page
              .locator("text=/.*.(csv|xlsx|json|xml).*/i")
              .count();
            if (supportedFormats > 0) {
              console.log(
                `📄 지원되는 파일 형식 정보 ${supportedFormats}개 발견`,
              );
            }
          }
          ...
          const closeButton = page
            .locator(
              'button:has-text("닫기"), button:has-text("Close"), button:has-text("취소"), button:has-text("Cancel")',
            )
            .first();
          if (await closeButton.isVisible({ timeout: 2000 })) {
            await closeButton.click();
      ...
      try {
        ...
        if (await shareButton.isVisible({ timeout: 3000 })) {
          ...
          const shareOptions = {
            ...
            permissions:
              'select[name="permission"], .MuiSelect-root:has-text("권한")',
              ...
              if (await element.isVisible({ timeout: 2000 })) {
                ...
                if (type === "link") {
                  ...
                  const copyButton = page
                    .locator('button:has-text("복사"), button:has-text("Copy")')
                    .first();
    ...
    for (const selector of collaboratorSelectors) {
      try {
        ...
        if (await collabButton.isVisible({ timeout: 2000 })) {
          ...
          // 협업자 목록 확인
          const collaboratorList = page.locator(
            ".collaborator-item, .MuiListItem-root, .member-item",
          );
          const collabCount = await collaboratorList.count();
          console.log(`👥 현재 협업자 수: ${collabCount}`);

          // 새 협업자 추가 버튼
          const addCollabButton = page
            .locator(
              'button:has-text("추가"), button:has-text("Add"), button:has([data-testid="AddIcon"])',
            )
            .first();
          if (await addCollabButton.isVisible({ timeout: 2000 })) {
            ...
            const userSearchInput = page
              .locator(
                'input[placeholder*="사용자"], input[placeholder*="user"], input[placeholder*="검색"]',
              )
              .first();
        ...
        if (await permButton.isVisible({ timeout: 2000 })) {
          ...
          for (const levelSelector of permissionLevels) {
            try {
              ...
              if (levelCount > 0) {
                console.log(
                  `🔐 권한 레벨 ${levelCount}개 확인: ${levelSelector}`,
                );
              }
            } catch (e) {
              continue;
      ...
      try {
        ...
        if (await activityButton.isVisible({ timeout: 2000 })) {
          ...
          const activityItems = page.locator(
            ".activity-item, .MuiTimelineItem-root, .log-entry",
          );
          ...
          const filterOptions = page.locator(
            'button:has-text("필터"), select[name="activityType"]',
          );
        ...
        if (await notifButton.isVisible({ timeout: 2000 })) {
          ...
          for (const optionSelector of notificationOptions) {
            try {
              ...
              if (optionCount > 0) {
                console.log(
                  `🔔 알림 옵션 ${optionCount}개 확인: ${optionSelector}`,
                );
              }
            } catch (e) {
              continue;
    ...
    if (projectCount > 1) {
          ...
          try {
            ...
            if (await menuButton.isVisible({ timeout: 2000 })) {
              ...
              const archiveMenuItem = page
                .locator(
                  'li:has-text("아카이브"), li:has-text("Archive"), .MuiMenuItem-root:has-text("아카이브")',
                )
                .first();
      ...
      if (archiveMenuFound) {
        ...
        const confirmationDialogElements = {
          ...
          confirmButton:
            'button:has-text("아카이브"), button:has-text("확인"), button:has-text("Archive")',
        ...
        if (
          await page
            .locator(confirmationDialogElements.message)
          ...
          const cancelButton = page
            .locator(confirmationDialogElements.cancelButton)
            .first();
          if (await cancelButton.isVisible({ timeout: 2000 })) {
            await cancelButton.click();
    ...
    for (const selector of archivedViewSelectors) {
      try {
        ...
        if (await archivedButton.isVisible({ timeout: 3000 })) {
          ...
          // 아카이브된 프로젝트 목록 확인
          const archivedProjects = page.locator(
            '[data-testid="archived-project-card"], .archived-project, .MuiCard-root',
          );
          ...
          if (archivedCount > 0) {
            ...
            for (const restoreSelector of restoreSelectors) {
              try {
                const restoreButton = firstArchivedProject
                  .locator(restoreSelector)
                  .first();
                if (await restoreButton.isVisible({ timeout: 2000 })) {
                  ...
                  const restoreConfirm = page
                    .locator('button:has-text("복원"), button:has-text("확인")')
                    .first();
            ...
            for (const deleteSelector of permanentDeleteSelectors) {
              try {
                const deleteButton = firstArchivedProject
                  .locator(deleteSelector)
                  .first();
