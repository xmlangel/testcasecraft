...
test.describe("프로젝트 전환 및 컨텍스트 관리 E2E 테스트", () => {
  ...
  async function selectProject(page, projectIndex = 0) {
        ...
        if (await tab.isVisible({ timeout: 2000 })) {
          ...
          for (const selector of projectSelectors) {
            try {
              ...
              if (count > 0) {
                console.log(
                  `✅ "${tabName}" 탭에서 프로젝트 발견: ${selector} (${count}개)`,
                );
                foundProjects = true;
                break;
              }
    ...
    if (buttonCount <= projectIndex) {
      throw new Error(
        `프로젝트 인덱스 ${projectIndex}가 사용 가능한 프로젝트 수 ${buttonCount}를 초과합니다.`,
      );
    }
  ...
  test("프로젝트 간 전환 기본 플로우 테스트", async ({ page }, testInfo) => {
    ...
    const dashboardElements = await page
      .locator(
        'h5:has-text("대시보드"), h6:has-text("QA 자동화"), [class*="dashboard"]',
      )
      .count();
    ...
    // 독립 프로젝트 탭으로 이동
    const independentTab = page.locator(
      'tab:has-text("독립 프로젝트"), [role="tab"]:has-text("독립 프로젝트"), button:has-text("독립 프로젝트")',
    );
    ...
    if (projectCount > 1) {
      ...
      const contextChanged =
        firstProjectContext.selectedProject !==
          secondProjectContext.selectedProject ||
        firstProjectContext.currentProject !==
          secondProjectContext.currentProject ||
  ...
  test("프로젝트 컨텍스트 상태 지속성 테스트", async ({ page }, testInfo) => {
    ...
    const localStoragePersisted =
      initialContext.localStorage.selectedProject ===
        restoredContext.localStorage.selectedProject &&
      initialContext.localStorage.accessToken ===
  ...
  test("프로젝트 네비게이션 히스토리 관리 테스트", async ({
    page,
  }, testInfo) => {
    ...
    const firstUrl = page.url();
    console.log(
      `📁 첫 번째 프로젝트: ${firstProject.projectTitle}, URL: ${firstUrl}`,
    );
    ...
    if (projectCount > 1) {
      ...
      const secondUrl = page.url();
      console.log(
        `📁 두 번째 프로젝트: ${secondProject.projectTitle}, URL: ${secondUrl}`,
      );
  ...
  test("프로젝트 전환 시 데이터 동기화 테스트", async ({ page }, testInfo) => {
    ...
    if (projectCount > 1) {
      ...
      const relevantApiCalls = apiCalls.filter(
        (call) =>
      ...
      // 데이터가 실제로 변경되었는지 확인
      const dataChanged =
        firstProjectData.currentUrl !== secondProjectData.currentUrl ||
      ...
      if (!loadingFound) {
        console.log(
          "⚠️ 로딩 인디케이터를 찾을 수 없음 (빠른 로딩이거나 캐시된 데이터)",
        );
      }
    } else {
      console.log("⚠️ 프로젝트가 1개뿐이어서 전환 테스트를 수행할 수 없음");
  ...
  test("프로젝트 컨텍스트 오류 처리 테스트", async ({ page }, testInfo) => {
    ...
    if (!errorHandled) {
      ...
      if (
        recoveredContext.selectedProject === null ||
        recoveredContext.selectedProject === "invalid-project-id-999999"
      ) {
