...
test.describe("폴더 생성 및 폴더 내 테스트 케이스 생성 테스트", () => {
  ...
  test("새로운 폴더를 생성하고 그 안에 테스트 케이스 10개를 생성한다", async ({
    page,
    testCasePage,
  }) => {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      await page
        .locator('div[role="button"]:has-text("테스트케이스 정보")')
        .click();
      ...
      await page
        .getByLabel("설명")
        .fill(`자동화 테스트로 생성된 ${i}번째 테스트 케이스입니다.`);
      ...
      // 전체 예상 결과 입력
      await page
        .getByLabel("Expected Results")
        .fill(`테스트 케이스의 전체 예상 결과입니다.`);
      ...
      const isExpanded = await folderLocator
        .locator("..")
      ...
      console.log(
        `✅ [${i}/10] 테스트 케이스 '${testCaseName}' 생성 및 확인 완료.`,
      );
    }

    await testCasePage.screen("create-10-testcases-complete");
    console.log(
      `🎉 폴더 '${folderName}' 및 그 안의 테스트 케이스 10개 생성 완료!`,
    );
  });
});
