...
test.describe("독립 프로젝트 생성 완성 테스트", () => {
  ...
  test("독립 프로젝트 생성 전체 플로우 완료", async ({ page }, testInfo) => {
    ...
    const orgSelect = dialog
      .locator('div[role="button"], select, .MuiSelect-root')
      .first();
    ...
    try {
      ...
      const errorMessage = dialog.locator(
        '.MuiAlert-root, .error-message, [class*="error"]',
      );
        ...
        if (response && Array.isArray(response)) {
          ...
          if (createdProject) {
            ...
            if (
              ...
              console.log(
                "🎯 organizationId가 null - 독립 프로젝트 특성 확인됨",
              );
            } else {
              console.log(
    ...
    try {
      ...
      console.log(
        "⚠️ 새 프로젝트가 UI에 즉시 표시되지 않음 (새로고침 필요할 수 있음)",
      );
    }
