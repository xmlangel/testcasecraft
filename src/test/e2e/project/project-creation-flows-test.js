...
test.describe('프로젝트 생성 플로우 E2E 테스트', () => {
  ...
  test('조직별 프로젝트 생성 플로우 테스트', async ({ page }, testInfo) => {
    ...
    // 프로젝트명 입력
    const nameInput = page.locator('input[name="name"], input[name="projectName"], input[placeholder*="이름"], input[placeholder*="name"]').first();
    ...
    for (const selector of autoMemberSelectors) {
      try {
        ...
        if (await autoMemberCheckbox.isVisible({ timeout: 2000 })) {
          ...
          console.log(`👥 멤버 자동 추가 설정 상태: ${isChecked ? '활성' : '비활성'}`);
          break;
        }
      } catch (e) {
        continue;
