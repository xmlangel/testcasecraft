...
test.describe("전체 프로젝트 생성 E2E 테스트", () => {
  ...
  test("전체 프로젝트 생성 전체 플로우 테스트", async ({ page }, testInfo) => {
    ...
    if (globalProjectSelected) {
      ...
      console.log(
        "ℹ️ 전체 프로젝트 유형 선택 옵션을 찾을 수 없음 - 기본 설정으로 진행",
      );
    }
    ...
    if (adminToken) {
      try {
        ...
        if (response && Array.isArray(response)) {
          ...
          if (createdProject) {
            console.log(
              `✅ 생성된 전체 프로젝트 API 확인: ${createdProject.name}`,
            );
            ...
            // 전체 프로젝트 특성 확인
            if (
              createdProject.visibility === "PUBLIC" ||
              createdProject.isPublic === true
            ) {
              console.log("🌐 전체 공개 프로젝트 특성 확인됨");
            } else {
              console.log(
                `ℹ️ 프로젝트 공개 설정: ${createdProject.visibility || "N/A"}`,
              );
            }
          }
        }
    ...
    console.log(
      `📊 결과: 프로젝트 "${projectName}" (코드: ${projectCode}) 생성됨`,
    );
  ...
  test("다른 사용자 계정으로 전체 프로젝트 접근 테스트 (API 중심)", async ({
    page,
  }, testInfo) => {
    ...
    if (testerToken) {
      try {
        ...
        if (response && Array.isArray(response)) {
          console.log(
            `📋 Tester가 API로 볼 수 있는 총 프로젝트 수: ${response.length}`,
          );

          const accessibleProject = response.find(
          ...
          if (accessibleProject) {
            ...
            console.log(
              "ℹ️ API에서 전체 프로젝트가 아직 Tester에게 보이지 않음",
            );
    ...
    const projectExists = await page
      ...
      .count();
    console.log(
      `🔍 UI에서 프로젝트 "${testProjectName}" 발견 수: ${projectExists}`,
    );
    ...
    for (let i = 0; i < buttons.length; i++) {
      try {
        ...
        if (await button.isVisible()) {
          ...
          console.log(
            `Button ${i + 1}: "${text}" (type="${type}", enabled=${isEnabled})`,
          );
        }
      } catch (e) {
        continue;
      ...
      for (let i = 0; i < checkboxes.length; i++) {
        try {
          ...
          if (await checkbox.isVisible()) {
            ...
            console.log(
              `Checkbox ${i + 1}: name="${name}", checked=${isChecked}`,
            );
          }
        } catch (e) {
          continue;
