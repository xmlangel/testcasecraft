...
test.describe('단순화된 다중 사용자 전체 프로젝트 접근 테스트', () => {
  ...
  test('API 기반 다중 사용자 전체 프로젝트 접근 테스트', async ({ page }, testInfo) => {
    ...
    try {
      ...
      if (testerResponse && Array.isArray(testerResponse)) {
        console.log(`📋 Tester가 볼 수 있는 프로젝트 수: ${testerResponse.length}`);
        
        const accessibleProject = testerResponse.find(p => p.name === testProjectName);
        if (accessibleProject) {
          console.log('✅ Tester API로 전체 프로젝트 접근 성공!');
          console.log(`📊 접근 가능한 프로젝트: ${accessibleProject.name}`);
          console.log(`🎯 프로젝트 ID 일치: ${accessibleProject.id === createdProject.id}`);
          ...
          console.log('ℹ️ 이는 권한 설정이나 동기화 지연으로 인한 것일 수 있음');
          
          // 프로젝트 목록 출력 (디버깅용)
          testerResponse.forEach((project, index) => {
            console.log(`프로젝트 ${index + 1}: ${project.name} (ID: ${project.id})`);
          });
        }
      } else {
    ...
    console.log(`🔍 UI에서 발견된 "${testProjectName}" 요소 수: ${projectCount}`);
    
    if (projectCount > 0) {
      console.log('✅ Tester UI에서도 전체 프로젝트 확인됨');
    } else {
      console.log('ℹ️ Tester UI에서는 즉시 보이지 않음 (정상적인 경우일 수 있음)');
    }
  ...
  test('전체 프로젝트 생성 후 즉시 다른 사용자 접근 검증', async ({ page }, testInfo) => {
      ...
      try {
        ...
        if (response && Array.isArray(response)) {
          ...
          if (quickProject) {
            ...
            console.log('ℹ️ 즉시 접근은 불가, 시간 지연 후 접근 가능할 것으로 예상');
          }
          
          console.log(`📈 Tester 전체 접근 가능 프로젝트 수: ${response.length}`);
        }
      } catch (e) {
