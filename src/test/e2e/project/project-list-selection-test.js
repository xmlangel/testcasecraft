...
test.describe("프로젝트 목록 조회 및 선택 E2E 테스트", () => {
  ...
  test("프로젝트 목록 조회 및 기본 표시 검증", async ({ page }, testInfo) => {
    ...
    await page.waitForSelector(
      '[data-testid="project-card"], .MuiCard-root, [class*="project"]',
      { timeout: 10000 },
    );
    ...
    // 프로젝트명이 표시되는지 확인
    const projectTitle = firstProject
  ...
  test("프로젝트 선택 및 전환 기능 테스트", async ({ page }, testInfo) => {
    ...
    // 프로젝트명 추출
    const projectTitle = await firstProject
      .locator('h6, h5, [class*="title"], .MuiTypography-h6, .MuiTypography-h5')
      .first()
      .textContent();
  ...
  test("프로젝트 상세 정보 조회 테스트", async ({ page }, testInfo) => {
    ...
    const projectInfo = {
      title:
        (await firstProject
          .locator(
            'h6, h5, [class*="title"], .MuiTypography-h6, .MuiTypography-h5',
          )
          .first()
          .textContent()) || "N/A",
      description:
        (await firstProject
          .locator('[class*="description"], .MuiTypography-body2')
          .first()
          .textContent()) || "N/A",
    };
    ...
    const metadataElements = await firstProject
      .locator('.MuiChip-root, [class*="chip"], [class*="badge"]')
      .count();
  ...
  test("프로젝트 목록 페이지네이션 테스트", async ({ page }, testInfo) => {
        ...
        if (await pagination.isVisible({ timeout: 2000 })) {
          ...
          if (pageButtons > 1) {
            ...
            const nextButton = pagination
              .locator(
                'button:has-text("2"), button[aria-label*="2"], button[title*="2"]',
              )
              .first();
            if (await nextButton.isVisible({ timeout: 1000 })) {
              ...
              const firstButton = pagination
                .locator(
                  'button:has-text("1"), button[aria-label*="1"], button[title*="1"]',
                )
                .first();
    ...
    if (!paginationFound) {
      console.log(
        "⚠️ 페이지네이션 컨트롤을 찾을 수 없음 (프로젝트 수가 적을 수 있음)",
      );
    }
