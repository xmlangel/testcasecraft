...
test.describe("조직별 프로젝트 생성 E2E 테스트", () => {
      ...
      try {
        ...
        for (let i = 0; i < count; i++) {
          ...
          if (await button.isVisible({ timeout: 2000 })) {
            createButton = button;
            console.log(
              `🆕 조직 프로젝트 생성 버튼 발견: ${selector} (${i}번째)`,
            );
            break;
          }
        }
  ...
  test("조직별 프로젝트 생성 - 기본 플로우", async ({ page }, testInfo) => {
    ...
    // 다이얼로그가 닫혔는지 확인
    try {
      await page.waitForSelector('.MuiDialog-root, [role="dialog"]', {
        state: "detached",
        timeout: 5000,
      });
    ...
    await takeSuccessScreenshot(
      page,
      testInfo,
      "org-project-creation-basic-flow",
    );
  ...
  test("조직 선택 및 멤버 미리보기 테스트", async ({ page }, testInfo) => {
    ...
    const orgSelect = page
      .locator(
        '.MuiFormControl-root:has(.MuiInputLabel-root:has-text("소속 조직")) .MuiSelect-root',
      )
      .first();
    if (await orgSelect.isVisible({ timeout: 3000 })) {
      ...
      if (await firstOrg.isVisible({ timeout: 2000 })) {
        ...
        for (const selector of memberPreviewSelectors) {
          try {
            ...
            if (await memberPreview.isVisible({ timeout: 3000 })) {
              ...
              // 멤버 수 확인
              const memberItems = memberPreview.locator(
                ".member-item, .MuiListItem-root, li",
              );
        ...
        if (!memberPreviewVisible) {
          console.log(
            "ℹ️ 멤버 미리보기가 별도 UI로 표시되지 않거나 다른 방식으로 구현됨",
          );
        }
      }
    }
  ...
  test("프로젝트 코드 중복 검사 테스트", async ({ page }, testInfo) => {
    ...
    const codeInput = page
      .locator('input[name="code"], input[name="projectCode"]')
      .first();
    if (await codeInput.isVisible({ timeout: 3000 })) {
      ...
      if (!duplicateErrorFound) {
        console.log(
          "ℹ️ 중복 검사 오류가 표시되지 않음 (코드가 중복되지 않거나 검사가 다른 시점에 수행됨)",
        );
      }
  ...
  test("필수 필드 유효성 검사 테스트", async ({ page }, testInfo) => {
    ...
    const submitButton = page
      .locator('.MuiDialogActions-root button:has-text("생성")')
      .first();
    if (await submitButton.isVisible({ timeout: 3000 })) {
      await submitButton.click({ force: true });
    ...
    if (validationErrors.length > 0) {
      console.log(
        `❌ 유효성 검사 오류 메시지들: ${validationErrors.join(", ")}`,
      );
    } else {
    ...
    const nameInput = page
      .locator('input[name="name"], input[name="projectName"]')
      .first();
  ...
  test("프로젝트 생성 완료 후 즉시 접근 테스트", async ({ page }, testInfo) => {
    ...
    if (!projectAccessible) {
      console.log(
        "⚠️ 생성된 프로젝트에 즉시 접근할 수 없거나 다른 방식으로 구현됨",
      );
    }
