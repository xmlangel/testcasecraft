...
test.describe("프로젝트 설정 관리 및 대시보드 E2E 테스트", () => {
  ...
  test("프로젝트 정보 수정 기능 테스트 (수정됨)", async ({
    page,
  }, testInfo) => {
      ...
      try {
        ...
        if (projects && Array.isArray(projects)) {
          ...
          if (targetProject) {
            ...
            console.log(
              `📊 프로젝트 정보: ${targetProject.name} (ID: ${targetProject.id})`,
            );
          }
        }
      } catch (e) {
    ...
    if (projectId && token) {
      ...
      try {
        ...
        console.log(
          `📡 수정 API 응답: ${updateResult.status} (성공: ${updateResult.success})`,
        );
  ...
  test("멤버 관리 기능 테스트 (수정됨)", async ({ page }, testInfo) => {
    ...
    const memberTestProjectName = await createTestProject(
      page,
      "멤버관리테스트프로젝트",
    );
    ...
    if (adminToken) {
      try {
        ...
        if (projects && Array.isArray(projects)) {
          ...
          if (testProject) {
            console.log(
              `📊 멤버 관리 테스트 프로젝트: ${testProject.name} (ID: ${testProject.id})`,
            );
          }
        }
      } catch (e) {
    ...
    if (testProject && adminToken) {
      ...
      try {
        const addMemberResponse = await page.evaluate(
          async (data) => {
            const { token, projectId } = data;
            const res = await fetch(
              `http://localhost:8080/api/projects/${projectId}/members?username=tester&role=MEMBER`,
              {
                method: "POST",
                headers: {
                  Authorization: `Bearer ${token}`,
        ...
        console.log(
          `➕ 멤버 추가 API 응답: ${addMemberResponse.status} (성공: ${addMemberResponse.success})`,
        );
      ...
      if (testerToken) {
        try {
          const testerProjectAccess = await page.evaluate(
            async (data) => {
              const { token, projectId } = data;
              const res = await fetch(
                `http://localhost:8080/api/projects/${projectId}`,
          ...
          console.log(
            `🔍 Tester 프로젝트 접근: ${testerProjectAccess.status} (성공: ${testerProjectAccess.success})`,
          );
  ...
  test("프로젝트 삭제 기능 테스트 (수정됨)", async ({ page }, testInfo) => {
    ...
    const deleteTestProjectName = await createTestProject(
      page,
      "삭제테스트프로젝트",
    );
    ...
    if (adminToken) {
      try {
        ...
        if (projects && Array.isArray(projects)) {
          projectToDelete = projects.find(
            (p) => p.name === deleteTestProjectName,
          );
          if (projectToDelete) {
            console.log(
              `🎯 삭제 대상 프로젝트: ${projectToDelete.name} (ID: ${projectToDelete.id})`,
            );
          }
        }
      } catch (e) {
    ...
    if (projectToDelete && testerToken) {
      try {
        ...
        console.log(
          `🚫 Tester 삭제 시도 결과: ${deleteAttempt.status} (${deleteAttempt.statusText})`,
        );
        ...
        if (deleteAttempt.status === 403 || deleteAttempt.status === 401) {
          ...
          console.log(
            "⚠️ 권한 제어 확인 필요 - Tester가 삭제 권한을 가지고 있음",
          );
        }
      } catch (e) {
    ...
    if (projectToDelete && adminToken) {
      try {
        const adminDeleteResult = await page.evaluate(
          async (data) => {
            const { token, projectId } = data;
            const res = await fetch(
              `http://localhost:8080/api/projects/${projectId}`,
        ...
        console.log(
          `🗑️ Admin 삭제 시도 결과: ${adminDeleteResult.status} (${adminDeleteResult.statusText})`,
        );
        ...
        if (adminDeleteResult.success) {
          ...
          if (remainingProjects) {
            const deletedProjectExists = remainingProjects.find(
