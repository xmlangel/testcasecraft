...
test.describe("대시보드 메인 화면 E2E 테스트", () => {
  ...
  test("대시보드 메인 화면 정상 렌더링 확인", async ({ page }, testInfo) => {
    ...
    await expect(
      page.locator(
        'h1:has-text("대시보드"), h4:has-text("대시보드"), h5:has-text("대시보드")',
      ),
    ).toBeVisible();
  ...
  test("대시보드 차트 및 시각화 요소 확인", async ({ page }, testInfo) => {
    ...
    if (
      (await organizationStats.count()) > 0 ||
      (await projectStats.count()) > 0
    ) {
      console.log("✅ CountUp 통계 숫자 표시 확인");
    }

    // 조직명 텍스트 확인 (범례나 조직 목록에서)
    const organizationTexts = page.locator(
      "text=개발팀, text=QA팀, text=데브옵스팀",
    );
  ...
  test("대시보드 네비게이션 메뉴 동작 확인", async ({ page }, testInfo) => {
    ...
    await expect(
      page.locator('.MuiButton-root:has-text("대시보드")'),
    ).toBeVisible(); // 버튼만 선택
  ...
  test("대시보드 빠른 액션 버튼 및 상호작용 확인", async ({
    page,
  }, testInfo) => {
    ...
    await expect(
      page.locator(
        'h1:has-text("프로젝트 선택"), h4:has-text("프로젝트 선택"), h5:has-text("프로젝트 선택")',
      ),
    ).toBeVisible();
  ...
  test("대시보드 Material-UI 컴포넌트 렌더링 확인", async ({
    page,
  }, testInfo) => {
    ...
    await takeSuccessScreenshot(
      page,
      testInfo,
      "dashboard-material-ui-components",
    );
  });
});
