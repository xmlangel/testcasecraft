...
test.describe("대시보드 통계 위젯 및 차트 E2E 테스트", () => {
  ...
  test("통계 카드 렌더링 확인", async ({ page }, testInfo) => {
    ...
    const currentUrl = page.url();
    expect(
      currentUrl.includes("/projects/") || currentUrl.includes("/dashboard"),
    ).toBeTruthy();
    ...
    // 최근 업데이트 Chip 확인
    await expect(
      page.locator('.MuiChip-root:has-text("최근 업데이트")'),
    ).toBeVisible();
    console.log("✅ 최근 업데이트 Chip 확인");

    // 프로젝트 정보 요약 카드가 있는지 확인 (프로젝트가 선택된 경우)
    const projectInfoCard = page
      .locator('.MuiPaper-root:has(.MuiChip-root:has-text("총 테스트케이스"))')
      .first();
    if (await projectInfoCard.isVisible()) {
      ...
      // 총 테스트케이스 수 Chip 확인
      await expect(
        page.locator('.MuiChip-root:has-text("총 테스트케이스")'),
      ).toBeVisible();
      console.log("✅ 총 테스트케이스 수 표시 확인");

      // 프로젝트 멤버 수 Chip 확인
      await expect(
        page.locator('.MuiChip-root:has-text("프로젝트 멤버")'),
      ).toBeVisible();
    ...
    const statisticsPapers = page.locator(".MuiPaper-root").filter({
      hasText: /최근 테스트케이스 결과|테스트케이스 결과 추이|오픈 테스트런/,
    });
  ...
  test("PieChart 차트 렌더링 확인", async ({ page }, testInfo) => {
    ...
    // 최근 테스트케이스 결과 위젯 확인
    const pieChartWidget = page
      .locator('.MuiPaper-root:has-text("최근 테스트케이스 결과")')
      .first();
    ...
    const legendItems = pieChartWidget
      .locator("div")
      .filter({ hasText: /성공|실패|차단됨|미실행/ });
    ...
    const hasSuccessLabel = await pieChartWidget
      .locator("text=성공")
      .first()
      .isVisible();
    const hasFailLabel = await pieChartWidget
      .locator("text=실패")
      .first()
      .isVisible();
    console.log(
      `✅ 테스트 결과 라벨 확인: 성공(${hasSuccessLabel}), 실패(${hasFailLabel})`,
    );
  ...
  test("LineChart 차트 렌더링 확인", async ({ page }, testInfo) => {
    ...
    // 테스트케이스 결과 추이 위젯 확인
    const lineChartWidget = page
      .locator('.MuiPaper-root:has-text("테스트케이스 결과 추이")')
      .first();
    ...
    await expect(
      lineChartWidget
        .locator('.MuiFormControl-root:has-text("최근 15일")')
        .first(),
    ).toBeVisible();
    ...
    await expect(
      lineChartWidget.locator("g.recharts-xAxis").first(),
    ).toBeVisible();
    await expect(
      lineChartWidget.locator("g.recharts-yAxis").first(),
    ).toBeVisible();
    ...
    // 범례 확인 (첫 번째 요소만 선택)
    const legendElements = lineChartWidget
    ...
    const notRunLineChartWidget = page
      .locator(
        '.MuiPaper-root:has-text("오픈 테스트런 미실행 테스트케이스 추이")',
      )
      .first();
  ...
  test("BarChart 차트 렌더링 확인", async ({ page }, testInfo) => {
    ...
    const barChartWidget1 = page
      .locator('.MuiPaper-root:has-text("오픈 테스트런별 테스트케이스 결과")')
      .first();
    ...
    if (barCount1 === 0) {
      ...
      console.log(
        `ℹ️ recharts-bar-rectangle가 없음, 일반 rect 요소: ${barCount1}개`,
      );
    ...
    const barChartWidget2 = page
      .locator(
        '.MuiPaper-root:has-text("오픈 테스트런 담당자별 테스트케이스 결과")',
      )
      .first();
    ...
    if (barCount2 === 0) {
      ...
      console.log(
        `ℹ️ 두 번째 BarChart recharts-bar-rectangle가 없음, 일반 rect 요소: ${barCount2}개`,
      );
    ...
    const legends1 = barChartWidget1
      .locator(".recharts-legend-wrapper")
      .first();
    const legends2 = barChartWidget2
      .locator(".recharts-legend-wrapper")
      .first();
  ...
  test("차트 인터랙션 기능 확인", async ({ page }, testInfo) => {
    ...
    const initialBoxShadow = await firstWidget.evaluate(
    ...
    const hoveredBoxShadow = await firstWidget.evaluate(
      (el) => getComputedStyle(el).boxShadow,
    );
    ...
    if (await pieChart.isVisible()) {
      ...
      if (svgBox) {
        console.log(
          `✅ PieChart 인터랙션 영역 확인: ${svgBox.width}x${svgBox.height}`,
        );
      }
    }
  ...
  test("테스트 플랜 셀렉터 및 최근 결과 테이블 확인", async ({
    page,
  }, testInfo) => {
    ...
    const testPlanWidget = page
      .locator('.MuiPaper-root:has-text("테스트 플랜별 최근 테스트 결과")')
      .first();
    await expect(testPlanWidget).toBeVisible();
    console.log("✅ 테스트 플랜 위젯 확인");

    // 프로젝트가 선택된 경우 테스트 플랜 셀렉터 확인
    const testPlanSelector = testPlanWidget
    ...
    if (await testPlanSelector.isVisible()) {
      ...
      const noProjectMessage =
        testPlanWidget.locator("text=프로젝트를 선택해주세요");
    ...
    const recentResultsArea = testPlanWidget.locator(
      '[data-testid="recent-test-results"], .recent-test-results, div:has(table)',
    );
    ...
    // 로딩 상태나 빈 데이터 상태 확인
    const loadingIndicator = testPlanWidget.locator(
      '.MuiCircularProgress-root, [role="progressbar"]',
    );
    const emptyMessage = testPlanWidget.locator(
      "text=데이터가 없습니다, text=결과가 없습니다",
    );
    ...
    await takeSuccessScreenshot(
      page,
      testInfo,
      "testplan-selector-results-table",
    );
  });
  ...
  test("전체 통계 위젯 레이아웃 및 반응형 확인", async ({ page }, testInfo) => {
    ...
    const allWidgets = page.locator(".MuiPaper-root").filter({
      hasText:
        /최근 테스트케이스 결과|테스트케이스 결과 추이|오픈 테스트런|테스트 플랜별/,
    });
    ...
    console.log(
      `📏 모바일 뷰 스크롤 가능: ${isScrollable} (body: ${bodyHeight}px, window: ${windowHeight}px)`,
    );
    ...
    await takeSuccessScreenshot(
      page,
      testInfo,
      "responsive-statistics-widgets-mobile",
    );
  });
});
