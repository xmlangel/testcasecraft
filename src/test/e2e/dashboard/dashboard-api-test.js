...
test.describe('대시보드 백엔드 API E2E 테스트', () => {
  ...
  test('프로젝트 테스트 결과 요약 API 테스트 (파이차트용)', async ({ page }) => {
    console.log('📊 테스트 결과 요약 API 테스트 시작...');

    // 직접 API 호출 테스트
    const directApiResponse = await fetch(`http://localhost:8080/api/dashboard/projects/${projectId}/test-results-summary`, {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Content-Type': 'application/json'
      }
    ...
    console.log(`✅ API 응답 데이터 검증 완료 - 총 케이스: ${responseData.totalCases}, 완료율: ${responseData.completeRate}%`);
  });
  ...
  test('프로젝트 테스트 결과 추이 API 테스트 (라인차트용)', async () => {
    ...
    // 직접 API 호출 테스트 (7일간)
    const directApiResponse = await fetch(`http://localhost:8080/api/dashboard/projects/${projectId}/test-results-trend?days=7`, {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Content-Type': 'application/json'
      }
    ...
    console.log(`✅ 추이 API 응답 데이터 검증 완료 - 데이터 포인트: ${responseData.dataCount}개`);

    // 다른 기간으로도 테스트 (30일)
    const extendedApiResponse = await fetch(`http://localhost:8080/api/dashboard/projects/${projectId}/test-results-trend?days=30`, {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Content-Type': 'application/json'
      }
    ...
    console.log(`✅ 30일 추이 데이터도 정상 응답: ${extendedData.dataCount}개 포인트`);
  });
  ...
  test('오픈 테스트런 결과 API 테스트 (바차트용)', async () => {
    ...
    // 직접 API 호출 테스트
    const directApiResponse = await fetch(`http://localhost:8080/api/dashboard/projects/${projectId}/open-testrun-results?limit=5`, {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Content-Type': 'application/json'
      }
    ...
    expect(responseData.resultCount).toBe(responseData.openTestRunResults.length);
    ...
    console.log(`✅ 오픈 테스트런 API 응답 데이터 검증 완료 - 결과: ${responseData.resultCount}개`);
  });
  ...
  test('프로젝트 대시보드 종합 정보 API 테스트', async () => {
    ...
    // 직접 API 호출 테스트
    const directApiResponse = await fetch(`http://localhost:8080/api/dashboard/projects/${projectId}/overview`, {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Content-Type': 'application/json'
      }
    ...
    console.log(`✅ 종합 정보 API 검증 완료 - 프로젝트: ${responseData.projectName}, 총 케이스: ${basicStats.totalTestCases}`);
  });
  ...
  test('프로젝트 전체 통계 API 테스트 (ICT-129)', async () => {
    ...
    // 직접 API 호출 테스트
    const directApiResponse = await fetch(`http://localhost:8080/api/dashboard/projects/${projectId}/statistics`, {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Content-Type': 'application/json'
      }
    ...
    console.log(`✅ 전체 통계 API 검증 완료 - 총 케이스: ${responseData.totalTestCases}, 실행률: ${responseData.executionRate}%`);
  });
  ...
  test('API 오류 처리 및 권한 검증', async () => {
    ...
    // 잘못된 프로젝트 ID로 테스트
    const invalidProjectResponse = await fetch(`http://localhost:8080/api/dashboard/projects/invalid-project-id/test-results-summary`, {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Content-Type': 'application/json'
      }
    ...
    console.log(`ℹ️ 잘못된 프로젝트 ID 응답 상태: ${invalidProjectResponse.status}`);
    // 상태가 200이 아니거나, 200이면 에러 메시지가 있어야 함
    if (invalidProjectResponse.status === 200) {
      const errorData = await invalidProjectResponse.json();
      // 에러 응답이거나 빈 데이터여야 함
      console.log(`ℹ️ 200 응답이지만 데이터 확인: ${JSON.stringify(errorData).slice(0, 100)}...`);
    }
    console.log(`✅ 잘못된 프로젝트 ID 처리 확인: ${invalidProjectResponse.status}`);

    // 권한 없는 요청 테스트 (토큰 없이)
    const unauthorizedResponse = await fetch(`http://localhost:8080/api/dashboard/projects/${projectId}/test-results-summary`, {
      headers: {
        'Content-Type': 'application/json'
      }
    });
    ...
    // 잘못된 토큰으로 테스트
    const invalidTokenResponse = await fetch(`http://localhost:8080/api/dashboard/projects/${projectId}/test-results-summary`, {
      headers: {
  ...
  test('프론트엔드 차트 렌더링과 API 데이터 연동 확인', async ({ page }) => {
    ...
    // 페이지 기본 요소 확인
    const pageElements = await page.locator('div, svg, h1, h2, h3, h4, h5, h6').count();
