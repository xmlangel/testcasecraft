...
test.describe("대시보드 레이아웃 및 네비게이션 E2E 테스트", () => {
  ...
  test("대시보드 헤더 영역 요소 확인", async ({ page }, testInfo) => {
    ...
    // 사용자 정보 영역 (아바타) 확인
    await expect(
      page.locator(".MuiToolbar-root .MuiAvatar-root"),
    ).toBeVisible();
    ...
    const navButtons = [
      {
        text: "대시보드",
        selector: '.MuiAppBar-root .MuiButton-root:has-text("대시보드")',
      },
      {
        text: "조직 관리",
        selector: '.MuiAppBar-root .MuiButton-root:has-text("조직 관리")',
      },
      {
        text: "사용자 관리",
        selector: '.MuiAppBar-root .MuiButton-root:has-text("사용자 관리")',
      },
      {
        text: "프로젝트 선택",
        selector: '.MuiAppBar-root .MuiButton-root:has-text("프로젝트 선택")',
      },
    ];
  ...
  test("네비게이션 메뉴 클릭 및 페이지 이동 확인", async ({
    page,
  }, testInfo) => {
    ...
    await expect(
      page.locator(
        'h1:has-text("조직 관리"), h4:has-text("조직 관리"), h5:has-text("조직 관리")',
      ),
    ).toBeVisible();
    ...
    const isProjectUrl =
      currentUrl.includes("/projects") || currentUrl.includes("/project");
    ...
    await expect(
      page
        .locator(
          'h1:has-text("프로젝트"), h4:has-text("프로젝트"), h5:has-text("프로젝트")',
        )
        .first(),
    ).toBeVisible();
    ...
    await takeSuccessScreenshot(
      page,
      testInfo,
      "navigation-menu-page-transitions",
    );
  });
  ...
  test("현재 페이지 활성 상태 표시 확인", async ({ page }, testInfo) => {
    ...
    const dashboardButton = page.locator(
      '.MuiAppBar-root .MuiButton-root:has-text("대시보드")',
    );
    ...
    const orgButton = page.locator(
      '.MuiAppBar-root .MuiButton-root:has-text("조직 관리")',
    );
    ...
    const projectButton = page.locator(
      '.MuiAppBar-root .MuiButton-root:has-text("프로젝트 선택")',
    );
  ...
  test("반응형 디자인 breakpoint 동작 확인", async ({ page }, testInfo) => {
    ...
    await expect(
      page.locator('.MuiAppBar-root .MuiButton-root:has-text("대시보드")'),
    ).toBeVisible();
    await expect(
      page.locator('.MuiAppBar-root .MuiButton-root:has-text("조직 관리")'),
    ).toBeVisible();
    await expect(
      page.locator('.MuiAppBar-root .MuiButton-root:has-text("사용자 관리")'),
    ).toBeVisible();
    await expect(
      page.locator('.MuiAppBar-root .MuiButton-root:has-text("프로젝트 선택")'),
    ).toBeVisible();
  ...
  test("사용자 정보 정확한 표시 확인", async ({ page }, testInfo) => {
    ...
    await takeSuccessScreenshot(
      page,
      testInfo,
      "user-info-display-verification",
    );
  });

  test("레이아웃 구조 및 Material-UI 컴포넌트 확인", async ({
    page,
  }, testInfo) => {
  ...
  test("키보드 네비게이션 및 접근성 확인", async ({ page }, testInfo) => {
    ...
    // ARIA 속성 확인
    const appBarRole = await page
      .locator(".MuiAppBar-root")
      .getAttribute("role");
    console.log(`♿ AppBar ARIA role: ${appBarRole || "none"}`);

    const toolbarRole = await page
      .locator(".MuiToolbar-root")
      .getAttribute("role");
