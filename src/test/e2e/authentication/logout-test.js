...
test.describe('로그아웃 플로우 테스트', () => {
    test.beforeEach(async ({ page }) => {
        ...
        await page.waitForSelector('button[aria-label="user menu"]', { timeout: 5000 });
    });
    ...
    test('로그아웃 후 대시보드 직접 접근 방지', async ({ page }, testInfo) => {
        ...
        // 성공 스크린샷 촬영
        await takeSuccessScreenshot(page, testInfo, 'logout-dashboard-access-prevention');
    });
    ...
    test('로그아웃 후 브라우저 뒤로가기 방지', async ({ page }, testInfo) => {
        ...
        if (hasLoginForm) {
            ...
            const userMenu = await page.locator('button[aria-label="user menu"]').isVisible();
        ...
        // 성공 스크린샷 촬영
        await takeSuccessScreenshot(page, testInfo, 'logout-back-button-prevention');
    });
    ...
    test('로그아웃 버튼 중복 클릭 방지', async ({ page }, testInfo) => {
        ...
        // 성공 스크린샷 촬영
        await takeSuccessScreenshot(page, testInfo, 'logout-duplicate-click-prevention');
    });
    ...
    test('로그아웃 후 재로그인 정상 동작', async ({ page }, testInfo) => {
        ...
        await page.waitForSelector('button[aria-label="user menu"]', { timeout: 10000 });
    ...
    test('네트워크 오류 시 로그아웃 처리', async ({ page }, testInfo) => {
        ...
        // 성공 스크린샷 촬영
        await takeSuccessScreenshot(page, testInfo, 'logout-network-error-handling');
    });
});