...
test.describe('세션 관리 및 토큰 처리 테스트', () => {
    ...
    test('페이지 새로고침 시 세션 유지 확인', async ({ page }, testInfo) => {
        ...
        // 성공 스크린샷 촬영
        await takeSuccessScreenshot(page, testInfo, 'session-persistence-after-refresh');
    });
    ...
    test('다중 탭에서 세션 동기화 확인', async ({ page, context }, testInfo) => {
        ...
        await page2.waitForSelector('h5:has-text("프로젝트 선택")', { timeout: 10000 });
    ...
    test('토큰 갱신 시나리오 테스트', async ({ page }, testInfo) => {
        ...
        // 원래 토큰 저장
        const originalAccessToken = await page.evaluate(() => localStorage.getItem('accessToken'));
        const originalRefreshToken = await page.evaluate(() => localStorage.getItem('refreshToken'));
        ...
        for (let i = 0; i < 3; i++) {
            await page.goto('http://localhost:3000');
            await page.waitForSelector('h5:has-text("프로젝트 선택")', { timeout: 5000 });
            await new Promise(resolve => setTimeout(resolve, 1000));
        }
        ...
        // 토큰이 여전히 유효한지 확인
        const finalAccessToken = await page.evaluate(() => localStorage.getItem('accessToken'));
        const finalRefreshToken = await page.evaluate(() => localStorage.getItem('refreshToken'));
    ...
    test('잘못된 토큰 형식 처리 확인', async ({ page }, testInfo) => {
        ...
        // 성공 스크린샷 촬영
        await takeSuccessScreenshot(page, testInfo, 'invalid-token-format-handling');
    });
    ...
    test('네트워크 오류 시 토큰 갱신 실패 처리', async ({ page }, testInfo) => {
        ...
        // 성공 스크린샷 촬영
        await takeSuccessScreenshot(page, testInfo, 'network-error-token-refresh-failure');
    });
});