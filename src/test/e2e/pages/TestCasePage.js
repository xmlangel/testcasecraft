...
class TestCasePage extends BasePage {
  ...
  constructor(page, testInfo) {
    ...
    this.modeExpandButton = page.locator(
      '[data-testid="input-mode-expand-button"]',
    );
    this.individualFormButton = page.locator(
      '[data-testid="mode-individual-button"]',
    );
    this.spreadsheetButton = page.locator(
      '[data-testid="mode-spreadsheet-button"]',
    );

    // Form sections
    this.basicInfoAccordion = page.locator(
      '[data-testid="accordion-basic-info"]',
    );
    this.stepsAccordion = page.locator('[data-testid="accordion-steps"]');
    this.expectedResultsAccordion = page.locator(
      '[data-testid="accordion-expected-results"]',
    );

    // Form elements
    this.createFormTitle = page.locator(
      'h6:has-text("테스트케이스"), h6:has-text("Test Case")',
    );
    ...
    this.preconditionInput = page.locator(
      '[data-testid="testcase-precondition-input"]',
    );
    this.overallExpectedInput = page.locator(
      '[data-testid="testcase-overall-expected-input"]',
    );
    ...
    this.addRowButton = page.locator(
      'button:has-text("행 추가"), button:has-text("Add Row")',
    );
  ...
  async selectIndividualFormMode() {
    if (await this.modeExpandButton.isVisible()) {
      const isExpanded =
        await this.modeExpandButton.getAttribute("aria-expanded");
      if (isExpanded !== "true") {
  ...
  async fillIndividualForm(data) {
    ...
    if (await this.basicInfoAccordion.isVisible()) {
      const isExpanded =
        await this.basicInfoAccordion.getAttribute("aria-expanded");
      if (isExpanded !== "true") {
    ...
    const innerBasicAccordion = this.page
      .locator(
        'h6:has-text("테스트케이스 정보"), h6:has-text("Test Case Info")',
      )
      .locator("xpath=./../..");
    if (await innerBasicAccordion.isVisible()) {
      const isExpanded =
        await innerBasicAccordion.getAttribute("aria-expanded");
      if (isExpanded !== "true") {
    ...
    if (data.description) {
      const descEditor = this.page
        .locator('[data-testid="testcase-description-input"] textarea')
        .nth(0);
    ...
    if (data.precondition) {
      const preEditor = this.page
        .locator('[data-testid="testcase-precondition-input"] textarea')
        .nth(0);
    ...
    if (data.steps) {
      if (await this.stepsAccordion.isVisible()) {
        const isExpanded =
          await this.stepsAccordion.getAttribute("aria-expanded");
        if (isExpanded !== "true") {
      ...
      for (let i = 0; i < data.steps.length; i++) {
        ...
        await this.page.waitForTimeout(300);
        const stepDesc = this.page.locator(
          `[data-testid="step-description-${i + 1}"]`,
        );
        const stepExpected = this.page.locator(
          `[data-testid="step-expected-${i + 1}"]`,
        );
    ...
    if (data.overallExpected) {
      if (await this.expectedResultsAccordion.isVisible()) {
        const isExpanded =
          await this.expectedResultsAccordion.getAttribute("aria-expanded");
        if (isExpanded !== "true") {
          await this.expectedResultsAccordion.click();
          await this.page.waitForTimeout(500);
        }
      }
      const overallEditor = this.page
        .locator('[data-testid="testcase-overall-expected-input"] textarea')
        .nth(0);
  ...
  async saveForm() {
    ...
    await this.page.waitForURL(/\/projects\/[a-f0-9-]+\/testcases/, {
      timeout: 30000,
    });
  }
  ...
  async deleteTestCase(name) {
    ...
    const deleteBtn = this.page
      .locator(`[data-testid^="testcase-delete-button-"]`)
      .first();
    if (await deleteBtn.isVisible()) {
      await deleteBtn.click();
      await this.page
        .locator('[data-testid="testcase-delete-confirm-button"]')
        .click();
      await this.page.waitForTimeout(500);
    }
  }
