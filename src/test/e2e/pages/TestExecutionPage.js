...
class TestExecutionPage extends BasePage {
  ...
  constructor(page, testInfo) {
    ...
    this.newExecutionButton = page.locator(
      '[data-testid="execution-add-button"]',
    );

    // Form elements
    this.executionNameInput = page.locator(
    ...
    this.startImmediatelyCheckbox = page.locator(
      '[data-testid="execution-start-immediately-checkbox"]',
    );
    ...
    this.deleteButton = page.locator(
      '[data-testid^="execution-delete-button-"]',
    );
    ...
    // Execution controls
    this.startExecutionButton = page.locator(
      '[data-testid="execution-start-button"]',
    );
    this.completeExecutionButton = page.locator(
      '[data-testid="execution-complete-button"]',
    );
    this.restartExecutionButton = page.locator(
      '[data-testid="execution-restart-button"]',
    );

    // Result Entry
    this.resultInputButton = page.locator(
      '[data-testid^="execution-table-result-button-"]',
    );
    this.resultSaveButton = page.locator('[data-testid="result-save-button"]');
    this.resultCancelButton = page.locator(
    ...
    // Pagination
    this.listPagination = page.locator(
      '[data-testid="execution-list-pagination"]',
    );
    this.tablePagination = page.locator(
  ...
  async createNewExecution(name, planNameOrId, startImmediately = false) {
    ...
    if (planNameOrId) {
      ...
      const optionById = this.page.locator(
        `[data-testid="execution-plan-option-${planNameOrId}"]`,
      );
      if (await optionById.isVisible({ timeout: 2000 })) {
        await optionById.click();
      } else {
        await this.page
          .locator(`role=option >> text="${planNameOrId}"`)
          .click();
      }
    } else {
      ...
      const option = this.page
        .locator('[data-testid^="execution-plan-option-"]')
        .nth(1);
      await option.click();
    }
    ...
    while (await firstDelete.isVisible()) {
      ...
      if (!(await firstDelete.isVisible())) {
        const nextBtn = this.page.locator(
          '[data-testid="execution-list-pagination"] button[aria-label="Go to next page"]',
        );
  ...
  async findExecutionAcrossPages(name) {
    let item = this.page
      .locator(`[data-testid^="execution-item-"]:has-text("${name}")`)
      .first();
    let found = await item.isVisible();

    while (!found) {
      const nextBtn = this.listPagination.locator(
  ...
  async findTestCaseInTableAcrossPages(testCaseId) {
    const btn = this.page.locator(
      `[data-testid="execution-table-result-button-${testCaseId}"]`,
    );
    ...
    while (!found && currentPage < maxPages) {
      const nextBtn = this.tablePagination.locator(
        'button[aria-label="Go to next page"]',
      );
      if ((await nextBtn.isVisible()) && (await nextBtn.isEnabled())) {
        await nextBtn.click();
  ...
  async startExecution() {
    ...
    await this.completeExecutionButton.waitFor({
      state: "visible",
      timeout: 15000,
    });
  ...
  async openResultInputDialog(testCaseId) {
    ...
    if (testCaseId) {
      ...
      if (!found) {
        throw new Error(
          `Test Case ${testCaseId} not found in execution table (checked across pages).`,
        );
      }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (i % 5 === 0) {
        console.log(
          `Waiting for Result button to be enabled (attempt ${i})...`,
        );
      }
      await this.page.waitForTimeout(500);
    }
    ...
    if (!isEnabled) {
      throw new Error(
        'Test Execution "Result Input" button remained disabled even after 10s wait. Check if execution status is "INPROGRESS".',
      );
    }
  ...
  async enterResult(testCaseId, result, notes) {
    ...
    const resultBtn = this.page.locator(
      `[data-testid="execution-table-result-button-${testCaseId}"]`,
    );
    await resultBtn.click();

    await this.page
      .locator(`[data-testid="result-selector-button-${result}"]`)
      .click();
