...
class ProjectListPage extends BasePage {
    ...
    constructor(page, testInfo) {
        super(page, testInfo);
        this.projectSelectButton = page.locator('[data-testid="project-selection-nav"]');
        this.projectCards = page.locator('[data-testid^="project-card-"]');
        this.newProjectButton = page.locator('[data-testid="new-project-button"]');
        this.openProjectButtons = page.locator('[data-testid="open-project-button"]');
        ...
        this.projectOrgSelect = page.locator('[data-testid="project-organization-select"]');
        this.projectDescriptionInput = page.locator('[data-testid="project-description-field"] textarea:not([aria-hidden="true"])');
        this.saveButton = page.locator('[data-testid="project-save-button"]');

        // Menu & Delete
        this.moreMenuButton = (name) => page.locator(`[data-testid^="project-card-"]:has([data-testid="project-name"]:has-text("${name}"))`).locator('[data-testid="project-more-menu-button"]');
        this.deleteMenuItem = page.locator('[data-testid="project-delete-menu-item"]');
        this.deleteConfirmButton = page.locator('[data-testid="project-delete-confirm-button"]');
    }

    async waitForLoad() {
        await this.page.waitForURL('**/projects', { timeout: 15000 });
        // 로딩 중인 경우를 대비해 스피너가 사라질 때까지 대기하거나 카드가 나타날 때까지 대기
        await this.page.waitForSelector('.MuiCircularProgress-root', { state: 'hidden', timeout: 15000 });
    }
    ...
    async openProjectByName(name) {
        const card = this.page.locator(`[data-testid^="project-card-"]:has([data-testid="project-name"]:has-text("${name}"))`);
    ...
    async createProject(name, code, organization = '', description = '') {
        ...
        await this.page.waitForSelector(`[data-testid="project-name"]:has-text("${name}")`, { timeout: 8000 });
    }
    ...
    async deleteProject(name) {
        ...
        await this.page.waitForSelector(`[data-testid="project-name"]:has-text("${name}")`, { state: 'detached', timeout: 5000 });
    }

    async isProjectInList(name) {
        return await this.page.locator(`[data-testid="project-name"]:has-text("${name}")`).isVisible();
    }
}
