...
class AutomationPage extends BasePage {
  constructor(page, testInfo) {
    ...
    this.uploadNameInput = page.locator(
      '[data-testid="automation-upload-name-input"]',
    );
    this.uploadDescriptionInput = page.locator(
      '[data-testid="automation-upload-description-input"]',
    );
    this.submitUploadButton = page.locator(
    ...
    this.detailRefreshButton = page.locator(
    ...
    this.editCaseButton = page.locator(
    ...
    this.editStatusSelect = page.locator(
      '[data-testid="automation-case-status-select"]',
    );
    this.editPrioritySelect = page.locator(
      '[data-testid="automation-case-priority-select"]',
    );
    this.editTagsInput = page.locator(
      '[data-testid="automation-case-tags-input"]',
    );
    this.editNotesInputWrapper = page.locator(
      '[data-testid="automation-case-notes-input"]',
    );
    this.editNotesTextarea = this.editNotesInputWrapper
      .locator("textarea, input")
      .first();
    this.saveCaseButton = page.locator(
      '[data-testid="automation-case-save-button"]',
    );
    this.cancelCaseButton = page.locator(
  ...
  async viewResultByName(fileName) {
    const link = this.page.locator(
      `[data-testid="automation-file-link"]:has-text("${fileName}")`,
    );
    await link.click();
  }
  ...
  async editTestCaseByIndex(index = 0, data) {
    ...
    if (data.status) {
      ...
      await this.page
        .locator(
          `[data-testid="automation-case-status-option-${data.status.toUpperCase()}"]`,
        )
        .click();
    }
    ...
    if (data.priority) {
      ...
      await this.page
        .locator(
          `[data-testid="automation-case-priority-option-${data.priority.toUpperCase()}"]`,
        )
        .click();
    }
