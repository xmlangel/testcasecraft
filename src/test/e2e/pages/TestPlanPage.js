...
class TestPlanPage extends BasePage {
  ...
  constructor(page, testInfo) {
    ...
    this.addTestPlanButton = page.locator(
    ...
    this.descriptionInput = page.locator(
      '[data-testid="testplan-description-input"]',
    );
  ...
  async selectAllTestCases() {
    ...
    const specificCheckbox = this.page.locator(
      'input[data-testid="testcase-check-all-input"]',
    );
    ...
    if (data.testCaseIds && data.testCaseIds.length > 0) {
      for (const id of data.testCaseIds) {
        ...
        const checkbox = this.page.locator(
          `[data-testid="testcase-checkbox-${id}"]`,
        );
  ...
  async saveTestPlan() {
    ...
    const formDialog = this.page.locator('[role="dialog"]').filter({
      hasText:
        /테스트 플랜 생성|테스트 플랜 수정|Create Test Plan|Edit Test Plan/,
    });
  ...
  async deleteTestPlan(planName) {
    ...
    const row = this.page
      .locator("tr")
      .filter({ has: this.page.locator("td", { hasText: planName }) });
    await row.waitFor({ state: "visible", timeout: 10000 });

    const deleteButton = row.locator(
      '[data-testid^="testplan-delete-button-"]',
    );
    ...
    const confirmDialog = this.page.locator('[role="dialog"]').filter({
      hasText: /정말로 이 테스트 플랜을 삭제하시겠습니까|Are you sure/,
    });
    await confirmDialog.waitFor({ state: "visible", timeout: 5000 });

    await confirmDialog
      .locator('[data-testid="testplan-delete-confirm-button"]')
      .click();
  ...
  async startExecution(planId) {
    const executeButton = this.page.locator(
      `[data-testid="testplan-execute-button-${planId}"]`,
    );
  ...
  async openAutomatedLinkDialog(planId) {
    const linkButton = this.page.locator(
      `[data-testid="testplan-link-button-${planId}"]`,
    );
    await linkButton.click();
    await this.page
      .locator('[data-testid="automation-search-input"]')
  ...
  async searchAndLinkAutomation(searchTerm, resultId) {
    ...
    const linkButton = this.page.locator(
      `[data-testid="automation-link-button-${resultId}"]`,
    );
    await linkButton.click();
    // Wait for link status to update
    await this.page
      .locator(`[data-testid="automation-unlink-button-${resultId}"]`)
      .waitFor({ state: "visible" });
  }
  ...
  async goToLastPage() {
    const lastPageButton = this.page.locator(
      'button[aria-label="Go to last page"]',
    );
    if (
      (await lastPageButton.isVisible()) &&
      (await lastPageButton.isEnabled())
    ) {
  ...
  async findPlanAcrossPages(planName) {
    ...
    const firstPageButton = this.page.locator(
      'button[aria-label="Go to first page"]',
    );
    if (
      (await firstPageButton.isVisible()) &&
      (await firstPageButton.isEnabled())
    ) {
  ...
  async searchPlan(name) {
    ...
    const searchInput = this.page.locator(
      '[data-testid="testplan-search-input"]',
    );
  ...
  async deleteMatchingPlans() {
    const deleteBtn = this.page
      .locator('[data-testid^="testplan-delete-button-"]')
      .first();
