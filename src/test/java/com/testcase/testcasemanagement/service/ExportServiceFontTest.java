...
/** ICT-234: ExportService 폰트 로딩 및 캐싱 메커니즘 단위 테스트 폰트 캐싱, 폴백 로직, 성능 최적화 기능 검증 */
public class ExportServiceFontTest {
  ...
  /** ICT-234: 폰트 캐싱 메커니즘 테스트 연속적인 PDF 생성 시 캐시된 폰트 사용 확인 */
  @Test
  public void testFontCachingMechanism() throws Exception {
    ...
    System.out.println(
        "성능 향상률: "
            + String.format("%.1f", ((double) (duration1 - duration2) / duration1 * 100))
            + "%");
  ...
  /** ICT-234: PDF 생성 기본 기능 테스트 한글 텍스트가 포함된 PDF가 정상 생성되는지 확인 */
  @Test
  ...
  /** ICT-234: 폰트 로딩 로직 접근성 테스트 private 메서드들이 정상 동작하는지 리플렉션으로 확인 */
  @Test
  ...
  /** ICT-234: 캐시 TTL 테스트 캐시 만료 시간 경과 후 새로운 폰트 로딩 확인 */
  @Test
  ...
  private Page<TestResultReportDto> createTestReportData() {
    TestResultReportDto report1 =
        TestResultReportDto.builder()
  ...
  private TestResultFilterDto createTestFilter() {
    TestResultFilterDto filter =
        TestResultFilterDto.builder()
            ...
            .displayColumns(
                Arrays.asList(
                    "folderPath", "testCaseName", "result", "executedAt", "executorName", "notes"))
