 ...
 *
 * <p>의도 분석은 스키마 설명과 판단 규칙을 합쳐 1,968자를 매번 보낸다. 실측에서 이 호출 하나가 7.3~7.6초에 1,275~1,414 토큰을 썼다. 인사 한마디에도
 * 같은 값을 보내는 것이 낭비다.
 *
 * <p><b>오판의 비용이 비대칭이다.</b> 잘못 걸러 조회를 건너뛰면 답변 품질이 떨어지지만, 잘못 통과시켜도 지금과 같다. 그래서 확실한 것만 걸러야 하고, 이 시험의
 * 절반은 <b>걸러서는 안 되는 질문</b>을 확인한다.
 */
