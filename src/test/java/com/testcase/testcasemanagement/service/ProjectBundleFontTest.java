...
/** ICT-238: 프로젝트 번들 한글 폰트 추가 검증 테스트 환경 독립적인 PDF 생성을 위한 번들 폰트 기능 검증 */
public class ProjectBundleFontTest {
  ...
  public void testProjectBundleFontResourcesExist() {
    ...
    String[] bundledFontPaths = {
      "/fonts/NanumGothicCoding.ttf", "/fonts/NanumGothicCoding-Bold.ttf"
    };
  ...
  public void testConsistentOutputAcrossEnvironments() {
    ...
    try {
      ...
      double sizeDiff =
          Math.abs(pdf1.length - pdf2.length) / (double) Math.max(pdf1.length, pdf2.length);
