 ...
 *
 * <p>회귀 배경: 제공자 공식 문서가 base URL 을 경로 포함 형태(예: {@code https://openrouter.ai/api/v1})로 안내하는데, 클라이언트가
 * 그 값 뒤에 자신의 호출 경로를 다시 붙여 {@code /api/v1/api/v1/chat/completions} 로 요청해 404 가 났다.
 */
public class LlmApiUrlNormalizerTest {
  ...
  public Object[][] convergingUrls() {
    return new Object[][] {
      ...
      {OPENWEBUI_PATH, "http://192.168.0.10:3000", "http://192.168.0.10:3000/api/chat/completions"},
      {
        OPENWEBUI_PATH,
        "http://192.168.0.10:3000/api",
        "http://192.168.0.10:3000/api/chat/completions"
      },
      {PERPLEXITY_PATH, "https://api.perplexity.ai", "https://api.perplexity.ai/chat/completions"},
      {
        PERPLEXITY_PATH,
  ...
  @Test(dataProvider = "동일주소로수렴", description = "URL 을 어떤 형태로 넣어도 호출 주소가 하나로 수렴한다")
