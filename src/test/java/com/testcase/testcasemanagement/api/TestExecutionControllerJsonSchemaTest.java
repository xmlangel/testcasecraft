...
@Feature("테스트실행")
@ActiveProfiles("test")
public class TestExecutionControllerJsonSchemaTest
  ...
  public void globalSetup() throws Exception {
    ...
    // Schema 로딩
    try (InputStream is =
        getClass().getClassLoader().getResourceAsStream("schemas/testexecution-schema.json")) {
      testExecutionSchema = new String(is.readAllBytes(), StandardCharsets.UTF_8);
    }
    try (InputStream is =
        getClass().getClassLoader().getResourceAsStream("schemas/testexecution-list-schema.json")) {
