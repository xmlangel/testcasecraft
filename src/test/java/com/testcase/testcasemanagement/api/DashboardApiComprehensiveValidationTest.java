 ...
 *
 * <p>이 클래스는 모든 대시보드 API에 대한 종합적인 JSON Schema 검증을 수행합니다. - 스키마 구조 검증 - 데이터 타입 검증 - 비즈니스 로직 검증 - 에지
 * 케이스 검증
 */
...
@ActiveProfiles("test")
public class DashboardApiComprehensiveValidationTest
  ...
  public void validateRecentTestResultsApiComprehensively() throws Exception {
    ...
    validateJsonSchema(
        response1.getBody().asString(), "schemas/dashboard-recent-test-results-schema.json");
    ...
    validateJsonSchema(
        response2.getBody().asString(), "schemas/dashboard-recent-test-results-schema.json");
    ...
    validateJsonSchema(
        response3.getBody().asString(), "schemas/dashboard-recent-test-results-schema.json");
    ...
    for (Map<String, Object> result : results) {
      if (result.get("projectId") != null) {
        Assert.assertEquals(
            result.get("projectId"), testProjectId, "프로젝트별 조회 시 모든 결과는 요청한 프로젝트의 것이어야 합니다.");
      }
    }
  }
  ...
  public void validateOpenTestRunAssigneeResultsApiComprehensively() throws Exception {
    ...
    for (Map<String, Object> result : results) {
      ...
      Assert.assertTrue(passRate >= 0 && passRate <= 100, "통과율은 0-100 범위여야 합니다. 실제: " + passRate);
    }
  }
  ...
  public void validateTestResultsTrendApiComprehensively() throws Exception {
    ...
    validateJsonSchema(
        response1.getBody().asString(), "schemas/dashboard-test-results-trend-schema.json");
    attachResponse(response1);
    ...
    validateJsonSchema(
        response2.getBody().asString(), "schemas/dashboard-test-results-trend-schema.json");
    ...
    for (Map<String, Object> result : results) {
      ...
      Assert.assertTrue(
          dateStr.matches("^(0[1-9]|1[0-2])/(0[1-9]|[12][0-9]|3[01])/[0-9]{4}$"),
          "날짜 형식이 MM/dd/yyyy 패턴이어야 합니다. 실제: " + dateStr);

      Integer completeRate = (Integer) result.get("completeRate");
      Assert.assertTrue(
          completeRate >= 0 && completeRate <= 100, "완료율은 0-100 범위여야 합니다. 실제: " + completeRate);
    }
  }
  ...
  public void validateProjectStatisticsApiComprehensively() throws Exception {
    ...
    Assert.assertEquals(statistics.get("projectId"), testProjectId, "프로젝트 ID가 요청한 ID와 일치해야 합니다.");
    ...
    Assert.assertTrue(executionRate >= 0 && executionRate <= 100, "실행률은 0-100 범위여야 합니다.");
    Assert.assertTrue(passRate >= 0 && passRate <= 100, "통과율은 0-100 범위여야 합니다.");
    Assert.assertTrue(testCoverage >= 0 && testCoverage <= 100, "테스트 커버리지는 0-100 범위여야 합니다.");

    // 개수 필드 검증 (음수가 아니어야 함)
    Assert.assertTrue((Integer) statistics.get("totalTestCases") >= 0, "총 테스트케이스 수는 0 이상이어야 합니다.");
    Assert.assertTrue(
        (Integer) statistics.get("executedTestCases") >= 0, "실행된 테스트케이스 수는 0 이상이어야 합니다.");

    // 필수 필드 존재 검증
    Assert.assertNotNull(statistics.get("calculatedAt"), "calculatedAt 필드는 null이 아니어야 합니다.");
    Assert.assertTrue(
        (Integer) statistics.get("dataFreshnessMinutes") >= 0, "데이터 신선도는 0 이상이어야 합니다.");
  }
  ...
  public void validateTestCaseStatisticsApiComprehensively() throws Exception {
    ...
    validateJsonSchema(
        response1.getBody().asString(), "schemas/dashboard-test-case-statistics-schema.json");
    attachResponse(response1);
    ...
    Assert.assertEquals(
        totalCases, sum, "각 상태별 개수의 합이 전체 개수와 일치해야 합니다. 전체: " + totalCases + ", 합계: " + sum);
  }
  ...
  public void validateTestExecutionProgressApiComprehensively() throws Exception {
    ...
    validateJsonSchema(
        response1.getBody().asString(), "schemas/dashboard-test-execution-progress-schema.json");
    attachResponse(response1);
    ...
    validateJsonSchema(
        response2.getBody().asString(), "schemas/dashboard-test-execution-progress-schema.json");
    ...
    for (Map<String, Object> result : results) {
      ...
      Assert.assertTrue(completionRate >= 0 && completionRate <= 100, "완료율은 0-100 범위여야 합니다.");
      Assert.assertTrue(passRate >= 0 && passRate <= 100, "통과율은 0-100 범위여야 합니다.");

      Assert.assertTrue((Integer) result.get("totalTestCases") >= 0, "총 테스트케이스 수는 0 이상이어야 합니다.");
      Assert.assertTrue(
          (Integer) result.get("completedTestCases") >= 0, "완료된 테스트케이스 수는 0 이상이어야 합니다.");
    }
  }
