...
public class AuthControllerJsonSchemaTest extends AbstractTestNGSpringContextTests {
  ...
  public void globalSetup() throws Exception {
    ...
    // Schema loading
    try (InputStream is =
        getClass().getClassLoader().getResourceAsStream("schemas/auth-register-schema.json")) {
      registerSchema = new String(is.readAllBytes(), StandardCharsets.UTF_8);
    }

    try (InputStream is =
        getClass().getClassLoader().getResourceAsStream("schemas/auth-login-schema.json")) {
      loginSchema = new String(is.readAllBytes(), StandardCharsets.UTF_8);
    }

    try (InputStream is =
        getClass().getClassLoader().getResourceAsStream("schemas/auth-userinfo-schema.json")) {
  ...
  public void registerUserWithExistingUsernameTest() {
    ...
    // 1. 정상 등록
    given().contentType(ContentType.JSON).body(existingUser).post("/api/auth/register");
