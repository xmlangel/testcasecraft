...
/** 조직 관리 API 통합 테스트 실제 API 호출을 통한 전체 워크플로우 테스트 */
@SpringBootTest
...
public class OrganizationControllerIntegrationTest extends AbstractTestNGSpringContextTests {
  ...
  @MockBean private OrganizationSecurityService organizationSecurityService;
  ...
  void setUp() {
    ...
    when(organizationSecurityService.isOrganizationMember(any(), any())).thenReturn(true);
    when(organizationSecurityService.hasOrganizationAdminRole(any(), any())).thenReturn(true);
    when(organizationSecurityService.canManageOrganization(any(), any())).thenReturn(true);
    when(organizationSecurityService.canAccessOrganization(any(), any())).thenReturn(true);
    when(organizationSecurityService.isOrganizationOwner(any(), any())).thenReturn(true);
    when(organizationSecurityService.canInviteMembers(any(), any())).thenReturn(true);
    when(organizationSecurityService.canDeleteOrganization(any(), any())).thenReturn(true);
  ...
  void 조직_전체_워크플로우_테스트() throws Exception {
    ...
    // 4. 조직 상세 정보 조회
    mockMvc
        .perform(get("/api/organizations/" + orgId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.name", is("테스트 조직")));
    // organizationUsers 검증 제거 - 순환 참조 문제로 컨트롤러에서 제외됨
    ...
    mockMvc
        .perform(get("/api/organizations/" + orgId + "/members"))
        .andExpect(status().isOk())
    ...
    // 8. 조직 삭제
    mockMvc.perform(delete("/api/organizations/" + orgId)).andExpect(status().isNoContent());
  ...
  void 인증_없이_접근_시_401_반환() throws Exception {
    ...
    mockMvc
        .perform(post("/api/organizations").contentType(MediaType.APPLICATION_JSON).content("{}"))
  ...
  void 존재하지_않는_조직_접근_시_403_반환() throws Exception {
    ...
    mockMvc.perform(get("/api/organizations/nonexistent-id")).andExpect(status().isForbidden());

    mockMvc.perform(delete("/api/organizations/nonexistent-id")).andExpect(status().isForbidden());
  }
}
