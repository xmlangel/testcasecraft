...
/** 조직 관리 API JSON 스키마 검증 테스트 API 응답이 정의된 JSON 스키마를 준수하는지 검증 */
@SpringBootTest
...
public class OrganizationControllerJsonSchemaTest extends AbstractTestNGSpringContextTests {
  ...
  void 조직_목록_조회_응답_스키마_검증() throws Exception {
    ...
    MvcResult result =
        mockMvc
            .perform(get("/api/organizations").header("Authorization", "Bearer " + adminToken))
    ...
    InputStream schemaStream =
        new ClassPathResource("schemas/organization-list-response-schema.json").getInputStream();
    ...
    assertTrue(validationMessages.isEmpty(), "응답이 스키마를 준수하지 않습니다: " + validationMessages);
  }
  ...
  void 조직_생성_응답_스키마_검증() throws Exception {
    ...
    // 조직 생성 응답 스키마 로드
    InputStream schemaStream =
        new ClassPathResource("schemas/organization-create-response-schema.json").getInputStream();
    JsonSchema schema = jsonSchemaFactory.getSchema(schemaStream);
  ...
  void 조직_상세_조회_응답_스키마_검증() throws Exception {
    ...
    MvcResult result =
        mockMvc
            .perform(
                get("/api/organizations/" + orgId).header("Authorization", "Bearer " + adminToken))
            .andExpect(status().isOk())
            .andReturn();
  ...
  void 멤버_초대_응답_스키마_검증() throws Exception {
    ...
    // 멤버 초대 응답 스키마 로드
    InputStream schemaStream =
        new ClassPathResource("schemas/organization-member-schema.json").getInputStream();
    JsonSchema schema = jsonSchemaFactory.getSchema(schemaStream);
  ...
  void 멤버_목록_조회_응답_스키마_검증() throws Exception {
    ...
    // 멤버 목록 스키마 로드
    InputStream schemaStream =
        new ClassPathResource("schemas/organization-members-response-schema.json").getInputStream();
    JsonSchema schema = jsonSchemaFactory.getSchema(schemaStream);
