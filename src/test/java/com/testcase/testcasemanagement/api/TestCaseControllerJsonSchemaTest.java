...
public class TestCaseControllerJsonSchemaTest extends AbstractTestNGSpringContextTests {
  ...
  public void globalSetup() throws Exception {
    ...
    // JSON Schema 로드
    try (InputStream is =
        getClass().getClassLoader().getResourceAsStream("schemas/testcase-schema.json")) {
      testCaseSchema = new String(is.readAllBytes(), StandardCharsets.UTF_8);
    }
    try (InputStream is =
        getClass().getClassLoader().getResourceAsStream("schemas/testcase-list-schema.json")) {
      testCaseListSchema = new String(is.readAllBytes(), StandardCharsets.UTF_8);
    }
    try (InputStream is =
        getClass().getClassLoader().getResourceAsStream("schemas/testcase-tree-schema.json")) {
  ...
  public void importValidCsvFileTest2() {
    ...
    Map<String, Object> projectRequest =
        getProjectRequest("CSV2-", "유효한 CSV 파일 업로드 시 테스트 케이스 정상 등록 검증 (다른 포맷)");
  ...
  public void importValidCsvFileTest3() {
    // 1. 프로젝트 생성
    Map<String, Object> projectRequest =
        getProjectRequest("CSV2-", "유효한 CSV 파일 업로드 시 테스트 케이스 정상 등록 검증 (줄바꿈)");
  ...
  public void importInvalidCsvFileShouldFail() {
    ...
    String mappingJson =
        "{ \"fieldMappings\": {" + "\"name\":\"name\"," + "\"type\":\"type\"" + "}}";
  ...
  private void deleteAllTestCasesByProjectId(String projectId) {
    ...
    testCases.sort(
        (a, b) -> {
          ...
          return typeA.equals("folder") && !typeB.equals("folder")
              ? 1
