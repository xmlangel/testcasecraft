...
/** 조직 관리 API 보안 및 권한 검증 테스트 실제 보안 시나리오를 검증하는 테스트 케이스 */
...
public class OrganizationSecurityTest extends AbstractTestNGSpringContextTests {
  ...
  void setUp() {
    ...
    adminUser = new User();
    adminUser.setUsername("test_admin_" + uniqueId);
    adminUser.setEmail("admin_" + uniqueId + "@test.com");
    adminUser.setName("Administrator " + uniqueId);
    adminUser.setPassword(passwordEncoder.encode("admin123"));
    adminUser.setRole("ADMIN");
    adminUser = userRepository.save(adminUser);

    testerUser = new User();
    testerUser.setUsername("tester_" + uniqueId);
    testerUser.setEmail("tester_" + uniqueId + "@test.com");
    testerUser.setName("테스터 " + uniqueId);
    testerUser.setPassword(passwordEncoder.encode("tester123"));
  ...
  void testUnauthenticatedAccessBlocked() throws Exception {
    // 조직 목록 조회
    mockMvc.perform(get("/api/organizations")).andDo(print()).andExpect(status().isUnauthorized());
    ...
    // 조직 상세 조회
    mockMvc
        .perform(get("/api/organizations/fake-id"))
        .andDo(print())
        .andExpect(status().isUnauthorized());
    ...
    // 조직 삭제
    mockMvc
        .perform(delete("/api/organizations/fake-id"))
        .andDo(print())
        .andExpect(status().isUnauthorized());
  }
  ...
  void testInvalidTokenBlocked() throws Exception {
    ...
    mockMvc
        .perform(get("/api/organizations").header("Authorization", invalidToken))
        .andDo(print())
        .andExpect(status().isUnauthorized());
  }
  ...
  void testExpiredTokenBlocked() throws Exception {
    ...
    String expiredToken =
        "Bearer"
            + " eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJhZG1pbiIsImlhdCI6MTAwMDAwMDAwMCwiZXhwIjoxMDAwMDAwMDAxfQ.invalid";

    mockMvc
        .perform(get("/api/organizations").header("Authorization", expiredToken))
        .andDo(print())
        .andExpect(status().isUnauthorized());
  }
  ...
  void testOnlyOwnerCanDeleteOrganization() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/organizations/{organizationId}/members", orgId)
                .header("Authorization", adminToken)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    mockMvc
        .perform(
            delete("/api/organizations/{organizationId}", orgId)
  ...
  void testOnlyMembersCanViewOrganization() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/organizations/{organizationId}", orgId).header("Authorization", adminToken))
        .andDo(print())
    ...
    mockMvc
        .perform(
            get("/api/organizations/{organizationId}", orgId).header("Authorization", testerToken))
        .andDo(print())
        .andExpect(status().isForbidden());
  ...
  void testNonExistentOrganizationReturns404() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/organizations/{organizationId}", nonExistentOrgId)
                .header("Authorization", adminToken))
        .andDo(print())
    ...
    mockMvc
        .perform(
            put("/api/organizations/{organizationId}", nonExistentOrgId)
                .header("Authorization", adminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(updateRequest)))
        .andDo(print())
    ...
    mockMvc
        .perform(
            delete("/api/organizations/{organizationId}", nonExistentOrgId)
                .header("Authorization", adminToken))
        .andDo(print())
        .andExpect(status().isNotFound());
  }
  ...
  void testSelfRemovalFromOrganization() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/organizations/{organizationId}/members/{userId}", orgId, "tester")
                .header("Authorization", testerToken))
        .andDo(print())
        .andExpect(status().isNoContent());
  }
