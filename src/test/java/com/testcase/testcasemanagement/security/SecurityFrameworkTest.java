...
public class SecurityFrameworkTest {
  ...
  @Autowired private GroupSecurityService groupSecurityService;
  ...
  public void testOrganizationAccessControl() {
    ...
    assertTrue(
        organizationSecurityService.canAccessOrganization(
            testOrganization.getId(), testUser1.getUsername()));
    assertTrue(
        organizationSecurityService.isOrganizationMember(
            testOrganization.getId(), testUser1.getUsername()));

    // testUser2는 조직 멤버가 아니므로 접근 불가
    assertFalse(
        organizationSecurityService.canAccessOrganization(
            testOrganization.getId(), testUser2.getUsername()));
    assertFalse(
        organizationSecurityService.isOrganizationMember(
            testOrganization.getId(), testUser2.getUsername()));
    ...
    assertTrue(
        organizationSecurityService.canAccessOrganization(
            testOrganization.getId(), adminUser.getUsername()));

    // testUser1은 일반 멤버이므로 조직 관리 불가
    assertFalse(
        organizationSecurityService.canManageOrganization(
            testOrganization.getId(), testUser1.getUsername()));
  ...
  public void testProjectAccessControl() {
    ...
    assertTrue(
        projectSecurityService.canAccessProject(testProject.getId(), testUser1.getUsername()));
    assertTrue(
        projectSecurityService.isProjectMember(testProject.getId(), testUser1.getUsername()));

    // testUser2는 프로젝트 멤버가 아니지만 조직 멤버도 아니므로 접근 불가
    assertFalse(
        projectSecurityService.canAccessProject(testProject.getId(), testUser2.getUsername()));
    assertFalse(
        projectSecurityService.isProjectMember(testProject.getId(), testUser2.getUsername()));

    // 관리자는 모든 프로젝트에 접근 가능
    assertTrue(
        projectSecurityService.canAccessProject(testProject.getId(), adminUser.getUsername()));

    // testUser1은 개발자 역할이므로 프로젝트 관리 불가
    assertFalse(
        projectSecurityService.canManageProject(testProject.getId(), testUser1.getUsername()));
    assertFalse(
        projectSecurityService.hasManagementRole(testProject.getId(), testUser1.getUsername()));

    // 관리자는 모든 프로젝트를 관리 가능
    assertTrue(
        projectSecurityService.canManageProject(testProject.getId(), adminUser.getUsername()));
  }
  ...
  public void testRoleBasedPermissions() {
    ...
    assertTrue(
        organizationSecurityService.canManageOrganization(
            testOrganization.getId(), testUser1.getUsername()));
    assertTrue(
        organizationSecurityService.hasOrganizationAdminRole(
            testOrganization.getId(), testUser1.getUsername()));
  ...
  public void testMemberRemovalPermissions() {
    ...
    assertTrue(
        organizationSecurityService.canRemoveMember(
            testOrganization.getId(), testUser1.getId(), testUser1.getUsername()));
    assertTrue(
        projectSecurityService.canRemoveMember(
            testProject.getId(), testUser1.getId(), testUser1.getUsername()));
    assertTrue(
        groupSecurityService.canRemoveMember(
  ...
  public void testResourceCreationPermissions() {
    ...
    assertFalse(
        projectSecurityService.canCreateProject(testOrganization.getId(), testUser1.getUsername()));
    ...
    assertFalse(
        groupSecurityService.canCreateGroup(
            testOrganization.getId(), null, testUser1.getUsername()));

    // 일반 사용자는 프로젝트 그룹을 생성할 수 없음 (프로젝트 관리자가 아님)
    assertFalse(
        groupSecurityService.canCreateGroup(null, testProject.getId(), testUser1.getUsername()));
  }
}
