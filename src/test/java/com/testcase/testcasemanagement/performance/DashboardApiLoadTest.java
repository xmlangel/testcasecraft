 ...
 *
 * <p>이 클래스는 대시보드 API의 성능을 다양한 부하 상황에서 테스트합니다. - 동시성 테스트 (동시 요청 처리 능력) - 캐시 성능 테스트 (캐시 히트율 및 응답 시간
...
public class DashboardApiLoadTest {
  ...
  private String authenticateAndGetToken() {
    try {
      ...
      ResponseEntity<String> response =
          restTemplate.exchange(baseUrl + "/api/auth/login", HttpMethod.POST, entity, String.class);
  ...
  public void testCachePerformance() throws InterruptedException {
    ...
    Assert.assertTrue(firstResponse.getStatusCode().is2xxSuccessful(), "첫 번째 요청이 성공해야 합니다.");
    Assert.assertTrue(
        firstResponseTime <= CACHE_MISS_MAX_RESPONSE_TIME,
        String.format(
            "캐시 미스 응답 시간이 %dms 이하여야 합니다. 실제: %dms",
    ...
    for (int i = 0; i < 10; i++) {
      ...
      ResponseEntity<String> cachedResponse =
          restTemplate.exchange(apiUrl, HttpMethod.GET, entity, String.class);
      long responseTime = System.currentTimeMillis() - startTime;
      cacheHitTimes.add(responseTime);

      Assert.assertTrue(cachedResponse.getStatusCode().is2xxSuccessful(), "캐시된 요청이 성공해야 합니다.");
      Assert.assertTrue(
          responseTime <= CACHE_HIT_MAX_RESPONSE_TIME,
          String.format(
              "캐시 히트 응답 시간이 %dms 이하여야 합니다. 실제: %dms", CACHE_HIT_MAX_RESPONSE_TIME, responseTime));

      Thread.sleep(50); // 50ms 간격
    }

    double averageCacheHitTime =
    ...
    Assert.assertTrue(
        averageCacheHitTime < firstResponseTime * 0.5, "캐시 히트 시 응답 시간이 50% 이상 개선되어야 합니다.");
  ...
  public void testConcurrentLoad() throws InterruptedException, ExecutionException {
    ...
    // 통계 계산
    int totalSuccessfulRequests = results.stream().mapToInt(RequestResult::getSuccessCount).sum();

    double successRate = (totalSuccessfulRequests * 100.0) / totalRequests;

    List<Long> allResponseTimes =
        results.stream().flatMap(r -> r.getResponseTimes().stream()).toList();
    ...
    Assert.assertTrue(
        ...
        String.format("성공률이 %.1f%% 이상이어야 합니다. 실제: %.2f%%", SUCCESS_RATE_THRESHOLD, successRate));

    Assert.assertTrue(
        averageResponseTime <= AVERAGE_RESPONSE_TIME_THRESHOLD,
        String.format(
            "평균 응답 시간이 %.1fms 이하여야 합니다. 실제: %.2fms",
  ...
  public void testEnduranceLoad() throws InterruptedException, ExecutionException {
    ...
    Assert.assertTrue(totalSuccessfulRequests > 0, "지속성 테스트에서 최소한의 요청이 성공해야 합니다.");
    Assert.assertTrue(
        successRate >= 90.0,
  ...
  private String getFirstProjectId() {
    try {
      ...
      ResponseEntity<String> response =
          restTemplate.exchange(baseUrl + "/api/projects", HttpMethod.GET, entity, String.class);
