...
public class DatabaseIndexPerformanceTest extends AbstractTransactionalTestNGSpringContextTests {
  ...
  public void verifyIndexesExist() {
    ...
    // 인덱스 존재 검증
    Assert.assertTrue(
        hasTestResultExecutionResult, "idx_test_result_execution_result 인덱스가 생성되지 않았습니다.");
    Assert.assertTrue(
        hasTestExecutionProjectStatusUpdated,
        "idx_test_execution_project_status_updated 인덱스가 생성되지 않았습니다.");
    Assert.assertTrue(
        hasTestExecutionStatusUpdatedAt, "idx_test_execution_status_updated_at 인덱스가 생성되지 않았습니다.");
  ...
  public void verifyTestExecutionQueryPerformance() {
    ...
    String query3 =
        "SELECT te.test_plan_id, te.status, COUNT(*) "
            + "FROM test_executions te "
            + "WHERE te.project_id = :projectId "
  ...
  public void verifyIndexOptimizationEffects() {
    ...
    String[] optimizedQueries = {
      ...
      // 복합 조건 쿼리
      "SELECT te.id FROM test_executions te WHERE te.project_id = 'test' AND te.status ="
          + " 'COMPLETED'"
    };
