...
public class ProjectRepositoryTest extends AbstractTestNGSpringContextTests {
  ...
  void testFindAll() {
    ...
    assertTrue(
        result.stream()
            .anyMatch(
                project ->
                    "Organization Project 1".equals(project.getName())
                        && project.getOrganization() != null));
    assertTrue(
        result.stream()
            .anyMatch(
                project ->
                    "Organization Project 2".equals(project.getName())
                        && project.getOrganization() != null));
    assertTrue(
        result.stream()
            .anyMatch(
                project ->
                    "Independent Project".equals(project.getName())
                        && project.getOrganization() == null));
  }
  ...
  void testFindAccessibleProjectsByUserId_Success() {
    ...
    assertTrue(
        result.stream().anyMatch(project -> testOrgProject1.getId().equals(project.getId())));
    assertTrue(
        result.stream()
  ...
  void testFindByOrganizationId_MultipleProjects() {
    ...
    assertTrue(
        result.stream().anyMatch(project -> testOrgProject1.getId().equals(project.getId())));
    assertTrue(
        result.stream().anyMatch(project -> additionalProject.getId().equals(project.getId())));
  }
