...
/** GroupRepository 단위 테스트 Task 17: Repository 레이어 그룹 데이터 접근 로직 테스트 */
@DataJpaTest
...
public class GroupRepositoryTest extends AbstractTestNGSpringContextTests {
  ...
  void testFindAll() {
    ...
    assertTrue(
        result.stream()
            .anyMatch(
                group ->
                    "Organization Group".equals(group.getName())
                        && group.getOrganization() != null));
    assertTrue(
        result.stream()
            .anyMatch(
                group -> "Project Group".equals(group.getName()) && group.getProject() != null));
    assertTrue(
        result.stream()
            .anyMatch(
                group ->
  ...
  void testFindAccessibleGroupsByUserId_Success() {
    ...
    assertTrue(
        result.stream().anyMatch(group -> testOrganizationGroup.getId().equals(group.getId())));
