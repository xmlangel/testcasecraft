...
public class TestCaseGoogleSheetExporterDbIntegrationTest extends AbstractTestNGSpringContextTests {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(TestCaseGoogleSheetExporterDbIntegrationTest.class);
  ...
  public void exportTestCasesToGoogleSheet_DB데이터_기록_확인()
    ...
    for (TestCase tc : dbTestCases) {
      log.info(
          "TestCase ID: {}, Name: {}, Type: {}, ProjectID: {}",
          tc.getId(),
          tc.getName(),
          tc.getType(),
    ...
    ValueRange response =
        sheetsService.spreadsheets().values().get(TEST_SPREADSHEET_ID, RANGE).execute();
    ...
    Assert.assertEquals(
        sheetValues.size() - 1, dbFlatRows.size(), "DB의 flat row 수와 시트의 row 수가 일치해야 합니다.");
  ...
  /** 구글 시트에서 읽은 값과 DB 값을 비교할 때, null, 빈 문자열, "null" 등 모두 null로 통일해서 비교 */
